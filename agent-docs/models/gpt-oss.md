...
Four templates ship under `jinja_templates/`, selected with `chat_template:`.

<!-- markdownlint-disable MD056 -- pipes inside a code span are literal to Python-Markdown -->

| File | Turns | Tools | Reasoning channel | Use for |
|---|---|---|---|---|
| `gpt-oss-instruct.jinja` | single | no | empty `analysis` | non-tool, reasoning-free SFT |
| `gpt-oss-thinking.jinja` | single | no | passes `reasoning_content` through | single-turn SFT carrying a CoT |
| `gpt-oss-multiturn.jinja` | multi | no | empty `analysis` | multi-turn reasoning-free SFT; ends each assistant turn with `<|return|>` |
| `gpt-oss-harmony.jinja` | multi | yes | full | multi-turn / tool-use SFT, and any GPT-OSS RL |

<!-- markdownlint-enable MD056 -->

`gpt-oss-harmony.jinja` is the upstream OpenAI-harmony template from `unsloth/gpt-oss-20b-BF16`, kept so the training render is **byte-identical** to vLLM's server-side render. Its one deviation: tool and parameter `description` is guarded with `is defined`, so a tool without descriptions does not crash the render. (The checkpoint's built-in template renders `param_spec.description` unconditionally and raises without it.)

For RL the training and serving templates must match, since log-probs are computed on the served prompt. GPT-OSS ships a built-in template, so overriding it needs **both** `chat_template: jinja_templates/gpt-oss-harmony.jinja` and `force_chat_template: true`. Pass the same file to vLLM via `VLLM_CHAT_TEMPLATE`.
