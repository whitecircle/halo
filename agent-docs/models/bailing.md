...
The Ling configs pin the toolkit's own chat templates with `force_chat_template: true` — `jinja_templates/ling-instruct.jinja` / `ling-multiturn.jinja` (Ling-mini-2) and `jinja_templates/ling3-instruct.jinja` (Ling 3.0) — because the hub templates drift across revisions and the training template must match the served one.
