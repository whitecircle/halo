...
- **Global layers** run matmul-softmax-matmul attention (fp32 softmax, GQA without a KV copy), because the mem-efficient kernel is the only SDPA backend left for them, while a layer's saved scores fit `EAGER_GLOBAL_BUDGET_BYTES` (2 GiB: 16 heads at 4,096 tokens per row), and mem-efficient SDPA past it. Fwd+bwd per layer at 2,048 tokens: 1.96 ms against 6.52 ms. At 8,192 tokens the scores exceed the budget and the layer runs mem-efficient SDPA. FlexAttention only fits shared memory at head_dim 512 with its smallest tiles, and those are slower than SDPA.
