...
- Liger: every decoder layer keeps a dense `Gemma4TextMLP` beside its experts, which the wrapper never touches; the toolkit's delegating spec fuses it (`geglu`, probing the activation the same way) so it stays fused under EP, while upstream's `gemma4_text` applier serves the norms. The fused loss is forced off for `Gemma4ForConditionalGeneration` checkpoints, whose own head runs ([Liger Kernels](../optimization/liger-kernels.md#fused-loss-under-a-multimodal-wrapper)).
