...
| `rollout_chat_template_kwargs` | `dict` | `{}` | Chat-template variables sent with every rollout request (`chat_template_kwargs`) and applied to the trainer's own renders, so both sides see one template state. Qwen3.x reads `preserve_thinking`: with the env's `carry_reasoning`, the carried previous-turn reasoning stays rendered past a user message (the cut-turn nudge). `reasoning_effort` is refused here; it travels top-level. |
