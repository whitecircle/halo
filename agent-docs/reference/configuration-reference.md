...
| `max_length_cutoff_recoveries` | `int \| null` | `null` | Cap on the engine-cut turns one episode may recover from (`>= 0`; `null` = every one within `max_turns`). A cut turn is nudged and retried but spends a turn and no tool budget, so without a cap an episode whose thoughts overrun their budget re-thinks until `max_turns`; the cut past the cap ends the episode truncated, priced like a `max_turns` overflow. Environments with effort profiles may tighten it per level (`code_contests`: profile key `max_length_cutoff_recoveries`). |
