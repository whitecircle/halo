...
| `VLLM_GROUP_HOST` | loopback (local server) / default-route NIC | Trainer interface advertised to vLLM workers for the NCCL weight-sync group. Set on multi-homed hosts where the default-route NIC is unreachable from a remote vLLM. Precedence: explicit `group_host` arg (env-GRPO `rollout_server_configs`; Online GRPO has only the env var) → this var → loopback when the server is local → default-route NIC. |
...
| `max_length_cutoff_recoveries` | `int \| null` | `null` | Cap on the engine-cut turns one episode may recover from (`>= 0`; `null` = every one within `max_turns`). A cut turn is nudged and retried but spends a turn and no tool budget, so without a cap an episode whose thoughts overrun their budget re-thinks until `max_turns`; the cut past the cap ends the episode truncated, priced like a `max_turns` overflow. A reasoning-effort profile may tighten it per level (profile key `max_length_cutoff_recoveries`, admitted by every environment; never above this value). |
...
Common `environment_kwargs` keys: `search_backend` (`qa_search` / open-book `exam_qa` — setting it with `open_book: false` is refused at construction: `duckduckgo`, `serper`, `brave`, `tavily`; `mock` only with `HALO_ALLOW_MOCK_SEARCH=1`), `open_book` (`exam_qa`, default `false`), `timeout_per_test` (`code_contests`, seconds/test, default `15`), `max_grading_seconds` (`code_contests`, wall-clock budget per submission grade, default `None` = unbounded — see [Code Contests](../training-methods/grpo/environments/code-contests.md#grading)), `verdict_detail` (`code_contests`, `full` default / `outcome` — `outcome` omits the expected and produced output of a wrong answer, leaving `Test i: FAIL`; stderr, crash, time-limit and output-limit diagnostics stay in both modes), `resubmission_penalty` (`code_contests`, magnitude charged per graded submission after the first, default `0`), `language` (`code_contests`: `python` default, `cpp`, `c`, or a list the model chooses from per call), `reasoning_effort_profiles` (every environment, effort → profile merged over the class defaults; the base admits `thinking_tokens`, `max_length_cutoff_recoveries`, `token_cost`, and `code_contests` adds `max_submissions`, `max_test_calls`, `tested_submission_reward` — see [Code Contests](../training-methods/grpo/environments/code-contests.md#reasoning-effort)), `tool_budgets` (native tool-use and ReAct environments, `{tool_name: cap}` per episode, `0` disables a tool — see [Native Tool-Use](../training-methods/grpo/environments/native-tool-use.md#per-tool-budgets)), `mcp_server` (`mcp`, e.g. `filesystem`), `include_python_tools` (`qa_search`, default `false`).
...
| `sync_weights_every_n_steps` | `int` | `1` | Sync weights every N training steps. Must be `>= 1`. |
| `rollout_temperature` | `float` | `0.7` | Rollout generation temperature. Overwrites `GRPOConfig.temperature` (logged) so log-probs are scored at the sampling temperature; the engine's logprobs must carry it too — vLLM `--logprobs-mode processed_logprobs`, SGLang's default (leave `SGLANG_RETURN_ORIGINAL_LOGPROB` unset). The startup probe runs against either engine and refuses raw logprobs. |
| `rollout_top_p` | `float` | `0.95` | Rollout top-p, in `(0, 1]`. With the trajectory geometric band (`isr_geo_band_*`) and a nucleus-renormalized logprob reference (vLLM `processed_logprobs`) it must be `1.0` — or drop `isr_geo_band_min`/`isr_geo_band_max`, since every uncertain position otherwise reads as drift. |
...
| `rollout_max_thinking_tokens` | `int \| None` | `None` | Per-turn reasoning-token budget for reasoning models (vLLM `thinking_token_budget`): caps the chain-of-thought, then forces the answer out of the rest of `rollout_max_tokens`. Needs a reasoning parser on the server (`--reasoning-parser qwen3` for Qwen3.x, the `openai_gptoss` plugin for gpt-oss) **and** `VLLM_USE_V2_MODEL_RUNNER=0`: Model Runner V2 answers `thinking_token_budget` with a 400 on every request, so rollouts error instead of generating. `None` = unbounded reasoning. Must stay **below** `rollout_max_tokens`, which bounds the whole turn: at or above it the answer headroom is 0 and every turn is cut mid-reasoning (raised at config time). Rejected under `rollout_backend: sglang` for every model — the trainer wires neither of SGLang's budget mechanisms, and harmony models have none server-side. |
...
| `isr_engine_reference` | `bool` | `False` | Re-score every training row on the rollout engine under the just-synced weights and feed the mask stages the engine's current-vs-sampling log-ratio — the trainer↔engine disagreement drops out, the engine's own decode-vs-prefill disagreement (≈−0.003 nats/token at sampling entropy 0.36) stays in the reference and grows with entropy; the IS weight stays trainer-vs-sampling. Logs `sampling/engine_logratio_mean`, `sampling/numerics_logratio_mean`, `sampling/engine_rescore_coverage`, `sampling/engine_rescore_miss_frac`. Requires the IS correction and `rollout_temperature` / `rollout_top_p` of 1.0; vLLM re-scores through the completions `prompt_logprobs` echo, SGLang through `/generate` with `logprob_start_len`. The vLLM server needs headroom for the per-chunk fp32 vocabulary log-softmax that pass materializes (8 GB at 8192 × 248k): `--gpu-memory-utilization` ≤ 0.80 or a smaller `--max-num-batched-tokens`, or the engine OOMs under load. See [the stability knobs](../training-methods/grpo/environmental-grpo.md#off-policy-mismatch-and-stability-knobs). |
| `skip_update_masked_frac` | `float \| None` | `None` | Trust-region circuit breaker: when more than this fraction of IS-corrected trajectories are fully masked by the stages above, **or** more than this fraction of corrected tokens (the masked trajectories are the long ones), the step's advantages are zeroed and its optimizer step is skipped — gradients dropped to `None`, so Adam's momentum cannot step the weights either; the KL term goes with the step. Logged as `sampling/update_skipped` with the two fractions (`sampling/is_masked_traj_frac`, `sampling/is_masked_token_frac`). Requires the IS correction **and** at least one mask stage (`isr_band`/`isr_geo_band`/`isr_veto`/`isr_opsm`) — raises otherwise, since nothing would ever be masked. |
...
| `effort_length_penalty_k0` | `float \| None` | `None` | Coefficient, at the lowest effort level, of the capped effort-conditioned reasoning-length penalty (`None` = off): per trajectory `-min(c_max, k(effort) × reasoning_tokens / l_norm)` with `k(effort) = k0 × exp(-(effort - effort_min) / tau)`, reasoning tokens summed over the assistant turns. A low level pays most for the same trace; the cap keeps a long trace from outweighing the task reward, which the uncapped per-token `token_cost` does not. Logged as `reward/effort_length_penalty` and `effort/<level>/length_penalty`. Positive and finite when set. |
| `effort_length_penalty_tau` | `float` | `25.0` | Effort units over which `k(effort)` falls by `e`. |
| `effort_length_penalty_c_max` | `float` | `0.5` | Cap of the penalty, in reward units. |
| `effort_length_penalty_l_norm` | `float` | `8192.0` | Reasoning tokens per unit of the penalty. |
| `effort_length_penalty_levels` | `dict[str, float]` | `{low: 25, medium: 50, high: 100}` | Scalar effort per categorical level for `k(effort)`; its minimum is `effort_min`. Must name exactly the effort levels (refused at trainer construction otherwise). |
...
| `request_timeout` | `float` | `120.0` | HTTP timeout per rollout-server request (whichever engine `rollout_backend` selects), in seconds. One request is one turn, so size it to at least twice the per-turn cap (thinking budget + answer headroom) divided by the per-sequence decode speed at peak concurrency ([Sizing a run](../training-methods/grpo/environmental-grpo.md#sizing-a-run)); a slower turn fails its episode. |
| `max_retries` | `int` | `3` | Max retries for transient rollout-server failures (exponential backoff); total attempts = `max_retries + 1`, and it must be `>= 0` (`0` = one attempt, no retry). |
...
It also inherits `AdvantageShapingArguments` (`src/args/mixins.py`): `advantage_mode`, `advantage_quantile`, `advantage_pos_scale`, `advantage_neg_scale`, `advantage_hard_group_threshold`, `scale_rewards_std_floor`, `drop_degenerate_groups` — same semantics as the [AsyncTrainingConfig](#asynctrainingconfig) rows, with two differences: `drop_degenerate_groups` defaults **`false`** here (it is `true` in env-GRPO, whose sparse verifiable reward makes dead groups dominate the batch), and `advantage_hard_group_threshold` gates on the **total weighted reward** — this arm carries no objective-reward decomposition, so set it for the combined scale (a run with more than one reward function warns).
