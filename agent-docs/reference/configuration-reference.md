...
| `HALO_WEIGHT_SYNC_CHUNK_MB` | `1024` | Host-side budget of one streamed weight-sync chunk (pinned host memory on the forwarding rank, plus the largest single tensor; the SGLang client also keeps two arenas of this size on the sync GPU, the vLLM client its two packed buffers). Each chunk is one engine round trip. Measured over EFA against vLLM: `2048` pushed a 16 GB model 15% faster than the default, `4096` slower than it. |
