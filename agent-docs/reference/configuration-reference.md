...
| `HALO_WEIGHT_SYNC_CHUNK_MB` | `1024` | Budget of one streamed weight-sync chunk, staged on the forwarding rank's sync GPU; a tensor above it is a chunk of its own, and each engine path stages its broadcasts in buffers of at least this size. Each chunk is one engine round trip, which bounds the vLLM rate ([measured curve](../infrastructure/rollout-servers.md#servers-on-other-nodes-efa)). |
...
| `request_timeout` | `float` | `120.0` | HTTP timeout per rollout-server request (whichever engine `rollout_backend` selects), in seconds. |
