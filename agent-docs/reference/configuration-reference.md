...
| `rollout_temperature` | `float` | `0.7` | Rollout generation temperature. Overwrites `GRPOConfig.temperature` (logged) so log-probs are scored at the sampling temperature; the engine's logprobs must carry it too (vLLM: `--logprobs-mode processed_logprobs`, probed and refused at startup otherwise). |
| `rollout_top_p` | `float` | `0.95` | Rollout top-p. With the trajectory geometric band (`isr_geo_band_*`) and a nucleus-renormalized logprob reference (vLLM `processed_logprobs`) it must be `1.0`, or every uncertain position reads as drift. |
