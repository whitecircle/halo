...
- `reasoning_effort_profiles` (every environment): effort → profile merged over the class defaults. The base admits `thinking_tokens`, `max_length_cutoff_recoveries`, `token_cost`; `code_contests` adds `max_submissions`, `max_test_calls`, `tested_submission_reward` ([Code Contests → Reasoning effort](../training-methods/grpo/environments/code-contests.md#reasoning-effort)). `thinking_tokens` is a vLLM request field: under `rollout_backend: sglang` a level's budget reaches no request field (warned once per process) and survives only as the target `reasoning_compliance_weight` prices against.
...
| `reasoning_compliance_weight` | `float` | `0.0` | Weight of the asymmetric reasoning-budget calibration reward (`0` = off). Active when the episode has a CoT budget (the env's per-effort budget when `reasoning_effort` is set, otherwise `rollout_max_thinking_tokens`): no penalty inside `[0.3, 0.9]×` the budget, mild penalty for under-use (scaled by `reasoning_compliance_under_use_weight`), strong penalty for over-use or truncation (down to `-weight`). `~0.15` shapes without dominating the task reward. |
| `reasoning_compliance_under_use_weight` | `float` | `0.3` | Weight of the calibration term's below-band (under-use) side, relative to the over-use side's `1.0`: a turn under `0.3×` the budget pays `-under_use_weight × shortfall` (the missing fraction of the band floor) before `reasoning_compliance_weight` scales it. `0` turns that side off, so a short repair turn after a verdict is not penalized and only the above-band side prices thinking that runs to the budget. Finite and `>= 0`. |
