...
| `max_completion_length` | Completion share of the budget, or — under RL — the number of tokens the policy may generate. Async GRPO with Environments overwrites it with `rollout_max_tokens` at construction (the rollout budget is the generation cap there). |
...
- **Async GRPO never truncates a trajectory.** `rollout_max_tokens` is the per-turn generation budget; the multi-turn trajectory accumulates across turns and is bounded only by the served model's context window — one that exceeds it fails ([trajectory length](../training-methods/grpo/async-grpo/rollouts.md#trajectory-length)).
...
| `VLLM_GROUP_HOST` | loopback (local server) / default-route NIC | Trainer interface advertised to vLLM workers for the NCCL weight-sync group, and the one address its rendezvous store listens on unless `HALO_WEIGHT_SYNC_BIND_ALL` is set. Set on multi-homed hosts where the default-route NIC is unreachable from a remote vLLM. Resolution order and the multi-homed cases: [Online GRPO → Multi-homed nodes](../training-methods/grpo/online-grpo.md#multi-homed-nodes-vllm_group_host) (async GRPO also takes a per-server `group_host` in `rollout_server_configs`; Online GRPO has only the env var). |
| `SGLANG_GROUP_HOST` | loopback (local server) / default-route NIC | Trainer interface advertised to SGLang workers for the NCCL weight-sync group, and the one address its rendezvous store listens on unless `HALO_WEIGHT_SYNC_BIND_ALL` is set: the `VLLM_GROUP_HOST` equivalent for `rollout_backend: sglang`. Separate per engine because the two servers can sit on different hosts. |
| `HALO_WEIGHT_SYNC_BIND_ALL` | off | `1` puts the weight-sync rendezvous store (both engines) on every interface instead of the advertised group address alone, for a trainer the server reaches through NAT or a port mapping; each group formation then logs a WARNING. Off, an advertised address this host cannot bind alone (not local, `0.0.0.0`, or a name resolving to loopback for a remote server) raises at group formation. An IPv6 group address raises in either mode: the store listens on IPv4 only. The store is unauthenticated, so set it only where no untrusted host reaches the group port ([Group rendezvous](../infrastructure/rollout-servers.md#group-rendezvous)). |
...
| `HALO_FLEX_SLIDING` | on | `0` builds an SDPA run with plain `sdpa` instead of `sdpa_flex_sliding` (FlexAttention on sliding-window layers, matmul attention on global layers SDPA can only run mem-efficient). See [Flash Attention](../optimization/flash-attention.md#sliding-window-and-wide-head-layers-on-sdpa). |
...
| `HALO_SANDBOX_BACKEND` / `HALO_SANDBOX_URL` / `HALO_SANDBOX_MAX_CONCURRENCY` | `local` / unset / `cpu_count()` | RL code-execution sandbox: backend (`local` / `bubblewrap` / `remote`; [which ones warn](../training-methods/grpo/environments/sandbox.md#choosing-a-backend)), the SandboxFusion endpoint required by `remote`, and the process-wide concurrent-execution cap. See [Sandboxes](../training-methods/grpo/environments/sandbox.md). |
| `HALO_NCCL_SYNC_TIMEOUT_SECONDS` | `600` | Deadline for draining one chunk's broadcasts on both clients (a vLLM packed buffer, or a SGLang chunk's sends, settled two chunks later). On expiry the group is aborted instead of parking the trainer, with a hint naming the transport causes (address, `NCCL_NET` / plugin mismatch, cuMem). The 120 s warm-up and the 30 s failure-path cleanup keep their own deadlines. Raise for large policies over sockets. |
| `HALO_WEIGHT_SYNC_CHUNK_MB` | `1024` | Budget of one streamed weight-sync chunk, staged on the forwarding rank's sync GPU; a tensor above it is a chunk of its own, and the SGLang staging arena is at least this size (the vLLM packed buffers stay at their own 1 GiB). Each chunk is one engine round trip, which bounds the vLLM rate ([measured curve](../infrastructure/rollout-servers.md#servers-on-other-nodes-efa)). |
| `HALO_VLLM_GENERATION_TIMEOUT_SECONDS` | 80% of the NCCL watchdog (`1440` at the default `DIST_NCCL_TIMEOUT_MINUTES`) | Per-request HTTP timeout (seconds) for a vLLM generation call from the NCCL weight-sync client, **online GRPO only**. The call blocks every peer at the next collective, so the default tracks `DIST_NCCL_TIMEOUT_MINUTES` and stays under it. Async GRPO generates from the Ray actors instead, bounded by `request_timeout` and `episode_timeout`. |
...
| `max_completion_length` | `int \| None` | `256` | Generation budget (online / vanilla GRPO; TRL `max_new_tokens`). Set it explicitly. **Async GRPO does not use it**: it generates with `rollout_max_tokens` and is bounded by the model context (see [Sequence length](#sequence-length-caps-vs-generation-budgets)). |
| `beta` | `float` | `0.0` | KL penalty coefficient (`0.0` = no reference model). |
| `temperature` | `float` | `1.0` | Sampling temperature. Async GRPO **overwrites** it (with a log) with `rollout_temperature`, so log-probs are scored at the temperature the rollout sampled at. |
| `top_k` | `int` | `0` | Top-k sampling (`0` = disabled). Online GRPO only. |
| `top_p` | `float` | `1.0` | Top-p sampling. Online GRPO only. With a `sequence_*` `vllm_importance_sampling_mode`, `top_p < 1` against a server whose logprobs are nucleus-renormalized (vLLM `processed_logprobs`) is refused at startup ([Online GRPO](../training-methods/grpo/online-grpo.md#configuration)). |
...
| `mask_truncated_completions` | `bool` | `False` | Drop completions the engine cut at its token cap from the loss. Async GRPO widens it to any truncated trajectory (turn overflow and a lost episode included) and applies it in its own drop set (its rollout path replaces TRL's generation), masking every row of a truncated trajectory and logging `sampling/truncated_masked_frac`. |
| `off_policy_mask_threshold` | `float \| None` | `None` | TRL's off-policy token mask. **Refused on async GRPO** (raises at construction): TRL's mask reads `sampling_per_token_logps`, which that trainer's batch never carries, so it would threshold a KL of exactly 0; use `isr_opsm_delta` instead. |
...
| `vllm_group_port` | `int` | `51216` | NCCL group port for weight sync, on either rollout backend. In multi-server mode it is the **base** port: a `rollout_server_configs` entry without its own `group_port` binds `vllm_group_port + index`. |
...
| `remove_unused_columns` | `bool` | `False` | Async GRPO **forces it `False`** at construction and warns when a config sets it true: the rollout context is the row's non-prompt columns, and pruning would strip `answer` and every `context_fields` column, leaving the environment grading episodes it was handed no ground truth for. |

Async GRPO samples on the rollout server, so `top_p`, `top_k`, `min_p`, `repetition_penalty` and `generation_kwargs` warn and are ignored when set away from their TRL defaults — use the `rollout_*` fields on [AsyncTrainingConfig](#asynctrainingconfig). It also ignores `vllm_importance_sampling_mode` and `vllm_importance_sampling_clip_min` (its IS correction is token-level and clips from above only), and forces `use_vllm: true` while that correction is on.

Both on-policy trainers force `use_liger_kernel` off at construction: TRL's Liger GRPO loss breaks global token normalization and the chunked log-prob path.
...
| `pp_schedule` | `pipeline_schedule` |
| `pp_microbatches` | `pipeline_microbatches` |
| `pp_split` | `pipeline_split` |
| `ep_fp32_router` | `fp32_router` |
| `ep_fp32_experts` | `fp32_experts` |

Every other field below keeps its name, except `world_size` / `gpus_per_node` (auto-detected, not settable), `expert_lora`, and `merge_expert_lora_on_save` (documented under `DistributedArguments` above).
...
| `fsdp_reshard_after_backward` | `bool` | `True` | `False` keeps params unsharded across a grad-accum window's microsteps (torch `set_reshard_after_backward`, re-armed for the window's last backward): drops the full-model re-all-gather FSDP2 otherwise issues after every microstep's backward to one per optimizer step — negligible over NVLink, ~15 s × `gradient_accumulation_steps` per step when the trainer's NCCL runs over TCP sockets. Saves nothing at `gradient_accumulation_steps=1`. Costs one unsharded bf16 param copy per GPU. Plain-DP/CP/EP torchrun path only; rejected with `fsdp_reshard_after_forward=True`, TP, or PP. See [Data Parallelism](../parallelism/data-parallelism.md#zero-2-vs-zero-3-reshard_after_forward). |
| `fsdp_shard_ep1_experts` | `bool` | `True` | Applies only when experts are truly replicated — `ep_group_size==1` (`ep_size==1` **and** `expert_tp_size==1`): FSDP-shards the replicated MoE experts so their reduce-scatter becomes the sole gradient sync. Default `True` frees memory that grows with DP (gpt-oss-20b −19%/−37% at 2/8 GPU), throughput-neutral and grad-equivalent; `False` keeps a full replicated copy per rank (max throughput). No effect when `ep_group_size>1`. Composes with `fsdp_reshard_after_forward` for full ZeRO-3 experts, and works for online / async GRPO (the weight-sync gather `full_tensor`-s the sharded experts first). `False` raises at config time under TP or CP (those paths shard ep1 experts unconditionally, so the flag would be ignored) and under PP whenever the EP wrappers are active (`use_grouped_gemm`, the default — `ParallelismConfig` has no model knowledge, so this catches dense runs too): a stage holding plain replicated experts beside a dense stage issues a different collective program and deadlocks at the gradient norm. |
...
| `use_chunked_grpo_logprobs` | `bool` | `False` | Vocab-chunked completion log-probs instead of full `[B, T, vocab]` logits: the shared `ChunkedLogprobsArguments` switch; covers the policy and the `kl_beta > 0` reference forward. Inert under PP (the last stage still materializes its own logits plane); the trainer warns at construction rather than leaving it to a surprise OOM. See [Offline GRPO](../training-methods/grpo/offline-grpo.md#memory-chunked-log-probs). Off, construction refuses a full-logits plane the device's free memory cannot hold; the check is skipped under PP and with `max_completion_length` unset ([the logits wall](../training-methods/grpo/async-grpo/performance.md#the-logits-wall)). |
...
| `use_clm_loss` | `bool` | `True` | Add the auxiliary CLM (SFT) loss alongside distillation. `False` = distillation only, and then `distill_alpha` must be `1.0` — the loss is not renormalized, so a smaller alpha would scale the sole remaining term (**rejected** at config time). |
...
Environment selection, reward terms and turn budget for Async GRPO with Environments. **Source:** `src/configs/environment_config.py`
...
- `verdict_detail` (`code_contests`, `outcome` default / `full`): what a failed test's verdict shows ([Code Contests → Grading rules](../training-methods/grpo/environments/code-contests.md#grading-rules)).
- `eval_protocol` (`code_contests`, `harness` default / `leaderboard`): `leaderboard` pins one submission and no scratchpad runs ([Code Contests → Evaluation protocols](../training-methods/grpo/environments/code-contests.md#evaluation-protocols)).
- `submission_reward`, `execution_progress_reward`, `resubmission_penalty`, `improved_resubmission_refund` (`code_contests`, the shaping components, default `0`; the refund is the fraction of the resubmission price an improving resubmission earns back; the objective's shape is the `environment` term's `exponent` in `rewards`; see [Code Contests → Reward ladder](../training-methods/grpo/environments/code-contests.md#reward-ladder)).
- `language` (`code_contests`: `python` default, `cpp`, `c`, or a list the model chooses from per call).
- `reasoning_effort_profiles` (every environment): effort → profile merged over the class defaults. The base admits `thinking_tokens`, `max_length_cutoff_recoveries`; `code_contests` adds `max_submissions`, `max_test_calls`, `tested_submission_reward` ([Code Contests → Reasoning effort](../training-methods/grpo/environments/code-contests.md#reasoning-effort)).
- `max_length_cutoff_recoveries` (every environment, `>= 0`; `null` = every one within `max_turns`): cap on the unproductive turns one episode may recover from — engine-cut, or ended with neither a tool call nor visible content; the turn past the cap, or on the episode's last turn, ends the episode truncated, priced like a `max_turns` overflow. A reasoning-effort profile may tighten it per level, never above this value.
- `carry_reasoning` (every environment, default `false`): sends the previous assistant turn's reasoning back to the engine; earlier turns stay as visible text. vLLM backend only: the trainer refuses it on `rollout_backend: sglang`. Where it renders is the chat template's call ([Chat template](../training-methods/grpo/async-grpo/rollouts.md#chat-template)).
- `tool_reward_cap` (every environment): per-episode cap on the summed `tool_success_reward`, default `tool_success_reward × max_turns` ([Native Tool-Use → Reward](../training-methods/grpo/environments/native-tool-use.md#reward)).
- `tool_budgets` (native tool-use and ReAct environments, `{tool_name: cap}` per episode, `0` disables a tool; see [Native Tool-Use → Configuration](../training-methods/grpo/environments/native-tool-use.md#configuration)).
- `requires_answer` (every environment): whether the reward grades against the dataset's `answer` column, so a dataset without one is refused at trainer construction. Defaults to the class's own declaration — `true` for code contests, `exam_qa`, `qa_search` and the ReAct presets (an `answer_validator` clears it), `true` for `swe` unless judge-only ([SWE → Reward](../training-methods/grpo/environments/swe-environment.md#reward)), `false` elsewhere.
- `mcp_server` (`mcp`, e.g. `filesystem`).
- `include_python_tools` (`qa_search`, default `false`).
...
`DistributedAsyncEnvironmentalGRPOTrainer`: Ray workers, rollout-server connections, weight sync, rollout generation, async prefetch. **Source:** `src/configs/async_training_config.py`
...
| `max_concurrent_rollouts` | `int \| None` | `None` | Per-rank in-flight cap, the throughput throttle. Size to one generation cycle's per-rank demand (`per_device_train_batch_size × steps_per_generation`, which itself defaults to `gradient_accumulation_steps`) with ~2× prefetch headroom; server-pool load is `this × world_size ÷ servers` (under TP every rank collects a full batch). Raising it past the actual rollout count does nothing. Refused below 1 when set; `null` derives 4 × this rank's share of the rollout workers (`num_rollout_workers ÷ world_size` on a shared Ray cluster, all of them locally), clamped to at least that share. |
| `eval_rollout_batch_size` | `int \| None` | `None` | Rows per rank in one evaluation rollout round; rows, not prompts, since the eval sampler has already repeated each prompt `num_generations_eval` times. Eval rounds run without prefetch, so a round's wall time is its slowest episode and `per_device_eval_batch_size`-sized rounds idle the servers between them; size it to what the servers sustain: `this × world_size` requests are in flight at once, and a turn that decodes slower than `request_timeout` allows fails its episode, so a whole split in one round can oversubscribe the servers that carry the training rounds fine. Keep it a multiple of `num_generations_eval`, at most `max_concurrent_rollouts` (a wider round runs in serial waves; refused at start), a divisor of the per-rank share (ceil(eval rows ÷ `data_parallel_size`); the last round is padded to whole global batches, so other sizes roll out duplicate rows), with `dataloader_drop_last` off (refused at construction). It bounds the loader's batch, not eval peak memory: the loss forward still chunks `per_device_eval_batch_size` rows, but the round's widest completion sets the padded width. Refused below 1 when set; `null` leaves the round at the eval batch (`per_device_eval_batch_size`), whose geometry is checked at construction the same way. |
...
| `rollout_backend` | `"vllm" \| "sglang"` | `"vllm"` | Rollout engine for generation and weight sync. `sglang` supports `routing_replay: rollout` (serve with `--enable-return-routed-experts --moe-runner-backend triton`), rejects `rollout_max_thinking_tokens` and `rollout_thinking_budget_scope: episode` at config time and the env's `carry_reasoning` at trainer construction, refuses the families 0.5.17's loaders cannot take an online update for (`SGLangWeightSyncClient.UNSERVABLE_MODEL_TYPES`, quoted at trainer construction), and must be served from the NCCL-aligned `Dockerfile.sglang` image with `NCCL_CUMEM_ENABLE=1` (the compose default); see [Rollout Servers](../infrastructure/rollout-servers.md#which-families-each-engine-serves). |
...
| `rollout_connection_timeout` | `float` | `120.0` | Seconds to wait for the rollout server. Refused: non-finite or `<= 0`. |
...
| `sync_weights_every_n_steps` | `int` | `1` | Sync weights every N training steps. Refused: `< 1`. |
| `rollout_temperature` | `float` | `0.7` | Rollout generation temperature. Overwrites `GRPOConfig.temperature` (logged) so log-probs are scored at the sampling temperature; the engine's logprobs must carry it too — vLLM `--logprobs-mode processed_logprobs`, SGLang's default (leave `SGLANG_RETURN_ORIGINAL_LOGPROB` unset). The startup probe runs against either engine and refuses raw logprobs. Refused: non-finite or `<= 0`. |
| `rollout_top_p` | `float` | `0.95` | Rollout top-p. With the trajectory geometric band (`isr_geo_band_*`) and a nucleus-renormalized logprob reference (vLLM `processed_logprobs`) it must be `1.0` — or drop `isr_geo_band_min`/`isr_geo_band_max`, since every uncertain position otherwise reads as drift. Refused outside `(0, 1]`. |
| `rollout_max_tokens` | `int` | `32768` | The per-turn generation budget (one `/chat/completions` call). The multi-turn trajectory accumulates across turns and is bounded by the served model's context window, not truncated (a trajectory exceeding it fails). Refused: non-finite or `<= 0`. |
| `rollout_max_thinking_tokens` | `int \| None` | `None` | Per-turn reasoning-token budget for reasoning models (vLLM `thinking_token_budget`): caps the chain-of-thought, then forces the answer out of the rest of `rollout_max_tokens`. Needs a reasoning parser on the server (`--reasoning-parser qwen3` for Qwen3.x, the `openai_gptoss` plugin for gpt-oss) **and** `VLLM_USE_V2_MODEL_RUNNER=0`: Model Runner V2 answers `thinking_token_budget` with a 400 on every request, so rollouts error instead of generating. `None` = unbounded reasoning. Refused when negative or non-finite, and at or above `rollout_max_tokens`, which bounds the whole turn: there the answer headroom is 0 and every turn is cut mid-reasoning. Rejected under `rollout_backend: sglang` for every model: the trainer wires neither of SGLang's budget mechanisms, and harmony models have none server-side. Under `rollout_thinking_budget_scope: episode` it is the ceiling one turn may take of the episode's budget, not a clamp on that budget. |
| `rollout_thinking_budget_scope` | `"turn" \| "episode"` | `"turn"` | What a thinking budget (a level's `thinking_tokens`, else `rollout_max_thinking_tokens`) covers. `turn`: every turn gets it whole, so a cut or empty turn plus its recovery nudge buys another full budget. `episode`: the budget is the episode's total; a turn's engine cap is the budget minus the reasoning the earlier turns spent, never below `rollout_thinking_turn_reserve` and never above `rollout_max_thinking_tokens`, while the per-turn total stays constant. The spend is the sampled ids up to and including `rollout_reasoning_end_token`, so it is vLLM-only (refused under `rollout_backend: sglang`), requires `train_on_sampled_tokens`, and every request asks for `return_token_ids`. With `rollout_max_thinking_tokens` unset, every episode's budget is its level's alone: trainer construction and the eval scripts refuse the scope unless the environment sets `reasoning_effort` and every level's `thinking_tokens`. Passed to the effort templates as `reasoning_budget_scope`; logs `episode/thinking_budget_exhausted`. See [Reasoning budget](../training-methods/grpo/async-grpo/rollouts.md#reasoning-budget). |
| `rollout_thinking_turn_reserve` | `int` | `512` | Under `rollout_thinking_budget_scope: episode`: the reasoning a turn always gets once the episode's budget is spent, so the model can still close its reasoning and act. Refused below `1`; under the episode scope also refused above `rollout_max_thinking_tokens` when that is set, and above any level's `thinking_tokens` (at trainer construction and by the eval scripts). |
| `rollout_reasoning_end_token` | `str` | `"</think>"` | The token that closes the model's reasoning (Qwen3.x `</think>`, Gemma 4 `<channel\|>`). Under `rollout_thinking_budget_scope: episode` a turn's spend is the sampled ids up to and including it (all of them for a turn cut before closing its reasoning). Resolved through the tokenizer by the trainer and the eval scripts, which refuse a string that is not one of its tokens; the episode scope refuses it empty. |
| `rollout_stop_tokens` | `list[str]` | `[]` | Special-token strings that end a turn (resolved to the engine's `stop_token_ids`). Set to the model's tool-call terminator so a turn stops when the model emits its call; needed for a terminator that is not an eos (e.g. gpt-oss `<\|call\|>` under harmony-disabled serving). Empty = only eos stops a turn. |
| `rollout_chat_template_kwargs` | `dict` | `{}` | Chat-template variables sent with every rollout request (`chat_template_kwargs`) and applied to the trainer's own renders, so both sides see one template state. The stock Qwen3.x template reads `preserve_thinking`: with the env's `carry_reasoning`, the carried previous-turn reasoning stays rendered past a user message (a recovery nudge). `reasoning_effort` and `reasoning_budget` are refused here: both are per episode; the level travels top-level, the budget is added to the nested form per request. `reasoning_budget_scope` is refused too: it follows `rollout_thinking_budget_scope`, which adds it run-wide under the episode scope. |
| `train_on_sampled_tokens` | `bool` | `True` | Train on the server's actual sampled token ids (each assistant turn a training row sharing the trajectory advantage) instead of re-tokenizing a chat-template re-render, which eliminates re-tokenization mismatch. On vLLM this needs the server's `--return-tokens-as-token-ids` (passed by `docker-compose.vllm.yml`); on SGLang the capture is per-request and needs no server flag. Falls back to a single re-tokenized row for any trajectory with an uncaptured turn. |
| `max_train_row_tokens` | `int \| None` | `None` | Longest training row (prompt + completion tokens) a training rank takes; `None` = the model's context window. A per-turn row over it is left out of the batch while the episode's other turns still train; a whole-trajectory row over it trains as a zero-weight row. Logged as `sampling/rows_over_cap_frac`: rows left out over rows left out plus rows that train. A memory bound for the training ranks, below the context check (which rejects rows the served model could not have produced). Refused: a non-positive or non-int value, or one at or below `rollout_max_tokens`, since a row is prompt + completion and such a cap would leave out every turn that used its budget. |
...
| `truncation_alarm_rate` | `float \| None` | `0.25` | Warn when a rollout round's `episode/truncation_rate` rises over this, and log `episode/truncation_alarm` (1 over, 0 under) every round; the warning repeats only after the rate drops back under. Refused outside `[0, 1)`; `null` = off. |
| `advantage_mode` | `str` | `"mean"` | Group-baseline / negative-side advantage surgery: `mean` (plain GRPO baseline), `qae` (per-group quantile baseline), `asymmetric` (scale positive/negative advantages), `neg_mask_hard` (zero negatives in groups where no member's objective reward reached `advantage_hard_group_threshold`). See [advantages](../training-methods/grpo/async-grpo/objective.md#advantages). |
| `advantage_quantile` | `float` | `0.4` | Baseline quantile for `advantage_mode: qae`. Refused outside `(0, 1)`, whatever the mode. |
| `advantage_pos_scale` / `advantage_neg_scale` | `float` | `1.0` / `0.4` | Positive / negative advantage multipliers for `advantage_mode: asymmetric` (`neg_scale: 0` = full negative mask). Refused: non-finite or negative, whatever the mode. |
| `advantage_hard_group_threshold` | `float` | `0.5` | `neg_mask_hard`: a group is hard (negatives zeroed) when no member's objective reward component reaches this value. With no `environment` term in `rewards:` there is no objective component and the gate falls back to the **total** episode reward — a different scale. |
...
| `isr_geo_band_min` / `isr_geo_band_max` | `float \| None` | `None` | Trajectory geometric-mean band: mask a whole trajectory when `exp(mean log-ratio)` over its corrected tokens leaves the band. Set both to activate. Size it above the entropy-dependent vLLM↔trainer numerical floor (≈0.002 nats/token at sampling entropy 0.4, ≈0.04 at 1.0; see [the trust-region masks](../training-methods/grpo/async-grpo/objective.md#trust-region-masks)); a tighter band masks every step. |
| `isr_veto_min` | `float \| None` | `None` | Catastrophic-token veto: mask a whole trajectory when any corrected token's raw ratio falls below this. Refused outside `(0, 1)`. |
| `isr_opsm_delta` | `float \| None` | `None` | Off-policy sequence masking: mask negative-advantage trajectories whose \|mean log-ratio\| exceeds this many nats; positives never masked. Refused: `<= 0`. |
| `isr_engine_reference` | `bool` | `False` | Re-score every training row on the rollout engine under the just-synced weights and feed the mask stages the engine's current-vs-sampling log-ratio: the trainer↔engine disagreement drops out, the engine's own decode-vs-prefill disagreement (≈−0.003 nats/token at sampling entropy 0.36) stays in the reference and grows with entropy; the IS weight stays trainer-vs-sampling. Logs `sampling/engine_logratio_mean`, `sampling/numerics_logratio_mean`, `sampling/engine_rescore_coverage`, `sampling/engine_rescore_miss_frac`. Requires the IS correction and `rollout_temperature` / `rollout_top_p` of 1.0; vLLM re-scores through the completions `prompt_logprobs` echo, SGLang through `/generate` with `logprob_start_len`. The vLLM server needs headroom for the fp32 vocabulary log-softmax that pass materializes; sizing on [Rollout Servers](../infrastructure/rollout-servers.md) (`isr_engine_reference` headroom). See [the trust-region masks](../training-methods/grpo/async-grpo/objective.md#trust-region-masks). |
| `skip_update_masked_frac` | `float \| None` | `None` | Trust-region circuit breaker: when more than this fraction of IS-corrected trajectories are fully masked by the stages above, **or** more than this fraction of corrected tokens (the masked trajectories are the long ones), the round's advantages are zeroed and every optimizer step the generation round feeds is skipped: gradients dropped to `None`, so Adam's momentum cannot step the weights either; the KL term goes with the round. Logged as `sampling/update_skipped` with the two fractions (`sampling/is_masked_traj_frac`, `sampling/is_masked_token_frac`). Refused outside `(0, 1]`. Requires the IS correction **and** at least one mask stage (`isr_band`/`isr_geo_band`/`isr_veto`/`isr_opsm`); raises otherwise, since nothing would ever be masked. |
| `scale_rewards_std_floor` | `float` | `0.0` | Floor (reward units) on the advantage-scaling std divisor: divide by `max(std, floor)`, bounding degenerate-batch noise amplification. `0` = off. Refused: non-finite or negative. |
| `routing_replay` | `str` | `"none"` | MoE routing replay. `recompute` (R2) captures each EP MoE layer's top-k selection in the no-grad logprob-recompute pass and replays it in the update/GC forwards, re-deriving gate weights from live router scores; it **raises** in a config that never runs that pass (`num_iterations: 1`, aligned accumulation, IS correction off, `beta: 0`). `rollout` (R3) replays the rollout engine's per-turn routing to close the cross-engine gap; it needs `train_on_sampled_tokens` and a capture-capable server: vLLM ≥0.22 with `--enable-return-routed-experts` and a non-FlashInfer MoE backend (`--moe-backend triton`), or SGLang with `--enable-return-routed-experts` and `--moe-runner-backend triton` (the `triton_kernel`/flashinfer runners bypass the capture hook). EP-wrapped MoE only; Gemma4 and Zaya are rejected. |
| `episode_timeout` | `float` | `1200.0` | Deadline (seconds) for one whole rollout episode: generation + tool execution + grading, unlike `request_timeout`, which bounds a single HTTP call. Pause-aware: the seconds the engines spend paused for a weight sync are credited back, so a sync never times an episode out. A timed-out episode is canceled and counted in `episode/error_rate`, and a straggler holds its peers at the per-step collective for up to this long. It must stay under the NCCL watchdog (`DIST_NCCL_TIMEOUT_MINUTES × 60`): a larger value **raises when training starts**, and ≥80% of it warns. The default sits at two thirds of the 30-min watchdog, leaving ~10 min of margin; raise `DIST_NCCL_TIMEOUT_MINUTES` before raising it. Refused: non-finite or `<= 0`. |
...
| `num_prefetch_batches` | `int` | `1` | Bound on the prefetch result queue. The pipeline is one round deep by construction (one submit and one pop per round), so a larger value only adds headroom. Must be `>= 1`; set `enable_prefetch: false` to turn prefetching off (`0` would make the queue unbounded). |
| `use_chunked_grpo_logprobs` | `bool` | `False` | Compute completion log-probs from the hidden state + a vocab-chunked softmax instead of full `[B, T, vocab]` logits, bounding the loss-forward peak by chunk size, not `B·T·vocab`. For large-vocab models on long trajectories. See [Chunked log-probs](../training-methods/grpo/async-grpo/performance.md#chunked-log-probs). Off, construction refuses a full-logits plane the device's free memory cannot hold ([the logits wall](../training-methods/grpo/async-grpo/performance.md#the-logits-wall)). |
| `effort_length_penalty_k0` | `float \| None` | `None` | Coefficient, at the lowest effort level, of the capped reasoning-length price (`None` = off). Per episode: `-min(c_max, k(effort) × reasoning_tokens / l_norm)` with `k(effort) = k0 × exp(-(effort - effort_min) / tau)`; reasoning tokens only, summed over the assistant turns. The same trace costs most at the lowest level, and the cap keeps a long trace from outweighing the task reward. Logged as `reward/effort_length_penalty`. Refused when on: a non-finite or non-positive `k0`, `tau`, `c_max` or `l_norm`. |
| `effort_length_penalty_tau` / `_c_max` / `_l_norm` | `float` | `25.0` / `0.1` / `8192.0` | Effort units per e-fold of the coefficient; the price's cap in reward units; reasoning tokens per unit of price. |
| `effort_length_penalty_levels` | `dict[str, float]` | `{low: 25, medium: 50, high: 100}` | Scalar effort per categorical level for `k(effort)`; the lowest value is `effort_min`. Must map exactly the effort levels — a missing or invented level is refused at trainer construction. |
| `effort_length_floor_weight` | `float` | `0.0` | Weight of the reasoning under-use floor (`0` = off): an episode whose reasoning tokens, summed over its turns, fall short of `effort_length_floor_budgets ×` its thinking budget (per turn, or the episode's total under `rollout_thinking_budget_scope: episode`) pays `-weight × shortfall / floor`. The only length term that pays for more reasoning. An episode with no thinking budget is free of it; a run where none can have one (no level sets `thinking_tokens`, no `rollout_max_thinking_tokens`) is refused at trainer construction. Logged as `reward/effort_length_floor`. Finite and `>= 0`. |
| `effort_length_floor_budgets` | `float` | `0.75` | The floor's reference, in thinking budgets, against the episode's reasoning tokens summed over its turns. The budget is the level's per-turn one, or the episode's total under `rollout_thinking_budget_scope: episode`; the floor is this multiple of it, so it moves with the budgets and is set against them (the episode-scope Qwen3.6 code-contests recipes: `0.375` × 24,576–36,000 = 9,216–13,500 tokens). Below 1 by default, so an episode of a single assistant turn can clear its floor without running into the cap the engine enforces per turn. Finite and `> 0` when the floor is on. |
| `model_name` | `str \| None` | `None` | Model name for `/v1/chat/completions`. `null` is filled with `model_name_or_path` at script start, so a request always names a model. |
| `request_timeout` | `float` | `120.0` | HTTP timeout per rollout-server request (seconds). Pause-aware like `episode_timeout`: a request in flight across a weight sync is credited the pause, so a sync never expires it (`async/requests_expired_in_sync` counts the ones that overran even so). One request is one turn, so size it to at least twice the per-turn cap (thinking budget + answer headroom) divided by the per-sequence decode speed at peak concurrency ([Sizing a run](../training-methods/grpo/async-grpo/performance.md#sizing-a-run)); a slower turn fails its episode. Refused: non-finite or `<= 0`. |
| `max_retries` | `int` | `3` | Max retries of one turn's rollout-server request after a transient failure (exponential backoff); the turn is re-issued, never the episode. Refused: negative. |
| `retry_base_wait` | `float` | `1.0` | Base wait (seconds) for backoff. Refused: non-finite or negative. The whole retry budget — `(max_retries + 1) × request_timeout` plus backoff — is only **warned** about at 80% of the NCCL watchdog, never refused: past it a stuck rollout keeps retrying until the watchdog fires and hangs the per-step barrier instead of giving up. |
...
| `test_size` | `float \| None` | `None` | Test split proportion (e.g. `0.05`). Leave empty to keep the dataset's own split: set on a dataset that already carries a `test` split, the two are **concatenated and re-split** at this proportion, so a curated split is not preserved. Ignored with a warning on a sharded or pre-processed dataset, whose split was fixed at preparation time. |
...
RLVR Online GRPO. **Source:** `src/args/rlvr_online_grpo_args.py`
...
| `reasoning_effort` | `"low" \| "medium" \| "high" \| "random" \| None` | `None` | Chat-template reasoning-effort steer: `low`/`medium`/`high`, `random` (sampled per prompt), or `None`. Passed via `chat_template_kwargs`; needs a template that reads `reasoning_effort` (e.g. gpt-oss harmony). |
| `rewards` | `list[dict]` | `[{source: accuracy}]` | Reward terms, each `{source, name?, weight?, exponent?, ...}`; sources `accuracy`, `format`, `judge`, `reward_model`. One TRL reward function per term. YAML-only. See [Reward Terms](../training-methods/grpo/rewards.md). |
| `use_chunked_grpo_logprobs` | `bool` | `False` | Vocab-chunked completion log-probs instead of full `[B, T, vocab]` logits, for large-vocab models on long completions. See [Online GRPO](../training-methods/grpo/online-grpo.md#chunked-log-probs). Off, construction refuses a full-logits plane the device's free memory cannot hold; the check is skipped with `max_completion_length` unset ([the logits wall](../training-methods/grpo/async-grpo/performance.md#the-logits-wall)). |
| `use_rlrr` | `bool` | `False` | RLRR relative-reward shaping: advantages come from intra-group rankings instead of the group mean/std. Refused together with `advantage_mode`, `scale_rewards_std_floor > 0` or `drop_degenerate_groups` — each would replace the advantages RLRR just computed. |
...
| `opd_positive_advantage_only` | `bool` | `True` | Charge the OPD term on positive-advantage tokens only. Refused with `use_sdpg: false`. |

The tuning fields behind the two toggles — `rlrr_*` (`RLRRArguments`, `src/args/mixins.py`) and the SDPG set (`SDPGArguments` plus `opd_positive_advantage_only`) — are **refused** at a non-default value when their toggle is off rather than ignored; `rlrr_*` are additionally range-validated whether or not `use_rlrr` is on. Per-knob list: [RLVR Online GRPO](../training-methods/grpo/online-grpo.md#configuration).

It also inherits `AdvantageShapingArguments` (`src/args/mixins.py`): `advantage_mode`, `advantage_quantile`, `advantage_pos_scale`, `advantage_neg_scale`, `advantage_hard_group_threshold`, `scale_rewards_std_floor`, `drop_degenerate_groups`, with the same semantics as the [AsyncTrainingConfig](#asynctrainingconfig) rows and two differences. `drop_degenerate_groups` defaults **`false`** here (it is `true` in async GRPO, whose sparse verifiable reward makes dead groups dominate the batch).

`advantage_hard_group_threshold` gates on the **total weighted reward**: this arm carries no objective-reward decomposition, so set it for the combined scale (a run with more than one reward function warns).
...
| `answer_field` | `str \| None` | `"answer"` | Field with the expected answer, carried into the row as `answer` when the column exists. Whether an answer is needed is the environment's `requires_answer`; a value other than the default must name a real column or startup raises. |
| `context_fields` | `list[str] \| None` | `None` | Additional fields passed as context to the environment. |

There is no `system_prompt`: the environment builds the rollout conversation from its own system prompt plus the dataset's user turn, so a config setting the key raises; set it with `environment_kwargs: {system_prompt: ...}` ([Async GRPO](../training-methods/grpo/async-grpo/README.md)).
