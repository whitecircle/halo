...
| `HALO_WEIGHT_SYNC_BIND_ALL` | off | `1` puts the vLLM weight-sync rendezvous store on every interface instead of the advertised group address alone, for a trainer the server reaches through NAT or a port mapping; each group formation then logs a WARNING. Off, an advertised address this host cannot bind alone (not local, `0.0.0.0`, or a name resolving to loopback for a remote server) raises at group formation. The store is unauthenticated, so set it only where no untrusted host reaches the group port ([Group rendezvous](../infrastructure/rollout-servers.md#group-rendezvous)). SGLang's store listens on every interface regardless. |
