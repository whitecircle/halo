...
- `submission_reward`, `execution_progress_reward`, `resubmission_penalty` (`code_contests`, the shaping components, default `0`; the objective's shape is the `environment` term's `exponent` in `rewards`; see [Code Contests → Reward ladder](../training-methods/grpo/environments/code-contests.md#reward-ladder)).
