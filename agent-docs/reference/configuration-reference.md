...
- `eval_protocol` (`code_contests`, `harness` default / `leaderboard`): `leaderboard` pins one submission, no scratchpad and `outcome` verdicts, and a contradicting value raises ([Code Contests → Evaluation protocols](../training-methods/grpo/environments/code-contests.md#evaluation-protocols)).
