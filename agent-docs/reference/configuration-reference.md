...
- `verdict_detail` (`code_contests`, `outcome` default / `full`): `outcome` shows a failed test's verdict class alone (`Test i: FAIL`, `Test i: RUNTIME ERROR`), none of the program's output, stderr, exit code or output size; `full` adds them and the expected output. A compile error shows the compiler's first error in both.
