...
- `max_length_cutoff_recoveries` (every environment, `>= 0`; `null` = every one within `max_turns`): cap on the unproductive turns one episode may recover from — engine-cut, or ended with neither a tool call nor visible content; the turn past the cap ends the episode truncated, priced like a `max_turns` overflow. A reasoning-effort profile may tighten it per level, never above this value.
