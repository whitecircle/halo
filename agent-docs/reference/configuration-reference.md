...
- `max_length_cutoff_recoveries` (every environment, `>= 0`; `null` = every cut within `max_turns`): cap on the engine-cut turns one episode may recover from; the cut past the cap ends the episode truncated, priced like a `max_turns` overflow. A reasoning-effort profile may tighten it per level, never above this value.
