...
| `fsdp_reshard_after_backward` | `bool` | `True` | `False` keeps params unsharded across a grad-accum window's microsteps (torch `set_reshard_after_backward`, re-armed for the window's last backward): drops the full-model re-all-gather FSDP2 otherwise issues after every microstep's backward to one per optimizer step — about 4–10% throughput over NVLink, ~15 s × `gradient_accumulation_steps` per step when the trainer's NCCL runs over TCP sockets. Saves nothing at `gradient_accumulation_steps=1`. Costs one unsharded bf16 param copy per GPU for the run, which the ZeRO-2 forward/backward peak already holds. Plain-DP/CP/EP torchrun path only; rejected with `fsdp_reshard_after_forward=True`, TP, or PP. See [Data Parallelism](../parallelism/data-parallelism.md#zero-2-vs-zero-3-reshard_after_forward). |
...
| `max_train_row_tokens` | `int \| None` | `None` | Longest training row (prompt + completion tokens) a training rank takes; `None` = no cap beyond the context check. A per-turn row over it is left out of the batch while the episode's other turns still train; a whole-trajectory row over it trains as a zero-weight row. Logged as `sampling/rows_over_cap_frac`: rows left out over rows left out plus rows that train. A memory bound for the training ranks, below the context check (which rejects rows the served model could not have produced). Refused: a non-positive or non-int value, or one at or below `rollout_max_tokens`, since a row is prompt + completion and such a cap would leave out every turn that used its budget. |
