...
| `effort_length_floor_budgets` | `float` | `0.75` | The floor's reference, in thinking budgets, against the episode's reasoning tokens summed over its turns. The budget is the level's per-turn one, or the episode's total under `rollout_thinking_budget_scope: episode` — a recipe that doubles its budgets for that scope halves this to keep the same floor. Below 1 by default, so an episode of a single assistant turn can clear its floor without running into the cap the engine enforces per turn. Finite and `> 0` when the floor is on. |
