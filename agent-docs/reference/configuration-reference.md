...
| `HALO_NCCL_SYNC_TIMEOUT_SECONDS` | 120 s (warm-up) / 600 s (per broadcast) | Deadline for the weight-sync stream drain on both clients — vLLM's packed broadcasts and SGLang's per-arena settle before reuse; on expiry the group is aborted instead of parking the trainer, with a hint naming the transport causes (address, `NCCL_NET` / plugin mismatch, cuMem). Raise for very large policies over sockets. |
| `HALO_WEIGHT_SYNC_CHUNK_MB` | `1024` | Budget of one streamed weight-sync chunk, staged on the forwarding rank's sync GPU (plus the largest single tensor; the SGLang client keeps two device arenas of this size). Each chunk is one engine round trip, which is what bounds the vLLM path: measured over EFA for a 16 GB model, 1 / 2 / 4 / 8 GiB chunks push at 54 / 63 / 70 / 73 GB/s; the SGLang path reaches the fabric's rate at the default. |
...
| `NCCL_CUMEM_ENABLE` | `1` in `docker-compose.sglang.yml` | Read by NCCL in the **SGLang server**: its engine entry point sets `0` process-wide unless the variable is pre-set, the trainer's NCCL has cuMem on, and the mismatch fails the first cross-container buffer import (`ncclP2pImportShareableBuffer ... invalid argument`). Keep both ends equal — the compose default does. |
...
| `rollout_backend` | `"vllm" \| "sglang"` | `"vllm"` | Rollout engine for generation and weight sync. `sglang` supports `routing_replay: rollout` (serve with `--enable-return-routed-experts --moe-runner-backend triton`), rejects `rollout_max_thinking_tokens` at config time, refuses every MoE family but GptOss (the fused expert layout), and must be served from the NCCL-aligned `Dockerfile.sglang` image with `NCCL_CUMEM_ENABLE=1` (the compose default) — see [Rollout Servers](../infrastructure/rollout-servers.md). |
