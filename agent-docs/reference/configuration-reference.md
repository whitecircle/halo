...
| `retry_base_wait` | `float` | `1.0` | Base wait (seconds) for backoff. Refused: non-finite or negative. |
...
The tuning fields behind the two toggles — `rlrr_*` (`RLRRArguments`, `src/args/mixins.py`) and the SDPG set (`SDPGArguments` plus `opd_positive_advantage_only`) — are range-validated whether or not their toggle is on, and the script **refuses** a non-default value set with the toggle off rather than ignoring it. Per-knob list: [RLVR Online GRPO](../training-methods/grpo/online-grpo.md).
