...
- `verdict_detail` (`code_contests`, `outcome` default / `full`): `outcome` shows a failed test's verdict class alone (`Test i: FAIL`, `Test i: RUNTIME ERROR`), none of the program's output, stderr, exit code or output size, nor an infra error's text (logged); `full` adds them and the expected output. A compile error shows the compiler's first error under `full`, and under `outcome` on `local` / `bubblewrap` only. `stop_on_first_failure` narrows the verdict to the first failing test.
...
| `use_chunked_grpo_logprobs` | `bool` | `False` | Compute completion log-probs from the hidden state + a vocab-chunked softmax instead of full `[B, T, vocab]` logits, bounding the loss-forward peak by chunk size, not `B·T·vocab`. For large-vocab models on long trajectories. See [Chunked log-probs](../training-methods/grpo/async-grpo/performance.md#chunked-log-probs). |
...
| `request_timeout` | `float` | `120.0` | HTTP timeout per rollout-server request (seconds). Not pause-aware: a turn in flight during a weight sync waits out the pause inside this budget, so it must exceed the longest turn plus one sync. One request is one turn, so size it to at least twice the per-turn cap (thinking budget + answer headroom) divided by the per-sequence decode speed at peak concurrency ([Sizing a run](../training-methods/grpo/async-grpo/performance.md#sizing-a-run)); a slower turn fails its episode. Refused: non-finite or `<= 0`. |
| `max_retries` | `int` | `3` | Max retries for transient rollout-server failures (exponential backoff). Refused: negative. |
