...
| `HALO_NCCL_SYNC_TIMEOUT_SECONDS` | 120 s (warm-up) / 600 s (per broadcast) | Deadline for the weight-sync stream drain on both clients — vLLM's packed broadcasts and SGLang's per-chunk broadcasts; on expiry the group is aborted instead of parking the trainer, with a hint naming the transport causes (address, `NCCL_NET` / plugin mismatch, cuMem). Raise for very large policies over sockets. |
| `HALO_WEIGHT_SYNC_CHUNK_MB` | `1024` | Host-side budget of one streamed weight-sync chunk (pinned host memory on the forwarding rank, plus the largest single tensor; the SGLang client also holds a chunk's uploads on the sync GPU until its broadcasts drain). Each chunk is one engine round trip. Measured over EFA against vLLM: `2048` pushed a 16 GB model 15% faster than the default, `4096` slower than it. |
