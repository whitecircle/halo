...
| `src/models/segment_markers.py` | which families' conv / linear-attention mixers read per-document segment markers, the GatedDeltaNet kernel refusal, and the markers built from a row's `position_ids` | the packing collators (through the factory) and SMPO's padding-free forward |
...
- [Parallelism](../parallelism/README.md) — EP, CP, TP, ETP, PP and valid combinations
