...
| **Gemma 4 environmental GRPO trains nothing** — `sampling/is_masked_traj_frac` 1, `sampling/update_skipped` 1, `sampling/logratio_mean` ≈ −0.7, on both engines | Open issue. On this env's multi-turn tool trajectories the trainer's log-probs of the engine-sampled tokens sit about 0.7 nats below the sampling log-probs the engine returned — far outside the shipped `isr_geo_band_min`/`isr_geo_band_max` `[0.95, 1.05]` — so `skip_update_masked_frac` zeroes every step; a plain-text prefill agrees with both engines within routing noise. | No workaround. Every shipped `examples/grpo/environmental/gemma4/*/*-code-contests-*.yaml` hits it; widening the band is not the fix. | [Gemma 4](../models/gemma4.md) |
