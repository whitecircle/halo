...
| **Async GRPO with Environments: completions fill with `Error: <tool>: missing a required argument: 'code'`**, episodes loop to `max_turns`, `episode/length_cutoff_turns` stays near zero | The thinking budget cut the turn mid-thought; the model resumed the thought inside its tool call's code argument until `max_tokens`, and vLLM labeled the salvaged call (`{}` arguments) a completed `tool_calls` finish. | Auto-handled: both rollout drivers read the cap off `usage.completion_tokens`, so the turn takes the nudge-and-retry path and its fragment is untrainable. If the count stays high, the level's `thinking_tokens` or — on vLLM, which is where `rollout_max_thinking_tokens` applies — the answer headroom (`rollout_max_tokens - rollout_max_thinking_tokens`) is too small for the task. | [Native tool-use](../training-methods/grpo/environments/native-tool-use.md#reward) |
| **Async GRPO with Environments: `episode/empty_turns` climbs**, episodes loop to `max_turns` while `episode/length_cutoff_turns` stays near zero | The model closes its reasoning and stops with neither a tool call nor an answer, below its budget. Each such turn is nudged and retried like an engine cut, and at `max_length_cutoff_recoveries: null` an episode can spend every turn this way. | Cap the recoveries (`max_length_cutoff_recoveries`, one cap for cuts and empty turns) and price them (`length_cutoff_penalty`); the turn itself is untrainable, so the price is the only gradient against it. | [Native tool-use](../training-methods/grpo/environments/native-tool-use.md#reward) |
...
| **`Dataset-map cache key cannot hash the content of tokenizer ... and keys on its name_or_path ... instead`** at the dataset map | Reading the tokenizer's content raised — a custom Python component the fast backend cannot serialize, or a remote-code `get_vocab` — so the map cache keys on the tokenizer's path. A resume that loads it from the run's checkpoint then re-runs every map, and a tokenizer changed in place at the same path reuses the previous run's rows. | Nothing to do for an unchanged tokenizer beyond the re-map on resume. After editing one in place, save it to a new path or clear `HF_DATASETS_CACHE`. | [Data Loading](../parallelism/data-loading.md) |
...
| **`net.ipv4.ip_local_port_range starts at N, which leaves K test worker(s) no port`** from a test | `tests/common/ports.py` hands out test ports between 20000 and the kernel's ephemeral range, so no kernel-picked port can take one; this host's range starts less than one port per xdist worker above 20000. | Move the range up to the Linux default: `sysctl -w net.ipv4.ip_local_port_range="32768 60999"` on the host (a `--network host` container shares it), or `docker run --sysctl net.ipv4.ip_local_port_range="32768 60999"` for a container with its own network. | [Contributing](../contributing/README.md#tests) |
...
| **`weight-sync group address ... is not a single address of this host that the server ... can reach`** at sync init | The advertised group address is one the trainer cannot bind alone (a NAT or port-mapped address, `0.0.0.0`, or a hostname resolving to loopback for a remote server), and the unauthenticated rendezvous store listens only on the address it advertises. | Set `VLLM_GROUP_HOST` / `SGLANG_GROUP_HOST`, or the entry's `group_host`, to this host's IP on an interface the server reaches. Behind NAT or a port mapping, set `HALO_WEIGHT_SYNC_BIND_ALL=1`, on a network where only trusted hosts reach the group port. | [Rollout Servers](../infrastructure/rollout-servers.md#group-rendezvous) |
| **`weight-sync group address ... has no IPv4 address on this host`** at sync init | The group address is an IPv6 literal or a name without an IPv4 address; the rendezvous store listens on IPv4 only, so IPv6 is unsupported. | Set `VLLM_GROUP_HOST` / `SGLANG_GROUP_HOST`, or the entry's `group_host`, to this host's IPv4 address on an interface the server reaches. | [Rollout Servers](../infrastructure/rollout-servers.md#group-rendezvous) |
...
| **`Could not verify the head path of <ModelClass> (...)`** at GRPO trainer construction with `use_chunked_grpo_logprobs: true`, or at the pipeline split | The chunked log-probs and the last pipeline stage compute logits from the hidden state, so the family's forward is first probed on a meta-device shell with a stubbed backbone and a stand-in output embedding. The probe could not run it; the parenthesized error says why (an output embedding that is not a plain `nn.Linear`, a `base_model` that is the model itself, a forward reading other model state on its head path). | Set `use_chunked_grpo_logprobs: false`. For a family you are adding, make its head path pass the probe. | [Adding a Model](../models/adding-a-model.md#declare-the-head-transform) |
| **`<ModelClass>'s forward transforms its head path beyond what is declared for it`**, at the same points | The forward applies something between the backbone and the logits (a hidden-state scale, a logit scale, a softcap, a vocabulary cut) that the family's `HeadTransformSpec` does not declare, so the hidden-state paths would score a different distribution than the model samples from. | Declare the transform on a `HeadTransformSpec` in `src/models/head_transform.py`, or set `use_chunked_grpo_logprobs: false`. | [Adding a Model](../models/adding-a-model.md#declare-the-head-transform) |
| **`The GRPO loss forward materializes full logits: …`** (`RuntimeError`) at GRPO trainer construction, or `Another rank's full-logits plane cannot fit` | With `use_chunked_grpo_logprobs` off, the loss forward holds a `rows × width × vocab` logits plane at 4 B a logit, and this device's free memory (or a peer's) cannot hold it. The message states the size and the batch-size and width settings behind it. | Set `use_chunked_grpo_logprobs: true`, or lower the batch size or the width setting the message names. | [Memory and Throughput](../training-methods/grpo/async-grpo/performance.md#the-logits-wall) |
...
| **`rollout_max_thinking_tokens`, `rollout_thinking_budget_scope: episode` or `carry_reasoning` refused** with `rollout_backend: sglang` | None is available on SGLang: the thinking budget (per turn or per episode) has no server-side mechanism, and the reasoning carry is refused until SGLang's forwarding of `reasoning_content` is verified. | Drop the knob, or run that recipe against vLLM. | [Async GRPO](../training-methods/grpo/async-grpo/setup.md#rollout-backend) |
| **`rollout_thinking_budget_scope='episode' would leave episodes with nothing to share`** or **`rollout_thinking_turn_reserve (N) exceeds the thinking_tokens of {…}`** at trainer construction or before an eval's first episode | With `rollout_max_thinking_tokens` unset an episode's budget is its level's `thinking_tokens` alone, so the environment must set `reasoning_effort` and every level a budget; no level's budget may sit below the reserve a spent turn keeps. | Set `reasoning_effort` and every level's `thinking_tokens`, or set `rollout_max_thinking_tokens`; keep `rollout_thinking_turn_reserve` at or below the smallest level budget. | [Rollouts](../training-methods/grpo/async-grpo/rollouts.md#reasoning-budget) |
...
| **`pytest's basetemp is too deep for the AF_UNIX sockets the GPU tests bind`** from the GPU test launcher | An AF_UNIX socket path caps at 108 bytes, and `datasets` binds a multiprocessing socket (`pymp-…/listener-…`, 32 bytes) under `TMPDIR` while it tokenizes; the launcher reserves 40 bytes for it. It gives each test a `TMPDIR` under pytest's basetemp, which sits under the host's `TMPDIR`, and moves one too deep for the socket to a short directory beside it; when even that leaves too few bytes, it refuses the run. | Point `TMPDIR` (or `--basetemp`) at a shorter path on the scratch volume; the message names how many of the 40 bytes the current one leaves. | [Contributing](../contributing/README.md#tests) |
...
| Expert LoRA `lora_r` not a multiple of 8 | Rejected when the EP layer builds its adapters | `torch._grouped_mm` reads the adapter's rank dimension as a stride spanning a multiple of 16 bytes (`GROUPED_MM_STRIDE_ALIGNMENT_BYTES`), and the adapter GEMMs run at bf16 at the narrowest; any other rank fails at the first expert matmul with `strides should be multiple of 16 bytes`. Use a multiple of 8 |
...
| `lora_target_modules` matching only modules no stock LoRA adapter fits | Rejected at PEFT setup | A wrapper holding its weight in a child module (a multimodal tower's `Gemma4ClippableLinear`) or a layer subclass whose forward is not its base layer's affine map (DeepSeek-V4's grouped `o_a_proj`) is excluded from injection; excluding every match would leave the adapter run training nothing. Name the backbone's own projections, or use `all-linear` ([PEFT](../optimization/peft.md#targets-peft-cannot-adapt)) |
...
`ep_scope: global`. The full matrix is in [Parallelism](../parallelism/README.md) and the per-mode
