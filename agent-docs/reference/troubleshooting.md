...
| **`net.ipv4.ip_local_port_range starts at N, which leaves K test worker(s) no port`** from a test | `tests/common/ports.py` hands out test ports between 20000 and the kernel's ephemeral range, so no kernel-picked port can take one; this host's range starts at or below 20000. | Move the range up to the Linux default: `sysctl -w net.ipv4.ip_local_port_range="32768 60999"` on the host (a `--network host` container shares it), or `docker run --sysctl net.ipv4.ip_local_port_range="32768 60999"` for a container with its own network. | [Contributing](../contributing/README.md#tests) |
