...
| **`OSError: AF_UNIX path too long`** in a `SyncManager` process, then `EOFError` out of `Map (num_proc=N)` | `datasets.map` with `num_proc > 1` starts a manager that listens on an AF_UNIX socket under `$TMPDIR` (`pymp-*/listener-*`, 32 characters deeper). Linux caps a socket path at 108 bytes including the terminating NUL, so a `TMPDIR` longer than 75 characters fails. | Keep `TMPDIR` short: a top-level directory on the large volume (`$D/tmp` in the standard launch), or a short symlink to a deep one. | [Docker](../infrastructure/docker.md#standard-training-launch) |
