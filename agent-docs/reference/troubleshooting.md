...
| **`pytest's basetemp is too deep for the AF_UNIX sockets the GPU tests bind`** from the GPU test launcher; outside it, a `datasets` map that logs `OSError: AF_UNIX path too long` and dies on a bare `EOFError` | An AF_UNIX socket path caps at 108 bytes, and `datasets` binds a multiprocessing socket (`pymp-…/listener-…`, about 40 bytes) under `TMPDIR` while it tokenizes. The launcher gives each test a `TMPDIR` under pytest's basetemp, which sits under the host's `TMPDIR`, and moves one too deep for the socket to a short directory beside it; when even that leaves too few bytes, it refuses the run. | Point `TMPDIR` (or `--basetemp`) at a shorter path on the scratch volume; the message names how many of the 40 bytes the current one leaves. | [Contributing](../contributing/README.md#tests) |
