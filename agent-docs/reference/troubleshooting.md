...
| Attention LoRA on Qwen3.5/3.6 MoE under environmental GRPO: `aten.embedding.default got mixed torch.Tensor and DTensor` at the first training step | Open issue | Full fine-tuning of the same model trains and syncs on either engine. The shipped `examples/grpo/environmental/qwen3_5/{vllm,sglang}/qwen3.6-35b-a3b-code-contests-lora-*.yaml` share the failing shape and have not been run through it. |
| Gemma 4 under environmental GRPO on multi-turn tool trajectories (the code-contests configs): every trajectory IS-masked (`sampling/is_masked_traj_frac` 1, `sampling/logratio_mean` −0.6 to −0.8, `sampling/update_skipped` 1), the run completes without training | Open issue, both engines | A plain-text forward and the single-turn e2e agree with both engines within MoE routing noise, so the trainer scores the sampled tokens under a different context than the engine sampled under — the re-render of earlier turns (thinking channel, tool calls) is the suspect. Watch those three metrics on any Gemma 4 RL run. |
