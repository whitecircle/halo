...
- **Async multi-turn RL** with Ray actors, NCCL weight sync, and built-in environments.
...
| Multi-node | **Blackwell tested** | Via Accelerate | Yes | Paid tiers | Yes | Yes | Yes | Yes |
...
- **Megatron-LM** adds deeper TE/FP8 optimization for 1000+ GPU dense pre-training; its alignment is split across four repos and needs MCore conversion. This toolkit's pipeline parallelism is [not yet available](../parallelism/pipeline-parallelism.md) (the seams ship, the engine lands in a future release); on large NVLink domains FSDP2 + EP/TP avoids the pipeline bubble — see [When PP is worth it](../parallelism/index.md#when-pipeline-parallelism-is-worth-it).
...
[Supported Models](../models/index.md), and [Adding a New Model](../models/adding-a-model.md) is the
procedure.
...
!!! warning "vLLM runs as a separate Docker container"
