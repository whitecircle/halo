...
Streams also shape **memory**: the caching allocator pools freed blocks *per stream*, since reuse within a stream is safe by ordering alone while cross-stream reuse would need per-block event tracking. So allocations made on a short-lived stream are stranded when it dies — the reason the RL weight broadcast (§10) keeps one persistent stream pair (vLLM) or send stream (SGLang) per server client ([Rollout Servers](../infrastructure/rollout-servers.md#weight-sync)) — and `reserved` far above `peak_alloc` in a memory reading means stranded pools, not live tensors ([Debugging](debugging.md#3-gpu-memory-profiling)).
...
The weight sync itself is a bandwidth question with the same shape as §9's ladder: a full-model broadcast (~42 GB at 20B) is cheap over NVLink and still ~1–2 s over host loopback, a tier below the ladder's bottom rung. Across nodes it rides the fabric: ~53 GB/s over EFA against ~10 GB/s on sockets, under a second per sync at 20B ([Rollout Servers](../infrastructure/rollout-servers.md#servers-on-other-nodes-efa)). Throughput anatomy and the tuning order live in [Environmental GRPO](../training-methods/grpo/environmental-grpo.md#throughput-tuning).
