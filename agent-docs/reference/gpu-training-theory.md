...
The weight sync itself is a bandwidth question with the same shape as §9's ladder: a full-model broadcast (~42 GB at 20B) is cheap over NVLink and still ~1–2 s over host loopback, a tier below the ladder's bottom rung. Across nodes it rides the fabric: measured 54 GB/s (vLLM client) and 73 GB/s (SGLang client) over EFA against 9.7 GB/s over sockets on the ENA — 0.8 s / 0.6 s / 4.3 s per sync at 20B ([Rollout Servers](../infrastructure/rollout-servers.md#servers-on-other-nodes-efa)). Throughput anatomy and the tuning order live in [Environmental GRPO](../training-methods/grpo/environmental-grpo.md#throughput-tuning).
