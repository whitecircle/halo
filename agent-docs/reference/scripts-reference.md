...
| `scripts/profiling/weight_sync_transport.py` | Weight-sync transport preflight (driver `src/diagnostics/weight_sync_transport.py`). Run from the trainer container as the trainer would launch (same image, devices, NCCL env) on a GPU the server does not own: forms the group against a live rollout server (`--server-url` required; `--backend vllm\|sglang`, `--group-port` default `0` = ephemeral, `--group-host` default auto, `--device` default `cuda:0`, `--connection-timeout` 120 s), pushes one real parameter of the served checkpoint (`--param`, default `model.embed_tokens.weight`, unchanged value so the served model is unchanged; `--model-id` overrides the checkpoint the server reports) over `--rounds` (default 3), and reports the transport NCCL formed on (`NET/Libfabric/…/GDRDMA` = EFA with GPUDirect, `NET/Socket`, `P2P/CUMEM` = same-host CUDA IPC), the `aws-ofi-nccl` build string, the libfabric provider and GB/s. `--expect efa\|socket\|p2p` makes it a gate — exit 1 on a mismatch, when the push altered the served model, or when the group fails to form; `--json` prints the full result ([Rollout Servers](../infrastructure/rollout-servers.md#servers-on-other-nodes-efa)) |
