...
byte-bounded chunks (`HALO_WEIGHT_SYNC_CHUNK_MB`, 1 GiB), never the model and never host RAM. The
forwarding rank's peak is the assembled EP layer being sent plus the chunk, the largest tensor's
snapshot and the engine path's buffers — each at least the chunk budget, grown past it only when a
