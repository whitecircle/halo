...
What the toolkit can and cannot do for very large models, long sequences, multi-week runs, huge
...
byte-bounded chunks (`HALO_WEIGHT_SYNC_CHUNK_MB`, 1 GiB), never the model and never host RAM.

The forwarding rank's peak is the assembled EP layer being sent plus the chunk, the largest tensor's
snapshot and the engine path's buffers. Each is at least the chunk budget, grown past it only when a
tensor exceeds it (a fused expert pair on a 100B+ MoE)
([Rollout Servers](../infrastructure/rollout-servers.md#weight-sync)). The ceiling is the
fabric behind one GPU (its NICs over EFA) and the single-producer fan-out below.
...
LoRA; PP is refused by the pipeline mixin. Dense, FSDP2, CP and TP have no parallel-write mode at all.

`merge_ep_shards.py` reassembles the artifact, streaming group by group: peak host RAM is one merged
MoE layer plus one pending output shard, not the model. `--delete_input_shards` frees the per-rank
shards once the merged checkpoint is complete.
...
| **Weight sync fans out from one producer** | Online and async GRPO broadcast updated weights from the trainer's forwarding rank to every rollout server, and the push time grows with the server count (the fan-out costs the sum of the pushes, not the slowest) — fine for a handful of servers, not a large inference fleet. | Keep the inference fleet modest, or shard weight sync (roadmap). |
...
| **Tokens per rank through the DeepEP dispatch** | One rank's tokens per MoE forward are capped by DeepEP's 32-bit wire index, and lower when the EP group crosses NVLink domains. | Batch 1 or a shorter `max_length`; for the cross-domain cap, node-local EP with DP across nodes. Ceilings, where each is enforced, and the override in [Dispatch wire-index limit](../infrastructure/deepep.md#dispatch-wire-index-limit-token-count-ceiling) and [EP over EFA](../infrastructure/deepep.md#expert-parallelism-over-aws-efa). |
| **Context parallelism is narrow** | CP runs only under SFT and SMPO, and is refused at load for families with sequence-mixing layers or no CP wrapper. Without it, the longest sequence is one rank's activation budget. | Requirements, including the head-count rule, in [Context Parallelism](../parallelism/context-parallelism.md#requirements). |
...
| **Async GRPO trajectories are never truncated** | A trajectory grows across turns up to the context window; a row over it raises on every rank and ends the run. | Budget the episode under the served `max_model_len`: [Context budget](../training-methods/grpo/async-grpo/performance.md#context-budget). |
