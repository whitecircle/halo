...
Dispatch is server-state-blind — `RolloutManager._next_url` is plain round-robin, so a server mid-sync still takes its turn. Watch `async/prefetch_hit_rate` (target > 0.8); `< 0.5` means the round's rollouts do not finish inside a training step — add servers, or raise `max_concurrent_rollouts` / `num_rollout_workers`. Raising `num_prefetch_batches` cannot help: the loop submits one round and pops one round, so the queue never holds more than a single entry. Prefetch makes each batch one sync stale (more if `sync_weights_every_n_steps > 1`), but that off-policy lag is corrected by the truncated vLLM importance-sampling ratio ([Training on sampled tokens](#training-on-sampled-tokens-train_on_sampled_tokens-default-on)).
...
`rollout_backend` selects the engine: `vllm` (default) or `sglang`. Both serve rollouts over `/v1/chat/completions`, receive weights over an NCCL group the trainer joins as rank 0, and support `train_on_sampled_tokens` through shared gather/merge/gate code. Both receive the family's own `gather_expert_state_dict` layout; what differs is which families each pinned release can take an online update for at all, refused per pair at construction with the engine's loader reason. That roster, engine capabilities, required serving flags, and the SGLang-only constraints (weight sync needs this repo's `Dockerfile.sglang` image; `rollout_max_thinking_tokens` rejected; `NCCL_CUMEM_ENABLE=1` in the server container) are on [Rollout Servers](../../infrastructure/rollout-servers.md#which-families-each-engine-serves).
...
A budget needs a reasoning parser on the server (Qwen3.x `qwen3`; Gemma 4 `gemma4`; gpt-oss the bundled plugin) and `VLLM_USE_V2_MODEL_RUNNER=0` on the vLLM container — [Rollout Servers](../../infrastructure/rollout-servers.md#vllm); sending one without a parser is rejected (400). The per-effort profile sends one on its own: with `rollout_max_thinking_tokens` unset, `code_contests`/`codeforces` still bind the level's 4k/8k/16k and every vLLM request carries `thinking_token_budget`, so those runs need the parser too. Reasoning is unbounded only where neither the knob nor a per-level profile applies.

A turn the engine cuts short — at its token cap, read off the completion's token count as well as the engine's label, or by aborting it (`finish_reason: abort`) — is nudged and retried within `max_turns` and counted in `episode/length_cutoff_turns`, whatever tool call the parser salvaged from it, but carries **no reward penalty of its own** — bound its frequency through the per-effort caps and the answer headroom instead ([Native tool-use](environments/native-tool-use.md#reward-knobs)).
...
Turns the rollout marked unusable — engine-cut (`truncated`: the token cap, or an engine abort, which is SGLang's pause mode and any engine restart) or every tool call naming a nonexistent tool (`calls_rejected`) — are **excluded** from the training rows. They stay in the next turn's prompt, since the model must condition on what it actually emitted, but training on them would reinforce the runaway or the invented call whenever the episode later recovers and earns a positive advantage. An episode whose assistant turns were **all** excluded yields one fully masked row (`completion_mask` all zero), keeping the row count rank-uniform.

Two fallbacks fire when capture is incomplete, and only one is loud. A trajectory where **any** assistant turn lost its completion ids drops whole to the single-row re-tokenize path with a one-time warning naming the engine's remedy — all-or-nothing, so a partial capture never silently drops turns. A turn that kept its completion ids but lost its `prompt_token_ids` re-renders **only its prompt** through the serving template, silently; a prefix the template rejects invalidates that episode rather than the batch.
...
The reference must be the **sampling distribution's** logprobs: the trainer scores at `rollout_temperature`, so the engine's values must already carry it — vLLM only does so under `--logprobs-mode processed_logprobs` (its default is pre-temperature; the training script probes every server at startup and refuses a raw one at any temperature ≠ 1), SGLang does by default ([rollout servers](../../infrastructure/rollout-servers.md)). A token the engine reports at probability 1 — a `</think>` the thinking budget forced, a nucleus collapsed onto one token — was no sampling choice, so it runs uncorrected at ratio 1 (`sampling/sampler_certain_frac`) and cannot trip a band or the veto. Under a nucleus-renormalized reference, top-p < 1 shifts every uncertain position by its nucleus mass, which the geometric band reads as drift: keep `rollout_top_p: 1.0` with the band on (also probed and refused).
...
No boundary is ever inferred from another turn, since a role header's token length is not turn-invariant (BPE merges its last token with the turn's first content token differently per turn). So a turn whose own context does not anchor — consecutive assistant messages, which `_add_action_message` cannot produce — and a template that rejects its own trajectory both **invalidate that episode** (a fully masked row, out of its group's baseline), never trained on a guessed span. The failure is data-local, so the run continues; a systematic cause reaches the all-invalid step halt below.
...
Three optional knob groups on `AsyncTrainingConfig` (all default off) target the engine↔trainer mismatch and failure-dominated batches. Each is validated at construction; the IS mask stages raise unless the IS correction is active, since without it they would silently do nothing.
...
`skip_update_masked_frac` is the circuit breaker over these stages. It reads two global fractions — IS-corrected trajectories fully masked (`sampling/is_masked_traj_frac`) and corrected tokens masked (`sampling/is_masked_token_frac`) — and trips when either exceeds the threshold: the masked trajectories are the long ones, so a trajectory count alone reads a step that lost most of its tokens as healthy. A tripped step zeroes its advantages **and skips its optimizer step** (gradients dropped to `None` at pre-optimizer-step, so no parameter is stepped): a zero gradient alone still moves the weights, since Adam keeps stepping on momentum for ~1/(1−β₁) step-equivalents after the last real gradient. The KL term is dropped with the step; the scheduler still advances; the next weight sync ships unchanged weights and the rollouts re-anchor. Logged as `sampling/update_skipped`.

The bands sit on top of a **numerical floor**: the trainer and the engine are different bf16 stacks, and under the sampler's own tokens their mean log-ratio is −KL(sampler ‖ trainer) — always negative, and steeper the flatter the distribution. Measured on Qwen3.6-35B-A3B with identical weights on both sides, the floor is ≈0.002 nats/token at a mean sampling entropy of 0.4 and ≈0.04 at 1.0 (0.15 on tokens above 2 nats). A band tighter than the floor of the entropy range the run holds masks every step with no policy movement at all, which the breaker then turns into a permanent skip. Size `isr_geo_band` above that floor (±0.05 for a run holding entropy ≤ 0.7) and treat a sustained `update_skipped: 1` at low entropy as a structural disagreement (weight sync, template, kernels) to fix, not a band to widen.

`isr_engine_reference: true` takes the floor out of the mask stages instead. After each weight sync the trainer re-scores every training row on the rollout engine (one prefill per row — vLLM's completions `prompt_logprobs` echo, SGLang's `/generate` with `logprob_start_len` — a trajectory's rows on one server so its turns share the prefix cache) and hands the geometric band, the veto and OPSM the log-ratio between the engine's **current** (prefill) and **sampling** (decode) log-probs. That takes the trainer↔engine disagreement out of the stages, and the run logs the split every step: `sampling/numerics_logratio_mean` (trainer − engine-prefill over re-scored tokens) next to `sampling/engine_logratio_mean` (engine-prefill − engine-sampling) and `sampling/engine_rescore_coverage`. What it cannot remove is the engine's own decode-vs-prefill disagreement, which stays inside the reference: with identical weights on both sides (a learning-rate-0 step) Qwen3.6-35B-A3B on vLLM reads −0.003 nats/token engine-prefill vs engine-sampling against −0.0003 trainer vs engine-prefill at sampling entropy 0.36 — the floor is mostly the engine's two kernel paths, and it grows with entropy like any such floor. Size the band above it as before (±0.02 has room at entropy ≤ 0.5). The IS weight itself stays trainer-vs-sampling: that ratio is an unbiased correction whatever its variance. The re-score needs server headroom the engine does not profile for — size it before enabling ([Rollout Servers](../../infrastructure/rollout-servers.md#throughput)). The shipped code-contests configs leave it off and run the ±0.05 band. A row whose request fails keeps the trainer diff (`sampling/engine_rescore_miss_frac`; a step with no successful request logs an error and trains on the trainer reference — the re-score never raises, since a rank-local raise there would desync the collectives behind it). Refused at construction without the IS correction and at any `rollout_temperature` / `rollout_top_p` other than 1 (the prefill returns the raw distribution, the sampling log-probs the processed one).
...
A rollout carrying no learning signal — a raised episode, an `episode_timeout` cancellation, a trajectory the chat template cannot re-render, or one the environment flagged `episode_invalid` such as a grading-infrastructure outage — enters as a zero-masked row excluded from its group's baseline, so it neither trains nor poisons the advantages.
...
It works under FSDP2 (the `lm_head` weight is gathered with `full_tensor()`, differentiable, so the sharded parameter still gets its gradient), ep1/EP and TP. Rows forwarded one at a time (`per_device_train_batch_size: 1`, the shipped code-contests shape) are trimmed to their real span before the backbone forward and the head sweep on every attention path — FA4 needs it for its positions and collective counts, and everywhere else a padded row would cost its rank's widest prompt+completion in every layer (quadratic in the full-attention layers) plus the padded completion width in the sweep; the multi-turn per-turn rows differ in length by an order of magnitude, so this is most of the training pass. Batched rows (`per_device_train_batch_size > 1`) keep the padded path on every implementation but FA4, which always trims. Text-only — multimodal inputs fall back to the full path, and that choice is all-reduced across ranks so a mixed batch keeps every rank on one collective path. The same flag exists on the online trainer ([RLVR Online GRPO](online-grpo.md#chunked-log-probs)); it does not apply to offline GRPO.
...
| `sampling/*` | engine↔trainer agreement | `is_correction_active`, `is_correction_coverage` (fraction of policy tokens with vLLM logprobs; `< 1` = some rollouts errored and run at ratio 1), `is_ratio_mean` / `is_ratio_max` over surviving tokens (`max → 3.0` = truncation biting), `logratio_mean`, `sampler_certain_frac` (policy tokens the engine emitted at probability 1, run uncorrected); the mask fractions plus the rank-local union `is_masked_frac`, and the global `is_masked_traj_frac` / `is_masked_token_frac` / `update_skipped` when `skip_update_masked_frac` is set; under `isr_engine_reference` also `engine_logratio_mean` (staleness), `numerics_logratio_mean` (the trainer↔engine floor), `engine_rescore_coverage`, `engine_rescore_miss_frac`; `degenerate_group_frac`, `truncated_masked_frac`, `invalid_episode_frac` |
