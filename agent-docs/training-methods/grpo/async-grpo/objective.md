...
A non-finite reward (or advantage) raises on every rank rather than being zeroed: under `batch` scaling one such value would turn the shared std non-finite and zero the whole step.
