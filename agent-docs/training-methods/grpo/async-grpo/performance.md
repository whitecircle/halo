...
With chunked log-probs off, every GRPO trainer weighs the plane at construction, after the parallel wrap: `rows × (completion width + 1) × vocab × 4 B` (an fp32 plane, or bf16 logits plus the saved log-softmax) against the device's free memory. The rows are `per_device_train_batch_size`, or `per_device_eval_batch_size` when evaluation runs and it is larger. A plane that cannot fit raises on every rank; one over half the free memory warns, since the optimizer state and activations are not yet allocated. Both name the settings behind the rows and the width: `rollout_max_tokens` here, or under `train_on_sampled_tokens: false` `max_train_row_tokens` (the model's context window when unset), and `max_completion_length` online and offline, where an unset one goes unchecked, as does offline GRPO under PP. The estimate is a floor, not the step's peak: the rows are one forward chunk, a trajectory that falls back to one whole-trajectory row is wider than a turn, and a model the trainer leaves on the host (accelerate's own FSDP) is weighed against an empty device.

## Chunked log-probs

`use_chunked_grpo_logprobs: true` (default off) removes the wall: completion log-probs come from the backbone's `last_hidden_state` through a chunked (sequence × vocab) matmul, so peak follows the tile size, not `B·T·vocab`. Each tile is a tensor-core matmul with an fp32 result, and the backward's GEMMs accumulate in fp32, so the logits are never rounded to bf16 and the head costs about what the full-logits path does (Qwen3-8B, 8k tokens: 51 ms forward+backward on a B300). Every code-contests recipe sets it; the online and offline GRPO trainers take the same flag.
...
Worked shape, the 4-rank code-contests recipe: Qwen3.6-35B-A3B on 4 ranks and 2 servers, `gradient_accumulation_steps: 24`, `num_generations: 8` — 96 episodes per step (12 prompts), 48 per server; every profile's per-turn cap is 30,000 tokens: each level's episode budget exceeds the 18,000 ceiling, so a turn's total is that ceiling plus the 12,000 answer headroom. It sets `request_timeout: 1200` and `episode_timeout: 2700`; KV is 48 × 13k × 20 KB ≈ 12 GB at a 13k mean context, and 48 × 30k × 20 KB ≈ 27 GB once every request holds a turn at that cap.
