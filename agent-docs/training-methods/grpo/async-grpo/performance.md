...
With chunked log-probs off, every GRPO trainer weighs the plane at construction, after the parallel
wrap: `per_device_train_batch_size × (completion width + 1) × vocab × 4 B` (an fp32 plane, or bf16
logits plus the saved log-softmax) against the device's free memory. A plane that cannot fit raises on
every rank; one over half the free memory warns, since the optimizer state and activations are not yet
allocated. Both name the setting behind the width: `rollout_max_tokens` here, or under
`train_on_sampled_tokens: false` `max_train_row_tokens` (the model's context window when unset), and
`max_completion_length` online and offline, where an unset one goes unchecked. The estimate is a
floor, not the step's peak: the rows are one forward chunk, a trajectory that falls back to one
whole-trajectory row is wider than a turn, and a model the trainer leaves on the host (accelerate's own
FSDP) is weighed against an empty device.
