...
| `sampling/*` | engine/trainer agreement | `logratio_mean`, `is_ratio_mean` / `is_ratio_max`, `is_correction_coverage`, `is_masked_frac`, `sampler_certain_frac`, `update_skipped`, `degenerate_group_frac`, `invalid_episode_frac`, `rows_over_cap_frac` |
| `routing/*` | MoE routing replay | `replay_flip_rate` |
...
- **Two episode-health rates a termination rate hides.** `episode/empty_answer_rate` is the fraction whose final answer had no visible content — a turn that stopped inside its reasoning, below the cap. `episode/reasoning_cjk_rate` is the fraction whose reasoning drifted into CJK script.
