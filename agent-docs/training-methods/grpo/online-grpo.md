...
- **`mask_truncated_completions: true`** — single-turn only: a completion cut at the token budget is dropped from the loss rather than scored as a failure. The env trainer implements the masking itself (it replaces the TRL generation path that would apply it) and reads the same flag; the shipped env recipes set it `false`, since there `truncated` marks the `max_turns` overflow rather than a cut turn ([Degenerate groups](environmental-grpo.md#degenerate-groups-and-reward-scaling)).
...
Forwarded params are staged on the sync GPU and shipped in packed broadcasts by `reset_prefix_cache()` (`src/distributed/nccl/clients/base.py`) — the name is TRL API compat, it calls no *prefix-cache* endpoint, and the KV flush rides the `/pause` → `/update_weights` → `/resume` sequence.
...
> [!NOTE]
> **Multi-server sync is an Environmental GRPO feature**
>
...
- **NCCL timeout / `Failed to broadcast barrier_id` / TCP store timeout** — a crashed vLLM server, a model/shape mismatch between trainer and server, blocked NCCL ports, or vLLM launched via `docker run` without host networking. Check server logs, match the model exactly, add `--network host` (compose handles this). On a host **without a fabric** both containers need the no-fabric recipe the compose file sets (`NCCL_NET=Socket`, `NCCL_IB_DISABLE=1`), so the two ends declare one net rather than each falling back on its own; on an EFA host, the compose EFA overlay plus `make ... EFA=1`, with `scripts/profiling/weight_sync_transport.py` as the preflight ([Rollout Servers](../../infrastructure/rollout-servers.md#vllm)). For genuinely slow collectives raise `DIST_NCCL_TIMEOUT_MINUTES` (default `30`).
