...
- **NCCL timeout / `Failed to broadcast barrier_id` / TCP store timeout** — a crashed vLLM server, a model/shape mismatch between trainer and server, blocked NCCL ports, or vLLM launched via `docker run` without host networking. Check server logs, match the model exactly, add `--network host` (compose handles this). On a host **without a fabric** the training image's baked OFI NCCL defaults wedge the weight-transfer group's first collective instead of falling back — both containers need the no-fabric defaults the compose file sets; on an EFA host, the compose EFA overlay plus `make ... EFA=1`, with `scripts/profiling/weight_sync_transport.py` as the preflight ([Rollout Servers](../../infrastructure/rollout-servers.md#vllm)). For genuinely slow collectives raise `DIST_NCCL_TIMEOUT_MINUTES` (default `30`).
