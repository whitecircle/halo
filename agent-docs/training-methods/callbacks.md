...
Auto-wiring passes the EP / TP / CP / ETP / PP sizes from `ParallelismConfig`, `max_seq_len` (`resolve_max_seq_len`), the `report_mfu_diagnostics` flag, and sets `include_num_input_tokens_seen="all"` — the field is tri-state in transformers 5 (`"no" | "all" | "non_padding"`), so a boolean is the wrong type and `"no"` is truthy. The expert count and router top-k are not parameters: they come from `detect_moe_experts_topk`, the same probe the balancing wiring and the MoE load metrics read, so no consumer can classify a family as MoE while another calls it dense.

**Attention-score term.** Each of this rank's decoder layers is costed by its `config.layer_types` kind — full, sliding (`sliding_window`), chunked (`attention_chunk_size`), GLM-5 sparse (top-k plus a pooled indexer), DeepSeek-V4 compressed (`compress_rates` plus the local band), or nothing for linear-attention / conv layers, with `hybrid` (full attention beside a convolution branch — Zaya's CCA, Inkling) costed as full and `hybrid_sliding` as sliding — at width `heads·(d_qk + d_v)` read per layer (Gemma 4's split `head_dim`, MLA's `qk`/`v` dims); the layer's own type attribute wins over list position, so a pipeline stage costs its real slice. A `layer_types` entry outside that vocabulary disables MFU with a warning rather than costing the layer as full attention, and so does a hybrid family that declares its linear layers only through `layer_group_size` (the remote Bailing/Ling and Ring-linear configs) (`src/models/attention_layout.py`). The keys attended are each **document's** length: the trainer costs every batch's documents (`cu_seq_lens_q`, `position_ids` resets, or the padded row) through that layout on the device, and the callback reads this rank's total once per optimizer step — no collective, no per-micro-batch host sync. Measured for SFT, distillation, DPO, reward, classification, prompts-SFT and VLM batches; every GRPO trainer (its batches are prompt lists until `compute_loss`), KTO, SMPO and embedding keep the config term, every token in a `max_seq_len` document. `max_seq_len` falls back to 2048 when nothing declares a bound, which is why the environmental-GRPO script passes its own multi-turn bound. The term divides by `tp_size` (heads are sharded) and, under Ulysses CP, by `cp_size`; the token count divides by `world_size` and `cp_size`, never by `ep_size` (EP is orthogonal to DP). The startup line `Attention layout (this rank): …` names the mix the run is costed with. Cost model and measured numbers: [Throughput](../optimization/throughput-benchmarks.md).
