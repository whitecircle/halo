...
2. **Tokenize each batch independently** to its own S3 prefix with the command above, varying only `--input`/`--output`. These jobs are embarrassingly parallel and CPU-bound — run them as separate CPU jobs ([Infrastructure](../infrastructure/README.md)).
