...
| `reference_kl_coef` | `0.0` | Alpha on a frozen-reference KL anchor; `0` loads no reference. Refused under EP/TP: the dense reference would run the unpatched path, so its log-probs would not match the policy's |
