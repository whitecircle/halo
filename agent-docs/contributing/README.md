...
| `HALO_TEST_ENV_GRPO_MODEL` / `HALO_TEST_ENV_GRPO_MAX_STEPS` | Checkpoint (default `Qwen/Qwen3-30B-A3B-Instruct-2507`) and step count (default `2`) of the Environmental-GRPO e2e body (`tests/common/env_grpo_e2e.py`). Both engine legs read it (the SGLang wrapper defaults to gpt-oss, the vLLM one to Qwen3-30B); a per-family pass points a wrapper at the family and serves the same checkpoint. The 4-GPU legs pass their own. |
