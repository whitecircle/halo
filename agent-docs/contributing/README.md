...
`$(HALO_SCRATCH)/models/`. The suites need a local checkpoint directory, not added tokens, so the tool
runs without `--patterns` and re-saves the source vocabulary unchanged:
