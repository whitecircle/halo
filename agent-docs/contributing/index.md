...
| Docs build (strict) | `make docs` | image (`mkdocs build --strict`) |
...
change, re-run the benchmark on the reference hardware, overwrite the file with the new payload, and
...
Update the owning doc page in the same PR when you change `src/` (`mkdocs.yml` `nav:` is the map) and
...
Two `docs.yml` jobs block a merge, and `make docs` runs only the first: `mkdocs build --strict`
(plus the relative-link check over `human-docs/`, `skills/` and the root markdown), and a `diagrams` job that
