...
- **No secrets to PR code.** The commented-out test triggers are `pull_request`, not `pull_request_target`, so enabling them still keeps repo secrets away from PR code. `pr-gate.yml` uses `pull_request_target` on purpose and checks out nothing.
