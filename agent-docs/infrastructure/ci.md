...
`.github/workflows/docs.yml`: `scripts/docs/check_links.sh` blocks on any broken relative link across `agent-docs/`, `human-docs/`, `skills/` and the root markdown. A `diagrams` job also blocks: it re-runs `scripts/diagrams/` in a `python:3.12-slim` container at `uv.lock`'s matplotlib pin and byte-compares the result against the committed figures under `agent-docs/assets/`, so a generator edited without re-committing its figure fails the PR — run `make diagrams` and commit. `markdownlint` is non-blocking. Blocking is enforced by the `main` ruleset's **required status checks** — without those, a red check is advisory and a PR still merges.
