...
Ray runs the rollout side of [Async GRPO with Environments](../training-methods/grpo/async-grpo/README.md).
...
constructed-but-untrained trainer starts no Ray. With `ray_address: null` (the default) each rank
starts a local Ray instance, unless a `ray start` instance already runs on the host, in which case
`ray.init` joins it.
...
- **The dashboard is off** (`include_dashboard=False`): Ray's dashboard/jobs API carries known
  ...
  opens port 8265. There is no YAML knob to re-enable it; monitor from the host shell instead (below).
    ...
    It governs only the instances `ray.init` starts: joining a head someone else launched inherits
    that head's setting, so a manual `ray start` must pass `--include-dashboard=false` itself
    ([below](#multi-node)).

- **Temp-dir fallback**: Ray's object-store socket is an AF_UNIX path with a ~107-byte cap, so when
  `TMPDIR` is longer than 40 characters Ray is redirected to `/tmp/ray`.

Teardown runs in `train()`'s `finally`: the prefetch thread stops, every actor's `shutdown()` is
awaited sequentially, weight-sync communicators close. `ray.shutdown()` is never called; actors die
by reference counting. A truly wedged actor can stall this teardown; kill the trainer container if
exit hangs.
...
- **Unlimited restarts**: actors are stateless between episodes, so a restarted actor beats a
  ...
  reward, excluded from the loss) rather than a crash.

    A *wholly* dead pool still halts the run: a step in which no episode anywhere in the world
    survived warns, and the second consecutive one raises (`EMPTY_ROLLOUT_STEP_LIMIT = 2`) rather
    than training on all-masked batches.

- **Default resources**: Ray reserves one CPU to place an actor, then releases it: a running actor
  is charged nothing. So Ray's scheduler never caps what the pool actually consumes; what does
  depends on the environment ([below](#what-bounds-actor-concurrency)).

- **Asyncio actor**: episode methods are `async def`, so one actor serves up to Ray's default 1000
  concurrent calls. The pool size is *not* the concurrency limit; `max_concurrent_rollouts` is.

- **Soft node affinity**: each rank pins its actors to its own node
  (`NodeAffinitySchedulingStrategy(soft=True, _spill_on_unavailable=True)`), so a shared cluster does
  not scatter one rank's pool cluster-wide, and placement never fails over it.

    `_spill_on_unavailable` is load-bearing: `soft` alone spills only when the node is dead or
    infeasible, so a live but saturated training node would pin every actor there and stall instead
    of spilling onto the CPU actor tier.
...
row; the in-flight episode is lost, not retried.
...
- `ray_address` set: each rank creates `num_rollout_workers // world_size` actors (floored at 1), so
  the cluster-wide pool approximates `num_rollout_workers`. The floor means a world size larger than
  `num_rollout_workers` overshoots.
...
([Rollout Servers — Throughput](rollout-servers.md#throughput)).

Actors are async, so size the pool to the env's **blocking** per-episode cost, not to HTTP
concurrency. Sandbox-backed envs (`code_contests`, `swe`) are CPU-bound and bursty; search/MCP envs
are latency-bound, where more actors help only until the remote endpoint rate-limits.

Leave the default unless `async/mean_rollout_latency` rises while the rollout servers sit idle. The
trainer-side batch knobs live under
[Async GRPO with Environments — Batch construction](../training-methods/grpo/async-grpo/objective.md#batch-construction).
...
- **Network-bound** (`qa_search`, `react_search`, `mcp`, open-book `exam_qa`): no gate; size against
  the search/MCP endpoint's rate limit, not against cores.
...
[Async GRPO with Environments — Multi-node deployment](../training-methods/grpo/async-grpo/setup.md#multi-node).
...
is the pin), and `ray.init` joining that head cannot turn it back off, so the head would serve the
jobs API on 8265 that the section above says is never opened.

The toolkit configures no other Ray ports; upstream defaults apply. What must hold across nodes:

- **Rollout-server URLs must resolve from every actor node**: actors call the servers directly over
  HTTP. The trainer warns when a loopback URL meets a multi-node run (a scattered actor would hit a
  node with no server and return masked rows).

- **Actor nodes need the environment's runtime**, not GPUs — per env: a sandbox plus a large
  `TMPDIR` for the coding envs and `swe` (an SWE session's whole working tree lives in `TMPDIR` for
  its episode); outbound network plus the backend's key for `react_search` / `qa_search` / open-book
  `exam_qa`; `npx`/`uvx` for `mcp`. The per-env list:
  [Environments](../training-methods/grpo/environments/README.md).
...
- **Actor processes inherit the `ray start` daemon's environment, not the trainer's.** Export
  ...
  `TAVILY_API_KEY`), and any HF/proxy vars *before* `ray start` on each node; nothing forwards the
  trainer's env to remote actors.
...
logprobs, and, under
[R3 routing replay](../training-methods/grpo/async-grpo/objective.md#routing-replay),
the routing mask, which stays base64-encoded end to end and so remains compact on this path. The
trainer gathers lightweight per-episode summaries across ranks, never full trajectories.

The object store itself runs at Ray's defaults (~30% of RAM, capped by `/dev/shm`); the standard
`--shm-size=128g` launch covers it, and the toolkit configures no spilling.
...
already gathers across ranks each round, so they are **job** totals, not one rank's pool.

No collective and no call into the actors happens inside `log()`. A `ray.get` fan-out there would
block this rank on single-threaded actors busy mid-rollout while its peers move on to the next NCCL
call, and NCCL then times out the job.

From the host: `ray status` (cluster resources, PENDING actors), `ray logs` (actor stderr). The
dashboard stays off (above). `py-spy` note: the toolkit's stack-dump helpers discover torchrun PIDs
only; actor processes are children of the raylet, so attach explicitly with
...
| `episode_timeout` errors while the env is healthy | Deadline below real episode latency (long SWE sessions hit this first) → raise it; it must stay under the NCCL watchdog ([`episode_timeout`](../reference/configuration-reference.md#asynctrainingconfig)) |
