...
- **Set `fsdp_reshard_after_backward: false` on the trainer** for step time: the
  ...
  ~1.4× of the same config on vLLM (gpt-oss-20b, 2×(TP=2)+DP=4 B300: ~215 s vs ~150 s per step);
...
- **Servers × TP**: split the serving GPUs into the fewest servers that hold the model — at
  gpt-oss-20b scale on 4 B300s, 2×(TP=2) and 4×(TP=1) measure the same step time on a tool-heavy
...
Undistributed MoE (`ep_group_size == 1`, EP wrappers present) works on gpt-oss-20b with
