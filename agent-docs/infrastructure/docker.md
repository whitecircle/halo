...
| uv 0.10.x, AWS CLI v2, Claude Code | Yes | Yes |
...
targets). Every `push-public-*` target is gated by `docker/scan_image.sh`, which refuses to push an
image carrying credential-shaped content, key material, shell histories, authenticated git remotes,
or coding-agent identity state (`/root/.claude.json` holds the build host's machine/user IDs; the
Dockerfile scrubs it — and the installers' backups/tmp/locks — in the same layer that creates them,
since an `rm` in a later layer hides the files but still ships the bytes). `make push-blackwell
ECR_ACCOUNT_ID=<yours>` (and friends) push the same images to your own private registry instead,
under `halo-`-prefixed tags and without the gate.
...
The build installs [Claude Code](https://code.claude.com) at `/root/.local/bin/claude` on a best-effort
basis — the step is non-fatal, so an image built while the installer was unreachable ships without it. Repo-aware
skills live under `skills/` (symlinked from `.claude/skills` and `.agents/skills`) and ship inside the image (`skills/` is copied in and the `.claude/skills` + `.agents/skills` symlinks recreated) and inside any container that mounts the repo at
`/workspace`.
