...
when `flash_attn` / `deep_ep` are missing) and replace the `s3://<your-bucket>/halo` mount with your
...
sky launch -c oss-120b launcher-configs/skypilot/nebius/gpt-oss-120b/crossnode-ep.yaml \
  ...
  --env MODEL=/data/checkpoints/oss-120b-sft \
  --env CONFIG_PATH=examples/sft/gptoss/gptoss-120b-multinode-ep.yaml \
  --env OUTPUT_DIR=/data/checkpoints/oss-120b-sft-v2
```
...
file_mounts:
  ...
  /data:      {source: s3://<your-bucket>/halo, mode: MOUNT}   # placeholder — set your bucket
```
...
```bash
aws s3 sync models/gpt-oss-120b-bf16-hf s3://my-bucket/models/gpt-oss-120b-bf16-hf/
```
