...
- **Sliding layers** run compiled FlexAttention with a `BlockMask` derived from the dense mask transformers built, so causality, the window, packed-document isolation and padding carry over and the tiles outside the window are skipped. Under SDPA the same layers compute every tile of the dense mask. The kernel compiles once with dynamic shapes, and its tuned tiles (head_dim 256, 16-bit inputs, SM100+) do not depend on the sequence length, so batches of varying length reuse the compiled kernel; other shapes use FlexAttention's defaults.
