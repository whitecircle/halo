...
the [supported-axis allowlist](index.md#supported-combinations); this page adds the locality rules —
...
!!! warning "Single-domain pure EP needs a single dispatch group"
...
What has run on real multi-node hardware, and what has not. Everything else on this page is enforced
in code and covered by one-host simulations (a `ParallelismConfig` built with `gpus_per_node` /
`nvlink_domain_size` below the world, so the domain-crossing branches execute over NVLink) — which
proves the rank math and the gradient algebra, not the fabric.

| Shape | Ran | Where |
|---|---|---|
| 2 × 8 B300, AWS EFA — cross-node EP `ep8`/`ep16`, the 8192 tokens/rank Gin dispatch ceiling, node-local `ep8×2` matching single-group `ep16` | ✅ | [DeepEP → AWS EFA](../infrastructure/deepep.md#expert-parallelism-over-aws-efa) |
| 2 × 2 / 2 × 4 / 2 × 8 B300, AWS EFA — plain FSDP2 DP, HSDP, node-local multi-group EP (deferred sync), cross-node EP, EP+TP, EP+CP, pure ETP, cross-node EP+ETP | ✅ | on-hardware runs, EFA only |
| **More than 2 nodes** (4-node, 8-node, the 512-GPU layouts in [Large-Scale Scenarios](large-scale-scenarios.md)) | ❌ | rank math only — `ParallelismConfig` is exercised at world 8/16/32, no run |
...
| **Hopper (H100/H200) multi-node, and InfiniBand/RoCE as a fabric** | ❌ | every recorded multi-node run is B300 over EFA; the IB path is config guidance, not a measurement |
