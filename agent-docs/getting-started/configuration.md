...
EP is orthogonal to data parallelism; only TP, CP, and ETP reduce it — `data_parallel_size = (world_size / pp_size) / max(tp_size, cp_size, expert_tp_size)`. See [Parallelism](../parallelism/index.md) for supported and rejected combinations.
