...
| [Distillation](../training-methods/distillation/index.md) | Conversation turns | Teacher | No | `scripts/training/distillation/teacher_distill.py` |
...
- **Environmental GRPO** — the only multi-turn option: the model acts, receives observations, and learns from trajectory rewards, with async Ray-actor rollouts and NCCL weight sync to vLLM or SGLang (`rollout_backend`). The environment comes from `environment_type`; see the [Environments overview](../training-methods/grpo/environments/index.md) for the built-in list and [Custom Environments](../training-methods/grpo/environments/custom-environments.md) to add one.
- **Teacher vs self distillation** — `teacher_distill.py` transfers from a larger model; two privileged-context self-distillation variants use one model as both student and teacher, offline (`self_distill.py`) and on-policy SDPG (`rlvr.py --use_sdpg=true`). See the [Distillation guide](../training-methods/distillation/index.md).
