...
| Gemma 4 MoE | Yes | Yes | No | No | Yes | No | No | No | KV-shared layers block CP/TP; no router-balancing path at all; attention LoRA on the multimodal checkpoint is refused at PEFT setup — the vision tower's projections share the `q_proj`…`o_proj` names in a `Gemma4ClippableLinear` PEFT cannot wrap (open issue); environmental GRPO on multi-turn tool trajectories trains nothing (every trajectory IS-masked, open issue) |
