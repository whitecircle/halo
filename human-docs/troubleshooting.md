...
| Gemma 4 environmental GRPO on multi-turn tool trajectories logs `sampling/is_masked_traj_frac` 1 and trains nothing | Open issue on both engines: the trainer's log-probs sit far below the engine's on these trajectories (mean log-ratio −0.6 to −0.8). Single-turn runs agree. |
