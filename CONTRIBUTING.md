...
- **Pass the gates.** `make lint`, `make format`, `make seed-hf-cache` once, then `make test-cpu`
  (and `make test-gpu-core` for GPU-affecting changes), `make docs`. Hosted CI runs `ruff`,
  `actionlint`, the docs checks and, on every non-draft PR, the CPU tier; its CPU tier uses the
  published image, so a dependency change still needs `make test-cpu` on a rebuilt image. The GPU
  tiers run on your machine, so state their result in the PR.
...
`CODE_OF_CONDUCT`, `AGENTS.md`, `CLAUDE.md`, `.github/*`) are the documented exception.
