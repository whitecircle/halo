"""Resolve, and after installing verify, the ``nvidia-nccl-cu13`` pin the images share.

``uv.lock`` is the source of truth for the NCCL runtime: the training image and both inference images
(vLLM, SGLang) dynamically link this wheel's ``libnccl.so.2``, and a version skew hangs the RL
weight-sync ``ncclCommInitRank`` until the NCCL timeout with no error. Every Dockerfile COPYs this
beside its ``uv.lock`` so the value is read the same way everywhere.

``--verify`` checks the wheel metadata and the library a process actually resolves: every image also
carries an older system ``libnccl.so.2`` (the NGC base's, the engine base images' own), so a build
that lost the wheel, or an ``LD_LIBRARY_PATH`` that puts the system copy first, would pass a
metadata-only check while dlopening a runtime the other end cannot pair with.
...
# DeepEP V2 (EPv2, Gin backend) is built against this floor. A pip step that re-resolves torch
# downgrades the wheel to torch's pinned 2.28.9 and leaves ``deep_ep._C`` with an undefined Gin symbol
# at import, so ``--verify`` asserts the floor as well as equality with the lock.
MINIMUM = "2.30.4"
...
def _version_tuple(value: str) -> tuple[int, ...]:
    ...
    for chunk in value.split("."):
        if not chunk.isdigit():  # a ``.post1`` or local suffix ends the comparable prefix
            break
