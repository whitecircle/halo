...
#     of the training image already bundles (its EFA_VERSION label); its MOFED rdma-core is replaced;
...
#
# Needs root, apt, curl, a CUDA toolkit at /usr/local/cuda and the nvidia-nccl wheel installed.
set -euo pipefail
...
apt-get install -y --no-install-recommends \
    autoconf automake libtool pkg-config git ca-certificates curl \
    hwloc libhwloc-dev
...
# module, MPI, and the installer's own NCCL plugin (no GIN) are not installed.
apt-get update
...
# NCCL loads the GIN plugin from this name; the net plugin is found as libnccl-net-ofi.so.
...
libefa=$(ldconfig -p | awk '/libefa\.so\.1 /{found=$NF} END{print found}')
