...
native harmony serving path returns malformed output and parser errors on this
stack. With harmony off, the model still follows its harmony chat template and
emits the channel structure, but vLLM detokenizes the output with the harmony
control tokens (``<|channel|>`` / ``<|message|>`` / ``<|constrain|>`` /
``<|call|>``) stripped, leaving the bare channel/role words and the payload as
plain text. A reasoning answer therefore arrives as::
...
parser extracts the calls from the text: every
...
# ``final`` channel marker: everything after it is the answer. With harmony control tokens stripped
# it decodes as ``assistantfinal`` (or a start-anchored ``final``). Anchored to start-or-``assistant``
# only, so ``final`` inside words such as "finally" in the CoT cannot split content.
...
def _coerce_json_args(args_str: str, name: str) -> str:
    ...
    The model emits its arguments under the harmony ``<|constrain|>json`` hint, but some
    generations slip into Python-dict syntax (single quotes, ``True``/``None``) or drop key
    quoting entirely. Returning such a string verbatim breaks the next turn: it replays the
    assistant ``tool_calls`` to the server, and vLLM's ``_postprocess_messages`` runs
    ``json.loads(arguments)`` on it, so an invalid string raises ``400 Bad Request`` and ends
    the rollout. A Python-literal slip is therefore recovered through ``ast.literal_eval`` with
    non-JSON keys canonicalized to strings, and text that stays unparsable is wrapped under
    ``_raw``: the request stays valid and the tool layer surfaces an argument error the model
    can recover from on a later turn.

    This function must not raise: any escaping exception becomes a 400 that ends the rollout.
    """
    try:
        ...
        pass
    try:
        # literal_eval raises ValueError/SyntaxError/MemoryError/RecursionError on hostile input, and
        # dumps raises TypeError on values JSON cannot encode (sets, bytes, complex). Both go to _raw.
...
def _resolve_tool_name(raw: str, request: ChatCompletionRequest) -> str:
    ...
    ``to=functions.submit_solution`` arrives as ``submit_solutionjson`` or
    ``submit_solutioncommentary``, and the greedy name regex swallows the suffix. The real tool is
    recovered by taking the longest declared tool name the captured token starts with. It only
    rewrites to a tool the request declares, so a hallucinated name is left untouched for the env to
    reject."""
    if not raw:
        return raw
...
class GptOssTextToolParser(ToolParser):
    ...
    # error. False keeps both forms going through extract_tool_calls, which reads the plain-text
    # format.
    ...
    def extract_tool_calls(
        ...
        **kwargs: Any,
        ...
        # With harmony disabled the model omits the `<|call|>` stop token, so it often keeps
        # generating after the first genuine call, hallucinating the tool result as more
        # `to=functions.*` text. For env-GRPO (one call per turn, executed by the environment) only
        # the first call is meaningful, so parsing stops after it. Parallel-call models would need the
        # harmony stop token.
        ...
        if m is not None:
            ...
            if args_str is not None:
                ...
                # next turn's replay and end the rollout (see _coerce_json_args).
            ...
            # No balanced JSON after the header means the call is not well-formed, and it is not
            # rescued: executing malformed calls would train the policy away from the
            # ``json{…}<|call|>`` format the template asks for. A dropped call becomes a no-tool turn,
            # which is the intended RL signal.
