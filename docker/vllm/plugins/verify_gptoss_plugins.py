...
No test suite can cover these: they import vLLM, which the training image deliberately does not
have (ABI-incompatible stacks), so the image build is the only gate. Each check covers a failure
mode that is silent at serve time, so a regression fails the build instead of the run.
...
# or vocabulary drift has to fail HERE rather than as a silently unbounded CoT in a training run.
HARMONY_MARKER_IDS = {
...
# a bare ``<|channel|>`` arming marker mistakes for reasoning already in flight and cuts at once.
...
def simulate_generation(
    ...
    many tokens, which is how a run that finishes reasoning inside its budget is distinguished from
    one that is cut. Returns the forced ids and how many tokens the model generated before the first
    of them.
    """
    output: list[int] = []
    ...
    # The holder keeps a reference to ``output``, exactly as the batch does to a request's ids.
    holder.sync_batch(
        BatchUpdate(
            batch_size=1,
            removed=(),
...
# Normal path: the model emits its own `<|start|>assistant<|channel|>final<|message|>`, which
# decodes to `assistantfinal`. The budget-forced cut appends the SAME ids, so it decodes the same
# way and takes this one path — the marker carrying the role tokens is what makes that true.
...
# A CoT cut mid-word, mid-space and while echoing the prompt's own "final": measured live at
# budget 48 on unsloth/gpt-oss-20b-BF16. A marker without the role tokens decodes to a bare
# `final` here, indistinguishable from the prose one, and stranded the whole answer in reasoning.
...
# special tokens (the detokenizer skips them). _split must find the answer behind it wherever the cut
# lands — including on whitespace, which is where a role-less marker decodes to a bare `final` no
# anchored pattern can tell from prose, stranding the whole answer in reasoning.
...
# Prose `final` alone is never a boundary — without the `assistant` anchor there is nothing to split on.
...
# A tool call must reach content — vLLM extracts tool calls from content only.
...
# the render never emits is silent: the server takes the field, the CoT runs to max_tokens, and only
# a live A/B notices. The rows below drive vLLM's own state machine over a simulated harmony-disabled
# stream, so a marker or render drift fails the build instead.
...
# A run that opens its own `final` channel inside the budget must be left alone: forcing there would
# cut the ANSWER, and the arming marker must not re-arm on the answer text either.
...
# ANTI-VACUITY: without the field nothing is forced, so the rows above measure the budget and not the
# simulated stream.
