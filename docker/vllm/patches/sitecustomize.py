...
vLLM must not be imported here, since that would pull torch in during ``site`` execution. Instead a
meta-path hook installed at interpreter start applies each patch when vLLM first imports the module
it targets.
"""
...
# vLLM module → the patch module that fixes it. One hook serves every entry; a new patch is a new
# row.
_PATCHES = {
...
class _PatchOnImport(importlib.abc.MetaPathFinder):
    ...
    def find_spec(self, fullname, path=None, target=None):
        ...
        # Re-entrancy guard rather than retiring the entry here: an import that fails part-way would
        # otherwise consume the single attempt and the retry would load the module unpatched,
        # reverting experts on every weight sync or leaking a communicator per trainer. The entry is
        # retired below, once the target has executed.
        ...
        def exec_module(module):
            ...
            except Exception:  # noqa: BLE001 — a broken patch must not break vLLM's own import
