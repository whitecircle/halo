...
# Every patch targets a file this vllm ships. Absence means upstream moved it, which fails the build
# rather than skipping into an unpatched server.
require_file() {
...
for HFILE in "${HARMONY_FILES[@]}"; do
    ...
    # Report per file what the sed did, rather than announcing success unconditionally.
    if grep -q -e "$HARMONY_CHECK" -e "$HARMONY_CHECK_C" "$HFILE"; then
...
# Backstop: fail if any unpatched `*_harmony = ... == "gpt_oss"` gate survives (matches the code form
# only, not the patched `= False  # ...` comment), so a stale HARMONY_FILES list fails the build.
grep_rc=0
