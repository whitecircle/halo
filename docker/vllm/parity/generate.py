...
``tests/common/models.py``, through the vendor config module the fixture ships where transformers has
no class for the family. A hub fixture, for a family whose export carries its source checkpoint's
schema, builds it from the release config at a pinned revision plus the remote-code modules that
schema names. ``unparseable/`` holds the same configs without the rewrites: the form each rewrite
exists to avoid, which the server's transformers must still refuse. ``check.py`` asserts all of them
against the server image at build time.

The roster is derived rather than listed: every EP family whose layer class admits weight sync
(``_supports_weight_sync``) needs a fixture here, because the server has to parse that family's
checkpoint before a tensor can be synced into it.
...
class RemoteCodeFixture:
    ...
    That module is the vendor's own, checked in beside the config rather than rendered here, and
    declares neither ``model_type`` nor ``auto_map``. A checkpoint's ``config.json`` carries those
    two, and without them nothing resolves the family or finds the module, so the fixture supplies
    them.
    """
...
class HubFixture:
    ...
    ``revision`` is pinned so a fixture never re-renders against a moved ``main``, which would gate on
    a schema no checked-in code was reviewed against. The snapshot carries the config and every
    remote-code module, which is what the source-schema export reads.
    """
...
# rewrites themselves read, so a family that starts declaring either one gets its control without an
# edit here.
UNPARSEABLE = tuple(
    fixture
...
def uncovered_families() -> list[str]:
    """Names of the admitted families no fixture covers; this list is expected to be empty."""
...
def render(fixture, *, rewrite: bool) -> dict:
    """The ``config.json`` payload the toolkit writes for ``fixture``: through the export rewrites by
    default, or the plain transformers serialization for the negative control."""
...
def config_module(payload: dict) -> str | None:
    """The remote-code file a payload's ``auto_map`` names for ``AutoConfig``: the only module beyond
    the config itself that the gate's parse imports. None for a natively-parsed family."""
...
def write_all() -> None:
    ...
    for role, fixtures, rewrite in (("fixtures", FIXTURES, True), ("unparseable", UNPARSEABLE, False)):
        for fixture in fixtures:
            ...
            # Only the served fixtures get the module: the control is refused as the unrewritten
            # export is, and that export ships no remote-code modules either.
