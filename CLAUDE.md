...
**RL serving:** vLLM 0.26.0 runs as a **separate** container (`Dockerfile.vllm` + `docker-compose.vllm.yml`, both `network_mode: host`) with native NCCL weight transfer; the training env uses a vendored NCCL client (`src/distributed/nccl/`), never imports vLLM (ABI-incompatible stacks). Two gates keep expert sync honest: the server's layerwise-reload patch must skip the layers the reload lifecycle cannot carry — `FusedMoE`/`RoutedExperts` experts and `OAIAttention` sinks, which write by direct `copy_`, plus `Gemma4Router`, whose partial `scale` load takes layerwise's delayed-processing branch and materializes its non-checkpoint `root_size` buffer from uninitialized memory (`RoutedExperts`, `OAIAttention` and `Gemma4Router` asserted at build), and the server must run `--moe-backend triton` (auto FLASHINFER/CUTLASS repack expert weights and corrupt updates). A third gate is trainer-side: weight sync refuses a GptOss whose sinks the FA2 `reset_sinks` reset removed. The image also asserts its weight-transfer re-init patch at build, without which the engine strands an NCCL communicator per trainer connection until `ncclCommInitRank` fails (the trainer's own client aborts its communicator on close for the same reason), and that `/pause` accepts `mode=keep`, the form the client pauses with so in-flight requests survive a sync. Async GRPO with Environments can target **SGLang** instead (`rollout_backend: sglang`, `Dockerfile.sglang` + `docker-compose.sglang.yml`, NCCL matched to the training image). Both engines read a family's experts in the layout its own `gather_expert_state_dict` emits; what differs is which model types each pinned release can take an online update for, declared per client (`UNSERVABLE_MODEL_TYPES` in `src/distributed/nccl/clients/`) and refused per model+backend pair at construction by `validate_weight_sync_support`. Both refuse Mistral4, Zaya, DeepSeek-V4 and the Ling 3.0/Ring spellings; SGLang also refuses Laguna and Step-3.7 (loaders that assert full coverage per call). Every other family syncs on either engine, expert distribution included; `_supports_weight_sync = False` is engine-independent and holds only for Inkling, GLM-5 Next and Cohere2 MoE. SGLang sync needs this image, not upstream: it patches the GLM-4 gate's cached fp32 weight and the Gemma 4 router's latched `scale` fold (`docker/sglang/patches/`), and the client keeps the co-loaded MLA pair (`q_a_proj`/`kv_a_proj_with_mqa`) in one chunk (`CO_LOADED_PARAM_GROUPS`). Its container needs `NCCL_CUMEM_ENABLE=1` (compose default; SGLang otherwise turns cuMem off process-wide and the first cross-container import fails). SGLang spells the same repack gate `--moe-runner-backend triton` (compose default, required for the sync itself), and R3 capture there adds `--enable-return-routed-experts`; `rollout_max_thinking_tokens` stays vLLM-only. All three images carry the same EFA userspace (`docker/efa/install_efa_userspace.sh`: rdma-core, AWS libfabric and one pinned `aws-ofi-nccl` build — two containers whose builds differ hang the sync group at its first collective), and the compose EFA overlays (`docker-compose.{vllm,sglang}.efa.yml`, trainer `make ... EFA=1`) put the sync to a server on another node on EFA; `scripts/profiling/weight_sync_transport.py --expect efa` is the preflight. Online GRPO is vLLM-only by construction. See `agent-docs/infrastructure/rollout-servers.md` and `agent-docs/training-methods/grpo/async-grpo/README.md`.
...
│                        #     checkpointing), segment_markers.py (per-family document-boundary markers for
│                        #     packed / padding-free rows),
...
├── rewards/             # reward terms (spec), scorers (judge, served reward_model), composer, TRL adapters
│                        #   (functions), RLVR graders (verifiable), answer matching — shared by both GRPO arms
├── environments/        # RL envs: base.py (the env protocol) + episode.py (the rollout-driver contract),
│                        #   registry, ray_actors, eval_runner, engine_wire (rollout wire format),
...
```
`scripts/` uses pipeline-stage folders: `training/ inference/ before_training/ after_training/ profiling/ environments/` (plus `diagrams/`, the docs-figure generators). Entry scripts share their flag surface per subtree — `scripts/inference/_common.py`, `scripts/inference/reward_model/_common.py`, `scripts/environments/_common.py` — or above them where the tools chain across subtrees (`scripts/_common.py`, the checkpoint tools' flags) — while the driver stays in `src/`. Configs in `examples/` split per method then per model family; the async GRPO recipes further split per rollout backend (`vllm/`, plus `sglang/` for gpt-oss, Qwen3.5/3.6 and Gemma 4 — ep1 variants only) with one file per adapter (`-lora-`/`-full-`) × expert distribution (`-ep1`/`-ep4`), while the Qwen3.6 vLLM full-ep1 recipe exists only as its three curriculum stages (`-stage1-codeforces`/`-stage2-hard`/`-stage3-extra-hard`). Layer map and leaf-module contracts: `agent-docs/reference/architecture.md`; per-topic depth via the doc index below.
...
**Add a method:** arg/config class in `src/args/` or `src/configs/` → subclass `DistributedTrainerMixin` in `src/trainers/<family>/` → script in `scripts/training/` → YAML in `examples/` → tests in `tests/`. Trainer `__init__` does `kwargs = self._init_distributed_config(kwargs, ctor_args=args, ctor_positions=_CTOR_POSITIONS)` (the TRL base's slot table, so a positional call gets the same setup as a keyword one) then `super().__init__(...)` then `self._setup_distributed_modes()`.
