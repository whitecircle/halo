...
flags and the ``Namespace``-reading output writer) lives here with the scripts that use it.
"""
...
def rollout_config_from_args(args: argparse.Namespace, *, temperature: float, max_tokens: int) -> RolloutConfig:
    ...
    :func:`write_eval_outputs`: each script registers its own flag and default.
    """
