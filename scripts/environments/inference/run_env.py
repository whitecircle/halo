...
`scripts/environments/inference/run_code_contests.py` instead — it owns the dataset adapters, solution
language, and rating-bucketed reporting, so that task-specific logic stays out of this generic runner.
...
passed through with `--context_fields choices`) and an `answer` that is already a letter A-J. Point
it at a raw `cais/mmlu` — whose `answer` is an int INDEX — and every episode scores zero with no
error. Convert the index to a letter during dataset preparation; nothing on the eval path can, since
the grader compares two strings and never sees the choice ordering. Schema and dataset notes:
...
def parse_args() -> argparse.Namespace:
    ...
    # No default: each env class carries its own, and passing one unconditionally would truncate
    # every env at a number none of them chose.
