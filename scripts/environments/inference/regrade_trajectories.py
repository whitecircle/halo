#!/usr/bin/env python
"""
Offline re-grading of recorded code-contest trajectories.

Online evaluation generates trajectories with many concurrent rollout workers per process and several
processes per host. Grading runs untrusted solutions in subprocesses, so at high aggregate
concurrency the wall-clock a solution is measured against is inflated by host load and a correct,
fast solution can be reported as time-limit-exceeded. Decoupling the two stages avoids that: generate
in parallel (API-bound), then re-grade the recorded submissions here in a single process with bounded
concurrency, where each run gets an effectively dedicated core.

This reads the JSONL files written by ``write_trajectories_jsonl`` (a ``{"type": "meta", ...}`` line
then one episode per line), rebuilds each problem's hidden tests from its dataset by index in the
order ``build_examples`` used, and re-runs every recorded ``submit_solution`` through the same
``grade_solution`` the environment uses, reproducing the env's comparison, checker and time limit. It
reports first-submission and within-budget solve rates (``s@1`` / ``s@2``).
...
def validate_meta(path: str, meta: dict[str, Any]) -> None:
    """Raise before any grading when the trajectory's meta line lacks a key the re-grade needs.
...
def _load_payloads(adapter_name: str, dataset: str, config: str | None, split: str) -> tuple[dict[str, Any], ...]:
    """Rebuild a dataset's per-problem grading payloads (tests, checker, time limit) in the order
...
def regrade_file(path: str, workers: int) -> dict[str, Any]:
    ...
    # Reproduce the environment the run used rather than a default one: env_kwargs carries the
    # interaction budgets (max_submissions, reasoning_effort_profiles) that decide how many
    # submissions count. Without them a run that raised max_submissions re-grades against the class
    # default and reports a lower s@k than the online number.
    env_config = {
    ...
    # not inherit. ``stop_on_first_failure``: s@1/s@2 need the all-pass verdict rather than the pass
    # fraction, so short-circuiting matches full grading while skipping the remaining expensive tests
    # of an already-failed solution. ``max_grading_seconds`` exists to stop one episode pinning a
    # rollout worker; applying it here would score a correct-but-slow solution as unsolved. A run that
    # stamped no block leaves the rebuilt env's own spec in place.
    ...
    # One grading task per (episode, submission within budget); a fixed worker count keeps wall-clock
...
def main() -> None:
    ...
    with contextlib.ExitStack() as stack:
        ...
        for path in args.trajectories:
            ...
            # print rather than logging, since third-party imports reconfigure the root logger, and
            # persist each result as it lands so partial runs are not lost.
            print(
