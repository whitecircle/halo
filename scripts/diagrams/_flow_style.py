...
Distinct from its sibling `_theory_style.py`: the theory figures use the IBM
Plex family and a slate accent palette, the flow figures use matplotlib's
bundled DejaVu Sans and a lighter "card" palette (slate ink on white, blue
accent). The font is pinned to the bundled face so the PNG bytes are identical
on every host, since CI checks the committed figures by hash.

Each flow script sets any base font size other than 10 after the import with
`plt.rcParams["font.size"] = <n>`, and defines its one-off tokens
...
class CardLayout:
    ...
    A figure-level knob rather than a constant: the pipeline figures are laid out at slightly
    different scales and CI byte-compares the committed PNGs, so each caller keeps the metrics its
    figure was drawn with. The bullet text always starts 0.15 right of its marker.
    """
...
def bind_drawers(ax, layout=CARD_LAYOUT):
    ...
    A flow figure has one axes and one set of card metrics, so binding them once here leaves each
    call site with only the geometry that varies.
    """
    return (
