...
fprop + dgrad + wgrad = 6P FLOPs (P = params) — over a proportional view of what fills HBM:
weights, gradients, and optimizer state per parameter, plus activations (which
scale with the batch, not the parameter count).
"""
...
def arrow(x0, x1, label=None, color=INK2):
    # inset both ends so the arrow sits cleanly in the gap, never on a border
    ax.annotate(
        "",
        xy=(x1 - 0.08, mid_y),
