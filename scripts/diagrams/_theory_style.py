...
fonts; without this the diagrams silently fall back to DejaVu Sans. Weight
resolves within one family: `fontweight="bold"` picks Plex SemiBold, so the
per-figure scripts keep using plain `fontweight=` calls.
"""
...
SANS = "IBM Plex Sans"
# A list, not a bare name, so matplotlib's per-glyph fallback borrows glyphs Plex Mono lacks (e.g. ᵀ) from DejaVu.
...
# Roles stay consistent across figures: BLUE=compute/weights, AMBER=memory/activations, TEAL=throughput,
# VIOLET=communication, ROSE=bottleneck/waste, SLATE=neutral structure; each hue pairs a stroke with a tint.
...
def on_fill(hexcolor):
    ...
    Uses perceived luminance so labels sitting on a saturated bar/segment
    never come out too pale (white on teal/blue/rose, dark ink on amber).
    """
    h = hexcolor.lstrip("#")
