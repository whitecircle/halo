...
(harmony EOS included) sits below it — so a shrink buys nothing but padding, while any target under
...
the save did not produce is swept away.
...
def parse_args():
    ...
    # ignores. Pin the revision by downloading the source first, then pointing --model_id at it.
    add_hub_source_args(
...
def add_tokens_to_model(
    ...
    # Grow only. Models like gpt-oss ship an embedding padded LARGER than len(tokenizer); the rows above
    # len(tokenizer) are unassigned, and every special (harmony EOS, <|call|>) sits below it — a shrink saves
    # only padding, and any target under len(tokenizer) cuts the specials. The added tokens fit the padding.
...
def main():
    args = parse_args()
    # Refused up front: an in-place patch has no undo, and a per-rank EP/TP-sharded source would load with
    # randomly initialized experts (MISSING keys only warn) while the patched save looked complete.
    ...
    # source's size on disk (silent for a Hub id — nothing local to measure).
    ...
    # Full processor for VLMs (its processor config + chat template must survive the save), else a plain
    # tokenizer. New tokens go into this object's tokenizer, so the patched VLM reloads as multimodal.
    ...
    # auto_load_model resolves the widest Auto* class (AutoModelForCausalLM would drop a vision tower)
    # and routes through the checkpoint-coverage gate: a truncated or key-mismatched source would
    # otherwise load with randomly initialized tensors and be saved as a complete-looking patch.
    ...
    if args.reset_sinks:
        ...
        # patched checkpoint and a trained one cannot disagree about what their sinks mean.
        apply_sinks_policy(model, model.config, policy=SinksPolicy.NEUTRALIZED)
        # It returns silently on a family that carries none, and the stamp is the only signal it did
        # anything — unchecked, the flag saves an untouched checkpoint that every downstream tool
        # (provenance, the merge tools, the RL sink gate) then reads as deliberately sink-free.
    ...
    # Save → restore model_type → sweep; the processing class carries the patched tokenizer.
    save_full_checkpoint(
        model,
        args.output_dir,
