...
``<name>_scale_inv`` — one scalar per 2-D matrix, one scale per expert for the fused 3-D expert
banks. This tool dequantizes shard-by-shard and re-saves sharded BF16 with no full-model RAM
footprint; every other floating tensor is cast to bf16 too, so the output is uniform. The
activation scales (``*_activation_scale``) describe the fp8 forward and are dropped with the
``quantization_config``.
...
# The quant sidecars, exactly: the per-weight dequant scale and the static activation scales.
