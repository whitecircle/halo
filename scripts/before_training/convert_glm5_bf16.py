...
``ceil(rows/128) x ceil(cols/128)``, ``scale_inv[i, j]`` MULTIPLYING block ``[i, j]`` (the same
convention as transformers' ``Fp8Dequantize`` and vLLM's block dequant). EP training needs plain
BF16 expert tensors, so this tool dequantizes block-wise and re-saves sharded BF16, streaming
shard-by-shard with no full-model RAM footprint. Every tensor the release keeps unquantized (the
KDA linear-attention stack, norms, router, hyper-connection tensors, vision tower —
...
input in the forward instead — and ride through at whatever dtype the release stores. The MTP layer
(checkpoint layer 45) is carried across dequantized; transformers drops it on load and exports
therefore ship without it. The emitted ``config.json`` loses its ``quantization_config``.
...
# The quant sidecar suffix, exactly. GLM-5 also stores hyper-connection tensors named
# ``hc_attn_scale`` / ``hc_ffn_scale`` — real weights that any looser "scale" match would drop.
...
def read_block_shape(source: str) -> tuple[int, int]:
    """The fp8 block shape the checkpoint's own config declares — the gate on the quant scheme."""
