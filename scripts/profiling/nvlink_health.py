...
A marginal NVLink lane shows up as a flood of nonfatal ``Xid 145`` (``RLW_RXPIPE``) lines in
``dmesg`` — thousands per hour — while the link keeps running at full speed because forward error
correction absorbs the damage. That noise is easy to misread in both directions: alarming when the
fabric is fine, and drowning out the counters that DO mean data is at risk.

This separates the two. A link is unhealthy when it shows **hard** errors (Rx, symbol, link recovery,
link integrity — the ones FEC did not absorb). Corrections reaching the deepest FEC bins (the
correction budget nearly exhausted) or a correction rate high enough to warrant a physical
inspection mark it marginal: reported for inspection, not failed. Plain correctable churn is
reported only.
...
# Counters that mean FEC did NOT absorb the error: any non-zero value is a real fabric fault.
HARD_ERROR_KEYS = (
...
# consuming much of the correction budget — worth watching, but a handful of them across ~1e13
# codewords is normal link noise, so depth alone is a WATCH signal, never a fault.
DEEP_FEC_BIN = 3
...
def _nvidia_smi(*args: str) -> str:
    """Run nvidia-smi, failing loud with its stderr rather than returning partial output."""
...
def assess_link(counters: dict[str, int]) -> dict:
    ...
    ``marginal``: the link is carrying data correctly at full bandwidth, but its physical margin is
    lower than its peers and it is the usual source of nonfatal Xid 145 spam.
    """
