...
and NCCL env), on a GPU the server does not own. Exits 1 when the transport differs from
``--expect``, when the push altered the served model, or when the group fails to form.
"""
...
def main() -> int:
    ...
    parser.add_argument("--param", default=DEFAULT_PARAM, help="checkpoint parameter to push")
    parser.add_argument("--model-id", default=None, help="checkpoint id or path (default: what the server serves)")
