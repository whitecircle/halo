...
Out-of-process CLI over the :mod:`src.diagnostics.debugging` helpers — no profiling flags
needed at launch time, so it works on a job that is already running (or already hung). Discovers
this node's torchrun training ranks (or takes explicit ``--pid``) and either:

  - ``dump``   — instantaneous py-spy stack dump of every rank (the hang-triage tool: one hung
    collective shows as N-1 ranks inside NCCL and one rank elsewhere),
  - ``record`` — per-rank CPU flame-graph SVGs sampled over ``--duration`` seconds (the tool for
    dataloader stalls, tokenization, and Python overhead between kernels).

GPU-side traces are the ``TorchProfilerCallback``'s job (``enable_torch_profiler: true``); this
covers the host side. Attach from the same pid namespace as the job — for a containerized run,
``docker exec`` into the training container first.
...
def main() -> None:
    ...
    # (the docstring's `record --pid N` form) — options on `parser` itself would not be.
