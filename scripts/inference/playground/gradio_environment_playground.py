...
# search dump) is capped at 16k chars by the env itself and would bury the turn it belongs to.
...
def render_transcript(trajectory: Trajectory | None) -> list[dict[str, str]]:
    ...
    turn rather than dropped, because the observation is what the next turn conditions on.
    """
...
def render_summary(env_type: str, trajectory: Trajectory | None, elapsed: float, max_turns: int) -> str:
    ...
    # A turn the engine cut off at its token cap is why an otherwise sane episode scores zero.
...
def run_playground_episode(
    ...
    The driver, not a local copy of the loop: it stamps ``finish_reason`` (so a length-cut turn is
    recovered here as it is in training, not graded as a final answer), binds the episode's
    reasoning-effort level and its token budget, and finalizes a truncated episode.
    """
    ...
    trajectory = asyncio.run(
        run_episode(
            ...
            rollout=RolloutConfig(
                # None, never "": an unset model name is dropped from the body and a single-model
                # server answers with what it serves, while an empty string 404s.
