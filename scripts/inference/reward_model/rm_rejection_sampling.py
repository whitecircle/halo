        ...
        try:
            ...
            for _ in range(args.n_hypos):
                ...
                if finish_reason == FINISH_REASON_LENGTH:
                    ...
                    # score it as a finished answer, and it would land in the preference file as the
                    # chosen or the rejected side on that number.
                    truncated += 1
                    continue
            ...
            # The first turn of a follow-up row was cut: one truncated generation, and every
            # hypothesis would have conditioned on it.
            stats["truncated"] += 1
            stats["skipped_truncated"] += 1
            print(f"Truncated at --max_gen_tokens for {row.get(args.id_field, '?')}: {e}")
        except Exception as e:
            # Counted, not just printed: a row that never reaches the scoring queue is a dropped row,
            # and an uncounted one makes a dead generation endpoint look like a clean short run.
            stats["failed"] += 1
...
def build_preference_result(
    ...
    The record itself is :func:`preference_record`: one writer for the trainers' contract, since both
    rejection samplers feed the same readers. Each hypothesis is the last turn of one scored
    conversation, and the reward model that graded it is this sampler's own provenance column.
    """
...
def build_offline_grpo_result(
    ...
    The record itself is :func:`offline_grpo_record`: one writer for the trainer's contract, since
    both rejection samplers feed the same reader. Each completion is the last turn of one scored
    conversation — the hypothesis the reward model graded.
    """
...
async def score_and_select(
    ...
    while True:
        # Outside the try: the cancellation that stops this worker arrives here, and a task_done()
        # in the finally would then be counted against an item that was never taken.
        ...
        try:
            ...
            # The single-output shape guard is a deterministic misconfiguration (wrong RM), not a
            # transient row error — record it as fatal so main aborts instead of dropping every row.
            stats["fatal"] = str(e)
            stats["failed"] += 1
        except Exception as e:
            stats["failed"] += 1
...
async def save_results(
    ...
    while True:
        ...
        try:
            ...
            # A write failure is never row-local — ENOSPC, a read-only mount, a deleted output
            # directory all fail identically for every following result. Swallowing it drops the
            # whole run's output while the counters and the exit code report success.
            raise RuntimeError(
...
async def join_worker_queue(queue: asyncio.Queue, worker: asyncio.Task) -> None:
    ...
    ``Queue.join`` waits for one ``task_done`` per item, so a worker that raised leaves it waiting
    forever on items nobody will take. Racing the two turns that deadlock into the worker's own
    traceback.
    """
    join = asyncio.create_task(queue.join())
    done, _ = await asyncio.wait({join, worker}, return_when=asyncio.FIRST_COMPLETED)
    # Worker first: a worker that died as the queue drained is in `done` alongside the join
