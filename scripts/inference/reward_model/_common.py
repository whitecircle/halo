...
# Longest basename these scripts will build, in bytes. Every common filesystem caps a single name at
# 255 bytes, and the failure lands as an uncaught OSError(ENAMETOOLONG) out of the first
# `output_path.exists()` — after the reward model is already resident on the GPU. Reachable without
# anything exotic: vLLM's --served-model-name defaults to the served model's PATH, which is what
# --model_name then carries.
...
class TruncatedGenerationError(RuntimeError):
    ...
    Its own class, not ``ValueError``: the scoring scripts treat ``ValueError`` as a fatal
...
class OverlongConversationError(RuntimeError):
    ...
    A per-row drop like :class:`TruncatedGenerationError`, never a truncation: right-truncation cuts
    the assistant turn, and the reward model would score the prompt fragment as the answer's reward.
    """
...
def build_output_path(output_folder: str, prompts_source: str, model_name: str, suffix: str) -> Path:
    ...
    in this file: a name without it has a second model's run read the first model's rows as its own and
    ...
    down to fit, keeping the suffix and a digest of the identifying pair — measured in BYTES, since the
    cap is a byte cap and either half (a long prompt path, a long served-model path) can overrun it on
    its own, and the digest is what stops two cut names from colliding back into the collision this
    filename exists to prevent.
    """
...
def load_reward_model(
    ...
    """
    # A per-rank EP-sharded dir loads its non-expert weights under their true names while every
    # expert silently randomly-initializes, so the scores would look plausible and be garbage.
    ...
    # After the cast, never before: transformers 5 hands every non-persistent buffer back
    # uninitialized (a remote-code family overriding ``_init_weights`` scores off a dead RoPE), and
    # ``Module.to(dtype=...)`` downcasts float BUFFERS too — repairing first writes the fp32 rotary
    # table the seam exists for and then rounds it to bf16.
...
def boot_scoring_run(args) -> tuple[Any, Any, Any, torch.device]:
    ...
    One call for both scripts, so neither can drift onto a different set of ``--rm_*`` knobs than the
    other — every flag the reward model is loaded under is threaded here.
    """
...
async def generate_chat_message(client, messages: list[dict], args, response_format: dict) -> tuple[dict, str]:
    ...
    because a hypothesis cut at the token cap is a fragment: the reward model scores it as a complete
    answer and the number lands in a preference / offline-GRPO file as if the policy had stopped there.
    """
...
async def prepare_generation_prompt(client, row: pd.Series, args) -> tuple[list[dict], dict]:
    ...
    if isinstance(follow_up, list) and len(follow_up) > 0:
        ...
        if finish_reason == FINISH_REASON_LENGTH:
            # The follow-up turn conditions on this answer, so a fragment here poisons every
...
def prepend_answer_context(conversation: list[dict], correct_answer: str | None) -> list[dict]:
    ...
    Both scoring paths must show the RM the same context, or their scores are not comparable — which
    is why the image refusal sits here: this is the one seam every scored conversation passes on its
    way to the reward model's chat template. Reward models are text-only sequence classifiers, so an
    image part would render as placeholder tokens with no pixels behind it and the returned "reward"
    would be a number computed over holes — which rejection sampling then writes into a preference /
    offline-GRPO training file.
    """
...
def require_single_output(logits) -> None:
    ...
    arbitrary — or misaligned per-sample — class logit as the "reward", which reads as plausible.
    """
    if logits.shape[-1] != 1:
        raise ValueError(
...
def encode_for_scoring(tokenizer, conversations: list[list[dict]], correct_answer: str | None) -> dict:
    ...
    Render through the chat template, then :func:`tokenize_rendered` — the Bradley-Terry reward
    map's own path (``build_reward_preprocess_fn``) — so BOS lands exactly once on a tokenizer whose
    post-processor prepends it; re-tokenizing the rendered text would double it, and the scores
    would come from a token sequence the model never trained on. A conversation past
    ``tokenizer.model_max_length`` (``--rm_max_seq_len``) raises :class:`OverlongConversationError`.
    """
    encoded = [
        tokenize_rendered(
            tokenizer,
...
async def score_conversations_offloaded(
    ...
    flight on the same loop. One offload for both scripts, so neither can drift off the other's
    argument order into scoring with the wrong batch size or a missing answer context.
    """
...
def load_local_jsonl_resume(output_path: Path, id_field: str) -> tuple[set, pd.DataFrame]:
    ...
    both scripts append one finished row per line, so the ids in the output file are exactly the work
    a re-run must skip. An output that does not exist yet resumes nothing — and the rows come back
    because a caller whose summary statistics span the whole job needs them, not just their ids.
    """
