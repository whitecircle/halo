    ...
    async with semaphore:
        try:
            ...
            if finish_reason == FINISH_REASON_LENGTH:
                # Scoring a fragment as if it were a finished answer writes a reward for text the
                # policy never got to finish; the row is dropped, counted and reported.
                stats["truncated"] += 1
            ...
            # The single-output shape guard (score_conversations) is a deterministic misconfiguration
            # — re-raise so the run aborts loudly instead of silently scoring nothing.
            raise
        except Exception as e:
            # Counted, not just printed: the statistics below are computed over the rows that
            # survived, so an uncounted drop makes a run that scored a tenth of its prompts read as
            # a complete one.
            stats["failed"] += 1
...
async def main():
    ...
    if not pending.empty:
        ...
        # Keyed on the growth: a length check passes the moment a single resumed row is loaded, so a
        # resumed run against a dead endpoint would report the PREVIOUS run's rows as its own output
        # and exit 0. An empty `pending` never reaches here — producing nothing is correct there.
