...
A self-contained replacement for external merge tools — no mergekit dependency. Supports the simple,
widely-used weight-space methods:
...
never the merged model. Output is HF-sharded safetensors (``--max_shard_size`` sets the per-file cap). Config, tokenizer and any
...
def _parse_model_spec(spec: str) -> tuple[str, float | None]:
    ...
    Splits on the LAST colon so Windows-free absolute paths and ``hf/org/model`` ids are safe; the
    suffix must parse as a float to be treated as a weight (else the whole string is the path).
    """
    if ":" in spec:
...
class _TensorReader:
    ...
    def _handle(self, key: str):
        ...
        # against model[0] ahead of the merge loop's own coverage check, so the refusal lives here:
        # a bare KeyError there names neither the model that lacks the key nor why it was wanted.
...
def _reference_keys(method: str, readers: list[_TensorReader], base_reader: _TensorReader | None) -> list[str]:
    ...
    incomplete, unloadable merged checkpoint. ``linear``/``slerp`` have no base → use model[0]."""
...
def _merge_linear(tensors: list[torch.Tensor], weights: list[float]) -> torch.Tensor:
    # Always a weighted AVERAGE — divide by the weight sum. A raw sum (default weights 1.0) would
    # scale every tensor by the model count and blow up activations with no error.
    total = sum(weights)
    if total == 0:
...
def _merge_slerp(t0: torch.Tensor, t1: torch.Tensor, t: float) -> torch.Tensor:
    ...
    identical vectors reads 0, and the "orthogonal" arc sums to ``-inf`` where the input was finite.
    """
    v0 = t0.float().flatten()
    v1 = t1.float().flatten()
...
class _MergeMethod(NamedTuple):
    """One merge method's whole contract: the op, the per-key tensors the loop hands it, and the
    scalar CLI knobs it reads with their defaults. The ops declare no defaults, so this table is the
    single home for them — the gate, the dispatch and ``--help`` all read it."""
...
def _check_method_knobs(method: str, explicit: set[str]) -> None:
    ...
    A silently ignored knob (``--models a:0.1 b:0.9`` under slerp, ``--t`` under linear) merges
    something other than what was asked, with no error.
    """
...
def merge_models(
    ...
    # Every knob the caller set must be one the method consumes — refused up front, before any I/O.
    ...
    # Symmetric coverage: the per-key loop raises on a reference key a model LACKS, but an EXTRA key is never
    # visited and vanishes silently. The realistic case is an untied head — reconcile_tie_word_embeddings flips
    # tie_word_embeddings only for the model whose lm_head diverged, so the merge would discard that head.
    ...
    # Created only past the last input gate, so a refused merge leaves no directory behind.
    ...
    # Peak RAM ≈ one fp32 copy of the largest tensor per contributing model (TIES stacks them all)
    # on top of the writer's pending output shard. Disk: the merged artifact is one input's size.
    ...
    for i, key in enumerate(ref_keys):
        ...
        # Integer/bool entries are structure, not weights (DeepSeek-V4's int64 tid2eid, expert-count buffers):
        # averaging them and casting to bf16 corrupts the model silently. Pass through when they agree, else refuse.
        ...
        # Balancing tensors export at trained dtype like every merge/gather path (mirrors
...
def _copy_aux_files(
    ...
    *,
    ...
    verbatim — critically including the **processor** files for multimodal models
    (``preprocessor_config.json``, ``processor_config.json``, ``chat_template.*``),
    ``generation_config.json`` and a remote-code family's ``.py`` modules, none of which a
    config+tokenizer re-save emits. A Hub id is downloaded (weights excluded) and copied the same
    way: re-saving its config instead would write the source's ``auto_map`` while shipping none of
    the modules it names, leaving a directory that raises ``does not appear to have a file named
    modeling_<x>.py`` for every consumer. The config's ``dtype`` is then set to the merged dtype so
    ``from_pretrained`` loads it consistently.
    """
    # A hub --tokenizer_source may ship legacy .pth/.gguf/.h5/.tflite/.ot exports and a vendor
    # weight dump (``original/``) — tens of GB config+tokenizer resolution never needs. The same
    # tuple copy_checkpoint_aux_files refuses, so a downloaded source and a local one drop alike.
    if not os.path.isdir(source_dir):
        source_dir = snapshot_download(source_dir, ignore_patterns=list(WEIGHT_FILE_IGNORE_PATTERNS))
    # No resume sidecars (rng_state*, scheduler.pt, router_balancing_biases.pt): they describe ONE
    # training run's state, and an N-way merged artifact has no such run — shipping one input's
    # sidecars would seed a resume/serve with state the merged weights never trained under.
    ...
    if not any(name.startswith(("tokenizer", "vocab", "spiece", "merges")) for name in os.listdir(output_dir)):
        # A merged artifact nobody can build a tokenizer for is not a deliverable, and warning about
        # it while exiting 0 hands the next pipeline stage a checkpoint that fails at ITS load.
    ...
    # Keep the saved config's dtype consistent with the merged weights, and write the CANONICAL torch name:
    # `--dtype bf16` is accepted but `torch.bf16` does not exist, so a raw write fails every from_pretrained.
    ...
    # The merged directory owes the same config contract as any other export: the merge tools and the
    # pinned servers read a family-less vendor config (Bailing/Ling) and a source-schema family
...
def main() -> int:
    ...
    # the help text renders each op signature's own default rather than a re-typed copy of it.
    parser.add_argument(
        "--density",
        type=float,
        default=None,
