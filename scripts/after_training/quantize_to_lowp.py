...
Pairs with the mixed-precision **QAT** training path (``lowp_precision: fp8|fp4|mxfp4``, i.e. mxfp8/nvfp4/mxfp4): a model trained
that way keeps bf16/fp32 **master** weights, and its forward already computes in the block-scaled format.
Quantizing those master weights to the SAME format here reproduces the training (QAT) forward *exactly*
— verified at ``relerr = 0`` for both mxfp8 and nvfp4 — so the converted model runs in low precision with
no accuracy loss beyond what training already saw. (A model trained in plain bf16 can also be converted,
but then it is post-training quantization, not QAT, and may lose more accuracy.)
...
Which weights those are is derived from the rosters that decide what QAT quantized — the trainer's dense
``MLP_PROJECTIONS`` plus the EP layer classes' expert layouts — so the export cannot quantize a weight the
training forward left in bf16. Every other tensor (attention, embeddings, norms, lm_head, biases, and a
VLM's vision tower / projector, which the dense conversion never reaches) is copied unchanged in its
original dtype. The four ``lowp_*`` SCOPE knobs the run trained with must be passed back in
(``--lowp_apply_dense_mlp`` / ``--lowp_apply_moe_experts`` / ``--lowp_keep_first_blocks`` /
``--lowp_keep_last_blocks``, defaulting to the training defaults): they are not recoverable from the
checkpoint, and quantizing a block the QAT forward held in bf16 is exactly the drift the rosters above
prevent everywhere else. A ``quantization_config.json`` records the scheme so a loader — an inference engine, or our own
:mod:`src.kernels.lowp.quantization` dequantizer — can reconstruct the weights. The block layout is OCP
microscaling (mxfp8) / NVFP4 (1x16 e4m3), the same formats TensorRT-LLM / vLLM consume via
``compressed-tensors`` / ModelOpt; mapping this manifest to a specific engine's loader is the remaining
integration step (the data + scale tensors are already in the right numeric layout).
...
# Fused MoE experts are bare nn.Parameters (3-D [num_experts, in, out], no .weight suffix). The names come
# from the EP layer classes' _HF_FUSED_EXPERT_KEYS, so a new fused family is covered without a second list.
_FUSED_EXPERT_SUFFIXES = hf_fused_expert_keys()

# What low precision covers, DERIVED from the two rosters that decide it rather than restated: the
# dense MLP projections the trainer converts (``MLP_PROJECTIONS`` — the QAT forward quantizes those
# and nothing else) and the expert-FFN weight names the EP layer classes declare, fused (3-D) and
# per-expert un-fused alike. A pattern written by hand here quantizes weights the QAT forward left
# in bf16 (any `fc1`/`fc2` or bare `mlp` match), which is exactly what this export must not do.
...
# families (Qwen-VL's ViT blocks carry gate_proj/up_proj/down_proj), but the trainer converts the
# TEXT backbone only (``_text_backbone_prefix``), so quantizing them would not reproduce the QAT forward.
_DEFAULT_EXCLUDE = (
...
# nvfp4 ≈ 0.08-0.15) with headroom. Above it, something structural is wrong — typically the
# contraction axis. The QAT forward is exact regardless of where a value lands in this band.
...
# Per-expert un-fused hub layout (``...experts.3.gate_proj.weight``): expert weights that are NOT the
# 3-D fused tensor, and so are told apart from a dense MLP projection by this segment alone. The
# container spellings are the EP layer classes' own (``routed_experts`` beside ``experts``).
...
def _is_expert_weight(name: str, ndim: int) -> bool:
    ...
    The seam ``--lowp_apply_moe_experts`` turns on, so it must be the complement of "dense MLP
...
def _should_quantize(
    ...
    *,
    ...
    """Quantize iff the tensor matches --include, not --exclude, is ≥2-D, is either a standard
...
def _reject_unexportable_experts(
    ...
    *,
    ...
    """Refuse an expert bank the scope would ship in bf16 under a claim of QAT parity.

    Two spellings escape ``_should_quantize`` silently: a 3-D bank under a name no EP layer class
    declares as fused — Step-3.7's hub ``moe.gate_proj``/``moe.up_proj`` ``[E, M, H]``, whose
    ...
    writes a ``quantization_config``. Either way the served experts would run in a precision
    training never used. Headers only, ahead of the output directory.
    """
...
def _fused_expert_axis(input_dir: str) -> int | None:
    ...
    ``model_type`` matches no registered family — the caller raises if a fused expert then shows up,
    rather than guessing an axis and silently corrupting every dequantized expert.
    """
...
def _backbone_block_count(input_dir: str, include: re.Pattern, exclude: re.Pattern) -> int:
    ...
    stacks (a vision tower the exclude fence did not catch, a draft/MTP head) share one index space
    here, so the keep-window would hold back whichever stack happens to own that index. There is no
    live module to resolve a text-backbone prefix from at export time, so this refuses rather than
    guesses which stack the window meant.
    """
    indices: set[int] = set()
    roots: set[str] = set()
...
def quantize_checkpoint(
    ...
    *,
    ...
    # Index-driven enumeration: a co-located adapter_model.safetensors is never quantized into the rebuilt
    # index, and a per-rank sharded input is rejected before the output directory exists — resolved here
    # rather than left to the lazy walks below, whose refusal would land after the output was created.
    ...
    # The shared walk opens each shard once, in order, so grouping its entries by shard is what carries the
    ...
    for shard, entries in groupby(iter_checkpoint_shard_entries(input_dir), key=itemgetter(0)):
        ...
        for _shard, reader, name in entries:
            ...
            if verify:
                ...
                # max(x, nan) returns x, so a non-finite weight would look like a clean round-trip; raise instead.
    ...
    # Anything but a single model.safetensors needs an index so the renamed weight_packed/scale keys stay
    # discoverable. Keyed on emitted FILENAMES: a lone model-00001-of-00001 input would otherwise get neither.
    ...
    # The index is in the keep-set only when THIS run wrote one, else a single-file run leaves a stale index.
...
def _write_manifest(
    ...
    manifest = {
        ...
        # Authoritative per-weight contraction axis: 3-D fused experts differ by family, so a loader must read
        # the axis per weight here and fall back to contraction_axis only for names absent from this map.
        ...
        # The low-precision SCOPE this export reproduced. Recorded because it is not derivable from the
        # weights: a re-export, or anyone comparing this artifact to its training run, needs the same
        # four ``lowp_*`` values back.
        "scope": scope,
    ...
    # Stamp the scheme into config.json — the standard key readers consult; it also flips the toolkit's
    # native-quantized detection (EP lazy loading routes such checkpoints to from_pretrained). Never set
    # quant_method to the raw format name: "mxfp8"/"mxfp4"/"nvfp4" are real transformers methods with a
    # DIFFERENT on-disk layout, which would trade the refusal for a wrong-layout load.
...
def parse_args():
    ...
    # The training run's lowp SCOPE, which the checkpoint does not carry. Same names and defaults as the
    # ParallelismConfig knobs, so a config's values transfer verbatim.
    p.add_argument(
