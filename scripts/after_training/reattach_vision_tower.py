...
A ``text_only_model`` run trains and exports the CausalLM sibling of a VLM checkpoint
(``model.*`` keys, ``*_text`` config, no vision tower) — an artifact vLLM 0.26.0 cannot load
natively: its registry carries only the multimodal wrapper class for Qwen3.5/3.6. (SGLang 0.5.17
registers the text CausalLM classes and loads the export as-is.) This script rebuilds the
wrapper-layout artifact:
the trained text weights are re-prefixed to ``model.language_model.*``, the UNTRAINED vision tower
...
wrapper-layout checkpoint is refused — it needs no re-attachment. The base must be the multimodal
...
def _grafted_composite_config(base_dir: str, text_config: AutoConfig, trust_remote_code: bool):
    ...
    # The graft is family-blind (a plain attribute assignment), so the base must be checked to be
    # the family the export came from: a text tower grafted into another family's wrapper is a
    # config that loads and serves garbage.
...
def reattach_vision_tower(
    ...
    *,
    ...
    # The base is streamed from while the writer's close sweeps every model*.safetensors it did not
    # write — an output aimed at the base directory would replace the base with the wrapper artifact.
    ...
    # Header-only, ahead of the output directory. The trained export supersedes the base's text
    # tower only where the base stores it under the wrapper prefix the export is re-prefixed to; a
    # base keeping a vendor namespace (renamed inside from_pretrained) would be carried whole beside
    # the trained tower — two text towers colliding on load — and a base with nothing else to
    ...
    # Tokenizer/template/etc. follow the TRAINED export; the processor files only the base has.
...
def main() -> None:
    ...
    # Both configs load through it, and --model_id may be a Hub repo: opt-in, like every sibling
    # whose source can be one.
