...
This is the after-training tool for an EXISTING checkpoint; a training run does not need it, because
...
def _passthrough_copy(checkpoint_dir: Path, output_dir: Path, dry_run: bool) -> None:
    ...
    A no-op reset still owes its caller the output it named: returning without writing leaves the
    next pipeline stage pointing at a directory that does not exist, or at a stale same-named one.
    """
...
def _reset_sinks_safetensors(safetensors_path: Path, output_dir: Path, dry_run: bool) -> int:
    ...
    # Write via a sibling temp file + atomic rename: under --in_place this REPLACES the only copy,
    # and a kill mid-write would otherwise lose it.
    ...
    # so a write that kept live sinks must never get that far.
    ...
    try:
        ...
        tmp_path.unlink(missing_ok=True)  # nothing that failed to verify is worth keeping
        raise
...
def _swap_staged_checkpoint(staging_dir: Path, output_dir: Path) -> None:
    ...
    Every entry is CLONED beside its final name first — a hard link, so it costs no I/O — and the
    clone is what gets replaced into place. Moving the staged entries instead would empty the
    staging directory as the swap proceeds, in the order the filesystem happens to hand back: a
    failure part-way would then leave NEITHER directory holding a whole checkpoint (the weights
    already moved are gone from staging, the ones still to come are missing from the target), and
    under ``--in_place`` that is the only copy. Cloning keeps the staged checkpoint whole until the
    last entry has landed, so a failure can KEEP and name it for a human to move by hand.
    """
...
def _reset_sinks_from_pretrained(
    ...
    # Sink models (gpt-oss patched checkpoints) are this script's whole purpose — load with remote
    # code allowed and the widest matching Auto* class.
    ...
    # The trainers' own seam, not a second walk: it neutralizes exactly what every trainer
    # neutralizes and RAISES when the layer walk finds nothing on a sinks-carrying model, rather
    # than saving a checkpoint whose sinks are still live under a name that says they are off. The
    # stamp it leaves is the only signal it did anything — a family that carries no sinks is
    # returned untouched.
    ...
    try:
        ...
        # Name the real error: a network/permission failure is not the same as a checkpoint that
        # genuinely ships no tokenizer, and the aux-file copy already covers the latter.
        logger.warning(f"Tokenizer not saved from {checkpoint_dir} ({type(exc).__name__}: {exc}).")
        tokenizer = None

    # Staged write → verify → swap, for the reason the single-file branch stages its temp file:
    # under --in_place this writes over the only copy, and save_pretrained writes many files — a
    # kill mid-write, or a save that silently kept live sinks, must never replace the source. The
    # source's aux files ride along (save_full_checkpoint copies them BEFORE writing, so the fresh
    # config wins).
    ...
    try:
        ...
        shutil.rmtree(tmp_dir, ignore_errors=True)  # nothing that failed to verify is worth keeping
        raise
    _swap_staged_checkpoint(tmp_dir, output_dir)
    # The verified save owns the directory now: drop the weight files it did not produce (a stale
...
def reset_sinks(
    ...
    Args:
        ...
        in_place: Rewrite ``checkpoint_dir`` itself. Explicit because it has no undo — every sibling
            conversion tool refuses an in-place run outright, and this one only allows it on request.
    ...
    """
    # The input gate runs FIRST: a per-rank EP/TP save lands on the from_pretrained branch, where
    # the real expert keys read as MISSING and are randomly initialized — that diagnosis is more use
    # than "you did not say where to write", and it holds whichever destination was named.
    ...
    if in_place:
        ...
        # A dry run only reads (every write below is gated on it), so it owes no destination. The
        # refusal guards the WRITING invocations, where defaulting to the input meant a mistyped
        # command rewrote the only copy with no undo.
        if not dry_run:
            raise ValueError(
        ...
        # Same refusal every sibling conversion makes, for the same reason: an --output_dir aimed at
        # the input is the in-place run the flag above exists to make explicit.
        reject_in_place_conversion(checkpoint_dir, output_dir)

    # Both branches hold the whole checkpoint in host RAM (a single-file load, or from_pretrained onto
    # the CPU), and a writing run stages a full copy beside its target.
    preflight_model_load_resources(
    ...
    if safetensors_path.exists():
        ...
        # Only this branch sweeps here: it copytrees the source and rewrites one file, so a stale sharded set can
        # ride along. Never in-place; the from_pretrained branch sweeps its own output after the staged swap.
...
if __name__ == "__main__":
    args = parse_args()
    # sanitize_generation_config logs through accelerate's rank-aware logger, which raises without this state
    # — and every gpt-oss checkpoint ships do_sample=False, so it fires on this script's whole reason to exist.
    PartialState()
    reset_sinks(
        args.input_dir,
