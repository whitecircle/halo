...
never the model — which is what lets a 397B artifact merge on an ordinary host.
...
# Union of every EP family's class-declared expert-weight roots (EPMoELayerBase._EXPERT_WEIGHT_ATTR_ROOTS),
# derived rather than hand-listed so a new family's expert shards can never pass through as non-expert weights.
_EP_EXPERT_SUFFIXES = sorted(expert_weight_roots(), key=lambda s: (-len(s), s))

# Expert keys are ...{container}.{suffix}. The container is captured verbatim (mlp, feed_forward, or the block
# itself) so transforms re-emit experts under the SAME container. Captures (prefix_with_container, param_suffix).
...
def _resolve_merge_transform(
    ...
    if not has_expert_groups:
        # Only expert params are sharded, so merged shards matching nothing means the suffix union missed this family.
...
def merge_ep_shards(
    ...
    """
    # Every input gate runs before the output directory is created, so a refused merge leaves no trace.
    ...
    # A gathered (or PP/TP) checkpoint already carries HF-layout expert keys that match the pattern below, so
    # merging one doubles the ``experts.experts.`` prefix and from_pretrained silently loads it as MISSING.
    ...
    # EP shards only — a bare .safetensors glob would pull in adapter_model.safetensors or a stale model.safetensors.
    ...
    with ExitStack() as stack:
        ...
        # Logical output tensors in the reference order: passthrough keys first, concatenated
        # shard groups after — exactly the order loading every file into one dict produced.
        ...
        # Peak RAM ≈ the largest group twice over (the concatenated inputs plus the transform's
        # copies) on top of the writer's pending output shard. Disk: the merged artifact is the
        # input shards' size (tensors are re-emitted at their stored dtype).
        largest_group = max(
            [
        ...
        def emit(state: dict) -> None:
            ...
            The sharded writer already applied ``save_dtype_caster`` — the module-tree keep-set
            (norms, balancing tensors, the family's fp32 pins) that the gathered save uses — so the
            ...
            ``A_log``/``dt_bias``, Inkling's short convolutions, DeepSeek-V4's ``attn_hc``), breaking
            merged-from-sharded == gathered on exactly the tensors pinned because bf16 breaks them.
            """
