...
Handles causal-LM, sequence-classification and bare-base checkpoints, optionally merging a LoRA adapter
into its base model first. ``--verify`` reads the saved safetensors headers and asserts the STORED
parameters are primarily bfloat16, raising on failure; ``--check_inference`` runs a short generation /
classification pass and reports its output. ``--output_dir`` is written fresh — every
``model*.safetensors`` or index the completed save did not produce is swept away afterwards.
...
# (``auto_load_model`` — AutoModelForCausalLM would drop a VLM's vision tower). One table so no
# call site can reach ``from_pretrained`` around the coverage gate every entry here goes through.
_MODEL_CLASSES = {
    "causal_lm": None,
...
# --verify bar: share of saved float PARAMETERS that must be bf16, weighted by numel — the fp32 norm leaves
# are ~⅓ of tensors but <0.01% of params. 0.99 not 0.9: a whole fp32 embedding+head still scores ~0.97.
_BF16_VERIFY_MIN_FRACTION = 0.99

# --check_inference smoke test: a handful of sampled tokens is enough to show the converted weights
# still produce text rather than garbage, and short enough that the check costs seconds on any host.
...
def load_model(model_path, model_type, is_peft=False, **load_kwargs):
    ...
    Raises on an unknown type, or a PEFT-unsupported one, rather than silently falling back to a
    different architecture head.

    ``is_peft`` loads the adapter's BASE and applies the adapter on top, instead of handing the
    directory to PEFT's own auto-class: that class reaches ``from_pretrained`` for the base
    directly, where a truncated or key-mismatched checkpoint random-initializes the absent tensors
    with a log line nobody sees — and this tool would then write them out as a finished bf16 model.
    The seq-cls head is the one absence that can be legitimate (a classification adapter trained on
    a plain causal-LM base carries it in ``modules_to_save``), and it is legitimate only then, so
    the excuse is read off what the adapter declares.
    """
...
def verify_adapter_save(adapter_path):
    ...
    PEFT restores LoRA ``A``/``B`` to fp32 as it writes them (``autocast_adapter_dtype``), so this
    path's own correct output is entirely fp32 — asserting bf16 on it asserts the wrong thing. What is
    checkable is that the adapter carries tensors at all: an empty one loads as a silent no-op.
    """
...
def verify_model_conversion(model_path):
    ...
    Deliberately not a ``from_pretrained`` reload: that path casts every tensor to ``config.dtype`` on
    the way in, so a checkpoint still stored in fp32 comes back 100% bfloat16 and the check can never
    fail — which is the whole job of ``--verify``. The header carries each tensor's dtype AND shape, so
    the parameter-weighted share costs no tensor reads.
    ...
    try:
        # Resolved eagerly, ahead of the (lazy) walk below, so the adapter fallback still sees the
        # FileNotFoundError instead of it surfacing from inside the loop.
    ...
    # Float tensors only: an integer buffer can never be bf16, so counting it would only dilute the share.
...
def run_test_inference(model_path, model_type, is_peft=False, trust_remote_code=True) -> None:
    ...
    Diagnostic only — a broken conversion shows up in the printed text/logits, not as a verdict this
    can return, so the caller has nothing to branch on. ``--verify`` is the gate that raises.
    ``is_peft`` picks the adapter auto-class, the same split :func:`verify_model_conversion` makes.
    """
...
def _cast_norms_to_fp32(model) -> None:
    ...
    Classified by the toolkit's own norm predicate — the one every save-dtype decision uses — rather
    than a second name rule that could disagree with it. Leaves only: a container that happens to
    classify as a norm must not drag its children up with it.
    """
...
def _merge_adapter_to_bf16(
    ...
    *,
    ...
    The gate order, the adapter's own sidecars and the merged save belong to
    :func:`merge_adapter_into_base`; what belongs to this tool is the dtype the base loads at and the
    fp32 norm leaves the merged model is written with.
    """
    _reject_unsupported_peft_type(model_type)
...
def _convert_checkpoint_to_bf16(
    ...
    *,
    ...
    # A per-rank EP/TP save reaches from_pretrained with expert keys under .shard_N: the real ones read as
    # MISSING and are randomly initialized (a warning, never a raise), which --verify's dtype count cannot catch.
    # Both guards run ahead of the load and of os.makedirs(output_path), so a refusal leaves nothing behind.
    if os.path.isdir(model_path):
        reject_sharded_checkpoint(model_path)
    reject_in_place_conversion(model_path, output_path)

    # Full processor for VLMs (keeps processor_config.json), else a plain tokenizer. A PEFT adapter dir
    # usually carries only a tokenizer, so resolve it against the base model too.
    if is_peft:
        # A later merge_and_unload cannot fold native EP expert-LoRA — refuse before re-saving one.
        assert_no_expert_lora_adapter(model_path)
        base_model_path = PeftConfig.from_pretrained(model_path).base_model_name_or_path
        # Under --peft the weights come from the BASE, so the guards above covered the wrong directory: an
        # --output_dir aimed at the base would overwrite the very checkpoint being read.
    ...
    if not is_peft:
        ...
        # reach the output — the sidecars are carried across verbatim instead, for the later merge.
    ...
    if is_peft:
        # Unmerged --peft writes ADAPTER files only: sweeping here would delete the full-model weights the
        # directory holds, and restore_model_type would stamp a family this save never wrote.
        sanitize_generation_config(model)
        model.save_pretrained(output_path)
        if processing_class is not None:
            processing_class.save_pretrained(output_path)
        # Only this branch has to carry them by hand: the adapter dir's sidecars must survive for
        # the later merge, and save_pretrained copies nothing.
...
def convert_to_bf16(
    ...
    # Refused before any I/O: merging is read only on the PEFT path, so without it the tool would exit 0
    # having written an UNMERGED checkpoint — the opposite of what a caller asking to merge wanted.
    ...
    # Raise, so a failed conversion cannot report success to a caller.
    ...
    if check_inference:
        # An unmerged --peft save is an ADAPTER dir with no config.json, so the plain loader would raise here.
        run_test_inference(
...
def main():
    args = parse_args()
    # The shared model_preparation helpers log through accelerate's rank-aware logger, which needs this state.
    PartialState()
    convert_to_bf16(
        args.input_dir,
        args.output_dir,
