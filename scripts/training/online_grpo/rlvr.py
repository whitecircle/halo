...
def accuracy_reward(completions, answer, **kwargs):
    ...
    **Strict boxed exact-match**, deliberately not the environments' validated-answer chain
    (``compute_answer_reward``, ``DEFAULT_METHODS = exact + numeric``, which every environment grader
    uses). The two differ on purpose and are not interchangeable: this one strips a GSM8K ``####``
    rationale prefix and ``,``/``$`` from both sides and then requires string equality, so it grades
    only what the policy put in the box; the environment chain instead normalizes case, re-extracts a
    box from the *ground truth* as well, and accepts a numeric match within ``rtol=0.01``. Swapping in
    the chain would silently re-grade every shipped RLVR recipe (``0.5`` vs ``.5`` and
    ``7 \\boxed{42}`` vs ``42`` flip verdicts), so the divergence is a pinned contract —
    ...
    for completion, gt in zip(completions, answer, strict=True):
        ...
        # An empty extraction is never correct, even against a blank ground truth: a missing answer column would
        # otherwise pay full reward for every box-less completion — the degenerate optimum a policy finds first.
...
def main():
    ...
    # hyperparameter; max_prompt_length is a dataset FILTER (rows over it are dropped, never truncated).
    ...
    def process_for_rlvr(row):
        ...
        if isinstance(prompt_data, list):
            # Conversational rows: inject the system prompt only when the conversation does not already open with
            # one (matches environmental_grpo) — an unconditional insert double-stacks system messages.
        ...
        # the per-episode analogue of the env-GRPO rollout (see src/environments/base.py).
    ...
    # A typo'd prompt/answer field silently yields empty answers → verifiable rewards all zero. Both
    # are checked here because this path declares no conversation_field for the loader to validate.
    ...
    processed_ds = process_dataset_with_map_and_filter(
        ...
        cache_key_extras={
            ...
            # fingerprint skips — omit it and a changed effort silently reuses the old rendering.
    ...
    # --use_sdpg swaps in the SDPG trainer (GRPO loss + privileged-teacher reverse-KL OPD on positive-advantage
    # rollouts); it reuses the same rollout + verifier machinery, so only the class and the OPD kwargs differ.
