...
``rollout_backend: sglang`` (gpt-oss only, no expert distribution). Every example config's header
carries the server flags that config needs — the ``--moe-backend triton`` gate, the tool-call and
...
# Assumed environment prompt overhead (tokens) when the chat template cannot render the env's system
# prompt + tool schema; deliberately conservative (tool-use preambles measure 300-1500 tokens).
...
def measure_env_prompt_overhead(environment, tokenizer) -> int:
    ...
    The dataset prompt filter measures only the templated user prompt, but the ROLLOUT prompt is
    ...
    prompt smaller than any the model will ever see. Falls back to a conservative margin when the
    template cannot render the preamble (logged with the assumption).
    """
...
def process_dataset(args: EnvironmentalGRPOScriptArguments, tokenizer, ds):
    ...
    # A typo'd answer_field silently yields answer-less rows: a full RL run with no ground truth and
    # all-zero rewards. prompt_field is NOT re-checked here — it is this script's conversation_field,
    # so the loader already ran the same guard over it.
    for knob, column in [
...
def main():
    ...
    # Same ownership rule for tools: the environment's tool registry supplies the schema sent to vLLM
    # (get_tools_schema), so a dataset tool column would never reach the rollout template. The
    # weight sync forwards trainer parameter names verbatim, and the text-only CausalLM sibling
    # spells its decoder model.layers.* where the multimodal checkpoint the server loads spells
    # model.language_model.layers.* — every dense tensor would miss its slot on the first sync.
    reject_unsupported_args(
    ...
    # The per-turn generation budget is rollout_max_tokens; a trajectory accumulates across turns and is
    # bounded by the context window (the trainer FAILS on overflow rather than truncating), so
    # max_completion_length is no length knob here — TRL reads it only as the dr_grpo normalization constant.
    grpo_config.max_completion_length = async_config.rollout_max_tokens

    # tokenizer.model_max_length is the model's context window — the SAME hard limit vLLM enforces during
    # rollout, and the trainer fails (never truncates) a trajectory that exceeds it.
    ...
    # process_dataset keeps only prompt/answer/context_fields, so an image column would be pruned
    # without a word and the run would train on the rows' text alone.
    ...
    # All ranks raise together. A single turn (prompt budget + per-turn generation) MUST fit the context;
    # the multi-turn worst case (prompt + max_turns × per-turn) is advisory — exceeding it can OOM the
    # forward before the fail-on-overflow check. Budgets come from a throwaway env (max_turns=None defers
    # to the env class default, and its system prompt + tool schema are measured into the prompt budget).
    ...
    # URL/key/model would otherwise mark every episode invalid, running the whole multi-GPU job on
    # zero advantage. Rank 0 probes and broadcasts so all ranks raise together.
