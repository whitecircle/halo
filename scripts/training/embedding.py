...
def build_sentence_transformer(
    ...
    """
    # QLoRA/k-bit is unsupported here: neither the EP/TP loader nor the SentenceTransformer path applies it,
    # so --load_in_4bit/--load_in_8bit would silently train full precision.
    ...
    if parallelism_config.is_ep_mode or parallelism_config.is_tp_mode:
        ...
        # The TP loader attaches the (dp, tp) mesh to the backbone, but the trainer reads it off the top-level
        # ST model — propagate it, or TP+DP mints a conflicting mesh and pure TP skips the TP grad-sync.
        ...
        # Main-first like every other load path: otherwise every rank downloads and materializes the model at
        # once. revision is threaded here as on the EP/TP branch, or a pinned checkpoint would depend on flags.
        ...
        # ST loads the backbone itself, so the parallel loader's sinks policy has to be applied here too — a
        ...
        # The checkpoint's modules.json decides pooling/normalization/length here while the EP/TP branch builds
        # them from the config; align them or those three knobs are silently inert on the default path.
...
def align_st_pipeline_to_config(st_model: SentenceTransformer, embedding_config: EmbeddingConfig) -> None:
    ...
    Warns on every value it changes: a checkpoint that ships its own pooling (Qwen3-Embedding is
    ``lasttoken``) is overridden by the config, which is what the config asked for but must be visible.
    """
...
def main():
    ...
    # The ST path has no setup_model_and_tokenizer / freeze stage, so these shared knobs would parse and do
    # nothing (a freeze request would silently become a full fine-tune) — reject them loudly.
    reject_unsupported_args(
        "Embedding training",
    ...
    # Inject LoRA with peft's inject_adapter_in_model, not SentenceTransformer.add_adapter (ST 5.5 gates that
    # on peft >= 0.18.2 while the image pins 0.18.1). It does not freeze, so freeze every non-adapter param.
    peft_config = get_peft_config(model_config)
    if peft_config is not None:
        # modules_to_save is worse than unsupported: the trainable copies ARE created, the freeze below re-freezes
        # them, and the wrapper renames the base tensor, so the saved ST module lacks the plain <mod>.weight.
