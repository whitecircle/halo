...
Dataset: RAW SFT conversations (``conversation_field``) + a privileged answer field (VLM adds an
...
def _build_text_dataset_and_collator(ds, args, tokenizer, max_length, model_config, num_proc):
    ...
    # Enforce the collator's length contract now, where the raise is world-uniform: at collate time only the
    # rank drawing the over-length row raises and its peers hang in the step's collectives until the watchdog.
    coordinated_map(
        train_dataset,
...
def _load_sdpg_reference(*, args, model_config, sft_config, dist_args, is_vlm: bool) -> PreTrainedModel | None:
    ...
    It goes through the shared frozen loader for the reason the KL term exists: ``L_ref`` is a
    divergence between this model's distribution and the policy's, so a reference on a different
    attention backend, or carrying live sinks the policy reset, moves the anchor rather than the model.
    """
    ...
    reference_model = load_frozen_auxiliary_model(
        ...
        # The pin only names a commit in the POLICY's repo, so it applies only when the reference is
        # that same repo; a separate reference repo takes its own main.
...
def main():
    ...
    # Inherited from SFTScriptArguments but unreachable: GenerateExamplesCallback needs a tokenized "generate"
    # split and self-distillation keeps the dataset raw, so wiring it would return None on every run.
    # num_eval_examples rides along on the default-comparing form — its own default (50) is truthy.
    reject_non_default_args("Self-distillation", args, "generate_eval_examples", "num_eval_examples")

    # Fail loud on inherited options the SelfDistill collators do not implement — silently ignoring
    # them trains something other than what the config states.
    ...
    # privileged columns must survive — HF's remove_unused_columns=True would strip them, leaving no inputs.
