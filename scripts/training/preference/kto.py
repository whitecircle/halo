...
def _reject_embedded_image_parts(dataset, completion_field: str) -> None:
    """Fail loud on image content parts in a dataset TRL would read as text-only.
    ...
    that no pixels back, and the run trains them as text. KTO renders nothing itself (TRL templates
    the raw ``prompt``/``completion`` columns), so the per-row ``reject_image_content`` backstop the
    other preference scripts inherit never runs here — this is the only guard the shape gets.
    """
    if dataset_declares_images(dataset):  # an image COLUMN — TRL's own probe sees it
        return
...
def _require_kto_columns(ds, args) -> None:
    """Fail loud on a configured KTO column the dataset does not carry.

    A configured field the dataset lacks is a typo, not an opt-out: skipping the rename silently
    leaves TRL's KTOTrainer to fail far downstream on a missing column, or — worse — to train on a
    stale ``completion``/``label`` column that happens to exist under the default name. Reported
    through the shared render-column guard, so a mistyped column reads identically here and on every
    other data path.
    """
...
def _rename_kto_columns(dataset, args):
    ...
    Renames exactly the pairs :func:`_require_kto_columns` checked — one roster, so a knob cannot be
    validated and then left unrenamed. Presence is that function's job, checked once over the whole
    DatasetDict before the first rename.
    """
    for knob, target in _KTO_RENAMED_FIELDS:
...
def main():
    ...
    # --- Model (text or VLM, auto-detected); padded preference takes the shared padded-workload backend
    # (SDPA, dropped under live sinks). One binding shared with the reference load: a logratio whose
    # halves came from different attention kernels is biased. ---
