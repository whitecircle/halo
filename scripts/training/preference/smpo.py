...
``images``/``image`` column (single image or list) — that column is what declares the VLM run.
"""
...
def main():
    ...
    # guards (the trainer enforces the same ones) fail before the potentially huge model load, and
    # pinned to the same revision as that load — hub `main` can name a different modality than the
    # commit this run trains.
