...
def main():
    ...
    # backend (SDPA, dropped under live sinks). One binding shared with the reference load: a
    # logratio whose halves came from different kernels is biased.
    ...
    # Vision routing keys on the DATASET, not the model: natively-multimodal models train on text preference
    # data through the normal text pipeline; only an image-carrying dataset takes TRL's vision path. On that
    # path the rows pass through untouched — TRL tokenizes them and auto-selects
