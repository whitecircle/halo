...
takes the text path — TRL chat-templates and tokenizes the raw columns itself. A run that declares
image data loads an ``AutoProcessor`` as the processing class and takes the vision pair: the
rendered-text map in ``src/data/pipeline/preferences.py``, placeholder expansion at collation in
...
def main():
    ...
    # would have nothing to decode. A flag that cannot take effect is refused, not parsed and dropped.
    reject_unsupported_args(
        "Reward modeling",
    ...
    # column, so the alias is what keeps the two verdicts identical (and a typo'd name loud).
