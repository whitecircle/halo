...
GPU tests are torchrun scripts, so this conftest does NOT import them. Instead it reads
...
with the spec's ``timeout`` (on expiry the agent *and* its workers are killed — torchrun starts
every worker in its own session, so killing the agent's process group alone leaves them running).
It then classifies:

* **ERROR**  — non-zero exit with no structured result line → infra / hang / import crash.
* **FAIL**   — a ``__HALO_TEST_RESULT__`` line with ``status="fail"``/``"error"`` (a real
               assertion failure), or a non-zero exit whose result line says ``pass`` (only rank 0
               emits it, so that combination means a NON-ZERO RANK failed).
* **SKIP**   — fewer GPUs than ``nproc`` available, or an OOM on the 8-GPU ``full`` tier
               (an OOM on a 2-GPU ``core`` smoke is a real ERROR — that config must fit). Zero
               GPUs is never a skip: it would report the whole tier green, so it is a usage error.
* **PASS**   — exit 0 (and ``status="pass"`` when the script uses ``gpu_test_main``).

This beats ``torchrun -m pytest`` (N-rank double collection, racing JUnit, duplicated
...
# Marker → (env var holding the base URL, default URL, compose command). Raw os.environ, not
...
# Set by the make server tiers: the run EXPLICITLY asked for this engine, so an unreachable server is
# a usage error, never a green tier of skips (mirrors the zero-GPU refusal below).
...
def _gpu_count() -> int:
    ...
    A *broken* driver is not 0 — a hanging or erroring ``nvidia-smi`` is the signature of a wedged
    GPU (exactly what a killed hang leaves behind), and answering 0 there would skip the whole tier
    green. Raise instead.
    """
...
def _launch_survivors(launch_id: str) -> list[int]:
    ...
    (rather than the script path) targets only processes this launcher spawned — never a co-tenant's
    job on a shared box.

    One pass is not a survey: a single ``/proc`` readdir drops a live entry whenever the process table
    churns underneath it (measured at ~15% against a process that both the preceding and following
    pass saw), and under-reporting here lets a wedged worker keep its GPU with nothing said. Union the
    passes so a miss has to happen twice in a row to be believed.
    """
...
def _kill_launch(proc: subprocess.Popen, launch_id: str) -> list[int]:
    ...
    group, then sweep the workers that outlive it — a wedged NCCL rank never exits on its own and
    holds both its GPU and this launch's stdout pipe open, which would stall the next node and the
    output drain.
    """
...
class GPUCase:
    ...
    def _launch_once(self, tmp_path: Path):
        ...
        # TORCHELASTIC_ERROR_FILE deliberately unset: one shared path makes the last writer win, and
        # the agent's collateral SIGTERMs would overwrite the real cause.
        env = {
            **os.environ,
        ...
        try:
            ...
            # Re-``communicate``, not ``proc.stdout.read()``: the first call discards its buffer on
            # the raise, so a bare read() returns only the tail. Bounded — an unkilled worker holds
            # the write end open forever.
            try:
    ...
    def run(self, tmp_path: Path):
        ...
        for attempt in range(attempts):
            ...
            # A script declining to run must print ``SKIP:`` to be reported as one; exit-0-with-no-
            # result-line still counts as PASS below (exit-code-only scripts have no other channel).
            # Substring, not prefix: ``log()`` stamps ``[Rank N] `` and torchrun interleaves mid-line.
            ...
            if result is not None:
                # Only rank 0 emits the result line, so pass + non-zero exit means a NON-ZERO RANK
                # failed: a real correctness failure, not an infra ERROR.
                pytest.fail(
...
def _absent_server(markers: tuple) -> str | None:
    ...
    is nothing to test rather than something broken, so the node skips like one needing more GPUs
    than are present — unless the run set ``HALO_TEST_REQUIRE_SERVER`` (the make server tiers do),
    where the caller turns this reason into a ``UsageError`` instead.
    """
...
def pytest_collection_modifyitems(config, items):
    ...
    if not cases:
        return
    have = _gpu_count()
    # Zero GPUs would skip every node and exit 0 — a whole tier green without running anything.
    if have == 0:
        raise pytest.UsageError(
