...
Validates that SmoothMarginPOTrainer works correctly with both expert parallelism
and context parallelism enabled simultaneously. EP distributes MoE experts across
GPUs while CP splits sequences via Ulysses attention.
...
def run_smpo_ep_cp_test():
    ...
    output_dir, cache_dir = setup_cache_dirs("smpo_ep_cp", rank)
    ...
    try:
        ...
        if success:
            log(f"\n{'=' * 70}")
            log("SMPO EP+CP TEST PASSED")
            log(f"  - EP size: {EP_SIZE}, CP size: {CP_SIZE}")
            log(f"  - Training loss: {train_result.training_loss:.6f}")
            log(f"  - Steps completed: {train_result.global_step}")
            log("  - All losses finite")
            log("  - Sequence lengths compatible with CP")
            log(f"{'=' * 70}")
        else:
            log(f"\n{'=' * 70}")
            log("SMPO EP+CP TEST FAILED")
            for issue in issues:
                log(f"  - {issue}")
            log(f"{'=' * 70}")
