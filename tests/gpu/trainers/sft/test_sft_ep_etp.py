...
def main():
    ...
    if world_size < EXPERT_TP_SIZE:
        log(f"\nERROR: Need at least {EXPERT_TP_SIZE} GPUs for expert_tp_size={EXPERT_TP_SIZE}, got {world_size}")
        teardown_distributed()
        return 1

    output_dir, cache_dir = setup_cache_dirs("sft_ep_etp_test", rank)
    ...
    try:
        ...
        log(f"\n{'#' * 70}")
        log(f"  SFT EP+ETP TEST {'PASSED' if all_passed else 'FAILED'}")
        if not all_passed:
