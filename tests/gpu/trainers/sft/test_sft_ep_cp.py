#!/usr/bin/env python
"""
SFT training test with Expert Parallelism + Context Parallelism (EP=2, CP=2).

Validates that DistributedSFTTrainer works in EP+CP orthogonal mode on a MoE
model. In this mode:
- EP distributes MoE experts across GPUs via DeepEP all-to-all routing
- CP splits sequences across GPUs via Ulysses attention
- With 2 GPUs: EP=2, CP=2 are orthogonal; data_parallel_size=1
...
Usage:
    torchrun --nproc_per_node=2 \
        tests/gpu/trainers/sft/test_sft_ep_cp.py
...
NUM_EVAL_SAMPLES = 8
MAX_SEQ_LENGTH = 4096  # must be divisible by cp_size=2
NUM_TRAIN_STEPS = 5
BATCH_SIZE = 1
LEARNING_RATE = 2e-5
SEED = 42
...
def main():
    ...
    try:
        ...
        log("Confirmed: trainer.is_cp_mode is True")
        ...
        # A check, not a log line: with no EP wrappers this file measures CP alone and reports
        ...
        # Read off the layer's own ep_size rather than the module constant: EP and CP overlay the
        # same two ranks here, so a group-size misresolution must fail here.
        ...
        log(f"\n{'#' * 70}")
        log(f"  SFT EP+CP TEST {'PASSED' if all_passed else 'FAILED'}")
        if not all_passed:
