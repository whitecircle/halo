...
Validates DistributedSFTTrainer on a MoE model under CP only (Ulysses sequence sharding,
experts local via grouped-GEMM). Complements the EP+CP gpt-oss test (this isolates CP).
...
def main():
    ...
    output_dir, cache_dir = setup_cache_dirs("sft_oss20b_cp_test", rank)
    ...
    try:
        ...
        # The trainer's logged loss is the reduced scalar every rank reports, so a spread here is a
        # desync, not CP's per-chunk normalization. All-gathered on every rank; judged on rank 0.
        ...
        if rank == 0:
            losses_list = [lv.item() for lv in all_losses]
        ...
        log(f"\n{'#' * 70}")
        log(f"  SFT CP (GptOss-20B) TEST {'PASSED' if all_passed else 'FAILED'}")
        if not all_passed:
