#!/usr/bin/env python
"""
SFT training test with Expert Parallelism + Tensor Parallelism (EP=2, TP=2).

Validates that DistributedSFTTrainer works in EP+TP mode on a MoE model.
In this mode:
- Attention layers are sharded via DTensor (TP)
- MoE experts are distributed via DeepEP all-to-all routing (EP)
...
Usage:
    torchrun --nproc_per_node=2 \
        tests/gpu/trainers/sft/test_sft_ep_tp.py
...
def main():
    ...
    log(f"\n{'#' * 70}")
    log(f"  SFT with EP+TP Mode (EP={EP_SIZE}, TP={TP_SIZE}) Test")
    ...
    output_dir, cache_dir = setup_cache_dirs("sft_ep_tp_test", rank)
    ...
    try:
        ...
        log(f"\n--- Loading model with EP={EP_SIZE}, TP={TP_SIZE} ---")
        ...
        log(f"EP layers found: {len(moe_layers)}")
        ...
        log("Confirmed: trainer.is_tp_mode is True")
        ...
        # A check, not a log line: with no EP wrappers this file measures TP on a dense-MoE model
        # and reports every other check green.
        ...
        # Read off the layer's own ep_size rather than the module constant: under EP+TP the two
        # axes overlay the same ranks, so a group-size misresolution must fail here.
        ...
        log(f"\n{'#' * 70}")
        log(f"  SFT EP+TP TEST {'PASSED' if all_passed else 'FAILED'}")
        if not all_passed:
