#!/usr/bin/env python
"""
MoE model SFT benchmark with Expert Parallelism + Tensor Parallelism (EP+TP).
...
Expert Parallelism (DeepEP all-to-all), and Tensor Parallelism (DTensor weight sharding).

EP+TP mode: attention/embeddings/lm_head are sharded via DTensor (TP), while MoE
expert layers are distributed via DeepEP (EP). TP must be node-local (NVLink
required for all-reduce), EP can be node-local or cross-node.

Usage:
    # EP=2 + TP=2, seq_len=4096, 10 steps
    torchrun --nproc_per_node=2 \
        tests/gpu/profiling/benchmark_sft_ep_tp.py --ep 2 --tp 2 --seq 4096 --steps 10

    # Short sequence benchmark
    torchrun --nproc_per_node=2 \
        tests/gpu/profiling/benchmark_sft_ep_tp.py --ep 2 --tp 2 --seq 2048 --steps 5

    # Long sequence benchmark
    torchrun --nproc_per_node=2 \
        tests/gpu/profiling/benchmark_sft_ep_tp.py --ep 2 --tp 2 --seq 16384 --steps 20 --warmup 3

    # 8 GPUs: EP=8, TP=8 (max sharding)
    torchrun --nproc_per_node=8 \
        tests/gpu/profiling/benchmark_sft_ep_tp.py --ep 8 --tp 8 --seq 4096 --steps 10

    # Custom model path
    torchrun --nproc_per_node=2 \
        tests/gpu/profiling/benchmark_sft_ep_tp.py --ep 2 --tp 2 \
        --model_path /path/to/moe/model --seq 8192

    # Disable Liger kernels
    torchrun --nproc_per_node=2 \
        tests/gpu/profiling/benchmark_sft_ep_tp.py --ep 2 --tp 2 --seq 4096 --no_liger
"""
...
def main() -> int:
    ...
    parser = create_benchmark_parser(
        description="MoE SFT benchmark with EP + Tensor Parallelism",
    )
    ...
    log(f"\n{'=' * 70}")
    log(f"  MoE SFT Benchmark (EP={ep_size} + TP={tp_size})")
    ...
    log(f"  World size: {world_size}, EP size: {ep_size}, TP size: {tp_size}")
    log(f"  Data parallel size: {world_size // tp_size}")
    ...
    if world_size < tp_size:
        log(f"\nERROR: TP size ({tp_size}) exceeds world size ({world_size})")
        dist.destroy_process_group()
        return 1

    # Isolate HF cache
    output_dir, cache_dir = setup_cache_dirs("bench_eptp", rank)
    ...
    try:
        ...
        # --- Load model with EP+TP ---
        log(f"\n[4/5] Loading model with EP={ep_size}, TP={tp_size}...")
        ...
        # --- Report ---
        log(f"\n{'=' * 70}")
        log(f"  BENCHMARK RESULTS (EP={ep_size} + TP={tp_size})")
        log(f"{'=' * 70}")
        ...
        emit_benchmark(f"sft_ep_tp_{args.model}_ep{ep_size}_tp{tp_size}_s{args.seq}", efficiency_cb)
        log(f"  TP: attention/embeddings sharded across {tp_size} GPUs")
        log(f"  EP: {num_experts} experts distributed across {ep_size} GPUs")
        log(f"{'=' * 70}\n")
