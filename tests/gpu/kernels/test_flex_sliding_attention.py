...
def test_packed_lengths_share_one_compilation():
    """Packed batches change length every step. The compiled FlexAttention serves every length from one
    compilation: a per-length specialization recompiles until Dynamo's recompile limit (8), after which
    FlexAttention runs unfused with the full score matrix, and under expert parallelism a mid-run recompile
    stalls the other ranks in the all-to-all.
    """
    lengths = [192, 288, 384, 448, 512, 640, 704, 832, 960, 1088, 1216, 1344]  # more than the recompile limit
    assert len(lengths) > torch._dynamo.config.recompile_limit
    torch._dynamo.reset()
    ...
    for seq in lengths:
        ...
        q = torch.randn(1, heads, seq, dim, device="cuda", dtype=torch.bfloat16, generator=generator)
        k = torch.randn(1, kv_heads, seq, dim, device="cuda", dtype=torch.bfloat16, generator=generator)
        v = torch.randn(1, kv_heads, seq, dim, device="cuda", dtype=torch.bfloat16, generator=generator)
        out, _ = flex_sliding_attention.flex_sliding_attention(
            _causal_module(), q, k, v, None, scaling=dim**-0.5, sliding_window=WINDOW
        )
        i = torch.arange(seq, device="cuda")
        window = (i[None, :] <= i[:, None]) & (i[:, None] - i[None, :] < WINDOW)
        ref = torch.nn.functional.scaled_dot_product_attention(
            q, k.repeat_interleave(2, 1), v.repeat_interleave(2, 1), attn_mask=window, scale=dim**-0.5
        ).transpose(1, 2)
        assert _rel(out, ref) < 2e-2, f"length {seq}: flex output differs from SDPA's sliding window"
    graphs = counters["stats"]["unique_graphs"]
    assert 1 <= graphs <= 2, (
        f"{graphs} compiled graphs for {len(lengths)} lengths: FlexAttention recompiles per length"
    )
