"""Test manifest — the single source of truth for launching the GPU test suite.
...
and asserts the exit code (and parses the structured result line when the script uses
...
:func:`unregistered_scripts` (the conftest fails collection on drift), so coverage can't
silently rot.

Markers (selection):
    gpu                        — every entry (the suite tier).
    core | full                — ``core`` is the PR gate *by intent*: small, fast, <=2 GPU, tiny
                                 model. The registered tier is wider than that intent — see
                                 ``agent-docs/contributing/index.md`` for the measured budget and the
    ...
    vllm_server                — needs the vLLM container from ``docker-compose.vllm.yml`` already
                                 ...
                                 Two launch requirements this tier cannot check for you (``make
                                 test-gpu-vllm`` sets both up):
                                   * the server must own a GPU the trainer does NOT use — weight sync
                                     is an NCCL broadcast, and a rank cannot broadcast to itself
                                     (drive the trainer with ``CUDA_VISIBLE_DEVICES`` excluding it);
                                   * on a host without InfiniBand, launch with ``NCCL_IB_DISABLE=1
                                     NCCL_NET=Socket``. The image's OFI/Gin defaults wedge the
                                     cross-container group instead of failing: both GPUs spin at 100%
                                     until the 120 s formation deadline, per test.
                                 A trainer killed while attached to the weight-transfer engine leaves
                                 the server's scheduler stuck (``/health`` still answers 200) —
                                 ...
                                 policy MOVED, so the server must serve the checkpoint being trained
                                 — dense (Qwen3-0.6B) for the ``not moe`` half, Qwen3-30B-A3B for the
                                 ``moe`` half. No one server satisfies both; run two passes
...
class TestSpec:
    ...
    Attributes:
        ...
        timeout: seconds before the launcher kills the process group (NCCL / FA hangs are
            live, so a hard kill is required — not a soft assert).
    ...
    ``gpu_test_main(exact_world_size=N)``, which is both authoritative and more precise
    than a manifest bool could be. The launcher always launches exactly ``nproc``.
    """
...
MANIFEST: dict[str, TestSpec] = {
    ...
    # never converge to a cached set (measured 1087s, kernel cache growing 4->59MB across runs).
    ...
    # Pure FSDP2 (no TP mesh anywhere) — no parallelism-axis marker, like the plain-FSDP entries.
    ...
    # The failure mode is a HANG (rank-divergent cache path -> non-main ranks never reach the sweep's
    # collectives), so the timeout IS the assertion: ~2 min per node measured, 420s fails fast.
    ...
    # tower). Marker UNION across the matrix; a family joins the matrix when it declares
    ...
    "parallelism/test_mistral4_all_parallelism.py": TestSpec(
        ...
        args_matrix=(
            ...
            # is a real shape too (ETP raises ep_group_size to the domain), absent only because it has
            # not been validated on THIS model.
    ...
    # The entries below assert the served policy MOVED, so the server must run the SAME checkpoint
    # the test trains — and the engines differ: VLLM_MODEL=Qwen/Qwen3-30B-A3B-Instruct-2507,
    # SGLANG_MODEL=unsloth/gpt-oss-20b-BF16 (the one family whose EP layer gathers SGLang's fused
    ...
    # --thinking-budget is deliberately NOT a row: the budget is a gpt-oss shape (its arming marker
    # lives in the server image's reasoning plugin, agent-docs/models/gpt-oss.md#serving-for-grpo-vllm),
    # while this file's server runs Qwen3-30B. Run it by hand against a gpt-oss server, with
    # HALO_TEST_ENV_GRPO_MODEL and VLLM_MODEL both pointing at that checkpoint.
    "trainers/grpo/test_env_grpo_vllm_e2e.py": TestSpec(
        ...
        # Measured on 2xB300 against a live vLLM Qwen3-30B-A3B: 2m14s-2m35s for the single-phase
        # rows, 4m03s-4m51s for the two that load under TP, 5m29s for the full-finetune resume
        # (two model builds plus a 61 GB checkpoint round-trip). 2400s keeps the FA4-JIT and
        # checkpoint-download cold paths inside the budget.
        timeout=2400,
    ),
    # Two axes at once — the shapes two ranks cannot form. Same URL and server GPU as the 2-GPU
    # entry above but a DIFFERENT checkpoint, so the two are separate marker passes:
    ...
    "trainers/grpo/test_env_grpo_vllm_4gpu_e2e.py": TestSpec(
        ...
        # Measured on 4xB300 against a live vLLM gpt-oss-20b-BF16: 2m34s-4m10s per row (the EP+TP row
        # is the slowest — it builds the DTensor attention plan on top of the EP wrappers). 1800s
        # covers the FA4-JIT and checkpoint-download cold paths.
        timeout=1800,
    ),
    ...
    "trainers/grpo/test_env_grpo_sglang_e2e.py": TestSpec(
        ...
        # collapses into runaway completions: every turn is cut at the token cap and excluded — the
        # zero-gradient batch the replay gate exempts (``_assemble_rollout_routing``).
        # Measured on 2xB300 against a live SGLang gpt-oss-20b-BF16: 4m23s-5m41s for the
        # single-phase rows (the replay ones at 4m52s-5m07s), 10m38s for the resume (two model
        # builds plus a 42 GB checkpoint round-trip, on an engine that quiesces for the whole of
        # every sync).
        timeout=2400,
        flaky=True,
    ),
    ...
    "trainers/grpo/test_offline_grpo_tp_resume.py": TestSpec(
        # FA4 backward JITs per-shape on Blackwell (~200s/step) and this resume test trains 6 steps
        # across a reload; 1500s timed out mid-resume, 2400s covers the compile.
        nproc=2,
        markers=("gpu", "core", "2gpu", "tp", "qwen3"),
        timeout=2400,
    ),
    ...
    # so they load the policy twice and carry the longest measured time in each file.
    "trainers/grpo/test_online_grpo_vllm_moe_e2e.py": TestSpec(
        ...
        # Measured on a warm HF cache: 139-166s for a training row, 244s for the longest resume row
        # (two 30B loads plus the gathered checkpoint); 1800s leaves room for a cold one.
        timeout=1800,
    ),
    "trainers/grpo/test_online_grpo_vllm_dense_e2e.py": TestSpec(
        ...
        # Measured: 46-91s per row on a warm cache; 900s leaves room for a cold one.
        timeout=900,
    ),
    # Runs the dense server through a dozen trainer connect/sync/disconnect cycles, so it is what
    # keeps the rows above able to share one long-lived server.
    "trainers/grpo/test_vllm_weight_transfer_reinit.py": TestSpec(
        nproc=1,
        markers=("gpu", "full", "1gpu", "qwen3", "vllm_server"),
        # Measured: 67s on a warm cache (12 cycles at ~1.2s plus the policy load); 600s leaves room
        # for a cold one.
        timeout=600,
    ),
    ...
    # `full`, not `core`: writes a whole merged checkpoint (40+ GB at the default GptOss-20B).
    ...
    # measured 105 s (lora) to 187 s (expert_lora) with the checkpoint warm. 1800 s matches the
    # sibling adapter-resume entries: it covers a cold 62 GB checkpoint read plus the first-use
    # FA4/flex_attention JIT, neither of which the measured runs paid.
    ...
    "trainers/other/test_checkpoint_roundtrip_gptoss_20b.py": TestSpec(
        ...
        timeout=1500,
        # Every implemented mode gets a row: an unregistered one reports coverage the run never performed.
    ...
    "trainers/preference/test_smpo_tp_resume.py": TestSpec(
        # 1778s measured: FA4 backward JITs per-shape (~200s/step) over a 6-step two-phase resume.
        nproc=2,
        markers=("gpu", "core", "2gpu", "tp", "qwen3"),
        timeout=2400,
    ),
    ...
    "trainers/sft/test_sft_checkpoint_resume.py": TestSpec(
        # Every mode the script implements gets a row — an unregistered one reports coverage the run
        # never performed. All four are 2-GPU (cp_size=2 / tp_size=2 / ep_size=2 on world 2).
        nproc=2,
        # The markers are the union over the rows, so `-m "gpu and cp"` selects the fsdp and tp rows too.
        markers=("gpu", "core", "2gpu", "cp", "tp", "qwen3", "gptoss"),
        args_matrix=("--mode fsdp", "--mode cp", "--mode tp", "--mode ep"),
        # 2400s: the cp row trains at MAX_SEQ_LENGTH_CP=4096 on auto-selected FA4 (JITs per shape), and
        # the same 6-step two-phase resume at 2048 measured 1778s (trainers/preference/test_smpo_tp_resume.py).
        timeout=2400,
    ),
    ...
    # Two 4-step Qwen3-0.6B arms + two model loads; measured 89s on 2xB300 warm, so ~6.7x headroom
    # for a cold HF cache and the FA4 kernel JIT.
    ...
    # `full`, not `core`: a plain SFT smoke on the 20B checkpoint, like every other `oss20b_*` entry.
    # The tier keeps gpt-oss correctness gates (`parallelism/ep/test_ep_correctness.py`), not smokes.
    ...
    "trainers/sft/test_sft_qwen3_dense.py": TestSpec(
        nproc=2,
        # One node per mode, so a regression names the mode instead of collapsing three verdicts
        # into one. The markers are the union over the rows — every row is the same dense model, so
...
# runner, never by the pytest launcher. Listing them here is what makes an *unlisted* `bench*.py`
# an orphan: the file rots unnoticed otherwise (nothing else globs it).
...
def unregistered_scripts() -> list[str]:
    ...
    The conftest fails collection if this is non-empty, so a new test can't be added
    without a launch spec (no silently-uncollected coverage). The launcher entrypoint
    (``test_suite.py``) is excluded — it is collected by pytest directly, not launched.
    ``bench*.py`` files are globbed too, against :data:`_UNMANIFESTED_BENCHMARKS`.
    """
    on_disk = {
