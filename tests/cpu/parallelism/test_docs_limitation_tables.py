...
_PARALLELISM_INDEX = _REPO_ROOT / "agent-docs" / "parallelism" / "index.md"
...
def test_supported_combinations_table_matches_the_allowlist():
    """``agent-docs/parallelism/index.md`` must publish exactly the sets ``ParallelismConfig`` accepts.
    ...
    assert not missing, f"in SUPPORTED_AXIS_SETS but absent from agent-docs/parallelism/index.md: {missing}"
