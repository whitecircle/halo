...
1. Snapshot an :class:`~src.callbacks.efficiency.EfficiencyCallback`
   ...
   metrics are hardware-anchored: tokens/s/GPU, cluster tokens/s, peak memory
   (GB), step time. MFU / S-MFU / achieved-TFLOPS stay under a ``diagnostics``
   sub-key, since they are setup-dependent and are not the gated number.

2. Emit a single machine-readable result line that the pytest launcher
   (``tests/gpu/conftest.py``) parses to distinguish a test failure
   (``status="fail"``) from an infra error (process died, no line emitted). The
   line is prefixed with :data:`RESULT_SENTINEL` so it survives interleaved
   torchrun stdout.
...
def format_benchmark_report(cb) -> str:
    ...
    memory, step time); MFU / S-MFU / achieved-TFLOPS follow under a labelled
    ``diagnostics`` block, since they are setup-dependent.
    """
