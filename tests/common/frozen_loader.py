...
Every unparallelized frozen model the toolkit scores a policy against — the DPO/KTO reference, the SDPG
KL anchor, both distillation teacher paths — loads through
:func:`~src.distributed.loading.frozen_models.load_frozen_auxiliary_model`, so its module namespace
is the ONLY place a test can intercept the hub fetches. That is also what makes these stubs a
consolidation check: a call site that re-implemented the load would fetch through its own names instead
of these, and :func:`captured_load` would find no recorded fetch.
"""
...
def stub_frozen_loader():
    ...
    with (
        ...
        # Patched where they are DEFINED, not where the loader imports from: the loader reaches them
        # through ``apply_sinks_policy``, so intercepting them here keeps the assertion end-to-end
        # (loader → shared policy → the branch that policy picks) instead of stopping at the seam.
...
def captured_load(caps, *, is_vlm: bool = False):
    """The one weight fetch the stubs recorded, with the config fetch and resolver call beside it.
