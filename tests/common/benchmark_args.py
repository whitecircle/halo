"""Common argparse setup for profiling benchmarks.

Replaces repeated argparse patterns across 10 benchmark files.
Individual benchmarks can extend the parser with add_argument()
...
def pp_topology_kwargs(pp_size: int, world_size: int) -> dict:
    ...
    each stage owns a whole simulated domain and every intra-stage EP/FSDP group stays
    NVLink-local. The benchmarks build ``ParallelismConfig`` directly, bypassing the config-time
    rejection in ``parallelism_config_from_args`` — so this shared chokepoint rejects ``--pp > 1``
    up front instead of failing after model load.
    """
    if pp_size <= 1:
        return {}
...
def resolve_benchmark_attn(
    ...
    :func:`resolve_attn_implementation` (GPU auto-detect + per-family limits + the sinks
    validator), but a direct ``from_pretrained`` does not — transformers just falls back to
    its own default, ``sdpa``. Benchmarks that bypass the loader must resolve the backend
    themselves or they silently measure a different kernel than production runs on.
    """
