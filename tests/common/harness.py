...
One home for the ~30-line lifecycle every torchrun test needs: ``init_distributed`` →
...
barrier, a dir leaked on the error path, a missing ``cleanup_ep``. With the
decorator owning it, a test body is just *load → train → assert*.
...
size — an infra ERROR, not a test FAIL).
...
class Ctx:
    ...
    def broadcast_seed(self, seed: int = 42) -> int:
        ...
        Use when every rank must generate the SAME data (e.g. a parallel-mode
        run compared against a single-GPU reference). Returns the shared seed.
        """
...
def record_check(checks: dict[str, bool], name: str, fn: Callable[[], None]) -> None:
    ...
    Every ``fn`` must therefore be rank-symmetric and collective-free: continuing past a failure is
    only safe while no later check enters a collective the failed rank's peers would then be
    misaligned on — which is why ``gpu_test_main`` skips its own barrier for a body that RAISED.
    """
    try:
        fn()
...
def gpu_test_main(
    *,
    ...
    Args:
        min_world_size: minimum GPUs the test needs (a smaller launch is a bad
            launch → exit 2, reported as an infra ERROR, not a FAIL).
    ...
    def decorator(run: Callable[["Ctx"], dict]) -> Callable[[], int]:
        ...
        def wrapper() -> int:
            ...
            try:
                ...
                # Every rank reports its own failure. Rank-0-only reporting made a non-zero rank's
                # exception invisible — the launcher showed exit code 1 with no message and no
                # traceback anywhere, which is indistinguishable from a teardown race.
                log_all(f"\nFATAL: {err}")
                traceback.print_exc()
            finally:
                ctx._run_finalizers()
