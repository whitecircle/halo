...
adapter × parallelism shape assert the same things about the same server, so the assertions live in
...
The assertion is on the SERVED policy, not on the trainer's tensors. ``test_online_grpo_vllm_e2e.py``
runs one GPU with no parallelism and stops at "train() returned with a finite loss", so a sync that
gathered the wrong shard — or an adapter fold that never ran — passes it. Only the engine's own
logprobs tell those apart.

The shape both on-policy families share — the probe, the perturbation round, the restore-point
snapshot, the parallelism-engaged verdict, the resume comparisons — is
:mod:`tests.common.on_policy_e2e`. What lives here is what differs: the axis × adapter table, the
vLLM-server ``GRPOConfig``, the two trainer classes, the TP + LoRA refusal, and SDPG's OPD term.
...
gather. The perturbation round therefore moves ``lora_B`` rather than a base weight — a base
perturbation would move the served policy whether or not the fold ran, which is the one thing these
rows exist to prove. The fold's other half is asserted too: the merge must be undone, leaving the
trainer's own base weights where it found them.

``resume`` proves the invariant TRL's ``_last_loaded_step`` sentinel carries — a resumed run must
generate its first rollout from the checkpoint's weights, never from whatever the engine still held.
Phase 1 trains to ``RESUME_MAX_STEPS`` with a checkpoint at ``RESUME_SAVE_STEP``, so when ``train()``
returns the engine holds exactly the checkpoint's policy (the last step's pre-generation sync pushed
it). The perturbation round then moves the engine OFF that policy. Phase 2 is a fresh model and a
fresh trainer resuming with exactly one step left, so the sentinel's push is the only thing that can
reach the engine before the resumed rollout: the served policy must be back where it was. A resume
that skipped the push leaves the engine on the perturbed policy. Both phases run in ONE process
against ONE server on the same weight-transfer group port, which
:mod:`tests.common.on_policy_e2e` documents.
"""
...
# Short on purpose: every step is a full engine round-trip, and the properties under test are "the
# sync landed" and "the resume pushed", neither of which needs a converged policy.
MAX_STEPS = 3
NUM_TRAIN_SAMPLES = 16
# Large on purpose — the sync has to move the served weights visibly within a handful of steps.
...
# Phase 1's last push merges into a base that has already been through the earlier steps'
# merge/unmerge round-trips while phase 2 merges into a freshly loaded one, and each round-trip
# rounds the base twice in bf16 — so the two pushes differ by that rounding even when the adapters
# are bit-identical. A full fine-tune restores the base itself and is compared exactly. The
# perturbation round moves these logprobs by whole nats, orders of magnitude above this.
...
class _Mode:
    ...
    mode belongs to, so neither script hand-repeats the list.

    ``checkpoint_optimizer`` carries the optimizer and scheduler into the resume checkpoint, which is
    what makes the continuity assertions possible. Off for a full fine-tune of the MoE policy, where
    that state is 4 B/param of AdamWBF16 moments — over a hundred GB of disk, and the same again in
    host RAM to compare — for a property the adapter rows and
    ``tests/gpu/parallelism/ep/test_ep_optimizer_resume.py`` already pin at EP scale.
    """
...
def reward_len(completions, **kwargs):
    ...
    A tied group has zero advantage and therefore a genuinely zero gradient: nothing moves, and both
    the adapter-movement and the SDPG OPD assertions would be measuring sampling luck.
    """
...
def _build_trainer(
    *,
    ...
    ``weights_source`` is where the WEIGHTS come from — the hub id normally, the checkpoint directory
    on a resume whose loader expects the model to be constructed from it.
    """
...
class _CheckpointOnce(TrainerCallback):
    """Write exactly the one checkpoint the resume targets.

    Under a step-based save strategy HF's default flow saves again when training stops
    (``DefaultFlowCallback.on_step_end``), which at MoE scale is a second copy of the model — and of
    its optimizer state — that nothing in this row ever reads. Registered after the default callback,
    so this verdict is the one that survives.
    """
...
def _weight_witnesses(model, spec: _Mode) -> dict[str, torch.Tensor]:
    ...
    Rank-local views, no collective: the expert shards a rank holds are the same ones in both phases
    (same ``ep_size``), so a rank-local comparison is exact and issues nothing. Empty for an adapter
...
class _RestorePoint(RestorePointSnapshot):
    ...
    against the checkpoint would fail on a perfectly correct restore — hence the callback.
    """
...
def _sdpg_checks(trainer, checks: dict) -> None:
    ...
    presence only proves an SDPG step ran. What a silent regression (the fused-Liger loss bypass, a
    gate that masks every token) would take away is a NON-ZERO ``opd_loss``, which needs the
    privileged-teacher forward and at least one positive-advantage rollout — both guaranteed here by
    a graded reward that keeps the group's advantage spread alive.
    """
...
def _lora_under_tp_refused(**build_kwargs) -> dict[str, bool]:
    ...
    ``_validate_lora_tp_compatibility`` runs inside ``_setup_distributed_modes``, i.e. after the
    trainer's own ``__init__`` has built the model wrappers AND TRL's weight-transfer client, so a
    config-level assertion never reaches it. The client that construction left behind is closed by
    the ``atexit`` hook ``init_communicator`` registers (``src/distributed/nccl/clients/vllm.py``) —
    the raise leaves no handle to close it through.
    """
    try:
...
def _run_resume(ctx, *, trainer_kind, spec: _Mode, model_name, tokenizer, server_url, group_port, checks) -> dict:
    ...
    # holds it — no extra merge stands between this probe and what phase 2 loads.
    checkpoint_policy = {}
    if ctx.rank == 0:
    ...
    # Off it in the sink stream too, which the dense delta above cannot speak for: the resume must
    # bring BOTH back.
    ...
    # phase 1's model on the GPUs next to phase 2's.
    saved.trainer = None
    del trainer
    cleanup_memory()
    ctx.barrier()

    # Phase 2: a fresh model and trainer, exactly as a restarted job builds them — that is what puts
    # TRL's ``_last_loaded_step`` back at its ``-1`` sentinel. The production decision, not a
    # test-local rule: under the EP/CP wrappers a full checkpoint's weights only load at
    # CONSTRUCTION, while an adapter-only checkpoint keeps the base and has its adapters restored.
    parallelism_config = _config_for(spec)
    ...
    # Train-begin captures, not post-train reads: one resumed step has already moved them. A save
    # that never fired leaves no witnesses, and the check above is what reports that.
...
def run_online_grpo_e2e(
    ...
    if ctx.rank == 0:
        ...
        # A failed update leaves the engine partially written, so prove it still answers.
