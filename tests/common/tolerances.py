...
class _Tolerances:
    ...
    # Identical broadcast batch — only the combine reduction order varies (measured 0.0);
    # 1e-3 is headroom. The 10x-looser DP band above would absorb a real mis-dispatch.
    ...
    # bf16 reduction reorder flips near-tied top-k picks (gpt-oss top-4-of-32): measured
    # 0.060 EP+TP / 0.064 EP+ETP vs 0.344 / 0.457 for rotated-expert controls, so >3x
    # separation survives. The generic bound above holds only for modes the router never
    # sees reordered (ETP on Mistral4 matches to 1e-2); stretching it would blind them.
    ...
    # Norm achieved after clipping vs the max_norm asked for. The clip coefficient is applied in
    # fp32 over bf16 grads, so the re-measured norm lands a rounding step off the target — a wrong
    # coefficient or a missed plain-vs-DTensor scale misses by a factor, not by a rounding step.
    ...
    # bounds (a corrupted checkpoint frame, e.g. a replayed DeepEP dispatch) moves them far more.
