...
These drive the SAME production path the training scripts use —
``split_expert_lora_targets`` → ``load_distributed_model(quantization_config=...)`` →
``setup_peft_model`` — so the tests exercise the real adapter wiring (attention PEFT, native
grouped expert-LoRA, QLoRA 4-bit base) as the GRPO scripts drive it, not a hand-rolled
approximation. The trainer families differ (offline GRPO / teacher-distill / self-distill), so each
test file builds its own dataset + trainer; this module owns the model/adapter build and the
adapter-invariant assertions shared across all of them.
...
def attention_target_modules(model_name: str, revision: str | None = None) -> list[str]:
    ...
    spell them ``q_a_proj``/``q_b_proj``/``kv_a_proj_with_mqa``, Zaya differs again, and PEFT ships
    no ``TRANSFORMERS_MODELS_TO_LORA_TARGET_MODULES_MAPPING`` entry for a single MoE model_type in
    this roster — so a fixed ``["q_proj", "v_proj"]`` makes PEFT raise "Target modules not found" on
    everything but the Qwen/GptOss spelling. Reading the shipped weight map means a new family needs
    no entry anywhere. Falls back when the checkpoint is single-file (no index), which the small
    dense test models are.
    """
    try:
...
def _mode_targets(mode: str, model_name: str, revision: str | None) -> list[str]:
    """``lora_target_modules`` for a mode. ``mixed`` names both namespaces in ONE list, which is what
    split_expert_lora_targets peels apart — attention names stay with stock PEFT, expert names become
    the native grouped adapters. That shape trains two adapter kinds at once and is the only one
    whose checkpoint needs both folds."""
    if mode == "expert_lora":
...
def parallelism_config_for(mode: str, size: int):
    ...
    One home beside :func:`model_name_for`, which resolves the other half of a mode: two copies of
    this map drift on their fall-through — one landing on plain FSDP2, the other on EP — and the two
    tests would then disagree about what an unsuffixed mode means.
    """
...
def load_peft_model(
    ...
    *,
    ...
    other MoE families (each has its own expert storage layout and export renames) instead of being
    ...
    # Peel expert targets to native grouped-LoRA BEFORE load (no-op for attention-only modes).
...
def is_lora_b_key(key: str) -> bool:
    ...
    ``...lora_B.weight``); the native grouped expert adapters end on it (``...experts.down_proj.lora_B``).
    B is the half that starts at zero, so "did it move" and "is this fresh" both key off exactly this set.
    """
...
def assert_only_adapters_trainable(model) -> tuple[bool, str]:
    ...
    This is the frozen-base guarantee — setup_peft_model / get_peft_model freeze the base (incl. the
    4-bit QLoRA weights) and leave only adapters (+ any modules_to_save) trainable, so the optimizer
    can only move adapters."""
...
def snapshot_adapters(model, *, expert_lora: bool) -> dict[str, torch.Tensor]:
    ...
    call this). Attention PEFT adapters are replicated/local, so a local clone suffices.
    """
    if expert_lora:
...
def assert_adapter_checkpoint(trainer, output_dir: str, rank: int, *, expert_lora: bool) -> tuple[bool, str]:
    ...
    non-zero B tensor — i.e. the trained adapter actually serialized (not an empty/garbage write).
    """
