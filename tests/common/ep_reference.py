...
A parallel MoE forward that dispatches every token to the *wrong* expert still produces a
finite loss and finite gradients, so no amount of finiteness checking can see it. The only
thing that can is the same math on the same weights with no distribution at all. This module
builds that reference — loss plus the first router gradient from a plain ``from_pretrained``
forward/backward — and the control that proves the comparison is actually sensitive to expert
identity: :func:`roll_expert_banks` rotates each rank's local expert bank by one expert, which
is precisely the corruption a dispatch/ownership bug produces, and must move the loss far
outside the tolerance the match is asserted at.

Reference and parallel model must be built with the SAME ``attn_implementation``. For a sinks
model (GptOss) that matters twice over: a sink-dropping backend shifts the loss by ~3 nats on
live sinks, so a mismatch there would swamp the signal this module exists to measure.
"""
...
# (``EPMoELayerBase._find_gate_or_router``), so the parameter path is identical in the sharded
# and the dense model — which is what makes the two directly comparable.
...
# ``strftime_now("%Y-%m-%d")`` into its system message, so an unpinned batch — and every loss
# threshold measured against it — drifts with the calendar date. The value is not free: it moves the
# rotated-expert control's loss shift, which at 2025-01-01 falls under ``control_min_loss_shift``.
...
def compare_grad(got: torch.Tensor, reference: torch.Tensor) -> tuple[float, float]:
    ...
    Split rather than collapsed into one relative error because the two bug classes are
    independent: a mis-scaled cross-rank reduction moves the ratio and leaves the cosine at 1,
    a routing/permutation corruption moves the cosine and can leave the ratio at 1. A single
    bound would have to be loosened past the weaker signal to absorb the other's noise.
    """
...
def fixed_chat_batch(
    ...
    *,
    ...
    rank 0's tensors with every rank, and the seed pins the tokenizer-independent parts so a rerun
    on one rank reproduces the same batch. The rendered date is pinned to
    :data:`CHAT_TEMPLATE_NOW`: ``strftime_now`` is a jinja global transformers injects into every
    chat template, and a same-named render kwarg shadows it — a no-op for templates never calling it.
    """
...
def roll_expert_banks(model, shift: int = 1) -> int:
    ...
    Router, dispatch and combine are untouched — only which FFN a dispatched token lands in
    changes, which is exactly what a wrong-expert routing or ownership bug does. Every rank
    applies the same rotation to its own bank, so the perturbed model stays rank-consistent and
    its reduced loss is still well defined. Returns the number of tensors rotated (0 means the
    control never engaged, which itself must fail the test).
    """
    rotated = 0
    with torch.no_grad():
