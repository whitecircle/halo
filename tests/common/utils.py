"""Shared utility functions for all test files.

One home for rank-aware logging, memory cleanup, repo-root resolution, by-path script import,
out-of-process probes, and the shared state-snapshot + assertion helpers.
"""
...
# The one spelling of the repository root for tests that read source files (drift pins, AST sweeps).
# Self-locating rather than cwd-derived: the training images bake a repo copy at /workspace, so a
# relative path can resolve against the wrong tree.
REPO_ROOT = pathlib.Path(__file__).resolve().parents[2]


def load_script_module(relative_path: str, name: str | None = None, *, register: bool = False) -> ModuleType:
    """Import a ``scripts/`` entry point by path — ``scripts/`` is not a package.
...
def probe_findings(script: str, marker: str) -> list[str]:
    ...
    classes, a funnel wrapper installed on ``transformers.dynamic_module_utils``) cannot run in the
    test process without leaking into every later test. It runs out-of-process instead and reports
    through one line, ``print(marker + "|".join(findings))`` — an empty list meaning "nothing wrong".
    A probe that dies, or prints no marker line, is the probe's own failure and raises here rather
...
def imports_name(path: pathlib.Path, name: str) -> bool:
    ...
    module-level import is the only way a call site can reach a symbol, while a mention in a
    docstring or a comment is not a use. ``asname`` counts — an aliased import binds the name too.
    """
...
def safetensors_state_dict(checkpoint_dir: str | pathlib.Path) -> dict[str, torch.Tensor]:
    ...
    Reads the parts directly: the files ARE the artifact under test, so nothing may re-spell or
    re-cast them on the way in the way ``from_pretrained`` does.
    """
...
def _current_rank() -> int:
    ...
    After ``destroy_process_group`` (tests that tear down before a rank-0-only phase) ``dist`` can
    no longer answer, but torchrun's ``RANK`` still can — falling back to 0 there makes every rank
    print ``[Rank 0]``, so a non-zero rank's (possibly vacuous) verdict is indistinguishable from
    rank 0's real one.
    """
...
def local_optimizer_state(model, optimizer) -> dict:
    ...
    The per-rank ``optimizer_shard_XXXXX.pt`` files hold exactly this view — ``get_optimizer_state_dict``
    at ``full_state_dict=False``, keyed by param FQN — so a snapshot taken here is comparable to what a
    resume restores. DTensor entries are reduced to their local shard: the comparison stays rank-local
    and issues no collective, and a per-rank divergence is not averaged away.

    ``reshard_fsdp2_modules`` first, for the same reason the save path does it: after an eval-only
    forward the FSDP2 modules still carry their transient UNSHARDED params, and ``get_optimizer_state_dict``
    then maps no optimizer param to an FQN (``KeyError: 0``, or an empty state that certifies nothing).
    """
...
def assert_optimizer_state_bit_exact(saved: dict, restored: dict) -> None:
    ...
    Fails when a ``load_state_dict`` drops, casts, re-quantizes, or resets any state tensor
    (or scalar, e.g. ``step``). ``saved`` must be non-empty — comparing two empty states would
    certify nothing.
    """
...
def optimizer_state_matches(saved: dict, restored: dict) -> tuple[bool, str]:
    ...
    Every rank has to reach the next collective: a bare assertion on one rank strands its peers, so a
    distributed test records the verdict and consensuses it rather than aborting the body.
    """
    try:
...
def tensors_equal_at_narrower_dtype(a: torch.Tensor, b: torch.Tensor) -> bool:
    """Bitwise equality of two tensors holding the same stored value at possibly different float
    dtypes — the lazy loader casts float parameters to the run's dtype where ``from_pretrained``
    honors a family's fp32 pins, and keeps a float buffer at the checkpoint's dtype where
    ``from_pretrained`` casts it; either side rounds the one stored value, so they agree at the
    narrower dtype. Shapes must match exactly."""
    if a.shape != b.shape:
        return False
