...
# ``project_name`` is annotated ``str``, and the parser converts these spellings to a real ``None``
# only on Optional fields — so on the CLI they survive as literal text and would name the run's
# tracking project. Compared against the exact lowercase spellings, like the parser's own null table.
...
class CommonScriptArguments(RangeValidatedConfig):
    ...
    # Annotated `float`, not `float | list[float]`, even though a per-dataset list is supported:
    # HfArgumentParser allows only `Optional[X]` in a union ("one type per argument") and refuses to
    # build at all otherwise. YAML goes through `parse_dict`, which assigns without argparse coercion,
    # so a list reaches `load_datasets` intact — it just cannot be spelled on the command line.
    ...
    def _apply_default_project_name(self, name: str):
        ...
        Every script-arg class calls this from ``__post_init__``, ahead of :meth:`_validate_ranges`
        — the placeholder must become a real name before the field is judged.
        """
    ...
    def _validate_ranges(self) -> None:
        ...
        Here rather than inside :meth:`_apply_default_project_name` so the CLI path is held to the
        same rule: ``--project_name=`` and ``--project_name=None`` land by ``setattr``, skipping
        ``__post_init__`` entirely, and would otherwise name the tracking project ``""`` or
        ``"None"`` with nothing in the logs to say so. A YAML ``project_name: null`` reaches
        ``os.environ`` as a bare ``None``, where the assignment raises a TypeError deep into the run.
        """
