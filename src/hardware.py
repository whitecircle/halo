"""Host and accelerator facts: the architecture predicates, GPU model detection, the dense
Tensor-Core peak-FLOPS table, and available host RAM.

Top-level because its readers sit in unrelated layers — the attention dispatch, the MFU callback,
the optimizer shard store. Two mechanisms on purpose: the kernel predicates read COMPUTE CAPABILITY
(what a kernel is compiled for), the FLOPS table is keyed by SKU name (peaks differ across one
architecture). Every MFU / S-MFU percentage divides by an entry of the table, so a wrong number
silently rescales every reported utilization.
"""
...
def is_blackwell_gpu() -> bool:
    ...
    The one arch predicate every backend choice shares (attention dispatch, the CP flash-attn
    resolver, the loader's CP substitution). ``>=``, not ``== 10``: a bare major-equality would demote
    a future SM11x device off FA4 while this detector still selected it. The open upper bound also
    admits consumer Blackwell (SM 12.x), where FA4's SM100 kernels do not run — not a supported
    target; bound it only if it becomes one.
    """
...
class GpuPeak:
    ...
    An SKU added to the table is classifiable through its own ``aliases`` without touching
...
def _classify_gpu_name(gpu_name_upper: str) -> str | None:
    ...
    Most specific alias first — token count, then length — so ``H100 PCIE`` outranks the SXM
    entry's plain ``H100``, and ``GB200`` outranks the ``B200`` its name contains.
    """
    candidates = sorted(
