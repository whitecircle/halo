...
def _warn_if_uneven_shards(params) -> None:
    """Say at build time when optimizer-state checkpointing cannot work, not N days into the run.

    flashoptim's ``state_dict`` raises for any DTensor param whose sharded dim does not divide the
    mesh, so the trainer's all-or-nothing shard save skips optimizer state on every checkpoint and
    ...
    if uneven and is_global_main_process():
        # Shapes are rank-uniform, so one rank saying it covers the fleet.
        name, shape = uneven[0]
        logger.warning(
