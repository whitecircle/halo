...
SR keeps the bf16 writes unbiased where nearest rounding would truncate a sub-ULP ``lr*step`` update
to zero (stale weights) and inflate the non-negative second moment. Writes go through ``p.detach()``
(``p.data`` would carry its own version counter) and bump ``_version`` explicitly, since the
raw-pointer Triton stores are invisible to ATen and the low-precision weight cache keys on it.
"""
...
# Launch tile shared by every SR kernel here and in Muon: the same elementwise pass over a
# flattened parameter, so a second tuned tile would only be a number to keep in sync.
BLOCK_SIZE = 1024
...
def _adam_bf16_sr_kernel(
    ...
    # SR the second moment: nearest rounding biases this non-negative accumulator upward (~+50% at
    # small grads), inflating sqrt(v) and shrinking the effective step below lr.
...
def _draw_sr_seeds(use_triton: bool) -> tuple[int, int]:
    ...
    the first), so the number of draws per parameter is path-independent — and ``step`` draws them
    structurally, even for a param it skips, so rank-nonuniform grad presence cannot shift the
    rank-synchronized ``_SR_RNG`` stream and drift replicated params apart.
    """
    if use_triton:
...
class AdamWBF16(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        # Resolved per PARAMETER at step time on the tensor's own device: the eager path is
        # equivalent, and gating on `torch.cuda.is_available()` sends a CPU param into Triton.
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                # Seed draw and step count are STRUCTURAL — advanced for every param even when its
                # grad is missing, or rank-nonuniform grad presence drifts replicas apart silently.
...
def build_bf16_optimizer(model: torch.nn.Module, args: Any, decay_parameters: Sequence[str]):
    ...
    """
    # Two groups even when one is empty: the count is the index space a saved optimizer state uses.
