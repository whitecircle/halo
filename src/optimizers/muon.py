...
# per-rank). Its own generator, not the internal AdamWBF16's: sharing one would make each
# optimizer's noise depend on how many params the other happened to route.
...
# Same-shape params are stacked for the batched Newton-Schulz; the chunk bounds that peak.
_GNS_CHUNK_SIZE = 32
...
def _muon_kernels_available() -> bool:
    ...
    The import alone is not enough — the kernel ABI can drift so the CALL raises — so exercise it on
    a tiny tensor and let :func:`create_muon_optimizer` fall back to pure-torch on any failure.
    """
...
def _weight_decay_update_kernel(
    ...
    The orthogonalized update has magnitude ~lr, below the bf16 weight ULP at typical LRs, so nearest
    rounding would truncate it to zero (the "stale weights" failure). SR keeps the bf16 write unbiased.
    """
    pid = tl.program_id(0)
    offsets = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    mask = offsets < n_elements
...
def _is_embedding_or_head(name: str) -> bool:
    ...
    Name-based fallback for models exposing no embedding accessors; :func:`_embedding_and_head_param_ids`
    is the authoritative signal. T5's tied 'shared.weight' matches only as an exact path component —
    a substring match would swallow MoE shared_expert FFN matrices, which belong on Muon.
    """
...
def _embedding_and_head_param_ids(model: torch.nn.Module) -> set[int]:
    ...
    Read off the module tree the model declares rather than from parameter spellings, so a family
    naming its embedding ``tok_embeddings`` or its head ``output`` routes correctly without
    extending a marker list; tied weights collapse to one id and a PP stage owning neither is empty.
    """
...
def _collect_params_with_sr_seeds(params) -> tuple[list, list[int]]:
    """Split ``params`` into those with grads, drawing one SR seed per param STRUCTURALLY.

    Every param consumes its ``_SR_RNG`` draw in deterministic order even when skipped for a missing
    grad, so rank-nonuniform grad presence cannot shift the seed stream and drift replicas apart.
    """
    with_grad, seeds = [], []
    for p in params:
...
class Muon(UpstreamMuon):
    ...
    ``OptimizerStateFingerprint``, and a resume whose recorded class differs is refused.
    """

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self.scalar_optimizer is not None:
            # The scalar leg is AdamWBF16's own fused step; upstream's torch.compile wrapper around
            # it only pays graph breaks around a kernel launch.
    ...
    def _muon_step(self, param_groups: list[dict]) -> None:
        ...
        GNS batching stacks same-shape params (tens of GB on large MoE), so it runs in
        ``_GNS_CHUNK_SIZE`` chunks whose entries are freed as consumed.
        """
        for group in param_groups:
            ...
            if isinstance(adjust_lr, str) and adjust_lr == "rms_norm":
                ...
                # Upstream has more string modes this step does not implement — refuse over a silent rescale.
                raise ValueError(
    ...
    def state_dict(self) -> dict:
        ...
        Upstream serializes only ``self.state``, so the internal AdamW moments would vanish from
        checkpoints and reset on resume; they merge under the combined param_groups index space.
        """
        sd = super().state_dict()
        if self.scalar_optimizer is not None:
    ...
    def load_state_dict(self, state_dict: dict) -> None:
        ...
        # so restored hyperparameters land in a dead attribute; copy them onto the live group dicts.
...
def create_muon_optimizer(
    ...
    if ns_use_kernels:
        ...
        # equivalent but not bit-identical, drifting replicas — the kernels run only where every
        # rank has them.
    ...
    for name, param in model.named_parameters():
        ...
        # where Newton-Schulz is empirically harmful. Names still cover models exposing no accessors.
    ...
    # The fused step SR-stores bf16 into the param pointer: a non-bf16 param pays fp32 memory for bf16 math.
    if non_bf16_muon:
        raise ValueError(
    ...
    if scalar_groups:
        # AdamWBF16, not torch.optim.AdamW: the scalar params are bf16 too, and a nearest-rounding
        # write truncates every sub-ULP step to zero (norm weights, ULP 3.9e-3, never move). Also
        # per-parameter, so the DTensor/plain mix under EP/TP that foreach/fused AdamW rejects works.
    ...
    # 2D-first: gram_newton_schulz keys its 3D->2D split on the FIRST tensor's dim-0, so a 3D-first group
    # whose 3D params differ in dim-0 (qwen3.5) IndexErrors. Per-param updates are otherwise order-free.
...
def _is_explicitly_set(args: Any, field_name: str) -> bool:
    ...
    The training-args classes are dataclasses, so the declared default is the only available signal
    for "the YAML asked for this value" — HF does not record which fields were provided.
    """
    for f in fields(args):
        if f.name == field_name:
            if f.default is MISSING:
                # Required and default_factory fields have no comparable literal default — a `!=`
                # against the MISSING sentinel would silently report every value as "explicitly set".
...
def build_muon_optimizer(model: torch.nn.Module, args: Any, decay_parameters: Sequence[str]):
    ...
    # The Adam betas configure only Muon's internal scalar AdamW, and its tuned defaults differ from
    # HF's: forward a beta only when explicitly set, or every Muon YAML silently retunes that optimizer.
