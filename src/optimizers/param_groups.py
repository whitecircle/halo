"""Parameter grouping for optimizer construction: the weight-decay split every custom-optimizer
builder hands its optimizer, and the tensor-type split a stock optimizer needs under FSDP2 + EP.

Dependency leaf — the optimizer modules import it and it imports none of them.
"""
...
def decay_groups(
    ...
    *,
    ...
    the group count is the index space a saved optimizer state and a scheduler's per-group updates
    are keyed on, so a builder whose optimizer always carries two groups must keep emitting two.
    """
...
def build_tensor_type_grouped_optimizer(
    ...
    # Factory-style optimizers (GaLore/LOMO/layerwise) couple kwargs to the model or a fixed
    # param list — forwarding those next to our own grouped params would double-bind them.
