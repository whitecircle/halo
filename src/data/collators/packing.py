...
# Pad tails must not become one attention segment per token: transformers turns every position-0
# reset into a varlen segment boundary and the FA4 backward pays a fixed cost per segment (measured
# 275x — 6.9 s vs 25 ms per layer — on a 34k-pad tail). Chunking bounds segments and garbage pairs.
PAD_TAIL_SEGMENT_CHUNK = 256

# The span/EOS helpers this module uses are imported from the ``spans`` leaf; that leaf is where
# every other caller reads them too.
...
def collate_preserving_precomputed_labels(
    ...
    masks) crash the parent's tensor conversion: pop them, collate, then re-pad them to the batch
    shape with the ignore index (honoring the padding side). Without precomputed labels the parent's
    output goes through the pad==eos restore instead — baked labels are authoritative, so the restore
    must not touch them. ``examples`` must already be a copy: the pop mutates the rows.
    """
...
def restore_eos_labels_when_pad_equals_eos(
    ...
    is the *real* turn-ending EOS too when pad and EOS coincide (e.g. GLM-4's ``<|endoftext|>``,
    any family on the pad=eos fallback), so the model never learns to stop. Restore labels at
    real-token positions (``attention_mask == 1``) from ``input_ids``; true padding
    (``attention_mask == 0``) stays masked. No-op when pad != eos (the parent already trains real
    EOS). Never apply over precomputed labels — their baked ``-100`` spans are authoritative.
    """
    if (
...
def flatten_packed_batch(batch: dict[str, Any], real_lengths: list[int]) -> dict[str, Any]:
    ...
    attention, silently leaking attention across document boundaries — so the rows must merge.

    ``real_lengths`` (per-row real token counts) is what makes the merge safe at scale. A plain
    reshape keeps each row's pad tail, and every pad carries position_id 0 — one length-1 attention
    segment PER PAD TOKEN once transformers derives cu_seqlens. A mostly-empty row (the dataset's
    final partial pack) co-batched with a full one contributes tens of thousands of such segments,
    each paying the varlen kernels' per-segment cost (:data:`PAD_TAIL_SEGMENT_CHUNK`), which lockstep
    FSDP collectives then impose on every rank. Real tokens never attend to pads and pad labels are
    ...
    # ndim == 2 exactly: the gather indexes with token strides, so [B, L, X] would be mangled.
    return {
...
class DataCollatorForCausalLMWithPadding(DataCollatorForLanguageModeling):
    ...
    The parent builds labels from ``input_ids`` but crashes converting ragged precomputed
    ``labels`` (``tokenizer.pad`` pads only model inputs); this pads them with the ignore index
    instead, keeping offline prompt-masking intact. On the raw-labels path the real turn-ending
    EOS is restored when pad == eos (the parent masks every pad-valued label).
    """
...
class DataCollatorWithPacking(DataCollatorForLanguageModeling):
    ...
    Cross-document isolation comes from the position_ids: FA derives varlen cu_seqlens from
    them, and transformers materializes a ``packed_sequence_mask`` from them for eager/SDPA
    whenever ``attention_mask`` is None (``masking_utils``). Both routes depend on the MODEL
    plumbing position_ids to its attention interface (flash) or its mask construction (dense) —
    family-dependent, see ``agent-docs/data/collators.md`` for the per-family isolation matrix.
    The non-flash path pays a dense ``[L, L]`` mask over the flattened row (``flatten_to_single_row``),
    so ``L`` is the whole batch's token count — up to ``per_device_train_batch_size × max_length`` —
    and the cost grows with the batch squared. :func:`src.data.collators.factory.select_data_collator`
    warns there rather than rejecting, because packing raises tokens per row on any kernel;
    ``padding_free`` IS rejected instead, its flattened ``L`` being no larger while the cu_seqlens it
    exists to emit go unread.

    Requires RIGHT padding: position_ids are laid out from index 0 forward, so leading
    pads would become an attended document and displace a real one out of the loss.
    """

    # Columns that must survive HF's signature-based column pruning; a caller pinning columns itself
    # unions this in, since without ``seq_lengths`` every row silently collates as ONE document.
    required_dataset_columns: tuple[str, ...] = ("seq_lengths",)

    # Flatten the packed ``[B, L]`` batch to ``[1, B*L]`` (:func:`flatten_packed_batch`). PP turns
    # this off: its dim-0 microbatch split IS the flatten.
    ...
    def torch_call(self, examples: list[list[int] | Any | dict[str, Any]]) -> dict[str, Any]:
        ...
        # One entry per example, non-dict rows included: a shorter list would re-attach a row's
        # lengths to the WRONG row below (the zip realigns), packing the batch by another row's
        # document boundaries.
        seq_lengths_list = []
        if has_packing:
            for ex in examples:
    ...
    def _real_row_lengths(batch: dict[str, Any], examples: list[Any]) -> list[int]:
        ...
        ``_handle_packing``), so its pads — indistinguishable without the lengths — stay, exactly as
        before. Rows are index-aligned with ``examples`` by the parent collation.
        """
...
class DataCollatorForCompletionOnlyLMWithPacking(DataCollatorWithPacking):
    ...
    def _mask_sequence(self, labels: torch.Tensor, input_ids: torch.Tensor | None = None) -> torch.Tensor:
        ...
        :data:`~src.data.spans.PACKED_SPAN_POLICY` — the named policy the offline bake also
        takes for a packed artifact, so a terminator-less final turn trains the same tokens whether
        this collator masks it or the bake did.
        """
...
class DataCollatorWithFlatteningAndCompletionMask(DataCollatorWithFlattening):
    ...
    def _apply_completion_mask(self, labels: list[int], input_ids: list[int]) -> list[int]:
        ...
        Spans are detected in ``input_ids``, never in labels: precomputed labels with the marker or
        EOS baked to ``-100`` would otherwise match nothing and silently zero the loss.
        """
        spans = resolve_spans_or_warn(
            input_ids,
            self.response_token_ids,
