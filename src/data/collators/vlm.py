...
The primitives they compose (the chat render, the processor call, the over-length check) come from
:mod:`src.data.vlm`, so a runtime batch and an offline bake of the same row tokenize identically.
"""
...
class VLMDataCollator:
    ...
    def _render(self, history: list[dict[str, Any]], example: dict[str, Any]) -> str:
        ...
        self-distill teacher branches so both render through the same call."""
    ...
    def __call__(self, examples):
        ...
        # No truncation: cutting expanded image placeholders desyncs text from pixel_values /
    ...
    def _build_labels(self, input_ids: torch.Tensor, attention_mask: torch.Tensor | None) -> torch.Tensor:
        ...
        response tokens, as OPD row alignment requires.
        """
...
class SelfDistillVLMDataCollator(VLMDataCollator):
    ...
    teacher labels reuse the parent ``_build_labels``, so both branches select the same tokens, as OPD
    row alignment requires. The text-only hint leaves the image grid unchanged, so the trainer reuses
    the student's image tensors. Uses the self-distill span policy (``_COMPLETION_SPAN_POLICY = None``),
    matching :class:`SelfDistillTextCollator`. Optionally emits ``confidence_weights``.
    """
    ...
    def __call__(self, examples):
        ...
        # No truncation on the teacher: the hint makes it systematically longer than the student, so
        # right-truncation would cut response tokens the student keeps and break OPD row alignment.
