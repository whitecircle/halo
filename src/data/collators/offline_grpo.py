...
Sits with the other collators rather than in the trainer file because it is the trainer's data-side
contract — the padding sides and the substituted-completion mask below are what
``OfflineGRPOTrainer.compute_loss`` assumes about every batch it receives.
"""
...
class OfflineGRPODataCollatorWithPadding:
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for example in features:
            ...
            if has_ref_logps:
                # Row-aligned with the completion as TOKENIZED: a length mismatch means the column
                # was produced under other length caps or another tokenizer, and shipping it would
                # anchor the KL to the wrong tokens. A substituted row carries none.
