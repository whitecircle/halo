...
The hint injection and the confidence weighting live here rather than in a shared leaf because the
VLM self-distillation branch is the only other caller and reaches them collator-to-collator.
"""
...
def mean_normalized_confidence_weights(
    ...
    total = weights.sum()
    if total <= 1e-9:
        # All confidences zero/absent — uniform weights, else the batch scales to a zero-gradient step.
...
class SelfDistillTextCollator:
    ...
    def audit_row(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        reject bound methods), where the coordinated map machinery turns the raise into a
        world-uniform failure. The same raise at collate time is rank-local: only the rank whose
        batch drew the over-length row reads the error while its peers sit in the step's
        collectives until the NCCL watchdog.
        """
...
def audit_self_distill_row(example: dict[str, Any], collator: SelfDistillTextCollator) -> dict[str, Any]:
    """Module-level seam for mapping :meth:`SelfDistillTextCollator.audit_row` with ``num_proc``
    workers, which reject bound methods (every worker would pickle ``self``). The collator rides
    ``fn_kwargs`` instead — its tokenizer and config pickle cleanly."""
