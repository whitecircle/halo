...
# ``find_spec`` checks drops the ``fla>=0.2.2`` floor and the CUDA-capable-torch conjunct.
...
def select_data_collator(
    ...
    # Emitting the markers is only half the isolation — the fast-path kernels alone read them.
