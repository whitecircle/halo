...
Provides the loss ignore index, the assistant-turn terminator set, the marker-to-terminator span
search, the span mask, the completion-only label builder and the construction-time chat-template
probe. The runtime collators, the offline label bake and the PP losses all read them from here.
"""
...
# The HF cross_entropy ignore_index, shared by the collators, the preprocessing label bake and the
# PP losses.
...
# label bake and ``build_completion_only_labels`` below select one instead of re-typing the kwargs.
# The packed policy differs only in ``eos_fallback_to_end``: a packed sequence carries no trailing
# padding, so a turn whose terminator the pack boundary cut away may end at the last position, which
# under padding would be a pad token. The offline bake picks by whether its artifact is packed, so a
# preprocessed run and the same YAML on a raw dataset train the same tokens for such a turn.
...
def require_response_marker(
    ...
    waits for collation arrives after the model load and the whole dataset map. ``subject`` names the
    caller in the error.
    """
...
def verify_marker_renders_in_chat_template(
    ...
    Probes the rendered text rather than the template source, since templates assemble the marker
    from pieces and a marker the template never emits masks every label. Two variants are probed
    because reasoning templates (gpt-oss harmony) emit the assistant marker only for a message
    carrying ``thinking``; the raise fires only when no variant emits it.
    """
    probe_conversations = (
    ...
    for conversation in probe_conversations:
        try:
            ...
            continue
        # isinstance: only actual text can be probed; anything else is skipped.
        if isinstance(rendered, str):
            rendered_texts.append(rendered)
    if not rendered_texts and PartialState().is_main_process:
        # Nothing rendered, so the marker check below never ran. Without this warning the outcome is
        # indistinguishable from a passing probe, and a marker the template never emits would show up
        # only as a run at loss ~0.
        logger.warning(
...
def ends_with_terminator(input_ids, tokenizer, eos_token_ids) -> bool:
    ...
    unterminated and appends a second terminator the model never emits. Same rule the SFT render path
    applies in ``_render_terminator_survived``. The walk stops at the first content token, so it costs
    one decode on an already-terminated row.
    """
...
def resolve_eos_token_ids(
    ...
    ``tokenizer.eos_token_id`` alone is unreliable: some chat templates (e.g. GLM-4) delimit turns
    with role markers listed only in ``config.eos_token_id``. Unions the model config's
    ``eos_token_id`` (also nested under ``text_config`` for VLM/composite) with the tokenizer's eos;
    ``model_config=None`` falls back to the tokenizer eos alone. A distinct pad token is not treated
    as a terminator: on a right-padded row whose turn lacks a real terminator, the first trailing pad
    would close the span, training the model to predict pad and bypassing the warn-and-mask path.
    When pad == eos, the eos collection already covers it.
    """
    ids: set[int] = set()
...
def warn_if_pad_equals_eos(tokenizer: PreTrainedTokenizerBase) -> None:
    """Warn when pad == eos: completion masking must then restore the turn-ending EOS from
    ``input_ids``, since the LM collator masks every pad position in labels."""
...
def filter_eos_after_responses(response_starts: list[int], eos_positions: list[int]) -> list[int]:
    ...
    which would unmask the intervening user/tool turns and a later assistant turn. Such a turn
    returns ``-1``, an empty no-op span.
    """
...
def _scan_completion_spans_sequential(
    ...
    Each span ends at the first terminator at or after the marker end, unbounded by the next marker
    start, and the marker search resumes past the span end, so a marker inside a terminator-less span
    is consumed by it. ``eos_fallback_to_end`` is per-span here: a terminator-less final span ends at
    the last position, otherwise it is an empty ``-1`` no-op span.
    """
    starts: list[int] = []
    ends: list[int] = []
...
def resolve_completion_spans(
    ...
    position ending its turn (``-1`` for an empty no-op span). Search ``input_ids`` rather than
    ``labels``: a labels search misses the turn-ending EOS masked to ignore_index when
    ``pad_token_id == eos_token_id``. ``train_on_last_assistant_only`` keeps only the final span.
    ...
    - ``include_marker``: span starts at the response template (collator) or after it (self-distill).
    - ``bound_by_next_start=True`` (collator): a terminator must precede the next marker start, else
      ...
      global: the last position substitutes only for a sequence with no terminator at all, which is
      safe only without trailing padding, i.e. on the packed/flattened paths.
...
def warn_completion_span_miss(
    ...
    *,
    ...
    is bounded on both axes, since a full 32k-token decode per row would dominate the step.
    """
    if matched_marker:
...
def resolve_spans_or_warn(
    ...
    *,
    ...
    The report distinguishes a marker that did not match from a span that did not close, since a
    matched turn with no terminator anywhere leaves ``eos_ends`` empty and would otherwise be
...
def _real_token_bounds(row: torch.Tensor, attention_mask_row: torch.Tensor | None) -> tuple[int, int]:
    ...
    batches alike. Without a mask the whole row counts as real.
    """
...
def mask_batch_to_completion_spans(
    ...
    The padded collator, the offline label bake and the self-distill / VLM label builder all land
    here, so a row masked offline and the same row masked at runtime carry the same targets. The
    packed and padding-free collators mask one sequence at a time but share this function's
    :func:`resolve_spans_or_warn` preamble under :data:`PACKED_SPAN_POLICY`.
    ...
    - ``span_policy`` selects the policy kwargs of :func:`resolve_completion_spans`, defaulting to
      :data:`COLLATOR_SPAN_POLICY` (no end-of-sequence fallback, which would train pad on padded rows).
...
def build_completion_only_labels(
    ...
    as OPD row alignment requires. Callers holding an ``attention_mask`` must pass it: the value-based
