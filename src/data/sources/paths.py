...
Pure string rules, no boto3/s3fs — the loader, the preprocessing pipeline and the standalone
scripts classify a path without pulling an S3 stack they never call.
"""

# A local path ending in one of these is a data FILE, not a saved-dataset directory. The classifier
# below and the loader's builder choice read the one mapping, so a newly accepted extension cannot
...
# The preprocessed-dataset stamp. In this leaf because both the pipeline and the uploader need it:
# ``push_folder`` orders it delete-first/upload-last so a torn upload cannot present as a valid
# preprocessed dataset, and a second spelling would break that ordering silently.
...
def parse_hub_spec(path: str) -> tuple[str, str | None, str | None]:
    ...
    One reader for the toolkit's spelling: ``@`` selects the split and ``:`` the config, in that
    order. A caller that re-derives it the other way round loads a *config* as a split name.
    """
...
def parse_dataset_destination(path: str) -> tuple[str, str | None, str]:
    ...
    dest_type is 's3', 'hf_hub', or 'local'. Unlike :func:`parse_dataset_source`, a bare
    "org/name" is LOCAL: guessing the Hub is acceptable for a read, but a write must never
    upload anywhere the caller did not name — only an explicit "hf://org/name" targets the Hub.
    """
    if path.startswith("s3://"):
        bucket, key = parse_s3_uri(path)
        return ("s3", bucket, key)
