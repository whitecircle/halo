...
and the per-shard cache of a sharded dataset share one set of crash and staleness semantics. The
...
# what the cache did lands nowhere. Plain logging, not the accelerate adapter: entry points that
...
# a set-but-empty override would resolve every cache path against the CWD, scattering multi-GB
...
# Written inside a published cache dir; its presence is what makes the cache servable. Shared with the
# sharded-dataset shard cache, which follows the same publish protocol.
...
def s3_cache_key(bucket: str, key: str) -> str:
    ...
    One spelling behind every cache on this volume — dataset trees, per-shard caches, control-file
    mirrors. A drifted second spelling would not fail; it would miss the cache a prior run
    pre-populated, so the run needs live AWS where it was meant to survive an outage.
    """
...
def publish_cached_download(
    ...
    * ``live_fingerprint`` (probed by the caller OUTSIDE the lock — network I/O) is compared with the
      marker's stored one, so an in-place re-push to the same URI invalidates the cache. ``None``
      means "cannot validate" and serves the cache as-is; a fingerprint-less marker is upgraded in place;
    * ``fetch(tmp_path)`` writes a unique temp dir published by ``os.rename``: flock does not reliably
      exclude across NFS clients, so cross-node writers race the harmless rename, not the bytes.
    """
    ...
    with lock:
        # Under the lock same-node writers are excluded; the age threshold covers cross-node ones it cannot see.
        ...
        if cache_valid and live_fingerprint is not None:
            ...
            if stored_fingerprint is None:
                ...
                # trust the cache, and stamp the live identity so the next probe can validate it.
        ...
        # another node is loading right now (loads run outside the lock, and flock does not exclude
        # across NFS clients), and deleting it under that reader raises FileNotFoundError mid-load.
        # Renaming keeps the reader's already-open files valid and narrows the window where
        # cache_path is absent to one rename; a crash in it leaves the path ABSENT (re-downloaded
        # next run), never a half-written tree behind a completion marker. The superseded tree wears
        # the same `.tmp-<uuid>` name a crashed writer's does, so one reaper covers both.
...
def _cleanup_stale_publish_tmp_dirs(
    ...
    Sibling dirs matching the exact temp pattern and older than ``max_age_seconds`` go — a live
    writer's dir is always fresh, so no race. Returns the removed paths.
    """
