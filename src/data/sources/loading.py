...
# run that declares its images anywhere else is aliased onto the first (:func:`alias_images_column`).
...
def _reject_divergent_split_presence(dataset: DatasetDict, path: str) -> None:
    """Fail loud when the SPLIT SET a load produced differs across ranks.
...
def _load_dataset_from_path(
    ...
    *,
    ...
    (``sharded=True`` — each DP rank then holds a rank-specific slice). Without a test split:
...
def _apply_conversation_field(
    ...
    the column's absence fatal instead of the silent no-op filtering below.
    """
