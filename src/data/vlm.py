...
def carried_image_columns(dataset) -> set[str]:
    ...
    One home for the probe every image-column guard runs — the text-only refusal, the offline bake's
    unconsumed-column refusal and the multimodality verdict — so all three read a dataset the same
    way. ``dataset`` may be a ``DatasetDict`` or a single split.
    """
...
def render_vlm_text(processor, history: list[dict[str, Any]], *, tools: list | None = None) -> str:
    """Chat-template one VLM conversation — the single home for the render every VLM path shares.
...
def run_vlm_processor(processor, texts, images, **overrides):
    """Invoke a VLM processor on rendered chat-template text — the single home for the call contract.
    ...
    images mismatch) is re-raised as ``ValueError``: leaked from a collator, ``StopIteration``
    silently ENDS the dataloader — training completes with zero steps and no error.
    """
...
def raise_if_over_length(encoded, max_length: int | None, subject: str) -> None:
    """Fail loud when a processed VLM batch carries a sequence over the run's ``max_length``.
    ...
    names the batch (the SDPG teacher and student batches are separately over-length).
    """
...
def get_image_token_ids(tokenizer, processor=None) -> set:
    ...
    Unions several sources because a missed placeholder is silent — its vision-pad tokens get trained
    as text.
    """
    image_token_ids = set()
...
def _conversation_schema_declares_images(column_feature) -> bool:
    ...
    Row-independent, so it sees what no row window can: Arrow unions the parts struct over the whole
    split, so one image-carrying row anywhere puts the image field in every row's schema. Read only
    for the parts form, and only on the fields the VLM path treats as image payload
    (:data:`~src.data.pipeline.conversation.IMAGE_PART_PAYLOAD_KEY`, or a ``datasets.Image`` feature
    under any name). A JSON-string conversation column has no struct to read — the first-row probe
    covers that one.
    """
...
def _declares_images_locally(dataset, conversation_field: str | None) -> bool:
    """:func:`dataset_declares_images` on THIS rank's slice, before the cross-rank agreement.

    A column in :data:`VLM_IMAGE_COLUMNS`, or images embedded in the conversation's content parts.
    The embedded form is read twice, and the pair is what makes the verdict hold for a MIXED dataset:
    the Arrow schema is exact wherever the column is parts-form struct data, and the first-row probe
    covers the JSON-string column, which has no schema. A row window alone would resolve a lightly
    image-bearing mixed dataset to a TEXT run and crash only once the model was on the GPUs.
...
def dataset_image_evidence(dataset) -> str | None:
    """Why ``dataset`` counts as image data on THIS rank, or ``None`` when nothing declares any.

    The same two declarations :func:`dataset_declares_images` reads, but with EVERY column probed
    for embedded parts rather than one named ``conversation_field`` — for a consumer that never
    learns the field name (a trainer handed the loaded dataset). ``dataset`` may be a
    ``DatasetDict`` or a single split; ``None`` reads as text. The returned text names the
    declaration, for a refusal that says what it saw.
    """
    if dataset is None:
        return None
...
def dataset_declares_images(dataset, conversation_field: str | None = None) -> bool:
    ...
    Under :class:`~src.data.sources.sharded_dataset.ShardedDatasetLoader` every local term is a
    rank-local fact — rank 0's first row is not rank 5's, and Arrow schemas are inferred per shard set
    — while the consumers branch a whole data pipeline on the answer, whose arms run different numbers
    of coordinated operations. One collective per call, on every input (``None`` included), so the
    count cannot depend on the verdict itself.
    """
...
def is_vlm_run(args, model_name_or_path: str, dataset=None, *, config=None, revision: str | None = None) -> bool:
    """Whether this RUN takes the VLM data path — a multimodal checkpoint AND image data to feed it.

    The checkpoint alone cannot decide it: every natively-multimodal family (Gemma 4, Qwen3.5/3.6) is a
    VLM by config while its text-only recipes are ordinary text runs, and the VLM path refuses
    packing / padding_free / train_on_last_assistant_only / interleaved_thinking. The run declares image
    data by naming the column (``images_field``) or carrying one; the model loads through its own
    multimodal class either way, so only the data prep, collator and eval split follow this verdict.

    Agreed across ranks ONCE for the whole verdict rather than per term — the two arms run different
    numbers of coordinated operations. ``config`` / ``revision`` pin the modality probe as in
    :func:`~src.models.modality.is_vlm_model`; pass the already-loaded ``model.config`` where there is one.
    """
        ...
        if isinstance(content, str):
            ...
            for item in content:
                if item.get("type") == IMAGE_PART_TYPE:
                    ...
                    # container) must fail loud in process_image. Skipping it while still appending
                    # the placeholder below builds a prompt whose image placeholder has no image.
                    ...
                    # templates read the KEY, not the type (Qwen-VL: `'image' in content`), so
                    # leaving it renders a vision placeholder no image ever pairs with.
