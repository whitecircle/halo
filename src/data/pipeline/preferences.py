...
The batch half of the vision pair is :mod:`src.data.collators.vlm_preference`.
"""
...
def split_rendered_completion(prompt_text: str, full_text: str, field: str) -> str:
    ...
    message lists, and the prefix strip guarantees ``prompt_text + completion_text`` reconstructs
    the full rendered conversation exactly. Fails loud on templates that break the prefix invariant.
    """
...
def normalize_preference_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    through. Fails loud on a continuation that ends up empty or not on an assistant turn — a
    mis-mapped dataset, not a shape to guess around.
    """
    ...
    for field in ("chosen", "rejected"):
        ...
        # A completion that repeats every prompt turn is a full conversation — strip the prefix.
...
def split_vlm_preference_row(
    ...
    """One raw VLM ``(prompt, chosen, rejected[, images])`` row → prompt history, images, completions.

    The pre-render half every VLM preference route shares: normalize to the preference contract,
    merge a raw image column into the prompt conversation, and extract the images back out, leaving
    the placeholders the processor expands at collation. ``subject`` names the route in the
    images-in-completion refusal, which keeps images in the shared prompt prefix — the only region
...
def vlm_preference_features(dataset: Dataset) -> Features:
    ...
    ``margin`` is the one source column the map keeps, so it belongs in the schema exactly when the
...
def render_vlm_preference_row(features: dict[str, Any], processing_class: ProcessorMixin) -> dict[str, Any]:
    ...
    The pre-render half is :func:`split_vlm_preference_row`, shared with the SMPO vision route. Each
    side is then chat-templated WHOLE — the reward head scores the full sequence, so there is no
    prompt/completion split to preserve and no prefix-strip invariant to lean on. The render goes
    through :func:`~src.data.vlm.render_vlm_text`, the one home every VLM path shares, so a
    conversation tokenizes identically here and on the SFT path. Pixels never reach the Arrow cache:
    the images travel as PIL objects and the patch geometry is a property of the processor call.
    """
...
def apply_chat_template_to_preference_data(
    ...
    # embedded-image row arrives here unseen.
...
def _normalized_generative_row(row, tokenizer, max_length, tools_field=None):
    """A raw preference row → tokenized generation prompt, through the contract normalizer.

    ``prepare_generative_row`` templates ``row["prompt"]`` as a message list, but the raw test split
    still carries the hub shapes, so a string prompt would die inside the chat template. The
    normalization composes here rather than inside that helper, which is family-generic (offline GRPO
    maps it over rows with no chosen/rejected). Module-level because ``num_proc`` maps must pickle it.
    """
