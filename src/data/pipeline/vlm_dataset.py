...
filter the SFT and distillation scripts share. Sits above the pipeline leaves it composes; the
training scripts are its only consumers.
"""
...
def vlm_map_features() -> datasets.Features:
    ...
    ``text`` key), and an all-empty ``images`` shard infers ``List(null)`` — multiprocess maps then
    fail to align shard schemas. Pinning the schema makes the map deterministic for every mix.
    """
...
# ``create_vlm_processor`` cannot be stripped here for want of a second edit.
...
def _filter_vlm_over_length(ds, processor, tokenizer, max_length: int, num_proc: int):
    ...
    The VLM collators fail loud on over-length batches (image tokens cannot be truncated), so
    pathological text rows must go first. Vision tokens are not counted here — this removes the text
    tail cheaply and the collator stays the exact backstop. The emptied-everything refusal is
    world-agreed: the surviving count is a per-rank fact on a presharded corpus.
    """

    def fits(row) -> bool:
        # The collator's own render, tools included, so the filter measures the sequence that will
        # actually be collated rather than a shorter twin of it.
