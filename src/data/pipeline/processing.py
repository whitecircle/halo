...
The main rank runs each map/filter to a deterministic cache file; the others wait and load it,
avoiding duplicate work and per-rank copies. Set HF_DATASETS_CACHE for a consistent cache dir.
Prefer the high-level ``coordinated_map`` / ``coordinated_filter``.
"""
...
def _resolve_dataset_num_proc() -> int:
    ...
    training (ranks x num_proc) each extra process adds memory and IPC cost.
...
# A post-map filter dropping at least this share of the corpus is reported at WARNING level:
# legitimate high drop rates exist, but this is usually a config error (see
# :func:`report_rejected_rows`).
...
def resolve_map_num_proc(configured: int | None) -> int:
    """``dataset_num_proc`` to map/filter worker count. Unset means the toolkit default, not 1 worker."""
...
def _referenced_local_functions(func: Callable, seen: set) -> list[Callable]:
    ...
    They shape the output as much as the function's own body, and a module global is invisible to
    source and closure inspection. Third-party callables are excluded: their source changes without
    changing this toolkit's outputs.
    """
...
def get_function_identifier(func: Callable, _seen: set | None = None) -> str:
    ...
    address), folding in the source of every repo-local helper the code references, so editing a
    helper invalidates the caches of its callers."""
    # partial hides its func and bound args from source inspection: unfolded, every partial would
    # share one id.
...
def _tokenizer_identity(val: Any) -> str | None:
    ...
    ``.tokenizer`` plus its own chat template; without the descent, two checkpoints sharing a
    processor class collide on one cache file. The descent is typed on ``ProcessorMixin``: any other
    ``.tokenizer`` holder keys by class name, and its knobs belong in ``cache_key_extras``.
    """
...
def _json_content_repr(val: Any) -> str | None:
    ...
    Nested collections (a dict of flags, a list of pairs) carry cache-relevant content that no scalar
    branch can read. Without this they key on their type name, so two different values of one knob
    share a cache file and the second run loads the first's rows.
    """
    try:
        ...
        try:
            ...
            if tok_sig is not None:
                ...
                if getattr(val, "tokenizer", None) is not None:
                    # Refused by the ProcessorMixin gate above. Keying by class name is correct but
                    # invisible: the render knobs it hides must ride cache_key_extras, or editing
                    # them reuses the previous run's rows.
                    _warn_unfingerprintable(f"fn_kwargs[{key}]", type(val).__name__)
                parts.append(f"{key}={type(val).__name__}")
        except Exception:
            # A kwarg the key cannot read cannot invalidate the cache, so report it.
...
def _scalar_collection_repr(val: Any) -> str | None:
    ...
    Sets are sorted so the fingerprint does not depend on iteration order. Dicts are not
    fingerprinted: captured dicts are schema carriers (``none_example``) treated as
    cache-irrelevant, and take the one-time skip warning instead.
    """
...
def _get_closure_fingerprint(func: Callable) -> str:
    ...
    for cell in closure:
        ...
        try:
            tok_sig = _tokenizer_identity(val)
        except Exception:  # a raising attribute/property lands in the skip warning, not in cache naming
            tok_sig = None
        if tok_sig is not None:
            parts.append(tok_sig)
            continue
        if isinstance(val, (types.FunctionType, functools.partial)):
            # A captured helper shapes the output as much as the wrapper's own source, so recurse
            # and let editing it invalidate the cache. Plain functions only; callable tokenizers
            # are handled above.
...
def ensure_cache_dir(writer_rank: Callable[[], bool] = is_local_main_process) -> str:
    ...
    fences the write; otherwise a read-only or full volume raises on one rank while its peers wait
    out the barrier instead of reporting the errno.
    """
...
def report_rejected_rows(original_size: int, kept_size: int, context: str) -> None:
    ...
    Shared by every drop-and-continue filter. Legitimately high drop rates exist, but most of a
    corpus vanishing usually indicates a config error. Local main only, like every other
    dataset-stage log.
    """
...
def require_render_column(dataset: DatasetDict, path: str, knob: str, column: str) -> None:
    """Raise when a declared render column is absent from a raw dataset.

    A mistyped column would otherwise surface inside an HF ``map`` worker as a ``KeyError`` naming
    neither the knob nor the dataset, and for an optional knob (``tools_field``) not at all.
    """
...
def _cached_map_or_filter(
    ...
    the ``datasets`` method and the cache-key namespace, so the two cannot drift on which kwargs they
    refuse, which callables they reject, or what enters the cache name."""
...
def coordinated_map(
    ...
    **map_kwargs,
    ...
    cache_key_extras fingerprints closure-captured tunables that affect output but are not visible in
...
def log_dataset_examples(
    ...
    *,
    ...
    non-shared filesystem gets the samples beside the log on every node while a shared one still has
    a single writer.
    """
...
def _run_or_record(operation_fn: Callable, operation_kwargs: dict, label: str) -> tuple[Any, BaseException | None]:
    """Run ``operation_fn``, returning ``(result, failure)``; a failure is recorded, not raised here.

    A rank-local raise leaves the peers blocked in the join that follows, so
    :func:`_reject_operation_failure` turns the record into a raise on every rank. ``BaseException``
    is caught so a ``KeyboardInterrupt`` also reaches the join, bounded by
    ``DIST_STORE_TIMEOUT_HOURS``.
    """
    logger.info(f"{label}...")
    try:
...
def _reject_operation_failure(failure: BaseException | None, operation_name: str) -> None:
    """Abort every rank when the rank that ran the operation failed. Collective-equivalent: all ranks
    call it, in the same order.

    The peers take a uniform ``RuntimeError`` carrying the cause; the rank that failed re-raises its
    own exception, keeping the type callers rely on. The join rides the c10d store rather than a
    collective, since peers wait out the writer's entire map and the NCCL watchdog would kill that.
    """
...
def coordinated_dataset_operation(
    ...
    **kwargs,
    ...
    Scope follows the input filesystem: on a shared one only the global main maps (avoiding an
    NFS/Lustre write race), on a non-shared one each node's local main does; the others wait and load
    the cache. ``operation_fn`` must capture the dataset in its closure. num_proc<=1 maps to None (HF
    still spawns subprocesses at 1).

    Both phases join through :func:`_reject_operation_failure` on the c10d store rather than a
    barrier, whose budget lets peers wait out an hours-long fresh-cache map the NCCL watchdog would
    kill.

    This function performs the rank ordering itself; do not wrap a call in a main-first block
    (``PartialState().local_main_process_first()``, :func:`fs_aware_main_first`), which would hold
    the peers outside the body and leave ``ensure_cache_dir``'s barrier on the main rank alone.
    """
    ...
    # HF's _fingerprint/cache_files diverge between writer and loader ranks, so stamp a stable key.
    if cache_file_name:
...
def pack_dataset_coordinated(
    ...
    # ensure_cache_dir() barriers, so it must stay outside the phase below: inside it, only the main
