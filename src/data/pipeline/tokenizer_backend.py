...
# Shorter than every probe's token count, so the truncated comparison actually truncates —
# production paths encode with truncation=True (e.g. tokenize_rendered), so parity must hold there.
...
def _equivalence_probes(tokenizer: PreTrainedTokenizer) -> list[str]:
    ...
    if getattr(tokenizer, "chat_template", None):
        ...
        except Exception as exc:  # a template needing extra kwargs isn't a backend defect
