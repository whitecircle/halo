...
# Per-row VLM failures (a bad image, a content shape the chat template refuses, a
# placeholder-vs-image count mismatch) are dropped with a warning; every other exception aborts the
# map, since a blanket catch would thin the corpus without notice. The two image errors are named
# rather than their shared ``OSError`` base so a full disk aborts instead of counting as a bad row.
...
def _completion_only_labels(
    ...
    the default), a packed one by :data:`~src.data.spans.PACKED_SPAN_POLICY`. ``response_token_ids``
    is the pre-tokenized ``assistant_template``, hoisted by callers out of the per-row map like
    ``eos_token_ids``; ``extra_ignore_token_ids`` (e.g. image tokens) are masked on top.
    """
...
def _drop_rejected_rows(
    ...
    *,
    ...
    Shared by the text and VLM bakes. An empty split indicates a config error rather than a length
    budget, and ``cause`` names the causes that apply to ``stage``.
    """
...
def _reject_fully_masked_labels(tokenized: Dataset, split_name: str) -> None:
    """Refuse a baked split whose probed rows are entirely ignore-index; it would train zero tokens.

    Probed after the bake, since labels do not exist before it, and at both head and tail, because an
    ordered pre-labeled dataset may legitimately open with fully-masked rows. Shared by the text and
    VLM bakes, since a mistyped assistant-message template masks everything on either path without
    tripping the row filters, which key on ``input_ids``.
    """
    total = len(tokenized)
...
def tokenize_dataset(
    ...
    # Every source column is dropped, as the runtime path drops them: a source column named `labels`
    # (classification/reward corpora carry one) would otherwise survive the map and be baked as this
    # dataset's loss targets, since the labels step below only fills in a missing column.
    # input_ids/attention_mask/labels below are the processor's output, added after removal.
    ...
    if config.train_on_completions_only:
        ...
        tokenized = coordinated_map(
            ...
            cache_key_extras={
                ...
                # Two artifacts of the same corpus can differ only by this policy; without it in the
...
def tokenize_vlm_dataset(
    ...
    # result is fixed across the corpus. Sorted so the closure fingerprint is order-independent.
    ...
    def process_vlm_example(example: dict[str, Any]) -> dict[str, Any]:
        try:
            ...
            # The runtime VLM path's own builder and renderer (create_vlm_processor into
            # VLMDataCollator), so a row baked here tokenizes exactly as an untouched run renders it.
            ...
            # No truncation of a VLM row: cutting expanded image placeholders desyncs text from vision.
            ...
            if config.train_on_completions_only:
                ...
                # explicitly so its value-based pad fallback cannot run here; that fallback erases
                # real EOS wherever pad_token_id == eos_token_id.
...
def preprocess_dataset(
    ...
    if config.is_vlm and "train" in result:
        ...
        if not has_pixel_values:
            # Legitimate for a text-only corpus rendered with a VLM processor, but also what a
