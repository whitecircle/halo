"""Chat-template rendering and the tokenization of rendered text, with probe-based special-token
ownership.

Sits directly over :mod:`src.data.pipeline.conversation`, so the row maps, the offline tokenizer and
the self-distillation collator reach the render path without the coordinated-map machinery.
"""
...
def _render_terminator_survived(encoded: Any, tokenizer: PreTrainedTokenizer, specials: TokenizerSpecials) -> bool:
    ...
    length-vs-``max_length`` comparison cannot decide it: a render that exactly fills the budget is
    never cut, yet measures the same as a cut one.
    """
...
def render_conversation(
    ...
    *,
    ...
    """Chat-template one text conversation, shared by every text path.
    ...
    template rendered it: :func:`tokenize_rendered` resolves the specials, and stripping here would
    delete BOS for families whose template emits it while their post-processor adds nothing
    (gemma-4).
    """
...
def render_generation_prompt(
    ...
    **template_kwargs,
    ...
    post-processor prepends one of its own, otherwise gemma-4-style templates lose theirs.
    ``max_prompt_length`` filters rather than truncates, since cutting a prompt removes the question
...
def tokenize_rendered(tokenizer: PreTrainedTokenizer, text: str, *, for_generation: bool = False, **tokenizer_kwargs):
    ...
    Templates disagree about BOS: some emit it, some rely on ``add_special_tokens``, and gemma-4
    emits it while its post-processor adds nothing. :func:`probe_tokenizer_specials` decides:
    ...
    sequence (modulo whitespace) and it survived into the ids
    (:func:`_render_terminator_survived`), where the appended copy is a duplicate. Truncation that
    cut the render's own terminator skips the dedup, since the appended copy is then the only one.
    """
