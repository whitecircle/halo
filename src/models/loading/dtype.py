...
A torch + TRL leaf — the name→``torch.dtype`` table every ``--dtype`` flag derives its choices from,
the ``model_init_kwargs`` normalizer, the run-dtype / quantization resolvers that must agree on one
answer, and the matmul pin — so a CLI or an entry point can resolve a dtype, or apply the pin, without
importing the model-loading stack (which reaches ``src.args`` and back into ``src.training``).
"""
...
def resolve_model_dtype(model_init_kwargs: dict) -> dict:
    ...
    if isinstance(dtype, str):
        ...
        # resolve rejects here the very spellings every ``--dtype`` CLI accepts.
...
def resolve_quantization_config(model_config, training_config):
    ...
    and is read by nothing else here — every other consumer takes the training config's ``bf16``/``fp16``
    through :func:`resolve_training_dtype`. Left alone, a bf16 QLoRA run dequantizes and computes every
    4-bit matmul in fp32. Every loader that accepts a quantized base must go through this, not
    ``get_quantization_config`` directly.
    """
...
def configure_float32_matmul_precision() -> None:
    ...
    on every model; pinning it in-process keeps the guarantee off the image's env. bf16 matmuls are
    unaffected. ``HALO_FP32_MATMUL_PRECISION=high`` opts back into TF32 — for cuDNN too, whose
    ``allow_tf32`` is a separate switch defaulting to True (Zaya's fp32 Conv1d).
    """
