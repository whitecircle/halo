...
def has_safetensors_checkpoint(model_path: str) -> bool:
    ...
    which owns which filenames count — re-listing them here would drift from it.
    """
...
def resolve_run_dtype(dtype: torch.dtype | None, config) -> torch.dtype:
    ...
    The fallback keys on the VALUE, not on the attribute being absent: a config declaring no dtype
    carries ``dtype = None``, which every consumer would resolve its own way — the meta shell to
...
def build_key_mapping(
    ...
    The caller-resolved conversion ``steps`` (``None`` = already canonical) run FIRST, rewriting
    ...
    map (first target) plus ``disk_key → ((model_key, ops), ...)`` for every converted key.

    Key translation comes from the declared entries alone: an unmapped key is skipped by the planner,
    so an unapplied rename means a whole module absent from the load.
    ...
    maps ``\\.norm\\.`` → ``.kv_norm.``, which also matches the canonical ``model.norm.weight``), and
    its own loader undoes such a rename too. Without the mirror, an EP/PP resume from a canonical
    checkpoint renames a tensor onto a key the model does not have and drops it.
    """
...
def _align_key(key: str, model_keys: set[str], _prefix: str, hub_renames) -> str:
    ...
    # ``language_model.model.*`` while the wrapper owns it at ``model.language_model.*``; mapped
...
class WeightPlan:
    ...
    ops: tuple = ()
    # Full extent the model expects along ``shard_dim`` (the config's expert count); 0 = unknown.
    shard_total: int = 0
...
class SafetensorsWeightLoader:
    ...
    def load_into_model(
        ...
        Every materialized tensor is shape-checked against the live target before assignment: this
        path bypasses ``from_pretrained``'s own size-mismatch check.
        """
        self._open()
        try:
            ...
            for plan in plans:
                ...
                # Every float parameter, deliberately overriding the class's
                # _keep_in_fp32_modules[_strict]: FSDP2 rejects mixed dtypes in one shard group, and
                # the EP from_pretrained fallback re-casts to match. Parameters only — a float BUFFER
                # may be fp32 deliberately (Zaya's balancing biases).
                if (
                    dtype is not None
    ...
    def _read(self, disk_key: str, plan: WeightPlan) -> torch.Tensor:
        ...
        if plan.shard_total and shape[plan.shard_dim] != plan.shard_total:
            # A ranged read is silently satisfiable by a LONGER axis: the per-rank slices would
            # cover only the config's expert count and no rank would ever own the rest. (A SHORTER
            # axis returns fewer rows and is caught by the post-read shape check.)
            raise RuntimeError(
...
def verify_loaded_shape(
    ...
    checkpoint's shape silently WINS over the config's (``from_pretrained`` raises instead). For an
    ...
    if not isinstance(live, torch.Tensor):
        # Every planned key comes from model.state_dict(), so this is unreachable in practice —
        # but a guard whose job is fail-loud must not fail open on its own resolution.
        raise RuntimeError(
...
def materialize_empty_like(model: nn.Module, key: str, device: str, dtype: torch.dtype | None) -> None:
    ...
    — an integer buffer (routing tables, position ids) would have its indices corrupted, and a float
    one may be fp32 deliberately (Zaya's balancing biases).
    """
...
def init_checkpoint_absent_modules(
    ...
    *,
    ...
    same values (the run seeds identically and nothing here consumes randomness). ``context`` names
    the caller in every message; ``dtype`` is the RUN's, not the shell's, which would leave the head
    alone in the checkpoint's dtype.
    ...
    # allocate a second vocab-sized tensor only to throw it away.
