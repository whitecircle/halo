...
The vocabulary is deliberately closed: each op acts on one declared dim, so the expert planner can
assert orthogonality to the expert shard axis and keep its ranged per-expert reads. An entry using
any other transformers op raises at translation rather than loading wrong tensors. Which family
declares which entries is injected by the EP package's hub-conversion seam.
"""
...
@dataclass(frozen=True)
class PermuteRope:
    """Rows of a complex-RoPE (interleaved-pair) head layout → the split-halves layout.
...
class Rename:
    pattern: re.Pattern
    replacement: str
    # Module path (relative to ``base_model_prefix``) whose keys alone this step sees; "" = every key.
    scope: str = ""
...
def translate_converter(entry: WeightConverter, config=None, scope: str = "") -> Convert:
    ...
    module does not have — refuse rather than materialize wrong tensors.
    """
...
def _permute_rope(op: PermuteForRope, config, entry: WeightConverter) -> PermuteRope:
    ...
    ``num_attention_heads`` (its own default of 1 applies where the config lacks the field)."""
...
def convert_disk_keys(
    ...
    """Disk key → canonical target(s) with ops, for keys the steps change; untouched keys are absent.
    ...
    by construction — pinned bitwise by the per-family lazy-load round-trip tests.
