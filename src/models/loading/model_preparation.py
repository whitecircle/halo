...
The leaf rules it builds on live beside it (config levels, dtype, tokenizer setup); the callers that
...
# explicitly because the stock GenerationConfig() instance carries None for all three fields —
# comparing against the instance reads a neutral temperature=1.0 as a sampling request.
...
def resolve_auto_model_class(model_config, *, text_only: bool = False):
    ...
    ``text_only=True`` forces ``AutoModelForCausalLM`` for a multimodal config — the deliberate
    ...
    Raises:
        ValueError: ``text_only=True`` for a family with no text-only CausalLM sibling — no
            ...
            (step3p7). Accepting the masquerade would silently load the remote-code VLM as the
            "text-only" model.
    """
    if text_only:
    ...
    # Bare mapping membership, NOT the value-inspecting
    # :func:`~src.models.modality.config_declares_multimodality`: the two deliberately disagree on
    # ``mistral4``, whose ITT entry IS its text decoder and which has no ``AutoModelForCausalLM``
    # entry at all, so unifying them would leave that family with no loadable class here — and would
    # send it down the VLM branch (a processor it does not ship) at every reader of that predicate.
...
def auto_load_model(model_name_or_path: str, *, trust_remote_code: bool = False, model_class=None, **kwargs):
    ...
    coverage-verified ``from_pretrained``. Every "only a path string" caller runs it — standalone
    tools, reference/teacher models, and the trainers' own ``load_model_from_pretrained``, which adds
    its dtype/cache handling around it. Distributed paths use ``load_distributed_model()``.

    ``model_class`` pins the class and skips the config fetch, for a caller that resolved its own
    (a task head).
    """
    # Before the CONFIG fetch, which is already enough to import a remote modeling file: this is the
    # seam every standalone tool loads through, so the shims cannot depend on each caller applying
    # them first (a CLI is the process's first remote-code load by definition). Idempotent.
...
def apply_family_attention_patches(model_config, attn_implementation: str) -> None:
    ...
    One seam for both loaders: a frozen teacher/reference paired with a different-family policy
    would otherwise miss its family's patch (the policy's load keys on the POLICY's config) and,
    e.g., score packed rows with cross-document flash attention while the student is isolated.
    """
    ...
    # position_ids plumbing as Mistral4 — Zaya's model forward never hands the tensor to its layers.
...
def sanitize_generation_config(model: PreTrainedModel) -> None:
    ...
    # None = explicitly unset, not a sampling request; a neutral value is not one either.
...
def finalize_run_model(
    ...
    """The sharding-agnostic finalization every loaded model owes its run, in one order.
    ...
    ``sinks_pretrained=False`` (a from-scratch build) skips the NEUTRALIZED policy alone — there are
...
def finalize_liger_after_direct_load(training_config, original_use_liger: bool, model) -> None:
    ...
    re-application a harmless identical re-patch. Otherwise the flag is cleared, so TRL does not
    re-apply Liger with its own defaults over the patched modules.
    """
