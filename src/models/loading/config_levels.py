...
field lives ONLY on ``get_text_config()`` while others live only on the wrapper — a one-level read
silently returns the default. This leaf owns the level enumeration, the reads and writes built on
it (plain and run-scoped), and the special-token-id snapshot/restore; every consumer resolves a
family's levels one way.
"""
...
# Sentinel for "this level did not declare the field at snapshot time" — composite wrappers carry no
# top-level ids, and align_special_tokens PLANTS one there; the restore must remove it.
_SPECIAL_TOKEN_ID_ABSENT = object()

# Pre-mutation values of run-scoped config writes, kept OUTSIDE the config object so nothing
# rides them into ``to_dict()``/``config.json``. Keyed by ``id``: transformers 5 configs define
# ``__eq__`` without ``__hash__``, which a WeakKeyDictionary rejects. ``_MISSING`` = the field was
# absent from the instance ``__dict__`` (serialization fell through to the class default).
_RUN_SCOPED_FIELD_ORIGINALS: dict[int, dict[str, Any]] = {}
# Config objects that take no weak reference (a ``__slots__`` stand-in), pinned so their ``id`` can
# never be recycled onto another config that would then inherit their originals — the very
# aliasing the finalizer prevents for everything else.
...
def text_config(cfg):
    ...
    wrapper-only read silently returns the default instead of raising.
    """
...
def config_sources(cfg) -> tuple:
    """``cfg`` plus its text sub-config, deduplicated — every level a field read or write must
    cover, for the reason :func:`text_config` states. ``get_text_config()`` returns ``self`` for a
    plain config, which therefore yields one level."""
...
def get_config_field(cfg, field: str, default=None, *, per_layer_reduce=None):
    ...
    The modeling code reads these off the TEXT config, so a wrapper-only read is not merely
    incomplete — it silently yields the default and turns a balancing decision into a no-op.
    """
    for source in config_sources(cfg):
        try:
            ...
            # name to keep this module the leaf its docstring pins.
...
def set_config_field(config, field: str, value, *, only_declared: bool) -> bool:
    ...
    level (always at least one, hence always ``True``) — what a toolkit-driven flag needs, since HF
    forwards read the text config while call sites hold whichever object ``model.config`` returned.
    """
...
def _config_storage_field(source, field: str) -> str:
    ...
    run-scoped registry must record and restore the MAPPED key, or the restore would target a slot
    the write never touched and leave the run-scoped value in ``config.json``.
    """
...
def set_config_field_run_scoped(cfg, field: str, value) -> None:
    """An ``only_declared=False`` write for RUN-scoped state — mutations that configure this training run
    (zeroed ``router_aux_loss_coef``, forced ``output_router_logits``, toolkit stamps) but do not
    describe the artifact. The pre-mutation value is recorded once per config source, and
    :func:`config_export_ready` restores it around every config serialization: without that,
    the exported ``config.json`` destroys the hub's aux coefficient and makes plain-transformers
...
def config_export_ready(cfg):
    """Temporarily restore every run-scoped field mutation on ``cfg`` (and its text config)
    so a serialization inside the block writes the ARTIFACT's fields; the run state is re-applied
    on exit. Instance-``__dict__`` based on both sides, mirroring how the run-scoped setter wrote."""
    if cfg is None:
        yield
        return
...
def snapshot_special_token_ids(config) -> list[tuple[object, str, object]]:
    ...
    HF's ``align_special_tokens`` (run by ``Trainer.train``) overwrites ``config.eos_token_id``
    with the TOKENIZER's single id — collapsing a list-valued stop set (Gemma 4's ``[1, 106]``,
    GLM's three role enders) in the config every export ships, so serving never stops on turn
    ends and a stage-2 run masking against the exported config trains zero tokens.
    """
    snapshot = []
