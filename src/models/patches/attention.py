...
# ``_flash_api_alternative_names`` entry for the first.
...
# tile count; the margin pushes the row past a sliding window, so the windowed variant is the one
...
def ensure_fa4_kernel_cache_env() -> None:
    ...
    mutes the DSL's deprecation flood; both are preconditions of the same import. Must run before the
...
def patch_transformers_flash_varlen_int_seqlen() -> None:
    ...
    def wrapped(*args, **kwargs):
        ...
        if torch.is_tensor(mq):
            ...
            # transformers aliases max_seqlen_k to the same tensor to avoid a second sync; keep that.
            if mk is mq:
...
class PositionIdsInjectingRegistry:
    ...
    # Dunders resolve on the type, bypassing __getattr__, so forward the Mapping protocol explicitly.
...
def install_packed_position_ids_injection(modeling_module, owner_cls, stash_targets) -> bool:
    ...
    dense causal sequence. Dense backends are unaffected, since the packed mask forms at the model
    level.
...
def warmup_fa4_kernels(model: PreTrainedModel, *, dtype: torch.dtype, device: torch.device | None = None) -> None:
    ...
    A first-use JIT compile (10 s or more) on one rank lets peers race to the next collective and
    deadlock, so the caller runs this on every rank behind one fence
    (:func:`~src.distributed.loading.warmup.warm_attention_kernels`); every verdict taken here is per
    rank and must not gate a collective. The dense and varlen entry points compile separately, so
    both are warmed, and ``dtype`` is the run's own.
    """
    try:
        ...
        # Warm every dispatched variant (dense/varlen x full/sliding-window x sink): a missed one
        # JIT-compiles later and desynchronizes the ranks.
        window_variants = [{}] + (
        ...
        # main_process_only=False: the skip is rank-local, and a rank without the warm-up desyncs later.
...
def model_fa4_backward_nan_prone(model_config) -> bool:
    ...
    256 as ``qk_nope_head_dim + qk_rope_head_dim`` with no partial-rotary field.
    """
...
def flash_varlen_fn(attn_impl: str):
    ...
    Varlen rather than the dense twin because that is the signature transformers reads: it builds its
...
def _attn_impl_handles_sinks(attn_impl: str) -> bool:
    ...
    path carries it only when the installed kernel accepts a sink argument transformers can forward,
    so the verdict is read off the kernel signature and a flash-attn upgrade that adds sink support
...
def _enable_sink_model_sdpa(model_config) -> bool:
    ...
    transformers sets ``_supports_sdpa = False`` on GptOss (SDPA cannot add the sink logit); with
    reset sinks the dropped column contributes 0, so SDPA matches eager/flex. Flips the class flag.
...
def validate_attn_implementation(model_config, attn_impl: str, sinks_reset: bool | None = None) -> str:
    ...
    # rejects it raises at model build) while the sinks escape hatch stays closed. A composite VLM
    # with no CausalLM sibling (glm5_next, mistral4) declares its flags on the conditional-generation
    # class, so the image-text mapping is the fallback.
    ...
    # sdpa drops sinks, so it is valid only without sinks, or when they were reset (here or earlier).
    sdpa_ok = supports_sdpa and (not has_sinks or recorded_reset is True)

    # Live (unreset) sinks require a sink-carrying implementation: a sink-dropping one shifts the
    # softmax away from the served policy.
    ...
    # Fallback chain: sinks models prefer FA2 over flex, whose compile path is unusable under EP.
    ...
    for candidate in candidates:
        if candidate == "sdpa" and not sdpa_ok:
            continue
        # The class flag is the family's own verdict (for glm5_next, KDA linear attention and the DSA
        # indexer have no flash path). transformers raises on the same flag at model build, after the
        # whole checkpoint has been fetched and placed.
...
def revalidate_attn_kwarg(model_kwargs: dict, model_config) -> None:
    ...
    deep in the load. Only the validated value is put back, and popping first keeps a second branch
    from leaking the unvalidated one through.
    """
