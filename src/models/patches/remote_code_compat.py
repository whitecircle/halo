...
# correct, only its import list is short.
...
def _halo_is_torch_fx_available() -> bool:
    ...
    Named, not a lambda: it is bound into a third-party module, where ``<lambda>`` names nothing in a
    traceback."""
    return True
...
def _patch_rope_init_default() -> None:
    ...
    would silently retune every position).
    """
...
def _repair_remote_module(module) -> None:
    ...
    training runs with the cache off, so it survives to the first person who uses what they trained.
    The name is never imported, so unlike the other shims here only the calling module can be given
    it. Absent names only.
    """
...
def _shim_dispatchless_eager_attention(module) -> None:
    ...
    lookup, so the requested implementation changes the MASK FORMAT while every layer still runs the
    eager score matmul — an ``[B, H, S, S]`` bf16 plane plus its fp32 softmax copy, ~190 GiB on a
    packed 80k row. The wrapper reads the attention module's own config per call, so everything but
    ``sdpa`` keeps the file's own eager. Replacing the module GLOBAL is what makes it stick: the
    forwards resolve ``eager_attention_forward`` by name at every call.
    """
    ...
    def eager_or_sdpa(mod, query, key, value, attention_mask, scaling=None, dropout: float = 0.0, **kwargs):
        ...
        attn_output = F.scaled_dot_product_attention(
            ...
            scale=scaling,
            # repeat_kv semantics (grouped, not interleaved) — what the eager helper expands to.
