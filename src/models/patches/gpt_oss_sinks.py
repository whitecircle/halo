...
# Where :func:`apply_sinks_policy` stamps the run's :class:`SinksPolicy`. A module attribute rather
# than a config field: it must not serialize into a saved ``config.json``, and attribute lookup falls
...
class SinksPolicy(enum.StrEnum):
    """The sink state a GptOss run trains under: one vocabulary for the trainer gates, the adapter's
    ``training_provenance.json`` record, and the merge tools that re-apply it. ``str``-valued so it
    serializes as plain JSON and compares against a loaded record directly."""
...
def is_sink_key(key: str) -> bool:
    ...
    Shared by every scan of a checkpoint's keys; a second copy would verify a different key set."""
    return ".sinks" in key
...
def neutralized_sink_value(dtype: torch.dtype) -> float:
    ...
    its softmax term. Shared by the reset, the save-time re-emission and every check that a written
    checkpoint's sinks are off."""
...
def _gpt_oss_sink_attentions(model, model_config=None):
    ...
    unrecognized layout yields nothing, with a warning. Always yields the module that owns the
    ``sinks`` parameter: after CP patching ``self_attn`` is the Ulysses wrapper, and the FA2 rebind
...
def neutralized_gpt_oss_sinks(model) -> dict:
    ...
    Under FA2 ``sinks`` is set to None (leaving ``named_parameters()``), so an EP/TP save would omit
    it. Returns ``{param_name: tensor}`` of sinks filled with dtype min; empty when sinks are present.
    """
    ...
    for index, attn in _gpt_oss_sink_attentions(model, config):
        ...
        # Named in the checkpoint's key space (a *ForCausalLM's backbone is ``model``) rather than the
        # caller's tree: these names merge into a state dict the wrappers do not appear in.
...
def gpt_oss_sinks_restored(model):
    ...
    save has no such hook, so an unparallelized FA2 run would write a checkpoint with no
    ``self_attn.sinks`` and reload it as an uninitialized Parameter. Restores ``None`` on exit, since
    FA2's kernel rejects a non-None ``s_aux``.
    """
    ...
    with torch.no_grad():
        for _, attn in _gpt_oss_sink_attentions(model, model_config):
            if not hasattr(attn, "sinks"):
                ...
                # neutralization that never happened, after which the run stamps NEUTRALIZED having
...
def install_fa4_trainable_sink_rescale() -> bool:
    ...
    The FA4 backward returns only ``dq, dk, dv``, so a trainable sink under the fused kernel receives
    no gradient. It does consume ``dlse``, which permits an exact reformulation outside it: with
    ...
    Both entry points are wrapped, plus transformers' cached references to them. The trainable branch
    returns ``(out, lse)`` with the sinked lse. Idempotent; False when ``flash_attn.cute`` is not
    importable.
    """
    try:
    ...
    def rescale(out, lse, sink, head_dim: int):
        ...
        # so autograd keeps the kernel's own bf16 output plus a [heads, tokens] gate rather than an
        # fp32 copy of the attention output. Returned lse is the sinked one, as the fused kernel does.
...
def apply_sinks_policy(
    ...
    *,
    ...
    that premise and then skipping the reset leaves a GptOss model running sdpa over live sinks. A
    model without sinks is left unstamped, since the stamp doubles as the family signal.
    """
    ...
    if not has_sinks:
        return
    # Installed for every FA4 sinks run, not only the trainable policy: it is inert for frozen sinks,
    # and a sink unfrozen by any other route (``unfreeze_layers_patterns``) then trains instead of
    ...
    if touched == 0:
        # The walk found no sink layers on a sinks-carrying model (unrecognized layout), so the
        # requested policy was not applied. Stamping anyway would silence the RL sink gate and record
        # a false provenance that a later merge re-applies onto a live-sink artifact.
        raise RuntimeError(
...
def stamped_sinks_policy(model) -> SinksPolicy | None:
    ...
    model carries no sinks. The provenance writer needs that distinction, since assuming
