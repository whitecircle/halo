...
All are idempotent. A renamed upstream internal fails the import or attribute access rather than
skipping a patch, which would leave the run with no balancing, allow GC into the cuDNN CCA fault, or
lose packed-attention isolation.
"""
...
def patch_zaya_router_load_recording() -> None:
    ...
    selections, so ``RouterBiasBalancingCallback`` would compute every update from zero loads and do
    nothing. Declares the ``expert_load_counter`` slot ``iter_balancing_routers`` keys on and records
    inside the gate's own forward, which serves EP and plain FSDP alike.
    """
...
def patch_zaya_flash_packed_position_ids() -> None:
    ...
    forward, unlike Mistral4's. Attention isolation is necessary but not sufficient: the CCA
    convolution still carries state across document boundaries on every backend
    (``agent-docs/models/zaya.md``).
    """
