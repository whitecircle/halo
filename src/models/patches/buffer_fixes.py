...
def default_rope_parameters(config, device=None, **kwargs) -> tuple[torch.Tensor, float]:
    ...
    Reading the dict first and the attribute second lets one formula serve both, which is why the
    remote-code shims register this back as the registry's ``"default"`` entry. ``kwargs`` absorbs
    the ``layer_type`` the registry passes to per-layer-type rotaries.
    """
...
def _register_recomputed_buffer(module: nn.Module, name: str, tensor: torch.Tensor) -> None:
    ...
    native rotaries do not, and forcing ``persistent=False`` would drop the key from every save. An
    unregistered buffer keeps the native non-persistent default.
    """
...
def _is_rotary_module(module: nn.Module) -> bool:
    ...
    They share no common base, so the ``*RotaryEmbedding`` class name is the remaining signal, the
    same rule :func:`~src.models.structure.is_normalization_module` applies to norms. Used only to
...
def _fix_layer_type_inv_freq(module: nn.Module, module_name: str, layer_type: str, rope_init_fn) -> bool:
    ...
    ``rope_parameters`` entry, as in Laguna and Gemma 4): its own init fn would raise on that entry,
    and building a table would invent state.
    """
...
def _fix_per_layer_type_rope(module: nn.Module, name: str) -> int | None:
    ...
    by class name (Gemma4 and DeepSeek-V4 reached this shape independently, and DeepSeek-V4 repeats
...
def _fix_single_inv_freq_rope(module: nn.Module, name: str) -> int | None:
    ...
    if getattr(module, "inv_freq", None) is None:
        ...
        # keeps whatever uninitialized values the load produced, which corrupts RoPE positions, so
        # name the module rather than skipping it.
...
def _retie_shared_weights(model: nn.Module) -> None:
    ...
    on meta until tying. transformers 5 honours a checkpoint shipping a distinct head even under
    ``tie_word_embeddings: true``, so when both sides are materialized and distinct an unconditional
    ``tie_weights()`` would overwrite the loaded head with the embedding.
    """
...
def finalize_loaded_model(model: nn.Module) -> None:
    ...
    never runs, and a zero inv_freq degenerates RoPE to NoPE at a plausible loss. Runs both fixer
    groups in one walk and re-ties shared weights. A buffer no fix covers stays on meta and is
    rejected at the trainer's device placement.
    """
