...
3.3x faster than mem-efficient SDPA at 2,048 tokens and 2.3x at 8,192 on B300. FlexAttention does not
...
# FlexAttention tiles for sliding layers of head_dim 256 on SM100+, used from 2,048 query tokens up, as
# measured on Gemma 4's sliding layers (window 1,024). Fwd+bwd on B300 at 2,048 / 8,192 / 32,768 tokens: 0.46 / 1.46 / 6.07 ms against 0.63 / 2.66 /
# 9.92 ms with the default tiles (and 0.78 / 1.74 / 6.98 ms for FlashAttention 2's local window); shorter
# queries run faster on the defaults.
...
_TUNED_MIN_QUERY_LEN = 2048
_TUNED_HEAD_DIM = 256


def _sliding_kernel_options(query: torch.Tensor) -> dict | None:
    """The tuned tiles where they were measured (16-bit inputs, head_dim 256, SM100+, long queries), else the
    defaults."""
    if (
        query.dtype not in (torch.bfloat16, torch.float16)
        or query.shape[-1] != _TUNED_HEAD_DIM
        or query.shape[-2] < _TUNED_MIN_QUERY_LEN
...
# dynamic=False: one specialization per (batch, length) shape, which fixed-length packed training hits once.
