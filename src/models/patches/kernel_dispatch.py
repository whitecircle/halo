...
def _segmented_conv1d(torch_function, hidden_states, *args, **kwargs):
    ...
    row, leaking left-context across document boundaries — only the CUDA kernel isolates. The conv is
    causal and depthwise, so per-document application is the kernel's definition, not an approximation.
    """
...
def _segmented_chunk_gated_delta_rule(torch_function, query, key, value, **kwargs):
    ...
    ONE recurrence, leaking recurrent state across document boundaries — only the fla kernel
    isolates. Running each document from a fresh initial state restores its semantics exactly; a
    cached call (``initial_state``) falls through unsegmented rather than guessing per-sequence state.
    """
