...
The single source of truth for a family's router-field spellings, for which balancing strategy a run
gets, and for where a trained routing bias lands in an exported checkpoint; applying that verdict to
a live model is :mod:`src.distributed.expert_parallel.balancing_strategy`. A leaf of the model layer
(no ``src.distributed``, no ``torch.distributed``) so its consumers — the callbacks package, the
checkpoint writers and tools, the PP split gate — cannot form an import cycle through each other.
The EP layer classes push their family declarations IN at subclass definition.
"""
...
# property — the one protocol name every balancing consumer keys on.
BALANCING_BIASES_ATTR = "balancing_biases"

# Instance flag the EP base sets when it adopts a family's NATIVE balancing slot (vs the transient
# side-buffer): one spelling for the setter and every reader, since a drifted copy would silently
...
# ``config.model_type`` → ``{flat legacy key: (layer_type, per-layer field)}``, filled by
# ``EPMoELayerBase.__init_subclass__`` from each family's ``_LEGACY_PER_LAYER_CONFIG_KEYS``. In this
# leaf so the config-export rewrite reads the roster without importing the EP package, which would
# close a cycle through ``parallelism_config``.
...
# each family's ``_EXPORTS_SOURCE_CONFIG_SCHEMA``. Same leaf and same reason as the map above.
...
# declares ``HF_MODEL_TYPES`` to exist at all. Doubles as the "has the roster been imported?" signal
# every reader here checks before answering off an empty registry.
...
class NativeBalancingSlot:
    ...
    ``attrs`` are the dotted paths the slot answers to — the declared one and its hub respelling
    (Laguna) — and ``config_flag`` the field an engine reads to decide whether to apply the tensor
...
# ``config.model_type`` → :class:`NativeBalancingSlot`, filled by ``EPMoELayerBase.__init_subclass__``
# from each family's ``_NATIVE_BALANCING_BIAS_ATTR``. Same leaf and same reason as the maps above: an
# offline export resolves a checkpoint's balancing slot without importing the EP package.
...
# bare ``top_k``: that name is also the generation sampling parameter.
...
# Routed-expert-count spellings, same single-registry rule: two copies would silently make a family
# MoE for the "is this MoE?" gate and dense for the load metrics.
...
# Same registry pattern for the per-expert FFN width — the dimension expert-TP shards. Every
# supported spelling names that dimension directly. The MoE-specific ones come first: a hybrid
# dense+MoE config carries both, and only the MoE one is sharded.
...
def register_legacy_per_layer_config_keys(model_types: tuple[str, ...], keys: dict[str, tuple[str, str]]) -> None:
    ...
    A spelling two families claim with different keys raises, as the EP class-claim maps do — the
    export would otherwise flatten by whichever class was defined last.
    """
...
def register_native_balancing_slot(model_types: tuple[str, ...], slot: NativeBalancingSlot) -> None:
    ...
    Two families claiming one spelling with different slots raises, as the registrars above do — an
    offline export would otherwise write the bias through whichever class was defined last.
    """
    for model_type in model_types:
...
def ep_roster_registered() -> bool:
    ...
    They fill at EP subclass definition, so a process that reaches a reader here without ever
    importing ``src.distributed.expert_parallel.layers.roster`` sees an EMPTY roster — every family
    answers "needs nothing" and an export silently drops the schema its servers require. Readers
    refuse that state instead of guessing.
    """
...
def has_ep_wrapper_class(model_config) -> bool:
    ...
    GEMM); only the roster says whether a wrapper is available for THIS family — Qwen3-Next has
    none, so patching leaves the stock expert loop in place. Both the wrapper's own ``model_type``
    and its text sub-config's are checked, which is where a multimodal checkpoint declares the MoE
    family.
    """
...
def resolve_expert_ffn_shard_width(cfg) -> int | None:
    ...
    The one home for "what does ETP actually shard", so the config-time divisibility gate and the
    layer that does the split cannot disagree: a second copy preferring another spelling would pass
    the gate on one width while ``EPMoELayerBase._etp_shard_size`` splits a different one, and the
    run would die minutes later on a number the gate never saw.
    """
...
def config_has_experts(config) -> bool:
    ...
    Single source of truth for "is this MoE?" — used by the loader gate and the trainer. The lookup
    itself is the router-field resolver ``ParallelismConfig`` asks the same question with, so the
    loader gate and the config-time gate cannot answer it differently for one checkpoint.
    """
    if config is None:
        return False
...
def mark_router_logits_forced_off(config) -> None:
    ...
    Run-scoped: the stamp configures THIS run and is restored away before config serialization —
    exported, it would suppress ``moe/*`` metrics in every later run loading the artifact.
    """
...
def resolve_balancing_slot(
    ...
    One home for the walk, shared by the EP layer that adopts the slot, the checkpoint-key derivation
    and the sidecar apply: a drifted copy disagrees invisibly until a served model routes unbiased.
    ``None`` when the path is empty, the owner submodule is absent, or — under ``require_tensor`` —
    the slot holds no tensor (LFM-2 with ``use_expert_bias: false``); the materializing caller passes
...
def native_balancing_bias_attrs(layer_cls) -> tuple[str, ...]:
    ...
    — must walk both spellings or go blind to the one the EP gather emits.
    """
    ...
    # once) — the two must name the same hub key or the dtype keep-set and the sidecar apply go
    # blind to the spelling the gather actually emits.
...
def balancing_param_keys(model: torch.nn.Module) -> frozenset[str]:
    ...
    Derived from the module tree like ``norm_param_keys``, never from a name list: an adopted native
    slot contributes its layer class's :func:`native_balancing_bias_attrs`, a hub-native router its
    registered ``balancing_biases`` buffer. Save paths keep these keys at their trained dtype — a
    bf16 round-trip would quantize away several 1e-3 sign-steps of trained balancing.
    """
    keys: set[str] = set()
...
def is_balancing_state_key(key: str) -> bool:
    """Whether a checkpoint key names a router-balancing tensor — the model-free twin of
...
def _materialize_hub_balancing_slot(
    ...
    A PEFT run saves adapters only — no ``config.json`` — so a merge starting from BASE weights
    ...
    ``None`` for families without a config-gated slot — their absence is structural, not assembly.
    """
...
def apply_router_balancing_sidecar(
    ...
    trained lives only in the sidecar — without this copy the merged model serves the pretrained
    routing. Sidecar keys are trainer-tree module names, mapped back to the hub tree by
    :func:`~src.models.structure.unwrapped_module_name` (PEFT and CP each add a level). A
    config-gated slot the base was assembled without is re-materialized (with its config flag
    flipped) before the copy — the run trained an exportable slot, and the merge must ship it. Only
    entries for families with no native slot at all trained a transient bias no artifact can serve —
    returned as skipped for the caller to warn about.
    """
...
def iter_balancing_routers(model: torch.nn.Module):
    ...
    BOTH, so the yield stays unique when a wrapper adopts an inner gate's native buffer: only the
    module that also records loads is the balancing router."""
...
def has_discard_expert_slot(model) -> bool:
    ...
    Declared by the router CLASS (``_has_discard_expert_slot``) — same shape as
...
def is_transient_balancing_router(module) -> bool:
    ...
    router's own buffer lives in ``_buffers``, so both correctly report False.
    """
...
def accepts_native_balancing_bias(model) -> bool:
    ...
    or one the layer can materialize — LFM-2 with ``use_expert_bias: false``). A model where this is
    False can only be bias-balanced transiently: the trained bias would never reach a served copy.
    """
    for module in model.modules():
...
def honors_output_router_logits_config(model) -> bool:
    ...
    aux loss never reaches the loss — balancing that silently does nothing.
    """
...
def resolve_balancing_mode(requested: str, model, is_moe: bool) -> BalancingMode:
    ...
    ``auto`` wants ``bias_update`` on any of three signals: a router exposing a ``balancing_biases``
    buffer (Zaya); EP wrappers that sever the aux-loss path while supporting the bias (DeepSeek-V4);
    or a forward that never declares ``output_router_logits`` while some wrapper accepts the bias. It
    commits only where the bias lands in checkpoint-exported state
    ...
    ``bias_update_transient`` is the deliberate opt-in.
    ...
    at all and ``auto`` resolves to ``none`` with the reason. An EXPLICIT ``aux_loss`` still raises
    there. ``none`` for dense.
    """
