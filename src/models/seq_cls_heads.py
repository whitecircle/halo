...
transformers 5.16 maps no seq-cls head for Gemma 4 or for MoE Qwen3.5/3.6, so reward modeling
and classification die at load on both families. Each head is the upstream Generic head
over the family's ``PreTrainedModel``, registered in both spellings a checkpoint can carry (the
composite wrapper and its text tower); a release that lands a native head wins.
"""
...
def register_sequence_classification_head(model_cls) -> None:
    ...
    Not ``register()``: transformers >= 5.12 silently no-ops it for a config class living under
...
# Driven off the classes above rather than a hand-kept pair list: a head added to this module
# registers by existing, under the config class it already declares.
