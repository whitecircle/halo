...
One home for the rules that map a wrapped, sharded tree back to plain hub spellings, shared by the
...
# Repo-wide, not per-family: a backbone spelling missing here is invisible to every consumer.
DECODER_LAYER_LIST_ATTRS: tuple[str, ...] = ("layers", "h")

# Parameter-name substrings marking a vocab-indexed embedding or output head (``score`` = the
# classification/reward head), read by Muon (sparsely-updated vocab rows stay on AdamW, not
# Newton-Schulz) and by the PEFT bf16 cast. Both reach here only when the module accessors are
# unavailable, so a per-consumer copy misroutes a family's head silently.
...
# ``UlyssesCPModelWrapper`` inserts (its ``_toolkit_inner_model_attr``). One spelling for every
# consumer that maps a live path back to the hub tree — a second copy drifts into an unresolvable key.
...
def unwrap_framework_wrappers(model: torch.nn.Module) -> torch.nn.Module:
    ...
    wrapper fails through it — both silently.
    """
...
def unwrap_model(model: torch.nn.Module) -> torch.nn.Module:
    ...
    Toolkit wrappers self-declare their inner-model attribute as ``_toolkit_inner_model_attr``, so a
    new wrapper type joins by declaring it. Correctness, not tidiness: a wrapper may yield inner
    names from ``named_parameters()`` while ``named_modules()`` still yields wrapper-prefixed paths,
    so a consumer cross-referencing the two (the EP save's expert classification) must walk this.
    """
...
def model_has_quantized_params(model: torch.nn.Module) -> bool:
    """Whether ``model`` carries quantized (non-floating-point) params — bnb 4/8-bit packed storage
    (QLoRA). Single home for the predicate: the FSDP2 routing in the trainer mixin and the GRPO
    weight-sync construction gate both key off it."""
...
def unwrapped_module_name(name: str) -> str:
    ...
    which ``unwrap_framework_wrappers`` deliberately keeps. Stripping only the PEFT prefix resolves
    nothing under PEFT+CP — the run trains fine and every offline consumer of its module names then
    fails on a key the target tree does not have.
    """
...
def normalize_peft_param_name(name: str, peft_prefix: str) -> str | None:
    ...
    One home for the spelling, shared by the vLLM weight sync and the merged EP save. Adapter params
    are dropped rather than renamed because both consumers run inside :func:`merged_adapters`: the
    delta is already folded into the base weight, so emitting ``lora_A``/``lora_B`` too would apply
    it a second time on any PEFT-aware reload.
    """
...
def strip_peft_adapter_segment(name: str) -> str:
    ...
    ``...lora_A.default.weight`` → ``...lora_A.weight``, ``...modules_to_save.default.weight`` →
    ``....weight``, and the ParameterDict spelling ``...lora_embedding_A.default`` — what the adapter
    SAVE writes, since a saved adapter file carries no adapter name. Every resume path rebuilding the
    saved-key → live-key map must strip identically: unmatched saved keys are "unexpected, dropped"
    to both adapter loaders, so those adapters silently stay at init.
    """
...
def merged_adapters(model: torch.nn.Module | None) -> Iterator[bool]:
    ...
    ``None`` — no PEFT model in the tree — is a no-op, so callers pass a lookup result directly.
    """
...
def decoder_layers(module: torch.nn.Module) -> torch.nn.ModuleList | None:
    ...
    The single probe for :data:`DECODER_LAYER_LIST_ATTRS`, so "which attribute holds the layers" is
    answered one way by the backbone descent below, the pipeline splitter and the MFU layer count.
    """
...
def backbone_with_layers(model: torch.nn.Module) -> torch.nn.Module | None:
    ...
    then ``model.transformer``. Framework/toolkit wrappers are NOT peeled — call :func:`unwrap_model`
    first when the input may be wrapped.
    """
    candidate = model
    seen: set[int] = set()
...
def persistent_buffers(model, exclude_prefixes=()):
    ...
    silently drops them and corrupts reload. Persistence is read from each owning module's
    ``_non_persistent_buffers_set``, never by diffing ``model.state_dict()``: that reshards the
    calling rank's parameters under FSDP2, and callers gate this on the save rank alone — leaving
    that rank out of step with its peers in the next collective over those params.
    """
...
def is_normalization_module(module: torch.nn.Module) -> bool:
    ...
    Structural first (isinstance against torch's norm bases); transformers' per-model norm
    classes share no common base, so their **class identity** is the remaining declaration —
    every roster family names them ``*RMSNorm`` / ``*LayerNorm`` (Zaya's EDA norm included).
    """
...
def fp32_pinned_param_names(model: torch.nn.Module) -> frozenset[str]:
    ...
    short convolutions), so every checkpoint writer must leave these at their trained dtype — a
    reload re-pinning the slot does not repair an export that threw the bits away.
    """
    pins = {
        pin
