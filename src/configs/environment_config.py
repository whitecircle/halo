"""Environment configuration for Environmental GRPO training (env selection + params, separate from AsyncTrainingConfig's infra)."""
...
class EnvironmentConfig(RangeValidatedConfig):
    """Environment selection and reward params, parsed from YAML; resolved via src/environments/registry.py (available environment_type values there)."""
    ...
    # No partial-credit reward: `compute_answer_reward` grades all-or-nothing, by design (a
    # similarity threshold rewards near-miss wrong answers).
    ...
    def _validate_ranges(self) -> None:
        ...
        # the cluster and the vLLM servers are already up. Fail at parse time instead.
    ...
    def to_env_config(self) -> dict[str, Any]:
        """Merge core reward/turn settings with environment_kwargs into the dict passed to resolve_environment(environment_type, config)."""
        config: dict[str, Any] = {
            "success_reward": self.success_reward,
            "failure_reward": self.failure_reward,
        }
        # Only an explicitly configured turn cap goes through — injecting the dataclass default
        # would silently clobber the per-class defaults (the ``max_turns`` help lists them).
