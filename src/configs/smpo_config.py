...
class SmoothMarginPOConfig(RangeValidatedConfig, TrainingArguments):
    """Config for SmoothMarginPOTrainer (SMPO): a reference-model-free, margin-based preference method with optional SFT loss. Per-field docs are in each field's ``help`` metadata."""
    ...
    def resolve_length_budget(self) -> tuple[int, int, int]:
        ...
        The single place the three length knobs relate to each other. An unset prompt share takes
        half of ``max_length`` so it scales with a context-resolved ``max_length`` (a fixed constant
        would silently truncate prompts to it on a long-context run); an unset completion share takes
        the remainder. The shares must fit inside ``max_length`` — SMPO truncates prompt and
        completion independently, so an over-budget split would exceed it without either cut firing.
