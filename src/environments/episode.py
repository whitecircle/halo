...
graded identically whichever one collects it. Imports :mod:`src.environments.base` (the env protocol);
nothing there reads this module.
"""
...
# penalty weights outside it (over-use hurts more than under-use). See
...
class EpisodeEffort:
    ...
    def stamp(self, trajectory: Trajectory | None) -> None:
        ...
        Every rollout driver stamps through here: re-tokenization has to render the same steer the
        model generated under, and the calibration reward prices CoT against the applied budget."""
...
def resolve_episode_effort(context: dict[str, Any] | None, env: BaseEnvironment) -> str | None:
    ...
    A context-supplied level wins because the trainer stamps ONE group-level draw into every group
    member's context — a ``'random'`` env setting drawn independently per episode would otherwise
    become intra-group conditioning noise (GRPO's group baseline assumes the members of a group share
    identical conditioning). Either source still resolves through :func:`resolve_reasoning_effort`
    (``'random'`` → a concrete draw).
    """
...
def bind_episode_effort(
    ...
    *,
    ...
    Every rollout driver (the Ray actor, the eval runner) binds through here, so an episode is
    generated under the same contract whichever one collects it. Call ONCE per episode: the level may
    be a ``'random'`` draw, and every turn must share it.

    The thinking budget alone caps only the reasoning channel; the visible/final channel would still
    run to the global ``max_tokens`` with nothing else governing it, and a visible channel free to grow
    into an essay displaces the tool call the turn exists to make. The per-effort total is therefore
    the level's budget plus the global answer headroom (``max_tokens - max_thinking_tokens``), so an
    effort level bounds the whole turn rather than its reasoning alone.
    """
...
def reasoning_calibration_penalty(reasoning_tokens: list[int], budget: int) -> float:
    ...
    The band and the two weights are the term's definition, not run knobs — the config help and
    ``agent-docs/training-methods/grpo/environmental-grpo.md`` state them, and the only per-run dial is
    ``reasoning_compliance_weight``, which scales the whole term.
    """
...
class RolloutResult:
    ...
    episode, so each one must have a consumer — pinned by ``tests/cpu/environments/test_ray_actors.py``.
    """
...
class TurnGeneration:
    ...
    the text is a fragment — an environment must not read it as a deliberate answer."""
...
def step_context_from_generation(context: dict[str, Any] | None, gen: TurnGeneration) -> dict[str, Any]:
    ...
    One owner for every rollout driver (the Ray actor's raw aiohttp transport, the eval runner's
    OpenAI client), because a driver that forgets ``finish_reason`` grades an engine-cut fragment as
    a deliberate final answer. The capture keys ride along only where they are id-aligned: an
    unaligned logprob or routing vector is silently WRONG training data, not merely a missing one.
    """
...
class EpisodeDispatcher:
    ...
    identically whichever one collects it. An :class:`AsyncBaseEnvironment` runs inline on the event
    loop; a sync env is offloaded to a worker thread, since its tool and grading handlers block on
    sandboxed execution for minutes and would freeze every episode sharing the loop.

    The offload reuses ONE context copied per episode rather than the fresh per-call copy
    ``asyncio.to_thread`` makes: a tool's ContextVar writes must still be visible on the next turn
    (the simulated file store), as they were when the calls ran inline.
    """
