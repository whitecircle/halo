...
# Per-execution wall-clock cap (seconds); generous so a slower-but-correct CPython solution isn't TLE'd.
...
# RLIMIT_NPROC for the run step. The kernel ignores it for root (how the training containers run),
# so it binds only unprivileged runs (a bubblewrap jail) and the timeout's process-group kill stays
# the real fork-bomb defense. Counts the user's WHOLE task set — must clear the trainer+Ray baseline.
LOCAL_NPROC_LIMIT = 4096

INTERPRETER_PLACEHOLDER = "$INTERPRETER"

# The REPL observation contract every backend renders results through, so the model sees one wording.
...
class SandboxInfraError(RuntimeError):
    ...
    the submitted code and stay ordinary string results. This is raised by the REPL layer so an
    infrastructure outage surfaces as a FAILED tool call instead of a string the protocol scores as a
    successful one.
    """
...
def require_language(language: str) -> LanguageSpec:
    ...
    The single home for the "unsupported language" message every construction-time caller needs.
    """
