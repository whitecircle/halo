...
# Big-int ``**``/``<<``/``*``/``factorial`` compute in ONE GIL-holding C call, so the wall-clock
# timeout cannot preempt them (``9**9**9`` wedges the worker forever) — bound the result size instead.
_BIGINT_MAX_RESULT_BITS = 1_000_000
_FACTORIAL_MAX_N = 10_000
# Same wedge, allocation-shaped: ``[0] * 10**9`` and ``sum(range(10**18))`` never return to bytecode.
# Python-level ``for`` loops run bytecode between items and stay preemptible.
...
def _guarded_mul(left, right):
    ...
    and other pairs take the cheap pass-through."""
...
def _guarded_range(*args):
    ...
    ``list``/``any``/``all``) drain a range inside one GIL-holding C loop, so an unbounded range is
    the same wedge as a big-int op. The cap leaves realistic loops (10**6-scale) untouched."""
    result = range(*args)
    try:
...
class _GuardedOpTransformer(ast.NodeTransformer):
    ...
    Python has no operator hook to intercept int-int operators, so the bound must be routed at the
    AST level."""
    ...
    def visit_AugAssign(self, node: ast.AugAssign):
        ...
        if not isinstance(node.target, ast.Name):
            # Fail loud: silently leaving x[i] **= y (or <<=, *=) unguarded would reopen the bombs.
...
def _run_with_timeout(fn: Callable[[], str], timeout: float) -> str:
    ...
    Daemon thread joined with a timeout: it's abandoned at interpreter exit, so a runaway
    ``while True:`` can't block process shutdown (ThreadPoolExecutor's shutdown(wait=True) would).
    ``fn`` must map its own exceptions to an ``Error: ...`` string; one escaping the thread is
    surfaced as the produced-no-result error, never silently dropped.
    """
    result: dict[str, str] = {}
...
def safe_calculate(expression: str, timeout: float = SANDBOX_DEFAULT_TIMEOUT) -> str:
    ...
    Empty ``__builtins__`` doesn't block attribute access, so run the AST escape-guard before ``eval``.
    The timeout keeps an expression the guard admits but that computes forever (a huge comprehension)
    from blocking the caller \u2014 sync tool execution runs on a Ray actor's event loop.
    """
    try:
...
def _validate_sandbox_ast(code: str, allow_imports: bool = False) -> None:
    ...
    Overriding ``__builtins__`` doesn't contain ``eval``/``exec``: ``().__class__.__bases__[0].
    __subclasses__()`` walks the dunder graph to arbitrary classes (file IO, subprocess). So dunder
    attribute/name access, frame/generator introspection attrs, dunder subscripts, and introspection/IO
    builtins are all forbidden. ``import`` is blocked unless ``allow_imports`` (safe only under external
    isolation). Defense-in-depth, not a hard boundary — see the module docstring.
    """
    try:
        tree = ast.parse(code)
    except SyntaxError:
...
def _run_python_sandboxed_inner(code: str) -> str:
    ...
    Every exception the submitted code raises is its *result* and is rendered as ``Error: ...`` so the
    model gets an actionable observation. Catching only an enumerated subset would let the commonest
    failures (a ``SyntaxError`` from the exec compile, an ``AssertionError``) escape the worker thread,
    surfacing as the opaque "produced no result" plus a raw traceback in the actor log.
    ``BaseException`` (``SystemExit``, ``KeyboardInterrupt``) still propagates.
    """
    ...
    # Captured print output beats the expression value, so ``print(x)`` returns x not None.
    try:
