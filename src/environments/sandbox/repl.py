...
def format_sandbox_repl_output(result: SandboxResult, timeout: float) -> str:
    ...
    and render as strings. A backend/transport failure (``result.error``) is NOT — it raises
    :class:`SandboxInfraError` so the tool layer records a failed call, instead of a string the
    protocol would score as a successful one.
    """
    if result.timed_out:
