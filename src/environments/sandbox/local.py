...
# Python program runs on the same interpreter the toolkit does.
...
class LocalSubprocessSandbox(SandboxExecutor):
    ...
    def _limit_wrap(argv: list[str], cpu_seconds: int, memory_mb: int, nproc: int | None = None) -> list[str]:
        ...
        Used instead of a ``preexec_fn``, which forces CPython down the ``fork`` path (copying the whole
        parent's page tables per execution — slow and GIL-serialized under a resident model). ``vfork``
        + ``exec`` keeps launch concurrency flat as resident memory grows; the kernel inherits the
        RLIMITs across ``exec`` (and into a bwrap jail). Bounds are per-call, so executions never race.
        ``nproc`` (run step only) caps process/thread count so a fork bomb can't outrun the timeout's
        process-group kill; the compile step omits it (the compiler's fork tree is trusted).
        """
    ...
    def _run_in_new_session(
        ...
        ``start_new_session`` puts the child in a fresh process group, so on timeout the WHOLE group is
        SIGKILLed — a plain child-kill leaves forked grandchildren (a fork bomb) running unbounded.
        """
