...
class BubblewrapSandbox(LocalSubprocessSandbox):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if bwrap_path:
            # An explicit path must point at a real executable, not silently accept a typo.
    ...
    def _verify_can_sandbox(self) -> None:
        """Fail fast if bwrap can't create a sandbox here.

        A blocked user/mount namespace makes ``bwrap`` exit non-zero before the child runs; without
        this probe that looks like a generic program exit and grading misreads it as a failed solution.
        """
    ...
    def _wrap_command(self, argv: list[str], workdir: str, *, allow_network: bool) -> list[str]:
        ...
        if not os.path.isdir(workdir):  # never bind a non-existent writable mount
