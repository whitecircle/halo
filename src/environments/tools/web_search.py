...
class HttpSearchSpec:
    """A keyed JSON search API, described once for both transports.
...
# Fabricated snippets pay ``tool_success_reward`` exactly like a real search, so a training YAML must
# not be able to name this backend — the run would teach the policy that invented evidence works.
# Its legitimate user is the demo playground, which opts in through the flag.
...
def _selectable_backends() -> dict[str, SearchBackend]:
    """The backends a caller may name. Read per call, so the flag is a runtime opt-in, not an
...
def validate_search_backend(backend: str | None) -> None:
    """Refuse a backend name this process cannot select (``None`` = auto-select, always valid).

    Called at tool-CONSTRUCTION time as well as per search, because every protocol turns a tool
    exception into an observation plus ``tool_error_penalty``: a per-call raise alone lets a whole
    training run play out against a ``web_search`` that never returns a result, scoring a fully broken
    environment instead of refusing the config.
    """
    if backend is None:
        return
...
def _backend_failure(backend: str, exc: Exception) -> RuntimeError:
    """Log a backend fault and build the error every search entry point raises.

    Raising, never falling back to mock results: invented "evidence" would also score tool_success.
    """
