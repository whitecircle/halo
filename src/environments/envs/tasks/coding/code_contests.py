...
# ``max_test_calls`` are per-episode interaction budgets, so effort buys iteration too. ``token_cost``
# prices total generated tokens (reward units per 1k): a price can only be avoided by economy where a
# length target is farmable, and pricing both channels closes the push-CoT-into-the-visible arbitrage.
...
# Accepted profile keys and the minimum each admits, so the two never drift apart.
...
class CodeContestsEnvironment(NativeToolUseEnvironment):
    ...
    def _run_test(self, code: str) -> str:
        ...
        An over-cap call raises :class:`ToolBudgetExhausted` so it classifies as a tool ERROR (charged
        ``tool_error_penalty``, never paid ``tool_success_reward`` — a call the env refused must not
        earn the success rung) without the traceback a genuine tool fault gets. The cap is per-episode
        state, so a direct call with no active trajectory (e.g. a unit test) runs uncapped.
        """
    ...
    def _episode_binding(self, trajectory: Trajectory) -> Iterator[None]:
        """Bind the active trajectory so submit_solution grades against ITS test cases."""
    ...
    def _submit(self, code: str) -> str:
        ...
        An over-cap call raises (tool ERROR, same rationale as :meth:`_run_test`) without overwriting
        the graded result.
        """
        trajectory = _ACTIVE_TRAJECTORY.get()
    ...
    def _apply_interaction_budgets(self, traj: Trajectory, context: dict[str, Any]) -> None:
        ...
        Runs whenever any profile binds interaction, because the tool descriptions then defer to the
        task message — so the contract must exist on every episode. A level without interaction keys,
        or one still undetermined at reset (:meth:`~src.environments.base.BaseEnvironment.reset_effort_level`
        returns ``None``), gets the class caps, stated rather than implied.
        """
        ...
        # Charged by the trainer against total generated tokens — the env never sees token counts.
