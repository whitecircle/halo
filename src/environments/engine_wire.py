...
One home for what goes out on a turn's request and what can be read back off its response: the two
engines accept different spellings of the capture flags and return the same three facts (sampled ids,
their logprobs, the engine's prompt ids) in different places. The rollout drivers own the transport
and the episode loop; this module owns the payload and the response shape.
"""
...
# The engines return the same three facts in different places; select the reader once, not per site.
...
def generation_control_fields(config: RolloutConfig, reasoning_effort: str | None = None) -> dict[str, Any]:
    ...
    ``reasoning_effort`` goes out TOP-LEVEL only: that spelling is the one both engines also derive
    their thinking toggles from (vLLM ``enable_thinking``, SGLang ``thinking`` + ``enable_thinking``),
    which the nested ``chat_template_kwargs`` form does not. Nor is the nested form needed as a
    belt-and-braces copy — vLLM merges it UNDER the top-level field and drops that field when unset —
    while sending both is ambiguous: vLLM resolves a disagreement to the top-level value, SGLang to
    the nested one.
    """
    fields: dict[str, Any] = {}
    ...
    if config.max_thinking_tokens is not None:
        if config.backend == "vllm":
            ...
            # A budget reaching here came from the env's per-effort profile (the global one is
            # rejected outright); it still reaches the reward side, so the drop is only half a no-op.
            warn_once(
                logger,
...
def build_payload(
    ...
    # capture flags would be a silent no-op there: gate them on the engine.
    ...
    if config.capture_token_ids:
        ...
        if is_vllm:
            # The server's own render is ground truth; a client-side re-render drifts.
    ...
    if config.capture_routed_experts and not is_vllm:
        # SGLang additionally wants a per-request opt-in. start_len stays 0 so rows cover the
        # full sequence — the convention assemble_rollout_masks replays prompt spans from.
