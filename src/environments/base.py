"""The environment protocol every multi-turn GRPO task implements: the conversation record, one step,
and the base classes an env subclasses (``AsyncBaseEnvironment`` runs its turn batch through
``asyncio.gather``, for I/O-bound tool/API calls). What DRIVES an episode is :mod:`src.environments.episode`."""
...
# Set in ``info`` when an episode completed but its reward carries no learning signal (grading infra
# died and forced the failure reward); the trainer excludes it from the GRPO group baseline.
EPISODE_INVALID_KEY = "episode_invalid"

# The task-outcome half of an env's ``reward_components``, the term advantage shaping gates on (it
# falls back to the total reward when absent). An env that misspells it is shaped on the total
# instead — silently, since a missing component is a legitimate shape.
...
# every other reasoning family ``reasoning_content``. Templates ignore the spelling they do not know,
# so emitting both renders the CoT once everywhere; the wrong one alone renders an EMPTY block.
...
def resolve_reasoning_effort(effort: str | None) -> str | None:
    ...
    ``"random"`` → a uniform pick of :data:`VALID_REASONING_EFFORTS` (call ONCE per episode so every
...
class Message:
    ...
    # would pickle a large array through Ray. The prompt ids are the engine's, which a re-render drifts from.
    ...
    # like a fragment, or a recovering episode reinforces the invented call that cost it a turn.
    ...
    def assistant(cls, content: str, **fields) -> "Message":
        ...
        the turn flags); an unknown name raises here rather than being silently dropped."""
...
class EnvStep:
    ...
    Gym-shaped: ``reward`` is this step's delta (the trajectory keeps only the running total), while ``done``/``truncated``/``info`` mirror the trajectory's own state at the
    moment the step returned — ``info`` by identity, so a caller mutating it mutates the trajectory.
    """
...
class BaseEnvironment(ABC):
    ...
    # Turn budget a class gets when the config names none. Per class: the generic budget cuts an
    # agentic edit-run-test loop mid-iteration, while a one-shot exam never spends it.
    DEFAULT_MAX_TURNS: int = 10

    # Declared here because ``_add_action_message`` trims stored ``tool_calls`` to it; ``None`` = no cap.
    max_tool_calls_per_turn: int | None = None

    # Every env is asked for the prompt its trajectories open with (eval records it in the trajectory
    # meta); the protocol layers that build one assign it. ``None`` = no system turn.
    system_prompt: str | None = None

    # What :meth:`_handle_length_cutoff` feeds back after the engine cuts a turn off at its token cap.
    # Per protocol, because the text must ask for that protocol's own next move; ``None`` means the
    # protocol has no recovery path and never routes a cut-off turn there.
    ...
    def __init__(self, max_turns: int | None = None, max_observation_chars: int = 16384, **kwargs):
        ...
        Any remaining keyword is rejected: the registry factories forward the whole ``env_config``, so
        a key no constructor in the chain binds is a typo or a knob meant for another ``env_type``, and
        absorbing it here would make the config claim behavior the run never gets.
        """
        max_turns = self.DEFAULT_MAX_TURNS if max_turns is None else max_turns
        # 0 turns makes the rollout loop a no-op: an all-zero batch that looks like a failing policy.
        if max_turns < 1:
    ...
    def reset_effort_level(self, context: dict[str, Any] | None) -> str | None:
        ...
        non-``random`` env setting. A ``random`` draw without a context stamp resolves actor-side
        *after* reset — resolving it here too would draw twice and diverge, so per-episode
        effort-conditioned setup (interaction budgets, prompts) must key off this and skip when it
        returns ``None``."""
    ...
    def rollout_metrics(self, trajectory: "Trajectory") -> dict[str, float]:
        ...
        # A natural-termination rate cannot show this: a cut-off turn looks like a deliberate answer.
    ...
    def _add_action_message(self, trajectory: Trajectory, action: str, context: dict[str, Any] | None) -> None:
        ...
        # Uncapped, the message advertises more calls than tool-result messages — the re-tokenizer rejects that.
    ...
    def _handle_length_cutoff(self, trajectory: Trajectory) -> tuple[Trajectory, float, bool, bool, dict[str, Any]]:
        """A turn the engine cut off at its token cap before it produced anything: a failed turn.

        Nudged and retried within ``max_turns``, never graded — the fragment would end the episode on
        a mid-sentence string that reads as a *natural* termination. Carries no reward penalty of its
        own (why: ``agent-docs/training-methods/grpo/environmental-grpo.md``). Owned by the base so
    ...
    def finalize_truncated(self, episode_ids: list[int]) -> list[EnvStep]:
        ...
        For drivers whose episode died mid-flight (generation failure, external abort): a fake
        empty-text step would take the plain-text terminal path and mark the episode ``completed``,
        paying completion-rewarded envs ``success_reward`` for an episode that never finished. Here
    ...
    def verify_backend(self) -> None:  # noqa: B027  optional no-op lifecycle hook; subclasses override
        ...
        service (an LLM judge, a remote sandbox) override it — a misconfigured backend must fail
