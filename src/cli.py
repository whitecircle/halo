...
# Not ``halo run`` tools: ``training`` belongs to ``halo launch``, and ``diagrams`` regenerates
# committed doc assets through generators that mostly lack a ``__main__`` guard, so indexing it
# would advertise the lone runnable one and hide its siblings.
...
# hands everything after the config to the SCRIPT, whose parser would refuse these as unknown config
# fields — naming the wrong problem. Multi-node is one torchrun per node, launched directly.
...
def _tool_roots(repo_root: Path) -> tuple[Path, ...]:
    ...
    Derived from the tree so a new scripts subtree (e.g. ``profiling``) indexes without a
    code change; non-package directories without runnable scripts drop out in ``_index``.
    """
...
def _lookup_key(name: str) -> str:
    ...
    The index is hyphenated (``offline-grpo``), but the script, its config directory and half the
    docs spell the same method ``offline_grpo`` — both must reach the one entry.
    """
...
def _stem_alias_key(stem: str) -> str:
    ...
    A bare-stem alias must uniquely identify one script; singular/plural stems like ``reward``
    and ``rewards`` are near-collisions a user cannot tell apart, so both count as claims on the
    same key and neither gets an alias — the qualified name is required.
    """
...
def _index(roots: Iterable[Path], repo_root: Path, name_base: Path) -> dict[str, Path]:
    ...
    Each script is indexed by its path relative to ``name_base`` (e.g.
    ``preference/smpo`` or ``after-training/merge-ep-shards``), plus a short alias of
    its file stem when that stem — folded with its singular/plural variant, see
    ``_stem_alias_key`` — is unambiguous across the whole index.
    """
...
def training_methods(root: Path | None = None) -> dict[str, Path]:
    ...
    A top-level script's path-derived name is registered before stem aliases, so an ambiguous stem
    never overwrites it.
    """
...
def resolve_config(config: Path, root: Path) -> Path:
    ...
    # Absolute, always: _execute chdirs to --root before exec, so a path validated relative to the
    # CALLER's cwd would re-resolve there and the run silently trains on a different file.
...
def reject_multinode_flags(args: list[str]) -> None:
    """Refuse torchrun's multi-node flags among the pass-through args (see ``_TORCHRUN_MULTINODE_FLAGS``)."""
...
def _lookup(index: dict[str, Path], name: str, kind: str, root: Path) -> Path:
    ...
    # An empty index is a wrong --root (or an install without the tree), not an unknown name.
    if not index:
    ...
    # A stem dropped for ambiguity still names real scripts — list them instead of "unknown".
    ...
    if candidates:
        ...
        # A qualified name and its own alias both match the stem — one script is no ambiguity.
        if len(scripts) == 1:
            return scripts.pop()
...
def _execute(argv: list[str], cwd: Path | None, *, dry_run: bool) -> None:
    ...
    ``halo launch`` passes the repo root: a training config's relative paths (dataset dirs, output
    dirs, accelerate configs) must resolve there whatever the caller's cwd, which is also why
    :func:`resolve_config` absolutizes the config itself.

    ``halo run`` passes ``None`` — a tool's flags are the caller's, and its path flags were typed
    against the caller's cwd. Chdir'ing would silently re-root a relative ``--output_dir ./out`` (or
    any multi-GB write) under the repo, which lives on the small root FS. Nothing needs the chdir:
    the indexed script path is already absolute, and the image puts ``src`` on ``PYTHONPATH``.
    """
    if dry_run:
