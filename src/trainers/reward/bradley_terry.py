...
:mod:`src.data.collators.vlm_preference`. Everything downstream of the batch is modality-blind:
image placeholder ids are ordinary content tokens to the pad-keyed pooling rule.
"""
...
def _rendered_sides_fit(row: dict, tokenizer, max_length: int) -> bool:
    ...
    the batch is expanded. This drops the worst text rows cheaply; the collator's over-length raise
    remains the backstop for image-heavy rows near the budget.
    """
    return all(
...
class DistributedRewardTrainer(DistributedTrainerMixin, RewardTrainer):
    ...
    # ctor is handed the inner tokenizer (see ``__init__``).
    ...
    def __init__(self, *args, **kwargs):
        ...
        # (``is_vlm_run``), so the modality of the processing class determines it: a text recipe on
        # a multimodal checkpoint arrives with a plain tokenizer and keeps TRL's own tokenize map
        # and collator unchanged.
        ...
        if self._is_vlm:
            ...
            # AttributeError before the first batch. The base therefore sees the inner tokenizer,
    ...
    def _prepare_dataset(self, dataset, processing_class, args, dataset_name: str):
        ...
        text ``tokenize_fn`` is bypassed, since tokenizing before placeholder expansion would
        produce ids that no longer match the batch the model sees. ``max_length`` stays a filter on
        the rendered text (see :func:`_rendered_sides_fit`).
        ...
        # so the check is agreed across the world and the raise lands on every rank at once.
    ...
    def _pp_loss_adapter(self) -> PPLossAdapter:
        ...
        labels plane, the per-row channel the runtime already ships. The loss pools and reads row
        validity off that plane: no ``input_ids`` side tensor, no second pooling pass.
        ``eval_normalizer`` counts the marked rows, so eval_loss means over pairs;
        ``extra_signature_columns`` keeps the preference dataset's columns past HF's pruning.
        """
    ...
    def _pp_bt_predictions(self, logits: torch.Tensor, inputs: dict) -> torch.Tensor:
        ...
        reward model is not a prediction: the score sits at one pooled position per row. The
        pairwise layout matches the convention preference metrics expect, so ``argmax == 0`` is the
        accuracy.
        """
    ...
    def _pp_bt_token_loss(self, logits: torch.Tensor, target: torch.Tensor | dict) -> torch.Tensor:
        ...
        pooled = pooled_outputs(logits, positions).squeeze(-1).float()
        chosen, rejected = split_pairs(pooled)
        # Eval row-padding appends inert rows whose pooled score is meaningless — mask those pairs.
