...
labels plane the runtime already ships, and the last stage's loss decodes it back. Both trainers
need the same seams, so they live here rather than in either one of them.
"""
...
def sequence_classification_pad_id(config, tokenizer_pad_id: int | None) -> int:
    ...
    Also raises when the two ids disagree, which is silent and total. PP pads every batch to
    ``max_length`` with the TOKENIZER's pad id, while pooling keys on the CONFIG's; if they differ,
    no filler token counts as padding and every row pools at ``max_length - 1`` — a reward model
    comparing two scores read off the tail of the pad run, or a classifier scoring a pad token. The
    loss stays finite and the run trains on nothing. The training scripts arrive here already synced
    (``setup_model_and_tokenizer`` records the tokenizer's pad id on every config level that declares
    it, whatever settled that id), so both raises gate a trainer built by hand around a checkpoint
    whose ``config.json`` and ``tokenizer_config.json`` disagree, or one whose tokenizer carries no
    pad id at all for the seam to record.
    """
...
def pooling_pad_id_for(trainer) -> int:
    ...
    themselves; the two raises that guard it live in :func:`sequence_classification_pad_id`.
    """
...
def last_non_pad_positions(input_ids: torch.Tensor, pad_token_id: int) -> torch.Tensor:
    ...
    row yields position 0. It must match the model's own rule bit-for-bit or PP would silently
    score different tokens than the unsplit model.
    """
...
def encode_pooling_plane(input_ids: torch.Tensor, pad_token_id: int, values: int | torch.Tensor) -> torch.Tensor:
    ...
    side tensors — and makes the eval path's inert row padding (all-ignore rows) self-identifying,
    which the raw ``input_ids`` are not: their filler carries the TOKENIZER's pad id, which need not
    equal the config's.
    """
