...
# The one metric this trainer may omit from an eval slice (undefined, not zero): a one-class slice
# makes the AUC meaningless. Naming it in ``metric_for_best_model`` is therefore a checkpoint-ranking
# hazard, checked at construction and again wherever the key is dropped.
...
def _validate_best_model_metric(args: Any) -> None:
    """Refuse a ``metric_for_best_model`` no evaluation will report.

    Otherwise the first eval raises ``KeyError``: transformers looks the metric up in the dict the
    trainer returns, and with ``compute_auc_roc`` off the key is never written. Checked at config
    time rather than at the run's first checkpoint.
    """
...
def _loss_inputs_fp32(logits: torch.Tensor, targets: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    weights themselves. Pooled logits are ``[B, num_labels]``, so the upcast is cheap.
...
def _filter_over_length(dataset: Dataset, max_length: int, num_proc: int | None, split_name: str) -> Dataset:
    ...
    Never raises — long-tailed datasets are legitimate — but a high drop rate is escalated to a
    warning by the shared reporter, which holds that threshold for every drop-and-continue filter.
    """
...
class ClassificationTrainer(DistributedTrainerMixin, Trainer):
    ...
    # Set from prepare_peft_model's result; False whenever no PEFT config was passed.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if compute_metrics is None:
            ...
            # Only for the default metrics: a caller's own compute_metrics decides its own keys.
    ...
    def _omit_auc_roc(self, reason: str) -> None:
        ...
        omission is one behavior. It raises instead when the run ranks checkpoints on the key, where
        transformers' best-metric lookup would otherwise fail with a bare ``KeyError``.
        """
    ...
    def _focal_loss(logits, labels, gamma=2.0, alpha=None, weight=None, reduction="mean"):
        ...
        value is ``p_y ** w_y`` rather than ``p_y``, and a class weight would distort the modulator
    ...
    def _balanced_class_weights(train_dataset: Dataset, model: nn.Module | None) -> torch.Tensor:
        ...
        the local shard alone diverge across ranks. Label counts are therefore summed over the world
        group, which is correct in every topology: DP shards are disjoint, while TP siblings and
        replicated datasets only add a uniform scale to every class count, and balanced weights
        ``n_samples / (n_present_classes * count_c)`` are invariant to that.
        """
    ...
    def _pooled_loss(
        ...
        objective, not just their loss after it — masking only the output still runs the backward
        over the row, where a saturated logit makes ``softmax`` NaN. The extra zeros lengthen the
        reduction, so the result matches summing the surviving rows alone to ~2 fp32 ULP.
        """
    ...
    def _pooled_loss_normalizer(self, targets: torch.Tensor, valid_rows: torch.Tensor) -> torch.Tensor:
        ...
        if isinstance(self._loss_fn, nn.CrossEntropyLoss) and self._loss_fn.weight is not None:
            ...
            # which then propagates into the whole reduced loss. Substituted only at zero, so a real
            # batch's denominator is untouched.
    ...
    def _pp_classification_predictions(self, logits: torch.Tensor, inputs: dict) -> torch.Tensor:
        ...
        Pooling here rather than after the broadcast keeps the chain hop small: the raw stage
        output is ``[B, S, num_labels]``.
        """
