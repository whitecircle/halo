...
# Stdlib, not accelerate's adapter: the one warning here fires per rollout ROW, on whichever rank
# drew it, and the adapter drops everything off rank 0 (warn_once cannot pass main_process_only).
...
class DistributedSDPGTrainer(StoredMetricsMixin, DistributedGRPOTrainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # popped by its fields and adopted as attributes, so the dataclass is their only spelling.
        ...
        # Scope of "once": the whole run — one answer-less row means the column is unusable.
        ...
        # Every rank constructs the trainer, so this raise is world-uniform — unlike a per-rollout one,
        # which would strand the peers of the rank that drew the bad row in the next collective.
    ...
    def _build_teacher_prompts(self, prompt_ids, prompt_mask, answers):
        ...
        for i in range(prompt_ids.size(0)):
            ...
            if answer is None or not str(answer).strip():
                # No hint at all rather than one asserting an empty answer as fact: the template
                # states the answer, so a blank renders "The correct answer is: ." and the
                # "privileged" teacher is not privileged — it is misled, and OPD distils toward it.
    ...
    def _compute_loss(self, model, inputs):
        ...
        # TRL normalizes inside _compute_loss, so an undivided per-microbatch OPD mean would inflate beta by grad_accum.
        if model.training:
