...
# alpha, beta and alpha + beta non-zero; this pair puts the higher exponent on the student.
...
def slim_loss(
    ...
    student_probs = softmax(student_logits, dim=-1, dtype=torch.float32)
    teacher_probs = softmax(teacher_logits, dim=-1, dtype=torch.float32)
    # Unscaled core, unlike the other temperature losses: SLIM's student-dependent coefficient also
    # multiplies the cross-entropy's teacher-entropy offset, which does not shrink as 1/T**2, so a
    # T**2 rescale would grow the gradient instead of holding it fixed.
...
def earth_mover_distance(student_logits: torch.Tensor, teacher_logits: torch.Tensor) -> torch.Tensor:
    ...
    Not ``torch.cdist``, which gives a position-coupled ``[batch, seq, seq]`` matrix.
    """
...
def consumes_hard_labels(loss_fn: Callable) -> bool:
    ...
    Read off the signature, which is what :func:`call_distillation_loss` dispatches on. Such a loss
    (``slim``) derives its own gold-token coefficient, so :func:`hard_labels_coefficient` must not be
