"""Off-policy teacher→student knowledge distillation.

A separate frozen teacher scores fixed dataset completions; 8 loss types in ``teacher_losses``.
EP/TP on the student; CP unsupported (it would require wrapping both models). The same-model
variants are ``self_distillation`` and ``sdpg``.
"""
...
class DistributedDistillationTrainer(StoredMetricsMixin, DistributedTrainerMixin, Trainer):
    ...
    def compute_loss(
        ...
        # A loss that takes hard_labels (slim) applies its own gold-token coefficient, so the shared
        # one would be counted twice; the dispatcher reads the same signature.
        ...
        # 0.0 * NaN propagates to every rank through the gradient reduce.
        ...
        # with use_clm_loss off) a grad-carrying [B, S, V] fp32 cross-entropy would be built for a
        # summand of exactly zero. The config is rank-uniform, so every rank builds the same graph.
        # The distillation leg is not branched to match: its forward is the reported metric at every
        # alpha.
