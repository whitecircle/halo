...
on a path that was never written and the next collective forward deadlocks instead of failing loudly.
...
TRL's sweep also shards its loader and gathers by GLOBAL rank, which is wrong whenever DP <
...
def _defining_module(instance: object) -> ModuleType | None:
    """Module object of the first MRO class BELOW this mixin that defines ``_precompute_ref_logps``.

    That is the concrete TRL trainer (``DPOTrainer`` / ``KTOTrainer``); its module is where the
    ``hash_module`` symbol used to build the cache key lives. Classes above the mixin — the mixin
    itself and any trainer subclass layering more logic over it — are skipped: their modules do not
    own ``hash_module``, and matching one would silently disable the rank-consistent patch.
    """
...
class PrecomputeRefLogpsRankConsistentMixin:
    ...
    docstring for the mechanism. It also owns the skip for a dataset that already carries the
    reference columns (``_required_ref_logps_columns``), which is what makes PP viable at all.
    """

    def _required_ref_logps_columns(self) -> tuple[str, ...]:
        """Reference log-prob columns whose presence makes TRL's sweep unnecessary.

        Each user names the columns its own TRL base would write; a silent empty default would turn
        the short-circuit off and strand PP, whose model cannot run the sweep at all.
        """
    ...
    def _precompute_ref_logps(self, dataset, name, batch_size):
        ...
        With every needed column already present the sweep would be wrong twice over: it would
        recompute values the caller supplied (then axis-1 concatenate duplicate columns), and under
        PP it cannot run at all — ``self.model`` is a bare pipeline stage. That check must precede
        the pre-sharded rejection below, whose own suggested workaround is to precompute these
        columns before sharding — such a dataset has to return here rather than hit the raise. The
        PP construction gate guarantees the columns exist, so the sweep below only ever runs
        outside PP.
        """
