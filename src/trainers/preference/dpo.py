...
CP is unsupported: the chosen/rejected forward needs global log-prob sums over full
...
Under PP the trainer is precompute-only: ``ref_chosen_logps`` / ``ref_rejected_logps`` must already
be dataset columns, since no pipeline rank holds a full model. The ``[chosen ⧺ rejected]`` concat is
re-laid out as interleaved pairs so no microbatch split separates a pair, and only the per-pair loss
types the PP last-stage loss re-implements are allowed (see ``_validate_pp_mode``).
"""
...
# Shared by the PP gate and the last-stage loss so the two cannot drift apart.
...
class DistributedDPOTrainer(DistributedTrainerMixin, PrecomputeRefLogpsRankConsistentMixin, DPOTrainer):
    ...
    def _validate_pp_mode(self, ctor_args: tuple, kwargs: dict) -> None:
        """Reject DPO configurations pipeline parallelism cannot honor, at construction.

        Runs before the model is split, so every raise is cheap and rank-uniform. Remaining ctor
        parameters are read from ``kwargs`` only: ``require_model_and_args_kwargs`` forces ``model``
        into kwargs, and any positional argument would rebind TRL's ``model`` slot.
        """
    ...
    def _pp_loss_adapter(self) -> PPLossAdapter:
        ...
        reference log-probs ship interleaved as a per-example extra target, keeping them row-aligned
        with their pairs across the microbatch split. ``pin_runtime_columns=False`` because DPO's
        dataset columns feed TRL's preference collator, which needs TRL's own signature set.
        """
    ...
    def _pp_dpo_batch_transform(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        microbatches comparing unrelated examples; interleaving keeps every pair together. Labels are
        the completion token ids at their positions — those TRL's ``shift_completion_mask`` scores.
        """
