...
CP is unsupported: like DPO, KTO needs global per-sequence log-prob sums over
...
class DistributedKTOTrainer(DistributedTrainerMixin, PrecomputeRefLogpsRankConsistentMixin, KTOTrainer):
    ...
    # balancing reaches the graph here (unlike the other preference objectives).
    ...
    def _validate_pp_mode(self, ctor_args: tuple, kwargs: dict) -> None:
        """Reject KTO configurations pipeline parallelism cannot honor, at construction.

        Runs before the model is split, so every raise is cheap and rank-uniform. The KTO options
        the PP loss reproduces — ``beta``, ``desirable_weight``, ``undesirable_weight`` — need no
        gate; the loss type is gated below. Remaining ctor parameters are read from ``kwargs`` only:
        ``require_model_and_args_kwargs`` forces ``model`` into kwargs, and any positional argument
        would rebind TRL's ``model`` slot.
        """
    ...
    def _pp_loss_adapter(self) -> PPLossAdapter:
        ...
        per-row desirability ``label`` ship as extra targets to stay row-aligned. The collator's
        ``completion_*`` keys are padded to the fixed shape in the batch transform, so no
        ``pad_spec`` is needed. ``pin_runtime_columns=False`` because KTO's dataset columns feed
        TRL's unpaired-preference collator, which needs TRL's own signature set.
        """
    ...
    def _pp_kto_token_loss(self, logits: torch.Tensor, target: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        makes a single-sided microbatch (all-desirable / all-undesirable) a plain partial sum, with
        no empty-tensor branch. Deviation from TRL's reduction: TRL takes ``losses.nanmean()``;
        sum ÷ row count is equivalent when no row is NaN, and lets a NaN from corrupted data or
        references surface instead of being dropped from the mean.
        """
