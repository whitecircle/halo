...
class SmoothMarginPOTrainer(StoredMetricsMixin, DistributedTrainerMixin, Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # tokenizer-eos-only for those models. Resolved once, because the tokenize map runs in
        # worker processes and fingerprints its fn_kwargs.
    ...
    def _resolve_vlm_mode(processing_class, is_vlm: bool | None) -> bool:
        ...
        trained on text-only preference pairs is a text run, and only the text path keeps CP,
        ``padding_free`` and PP. The script decides it with ``is_vlm_run`` (multimodal checkpoint
        plus image data); the model class and ``processing_class`` still follow the checkpoint, so a
        text run can arrive with a processor. Omitted (``None``), it falls back to the modality of
        ``processing_class``.
        """
        if is_vlm is None:
    ...
    def _validate_vlm_mode(self, parallelism_config: ParallelismConfig | None) -> None:
        """Reject option combinations VLM mode cannot honor, at construction.
    ...
    def _reject_pp_explicit_options(
        ...
        """Reject SMPO options pipeline parallelism cannot honor, at construction.
    ...
    def _prepare_dataset(
        ...
        if self.is_vlm:
            ...
            # does not carry. A text run on a multimodal checkpoint keeps the processor as
            # processing_class, so the row fn gets its inner tokenizer instead.
    ...
    def _get_current_margin(self) -> float:
        ...
        Read straight off ``self.model``, where the callback writes it; the recursive
        :meth:`_top_level_model` walk is paid only when a framework wrapper that does not delegate
        attribute access (DDP) causes a miss.

        No default: ``__init__`` (and the PP re-seed) puts ``target_margin`` on the model whenever
        the schedule is on, so a miss means the seeding path was skipped.
        """
    ...
    def _pp_smpo_predictions(self, logits: torch.Tensor, inputs: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        :meth:`prediction_step` reports these two batch-level masked logit means, so the pipeline
        reduces to them on the last stage rather than sending the raw ``[rows, S, vocab]`` plane
        down the chain. Chosen rows are the even indices of the interleaved batch, rejected the odd;
        inert eval-padding rows are all-ignore, so the loss mask drops them.
        """
    ...
    def _pp_batch_transform(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        tensor-splits along the batch dimension, so it is re-laid out as ``[c0, r0, c1, r1, ...]``
        to keep every pair in one microbatch, then right-padded to ``args.max_length`` because the
        P2P buffer shapes freeze on the first step. Also refreshes the per-step loss state: the
        full-batch normalizers and the live scheduled margin.
        """
    ...
    def _pp_normalizer(self, inputs: dict) -> float:
        ...
        rank-local: the non-PP loss is a rank-local mean whose gradients FSDP averages over the DP
        group, and the PP stage wrap averages over the same group.
        """
        del inputs
        return 1.0
    ...
    def _smpo_metric_sums(
        *,
        ...
        """One batch's metric sum contributions — the definition both loss paths build from.

        ``pair_valid`` is a per-pair 0/1 mask, so inert PP eval-padding pairs contribute nothing
        (off PP every pair is valid). Arguments are never re-reduced: under CP the log-probs, logit
        means and SFT losses arriving here are already CP-global means. Detached so no metric pins
        the autograd graph.
        """
        return {
            key: value.detach()
    ...
    def _smpo_metrics_from_sums(
        ...
        *,
        ...
        handed a unit divisor, which lets both loss paths share one expression.
        """
    ...
    def _pp_step_metrics(self) -> dict[str, torch.Tensor]:
        ...
        with zeros. Divisors are the full-batch constants the batch transform recorded, which makes
        the result equal to the non-PP ``get_batch_loss_metrics`` value.
        """
        state = self._pp_step_state
    ...
    def get_batch_loss_metrics(
        ...
        Metrics are detached 0-dim tensors (drained by ``StoredMetricsMixin`` at log time, so no
        per-microbatch host sync) and are rank-local estimates; TRL's DPO instead gathers them
        across DP ranks per compute_loss call, a per-microbatch collective.
        """
