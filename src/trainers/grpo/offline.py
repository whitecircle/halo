...
PP runs every loss type (grpo / bnpo / dr_grpo) — their whole-batch denominators are computable
from batch metadata alone — and the KL term at any ``kl_beta``: no pipeline rank holds a full model
to forward a reference, so the reference log-probs are scored once, before the first optimizer
step, by a forward-only sweep through the pipeline itself and ride into every step as a per-example
side tensor. See the ``_pp_loss_adapter`` section.
"""
...
def compute_group_advantages(
    ...
    if method == "z_norm":
        ...
        # and environmental z-norms take the torch path — an implicit default splits the two by
        # sqrt((n-1)/n), ~6% at the num_generations=8 GRPO actually runs at.
    ...
    if len(rewards_array) > 1:
        if best_completion_emphasis == "auto":
            ...
            # unlike the z_norm divisor above: this heuristic has no torch twin to agree with, and it
            # multiplies the best row under EVERY method — an alignment correction here would silently
            # move the advantages of the rank/minmax methods that never asked for one.
...
def tokenize_prompt_completion(
    ...
    *,
    ...
    # `tokenizer.model_max_length`, so an unset cap would silently truncate at whatever the run
    # pinned there instead of meaning "no cap".
...
def tokenize_offline_grpo_rows(
    ...
    *,
    ...
    for prompt, completions_list, rewards_list, idx in zip(
        ...
        # all-zero advantages, so its rows carry no gradient yet dilute the loss normalizer; a mere
        # NEAR-tie still trains here with full-scale advantages under the rank methods
        # (quantile_norm gives ±0.674 at any nonzero spread), so a spread threshold would silently
        # change the objective. Dropping at tokenization (not in the loss) also skips the rows'
        # forwards and holds across every mode, PP's metadata denominators included.
...
def live_min_log_prob(model: nn.Module, configured: float | None) -> float | None:
    ...
    ``VariableSchedulerCallback`` writes the scheduled value onto the UNWRAPPED module — the
    trainer's ``self.model`` — so the DDP/FSDP-wrapped module handed to ``compute_loss`` never
    carries it, and reading the floor off that wrapper would pin step 0's value for the whole run.
    """
...
class OfflineGRPOTrainer(ChunkedLogprobsCore, DistributedTrainerMixin, Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # three-way ``loss_type`` ladder still ends in a raise — its fallthrough would substitute a
        # different denominator, a wrong NUMBER rather than a crash — while the two-way PG choice ends
        # in a labelled else.
        ...
        if self.beta == 0.0 or is_peft_model(model) or self.parallelism_config.is_pp_mode:
            ...
            # runtime exists (_pp_precompute_reference_logps); a resident full model beside a stage
            # is exactly what PP is there to avoid.
            self.ref_model = None
        else:
    ...
    def _reject_inert_max_length(args: OfflineGRPOConfig, parallelism_config: "ParallelismConfig | None") -> None:
        ...
        tokenization applies — and ``max_length`` reaches nothing, so a config that sets it trains
        uncapped at whatever those two allow, while reading as though it caps sequences.
        """
    ...
    def _reject_pp_explicit_options(
        ...
        """Reject offline-GRPO options pipeline parallelism cannot honor (fail loud at construction).
    ...
    def _check_degenerate_drop(args: OfflineGRPOConfig, dataset, num_groups_in: int, what: str) -> None:
        ...
        silent 100% drop is indistinguishable from a mis-scored dataset, which the user must see.
        """
    ...
    def _create_full_ft_reference(model, model_id: str):
        ...
        # it (hub main can drift from the trained commit's format). Coverage-gated because a
        # reference silently loaded with missing keys is part random weights, and a wrong KL
        # reference biases every step with no other symptom. Remote code has already run in-process.
    ...
    def _build_grouped_dataloader(self, dataset, group_ids, *, batch_size: int, shuffle: bool) -> DataLoader:
        ...
        equalized to the global MIN, else faster ranks exit and block the next all-reduce. The
        sampler already shards, so accelerate does device placement only.

        A PRESHARDED dataset is already this rank's disjoint slice, so the sampler must not cut it
        again — doing so silently drops (dp-1)/dp of the data with no warning.
        """
        ...
        if not isinstance(dataset, torch.utils.data.IterableDataset):
            ...
            if get_global_world_size() > 1:
                # compute_loss gathers every batch: a rank with an extra batch strands its peers.
    ...
    def _buffer_sign_metrics(
        ...
        positive-advantage samples. The mode is the predicate log() drains under.
        """
    ...
    def _drain_sign_metrics(self, mode: str) -> None:
        ...
        # whole sample set — a world gather would strand the other stages in a collective they
        # never enter. The chain gets the drained scalars from ``log``.
    ...
    def _pp_batch_transform(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        column exists, and is the one caller allowed to.
        """
    ...
    def _pp_precompute_reference_logps(self, dataset: "datasets.Dataset", what: str) -> "datasets.Dataset":
        ...
        resumed ones, which is what ``create_reference_model`` anchors to off PP — so one forward-only
        sweep through the schedule before the first optimizer step scores every completion token,
        ...
        # one row longer would strand its peers on its extra batch. The shorter shards replay their
        ...
        try:
            for batch in loader:
                ...
                # Armed before the transform: it is what tells the transform (no column yet) and the
