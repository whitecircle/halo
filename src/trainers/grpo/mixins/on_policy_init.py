...
Both open their ctor identically — resolve a config that may have arrived positionally, disable
TRL's Liger GRPO loss, extract the distributed kwargs — and close it identically: realize the
parallel modes, gate the implicit reference model, wire weight sync, then kill dropout. The closing
ORDER is the load-bearing part, so it exists once here rather than in each ctor.
"""
...
class OnPolicyGRPOInitMixin:
    """Open/close halves of an on-policy GRPO ctor. Mixed into the trainer."""
    ...
    def _begin_on_policy_init(self, args: tuple, kwargs: dict) -> tuple[object, dict]:
        ...
        and EP/CP reentrant override would then silently no-op.
        """
    ...
    def _finish_on_policy_init(self) -> None:
        """Realize the parallel modes, gate the reference model, wire weight sync, kill dropout.

        Dropout goes LAST: it must reach the EP expert-LoRA dropout ``_setup_distributed_modes``
