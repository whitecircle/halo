...
class DistributedGRPOTrainer(
    ...
    def _recomputes_from_gathered_rewards(self) -> bool:
        ...
        Single home for the gate: the stash in :meth:`_calculate_rewards` and every consumer below
        must agree, and a knob covered by one but not the other is silently inert (the stash is
        ``None``, so the consumer early-returns and the user's setting never reaches the math).
        """
        return (
    ...
    def _install_advantages(
        ...
        Single home for the three steps every advantage-replacement hook owes: both hooks recompute
        on the full gathered set, so both must re-slice and both must re-log, and a hook that did one
        without the other would train on values the durable record does not carry (or the reverse).

        TRL fills ``_logs["advantages"]`` with ITS group-normalized values inside
        ``_generate_and_score_completions``, i.e. before either hook runs, so the completions record
        would otherwise report a set no gradient ever saw. The gathered tensor carries the same world
        order and length TRL appended, so overwriting that tail realigns the record row for row.
        """
    ...
    def _distributed_sync_weights(self) -> None:
        ...
        All ranks join the gathers; only global-main (and, under TP, TP-rank 0) forwards. The client lives
        on ``vllm_generation`` here (env-GRPO owns one directly) — the only difference between the two.
        """
