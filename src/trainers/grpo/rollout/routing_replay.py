...
Tiny numeric differences flip top-k selection near decision boundaries, so the update pass would otherwise
train through a different token distribution than the importance ratios were computed on (Qwen Routing
Replay, arXiv:2507.18071). Gate weights are always re-derived from the live router scores restricted to
the replayed selection — replaying weights or logits kills the router gradient (RSPO, arXiv:2510.23027).
...
def decode_rollout_routing(payload: str, num_layers: int, top_k: int) -> torch.Tensor:
    """Decode an engine's ``routed_experts`` payload → int16 ``[tokens, L, K]``.
    ...
    reshapes by the trainer's own ``(num_layers, top_k)`` and rejects a byte count they do not
    divide, so a shape disagreement surfaces here rather than as a silently misaligned mask.
    """
...
def assemble_rollout_masks(
    ...
    token count (``usage.prompt_tokens``) — the trainer's re-render can drift by a token, and
    anchoring on it would silently shift every completion position by one. The trainer-side prompt
    span is only filled when the two counts agree.

    Positions without engine routing stay ``-1`` (natural selection): padding always; whole rows
    without a mask; the prompt span on a count mismatch; rows whose coverage matches no known engine
    convention (counted, never raised — a per-rank raise here would desync ranks into a collective
    hang; the caller decides uniformly). Returns ``(masks, stats)`` with per-convention row counts —
    the observable for engine coverage drift.
    """
    rows = len(turn_masks)
    ...
    for i, turn in enumerate(turn_masks):
        ...
        if mask.min() < 0 or mask.max() >= num_experts:
            # Out-of-range ids would feed garbage into DeepEP dispatch — device-side corruption, not a clean error.
            raise ValueError(
...
class RoutingReplayInjector:
    """Owns capture/replay state across the EP MoE layers of one model.
    ...
    def capture(self, rows: int, seq_len: int, row_spans: list[tuple[int, int]] | None = None):
        ...
        scatters each row's segment back to its span; positions outside a span stay ``-1`` (natural
        routing — they are padding and never replayed). ``None`` means full-width forwards.

        Exception-safe: an error inside resets every layer's capture state and propagates unchanged
        (the assembly check must not shadow a real OOM/NCCL failure).
        """
    ...
    def disarm(self) -> None:
        ...
        Consumption model: only non-recompute forwards advance the cursor / consumed-total; the cursor
        wraps to zero on exact exhaustion so whole-pass reuse (a second full forward over the same armed
        mask) re-consumes cleanly, and GC backward recomputes re-read their frame's saved slice
        without consuming. A correctly-tiled microbatch therefore ends with consumed-total a POSITIVE
        multiple of the armed size — cursor==0 alone also matches a mask that was never consumed at all.
        Fails loud on any other end state, unless an exception is already propagating."""
        if not self._armed:
            return
        bad = []
        for layer in self._layers:
