...
(``full_tensor()`` and ``gather_expert_state_dict`` are collectives that hang if a rank skips them) while
only the forwarding rank (global-main, TP-rank 0 under TP) sends. PEFT/LoRA is folded into the base and
forwarded under base-model param names. The engine is a parameter, not an assumption: the expert
layout and the parallelism gate are read off the resolved client class (vLLM or SGLang).

Those sends sit BETWEEN the gathers, so every one of them runs under a :class:`DeferredRankFailure`
and the verdict is taken at a rank-uniform ``reject``: a forwarding rank that raised mid-loop would
otherwise leave every peer blocked in the next layer's gather until the watchdog fires.

Every forwarded key is the HUB spelling a gathered checkpoint would carry (:class:`_HubForwarder`):
the engine loads by hub name and silently skips an unknown one, so a family whose module tree is
spelled differently (Laguna's ``_EXPORT_KEY_RENAMES``, Step-3.7's vendor namespace behind
transformers' conversion mapping) would lose exactly those tensors.
"""
...
# held tensors sit on the forwarding rank's GPU next to the gather that produced them, so this is a
# rank-local allocation the streaming exists to avoid; the shipped claims (Step-3.7's vision q/k/v,
# one EP key) are megabytes, and a converter claiming a per-decoder-layer tensor would hold the
# whole stack instead. Sized at one chunk, the same budget the client streams by.
...
def validate_weight_sync_support(model: torch.nn.Module) -> None:
    ...
    Six failure classes are rejected here instead of opaquely (or silently) at the first sync:

    - **Quantized bases (QLoRA)**: ``_send_dense_weights`` iterates ``named_parameters`` and forwards
      raw storage under base-weight names, so a bnb-quantized base would ship packed
      non-floating-point tensors (``Params4bit`` uint8) the server fails on opaquely after full
      startup — and each sync's LoRA merge/unmerge round-trip through 4-bit weights is lossy.
    - **GptOss with sinks removed** (the flash_attention_2 ``reset_sinks`` reset): the removed
      ``sinks`` slots leave ``named_parameters``, so nothing is ever pushed for them and the rollout
      engine keeps serving the pretrained sinks against a sink-free trainer — permanently off-policy
      with no error at sync time.
    - **GptOss with trainable sinks** (``train_sinks``): an SFT-only policy. The frozen live sinks of
      ...
      validated end-to-end sync into either rollout engine, so the shape is refused rather than run
      against an unverified generator.
    - **Families whose layer class declares ``_supports_weight_sync = False``** (DeepSeek-V4,
      Inkling, Zaya, Cohere2 MoE, GLM-5 Next): the names this sync forwards are fed straight into
      vLLM's ``model.load_weights`` and cannot land — each class's ``_WEIGHT_SYNC_REFUSAL_REASON``
      states the family's actual gap. Enforced through live EP instances when present, else through the
      registry off ``config.model_type`` — a wrapper-less run carries the same contract.
    - **Model types no pinned engine can serve** (``_WEIGHT_SYNC_UNSUPPORTED_MODEL_TYPES``): the
      family flag holds for its servable siblings, but these spellings have no model class in the
      pinned engines, so the server cannot even load the base model.
    - **Live bias-update balancing state**: the sync payload is parameters only, so an adopted
      native slot (a buffer) or a transient side-buffer is never pushed — the trainer's routing
      would silently drift from the generator producing the trajectories. The shipped GRPO scripts
      downgrade ``moe_balancing`` to ``none`` before any bias state exists
      (``build_perf_callbacks``); this backstop catches hand-built drivers that enabled balancing
      themselves.
    """
    ...
    # enabled balancing itself. Presence-of-state probes stay silent on Zaya's always-present native
    # buffer (never adopted, never transient), which its _supports_weight_sync=False refuses above.
    balancing = sorted(
        {
            type(m).__name__
...
def _sync_contract_classes(model: torch.nn.Module) -> list[tuple[str, type[EPMoELayerBase]]]:
    ...
    (:func:`ep_layer_classes_for_config`) — ``ep_size == 1`` with ``use_grouped_gemm: false``
    leaves the stock HF module tree, so a gate that only walked live modules would silently admit
    exactly the families it exists to refuse.
    """
...
def _is_ep_expert_param(name: str, ep_layers: dict[str, EPMoELayerBase]) -> bool:
    """True if ``name`` is an expert weight owned by one of the EP layers (handled by the EP gather)."""
...
class _HubForwarder:
    ...
    inside EP layers (Laguna); the PEFT base-name normalization (adapter-only tensors dropped —
    their delta is already merged); and, for a family declaring ``_EXPORTS_HUB_NAMESPACE``
    (Step-3.7), transformers' own save-side conversion revert — the reversed ``WeightRenaming`` /
    ``WeightConverter`` entries ``save_pretrained`` and the gathered EP save apply, derived from the
    conversion mapping instead of a per-family name table. Renames are one-to-one and stream; a
    tensor a reverse ``WeightConverter`` claims (the vision tower's q/k/v → ``in_proj`` fusion, the
    fused ``gate_up_proj`` split into the hub's two stacks) is held until :meth:`flush`, which runs
    the revert over everything held — a many-to-one revert needs all of its sources together, and
    the engine loads one tensor at a time.

    Every forward runs under the caller's ``guard``: this object is the only thing in the sync that
    can fail on one rank alone (a host-OOM pinning the snapshot, an HTTP/NCCL error from the client,
    a tensor the engine's key space rejects), and it does so between two group-wide gathers.
    """
...
def wants_fused_experts(expert_layout: str) -> bool:
    ...
    Both declared spellings are recognized and anything else raises: with a two-valued knob an
    inequality test silently routes an unknown third layout into one branch, and the gather it picks
    is rejected on arrival — after the engine is paused and partly written.
    """
...
def expert_layout_for(trainer) -> str:
    ...
    Off the trainer's declared ``_rollout_backend`` rather than the client object: only the forwarding
    rank holds a client, and the gathers this selects are collective — reading it from the instance
    would have the ranks build different layouts and hang. Online GRPO leaves the attribute ``None``
    and keeps the base default, which is correct: it is vLLM-only by construction.
    """
...
def _send_ep_expert_weights(
    ...
    are collective, so it has to be decided identically on every rank — hence a value threaded in
    rather than read off ``forwarder``, which is ``None`` everywhere but the forwarding rank.

    The retained ASSEMBLY is the sync's largest rank-local allocation — the per-expert copies are
    built while the gather outputs are still live, ~28 GB for one 397B layer — so it runs under
    ``guard`` like the sends do: every retained gather finishes its collectives before it assembles,
    and an OOM there must reach the peers as a reason instead of dropping this rank out of the next
    layer's gather. Retaining then stops, since there is nothing left to send.
    """
...
def _send_dense_weights(
    ...
    # full-tensor name would corrupt, so skip here — the drain loop below sends them gathered.
...
def _flush_and_close(sender: Any) -> None:
    ...
    — the D2H completion of the tail's host snapshots, a chunk the engine rejects on arrival —
    leaves the engine quiesced behind an open reload with nothing else on this path to end it.
    """
    try:
...
def sync_weights_to_client(
    ...
    try:
        ...
        # queueing every rollout. Closing it is the forwarding rank's job alone.
        if sender is not None:
            sender.abort_weight_update()
        raise
    # The buffered broadcast lands after every gather, so a failure here strands no peer inside a
    # collective — but a peer that sails past it drives its next rollout round against an engine left
    # paused mid-update. Same uniform verdict as the push, taken on the flush's own rank-local work.
...
def gather_and_send_weights(
    ...
    # per-step push would ship a policy one optimizer step behind, and the merge below would fold
    # the adapter into a copy the next unshard discards. Same call the optimizer build and every
    ...
    # that rank alone. Raising there takes it out of the loop while every peer blocks in the next
    # layer's gather until the watchdog fires — so record and carry on, and let the reject decide.
    guard = DeferredRankFailure("weight-sync push to the rollout engine")
    with merged_adapters(model) as peft:
        # Live EP layers, NOT is_ep_mode: ep_size==1 is still EP-wrapped and the dense path ships an unloadable layout.
    ...
    # COLLECTIVE — every rank. Raises on all of them with the forwarding rank's real cause, after the
...
def sync_trainer_weights(trainer, client: Any | None) -> bool:
    ...
    EVERY rank must call this (all ranks join the gathers; only global-main forwards). ``client`` is the
    caller's own handle — the sole difference between the online and env sync paths.
    """
    ...
    # Hold every rank until the forwarding rank's push lands: peers would otherwise drive rollouts into
    # a mid-update engine (SGLang deadlocks against /pause_generation, vLLM reads half-written weights).
    # FENCED because the push is main-rank-only: a raise past the barrier its peers block in hangs the job.
