...
def _effort_template_kwargs(trajectory: Trajectory) -> dict[str, str]:
    """Chat-template kwargs pinning the SAME ``reasoning_effort`` the rollout used, so the template's
...
class TrajectoryTokenizeMixin:
    ...
    state; records fatal per-row failures in ``self._batch_build_error`` rather than raising, so the
    rank-uniform fence in the trainer raises them together.
    """
    ...
    def _context_limit(self) -> int:
        ...
        The tokenizer is consulted FIRST — the training script pins the *served* window there, which
        is the limit vLLM actually enforces during rollout. An unset value (``None``, non-positive, or
        at/above :data:`UNSET_MODEL_MAX_LENGTH`, the threshold HF's oversized "no limit" sentinels sit
        above) falls through to :func:`get_model_context_window`, which reads the model
        config (composite/VLM-safe) and raises when no window is derivable at all: returning a silent
        huge fallback would disable the trajectory-overflow check and let an oversize row OOM or
        corrupt the forward instead of failing with a reason. The tokenizer predicate is the shared
        resolver's own, so the two legs cannot drift apart.
        """
    ...
    def _masked_trajectory_tensors(self) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        reinforce P(EOS|garbage) — or, when every assistant turn was excluded as unusable, the exact
        fragment the exclusion exists to suppress — at that trajectory's advantage.
        """
    ...
    def _tokenize_trajectory(self, result: RolloutResult) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        Prompt plus completion is exactly ONE render of the whole trajectory, so the trained sequence
        is what the serving template emits; per-turn spans are located inside it
        ...
        try:
            ...
            # RECORDED, not raised: a per-rank raise ahead of the batch collectives is a watchdog hang.
        ...
        # No silent truncation (reward would decouple from trained tokens); recorded, not raised.
    ...
    def _tokenize_trajectory_turns(self, result: RolloutResult) -> list[TurnRow]:
        ...
        * ``sampling_logps`` — vLLM's SAMPLING log-probs, 1:1 with ``completion_ids``: the behavior-policy
          reference for the IS trust region. ``None`` when the turn has no aligned logprobs.
        * ``turn_routing`` — decoded engine routing plus its prompt-token count, else ``None``.
        ...
        # All-or-nothing: if any turn lacks captured ids, fall back rather than silently drop turns.
        if not assistant_turns or any(not m.token_ids for m in assistant_turns):
            if assistant_turns and not self._warned_capture_missing:
                ...
                # The remedy is engine-specific, and naming the wrong one sends the reader chasing a
                # flag their server does not take.
                remedy = (
        ...
        for idx, m in enumerate(messages):
            ...
            if m.truncated or m.calls_rejected:
                ...
                # condition on what it actually emitted), but training on it would reinforce the runaway
                # or the invented call whenever the episode recovers and earns a positive advantage.
                excluded_unusable = True
                continue
            # Engine prompt ids win: a client re-render drifts on effort steering, tool schemas, channel placement.
            if m.prompt_token_ids:
                prompt_ids = list(m.prompt_token_ids)
            else:
                # RECORDED, not raised, like every other render site: a template that rejects this
                # prefix (a turn ending on a `tool` message) raises on one rank only, and a per-rank
                # raise here desyncs the ranks into a collective hang.
                try:
            ...
            if len(prompt_ids) + len(comp) > context_limit and self._batch_build_error is None:
                ...
                # replace the root cause the user needs to see.
                self._batch_build_error = (
            ...
            # RECORDED, not raised: a per-rank raise here desyncs ranks into a collective hang.
            turn_routing = None
        ...
        if not rows:
            if excluded_unusable:
                # Re-tokenizing would weight EVERY assistant span, truncated ones included — the
                # exclusion above inverted into full-weight training on exactly what it suppresses.
