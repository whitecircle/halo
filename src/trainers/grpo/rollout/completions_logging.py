...
under one ``log_completions`` flag. :func:`emit_completion_artifacts` splits them — parquet + backend
table gated by ``save``, console table by ``console`` — so a run can keep the record without console
spam. Built from TRL's ``_logs`` (detokenized text, rewards, advantages, extras, images).
"""
...
# Cap per-cell text in the backend table (parquet keeps full text); untruncated ~0.5 MB cells stall the log call.
