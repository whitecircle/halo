...
# Output-queue delivery slice, retried until the stop event fires — a completed batch is never dropped.
...
class _RolloutStartCallback(TrainerCallback):
    """Starts generation at train-begin, which is the first hook past the RESUME restore.

    ``_init_async_components`` runs before ``Trainer.train``, i.e. before the checkpoint restore, so a
    first weight push issued there ships the PRE-restore weights: on a resumed LoRA run that is the
    zero-init adapter merged into the base, and every rollout until the next scheduled sync is drawn
    from a policy the trainer is not training. ``on_train_begin`` fires after both restore paths
    (``train``'s own and ``_prepare_for_training``'s FSDP/DeepSpeed one) and still ahead of
    ``eval_on_start`` and the first generation — and on every rank, which the push requires of a
    collective gather.

    Weak reference: the trainer owns the callback handler that owns this, and the push needs the
    trainer only while it is training.
    """
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        ...
        if trainer is None:
            # Silence here is every rollout of the run generated from the engines' launch weights.
            raise RuntimeError(
...
class AsyncRolloutMixin:
    ...
    Owns ``self._rollout_manager``/``self._weight_sync_client``/the prefetch queues and threads,
    ...
    def _rollout_engine_name(self) -> str:
        ...
        Every user-facing string in this mixin routes through it: the whole path is engine-generic
        and an SGLang run must not be told its "vLLM" sync failed.
        """
    ...
    def _init_async_state(self):
        ...
        self._loop = None
        # Two distinct questions: which client shape to build (a configs list, however long, carries
        # its own per-server url/ports) and how many engines actually serve — a one-entry list is a
        # single server dressed as a list.
        ...
        # The step at which the sync path was last ENTERED — what keeps it out of every microbatch of
        # one optimizer step. Not "the step the engines hold": the cadence gate
        # (sync_weights_every_n_steps) declines most steps, and only the push's own verdict says
    ...
    def _init_async_components(self):
        ...
        # Main-process only, and a bare rank-0 raise would strand peers at the first collective gather.
        error: str | None = None
        if self.accelerator.is_main_process:
            try:
    ...
    def _start_rollout_generation(self):
        ...
        The first thing that must be true before any rollout: the engines serve THIS trainer's
        weights, restore included. Called from :class:`_RolloutStartCallback` rather than from
        ``_init_async_components`` for that reason, and idempotent so a re-entry cannot push twice.
        """
        if self._rollout_generation_started:
            return
        self._rollout_generation_started = True
        # Stamped even though this push is forced: the first training step is at the SAME
        # global_step on a resumed run, and re-entering the path there would push the very weights
        # just sent.
    ...
    def _sync_weights_to_engine_fenced(self, force: bool = False) -> bool:
        ...
        server, trainer and engine on one GPU) is a bare rank-0 raise that strands every peer in the
        next collective until the watchdog. Both callers — the train-begin push and the per-step
        sync — need that fence, so it lives with the push rather than being written twice.
        """
    ...
    def _cleanup_async_components(self):
        ...
        self._loop = None
        # Re-arm: a second train() rebuilds the components above and owes the engines a fresh push,
        # at whatever step it starts from.
        self._rollout_generation_started = False
        self._last_sync_attempt_step = -1
    ...
    def _prefetch_worker(self):
        ...
        ``(prompt_count, None)`` failure marker when the collection raised — so the trainer-side
        in-flight accounting stays exact and a blocking consumer can never wait on a vanished batch.
        """
    ...
    def _put_prefetch_output(self, item: tuple[int, list[RolloutResult] | None]):
        """Deliver a completed (or failed) prefetch to the output queue, never silently discarding:
        a dropped item would strand the trainer's in-flight accounting (and throw away trained-for
        rollouts). Blocks in stop-aware slices; only shutdown abandons delivery (logged)."""
        while not self._prefetch_stop_event.is_set():
            try:
    ...
    def _wait_for_inflight_prefetch(self) -> list[RolloutResult] | None:
        ...
        A wedge is RECORDED in ``_batch_build_error``, not raised: prefetch hit/miss, the in-flight
        ...
        ``_raise_batch_error_uniformly`` then fails every rank together and names this one.
        """
    ...
    def _submit_for_prefetch(
        ...
        A skipped submission is LOUD: on the steady-state hit path this round's prompts train only
        via the prefetched set, so silently dropping them here would silently drop them from
        training. The bounded blocking put cannot deadlock — see ``_PREFETCH_SUBMIT_TIMEOUT_S``.
        """
        try:
    ...
    def _sync_cadence_declines(self, force: bool) -> bool:
        ...
        Single home for the predicate: both push paths gate on it, and the collective one must
        evaluate it identically on every rank or the ranks split over a gather.
        """
    ...
    def _sync_weights_to_engine_single(self, force: bool = False) -> bool:
        ...
        the sync. Returns whether weights were actually pushed — the caller must not record a sync
        the cadence gate declined.
        """
        if not self.accelerator.is_main_process:
            return False
        ...
        # Rolling sync lives on the raw-model client API, which forwards ``named_parameters()``
        # untouched — reachable for plain, adapter-free dense models only.
        if self._multi_server_mode and not named_ep_layers(unwrapped_model) and not is_peft_model(unwrapped_model):
            ...
            # wrappers are present even at ep_size==1), PEFT (the gather merges the adapters; the
            # engine serves the merged base), and plain dense — where ``update_model_params`` would forward raw
            # ``named_parameters()``, which FSDP2 hands out as DTensors describing a LOCAL shard under
            # the GLOBAL shape, so the broadcast reads past the shard.
            sync_weights_to_client(
                unwrapped_model,
    ...
    def _sync_weights_to_engine(self, force: bool = False) -> bool:
        ...
        route through the all-ranks collective gather. Returns whether weights were actually pushed,
        which is what the caller records — the cadence gate declines most steps, and a stamp taken
        on the attempt claims the engines hold weights nobody sent them.
        """
        ...
        # Step gate must be evaluated identically on ALL ranks — collectives need all to participate.
        if self._sync_cadence_declines(force):
            return False

        # Called on every rank of the collective path: the method is main-process-gated and
        # idempotent, so a caller-side rank gate here would only put the fence on the wrong side of
        # a push every rank has to enter.
        self._init_weight_sync_client()
