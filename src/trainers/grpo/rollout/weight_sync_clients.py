...
def _fetch_served_max_model_len(client_cls: type[BaseWeightSyncClient], url: str) -> int | None:
    ...
    card without ``max_model_len``) — a preflight probe must never be what fails the run."""
    client = None
    try:
...
def verify_context_window(
    ...
    raises. ``full_trajectory_tokens`` (worst-case multi-turn budget) is advisory — a rollout growing
    past the context OOMs the trainer's forward before the fail-on-overflow check — so this warns.
    """
    for url in urls:
...
def verify_context_window_synced(
    ...
    runs on every rank — it reads no server, and a rank-0-only raise there would strand the peers in
    the broadcast below.
    """
...
class InferenceClientManager:
    ...
    def __init__(
        ...
        declares no ``group_port`` of its own as ``base_group_port + N`` — the run's
        ``vllm_group_port``, so the knob means the same thing whichever client shape is built.
        ...
        # Bytes buffered since the last chunk went out — the manager owns the chunk decision because
        # only it knows when every server is done with the shared snapshots.
    ...
    def update_model_params(self, model: torch.nn.Module):
        ...
        for i, client in enumerate(self._clients):
            ...
            try:
                ...
                # The raise alone does not name which of N servers refused the weights.
    ...
    def update_named_param(self, name: str, weights: torch.Tensor):
        ...
        pinned host RAM stays ~1× chunk rather than N_servers× (host OOM at 4×100B+), and a client's
        flush/clear drops only its own references.

        The chunk decision is the MANAGER's, not each client's: the shared snapshot may only be
        recycled once every server has sent the chunk holding it, and only this loop knows when that
        is true. So the buffer is drained here — every server, concurrently — and the pool released
        after, which is what keeps the host footprint at one chunk instead of one model.
        """
        if not self._initialized:
            raise RuntimeError("InferenceClientManager not initialized. Call init_communicators() first.")
    ...
    def reset_prefix_cache(self):
        ...
        Servers flush CONCURRENTLY — each client owns its own NCCL communicator and per-call streams —
        so the trainer stall is ~max(per-server flush) instead of the sum. The async D2H snapshot
        copies are completed before any producer thread reads host memory. Returns only once every
        flush is complete: the invariant that makes the pooled host buffers safe to recycle.

        Raises RuntimeError if a server still fails after ONE reconnect + re-flush attempt (a
        restarted engine container cannot rejoin the old NCCL group — see ``reconnect_client``);
        swallowing it would leave that server serving stale-policy rollouts.
        """
    ...
    def _with_recovery(self, operation: Callable[[int], None], what: str, index: int) -> str | None:
        """Run one client's step; on failure make ONE reconnect + retry attempt where that can work.
        ...
        when the retry also fails — clearing first would lose the very params the retry needs.

        A client that has already streamed a chunk cannot be recovered: the trainer keeps no copy of
        what went out, so a fresh engine would receive the remainder alone and serve a model that is
        part old and part new while answering ``/health``. Reported instead, with the reason.
        """
        ...
        if not client.can_replay_sync:
            client.drain_param_buffer()  # dropped, not re-broadcast: nothing can replay this sync
            return (
    ...
    def reconnect_client(self, index: int):
        ...
        configured ``group_port`` (retiring the old client releases that listener, and the port is the
        one the engine node is configured to reach): retire the old client (draining its pending buffer
        and lifting any pause it left on the server), wait for the server (the client constructor's
        ``check_server`` with ``connection_timeout``), form the NCCL group on the trainer device, move
        the drained buffer onto the new client, and swap it into the pool. Returns the new client;
        raises if the server stays unreachable.

        The replacement starts on base-checkpoint weights, so it is only usable inside
        :meth:`_with_recovery`, whose retained buffer IS this sync's full param snapshot
        and is re-flushed immediately; any other caller owes the new client a full push of its own.
        """
        ...
        # Retire the old client FIRST: it owns the /resume for the pause its failed sync left behind.
