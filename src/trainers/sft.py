...
# The CP metric row deferred to log time: every column is a SUM, which is what makes one reduce per
# log equal to one reduce per micro-batch. Fixed width, so the collective's shape never depends on
# what a rank happened to accumulate.
_CP_METRIC_COLUMNS = (
    "microbatches",
    "correct_tokens",
    "loss_tokens",
...
class DistributedSFTTrainer(DistributedTrainerMixin, SFTTrainer):
    ...
    def _loss_is_own_mean(self) -> bool:
        ...
        token-scaled loss is what this trainer wants."""
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        """Validate inputs carry training signal; in CP mode reconcile local-chunk logits with full-sequence labels (metrics computed on the local chunk)."""
    ...
    def _compute_cp_metrics(self, outputs, full_labels, full_attention_mask=None):
        ...
        Per-micro-batch this stays LOCAL — no collective and no ``.item()``. Each quantity is a plain
        sum, so folding the micro-batches into one on-device row and reducing that row once per log
        (:meth:`_drain_cp_metrics`) gives the same totals as reducing every micro-batch separately,
        for one collective per log instead of five or six per micro-batch.
        """
        ...
        with torch.no_grad():
            ...
            columns = {
                # Whether this window saw ANY CP forward, world-wide — the drain emits nothing
            ...
            # Stacked BY NAME, so a renamed or dropped column raises here instead of transposing two
            # metrics into each other's slot; the width check catches the other direction, a column
            # added to the dict alone, which the stack would drop and the drain's zip never see.
            # Cast per column, not on the stack: the counts are int64 and the sums float, and
            # torch.stack refuses a mixed-dtype list outright.
    ...
    def _drain_cp_metrics(self):
        ...
        the sum is ever used, and an all-gather would put an O(world) tensor on every rank.

        The emitted ratios are token-weighted over the window rather than an unweighted mean of the
        per-micro-batch ratios. Identical whenever the micro-batches carry equal token counts — which
        CP's pad-to-a-multiple-of-cp_size sequences do — and better defined when they do not.
        ``num_attended_tokens_seen`` is a pure sum, so it is exactly unchanged.
        """
        ...
        # AFTER the reduce, and read off the reduced count, so the decision is world-wide and every
        # rank makes it identically. A window with no CP forward anywhere (the final log of a run,
        # or a mode that never ran) must emit nothing — a 0.0 accuracy is a data point, not a gap.
    ...
    def _should_skip_eval_logits(self, prediction_loss_only: bool, inputs) -> bool:
        ...
        so eval otherwise materializes full ``[B, S, vocab]`` fp32 logits (an OOM the training step
        never hits). Every clause names a consumer that still needs real logits:
        ``prediction_loss_only=False`` (the eval loop returns them), ``_loss_reads_logits`` (a
        subclass builds its own loss from ``outputs.logits`` and forwards ``inputs`` onward WITHOUT
        labels, where ``skip_logits=True`` makes lce_forward raise), CP mode (``_compute_cp_metrics``
        reads them), and no Liger / no ``skip_logits`` parameter (nothing to route).
        """
        return bool(
