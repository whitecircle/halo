...
# Read off each loss class's own signature: a hand-listed set silently drops `scale` the moment
# sentence-transformers adds or removes the parameter on a registered loss.
...
class EmbeddingTrainer(DistributedTrainerMixin, SentenceTransformerTrainer):
    ...
    def _validate_injected_lora_parallelism(self) -> None:
        ...
        of a plain gathered state dict (:func:`_merge_injected_lora_state_dict`), which under EP holds
        this rank's expert shards under their local names — a checkpoint no loader accepts. TP is
        rejected one level up by the mixin's LoRA gate. Runs after wrapping, on the live model, so a
        future injection site is covered without registering itself here.
        """
    ...
    def _should_capture_embedding_metrics(self) -> bool:
        ...
        # max_steps there, and max(0.1, 1) would silently capture on EVERY step — a no-grad encode,
    ...
    def _encode_and_compute_metrics(
        ...
        # NOT try/except: ``model(features)`` is a collective — swallowing on one rank hangs its peers.
    ...
    def _cap_group_samples(self, features: dict[str, Any]) -> dict[str, Any]:
        ...
        sentence-transformers emits packed/varlen features instead (``cu_seq_lens_q`` offsets over a
        single flattened row, plus scalar ``max_length_q`` and string ``modality`` entries): there dim 0
        is not the sample axis, a positional slice would desynchronize the offsets from the tokens, and
        the non-tensor entries cannot be sliced at all.
        """
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        ``dp_size`` < ``world_size``. A pre-sharded dataset takes it too — ST would re-shard an
    ...
    def _checkpoint_context(self) -> CheckpointContext:
        ...
        ``SentenceTransformer`` ``nn.Sequential`` — the savers must gather and write the
        ``auto_model`` backbone instead, and the tokenizer lives on the ST's first module when the
        trainer carries none.
        """
        model = self._top_level_model()
    ...
    def _save_distributed_embedding_model(self, ctx: CheckpointContext, output_dir: str, _internal_call: bool = False):
        ...
        Everything but in-place-injected LoRA goes through the shared ``save_checkpoint`` ladder,
        so embedding exports get the same save dtype, hub expert layout, shard size and ``.bin``
        fallback as every other trainer. Injected LoRA is the one genuine difference: the model is not
        a ``PeftModel``, so the adapters must be folded into the gathered dict before it is written.

        Every branch gathers on EVERY rank (collectives) and writes only on the save rank.
        """
        backbone = ctx.model

        # Fenced: one rank writes while all must reach the trailing barrier — an ENOSPC would strand peers.
        with barrier_on_exit():
            if self._has_injected_lora(backbone):
                ...
                # No active parallelism left to gather: ST's own writer owns the plain-tensor layout.
                # Not the mixin's save_model — that would rebuild a context around the ST Sequential.
