...
Subclass alongside a Trainer. The mixin creates the accelerator and sets up FSDP, so subclasses
must not override ``create_accelerator_and_postprocess()``. The sibling sub-mixins (gradient sync
and clipping, checkpointing, dataloaders, EP introspection, validation, pipeline) compose into the
class below.
"""
...
def thin_memory_margin_message(peak_bytes: int, total_bytes: int, rank: int) -> str | None:
    ...
    device free, else ``None``. Kept free of trainer state so the threshold is CPU-testable."""
...
def oom_banner(rank: int, exc: BaseException) -> str:
    """Banner text for a rank-local OOM inside a distributed step.
    ...
    Xid 43. Those secondary errors read like a compute bug, so this banner marks the primary
    failure.
    """
    return (
...
def emit_primary_failure(rank: int, exc: BaseException) -> None:
    ...
    ``main_process_only=False`` is required: this module logs through accelerate's
    ...
    leaving a post-mortem with peer watchdog timeouts and no primary cause.
    """
...
class DistributedTrainerMixin(
    ...
    def _pp_loss_adapter(self) -> PPLossAdapter:
        ...
        The default is the causal-LM contract (summed shifted CE, token-count normalizer), which is
        what SFT needs. A trainer enabling ``_supports_pp`` with a different objective overrides
        this to declare its loss, pair layout, batch transform, normalizer and per-example side
        tensors (see :class:`~src.distributed.pipeline_parallel.losses.PPLossAdapter`).
        """
    ...
    def _init_distributed_config(self, kwargs: dict, training_args=None, ctor_args: tuple = (), **explicit) -> dict:
        ...
        # The PP split gate and _validate_router_aux_loss_consumable key raise-vs-warn off this value.
    ...
    def _configure_mixed_precision(self, kwargs: dict, training_args: Any):
        ...
        (``self._bf16_optimizer``). Whether the selected stock optimizer can be built over a mixed
        plain/DTensor parameter set is decided later, in create_optimizer."""
        ...
        if bf16_optimizer_explicit is not None:
            ...
            # Replicated DDP is outside the validated SR matrix (FSDP/EP/TP/HSDP).
    ...
    def _disable_dropout_for_onpolicy(self):
        ...
        recomputed log-prob (Jensen), biasing the importance ratio, the k3 KL estimator and the PPO
        ratio. Idempotent.
        """
    ...
    def _setup_distributed_modes(self):
        ...
        # bucketed grad sweeps in this mixin (the TP replicated sweep, the QLoRA sweep) read
        # HALO_GRAD_BUCKET_MB over groups that span OS nodes, and neither goes near DeepEP: a
        # trainer built without the script scaffold would otherwise have no uniformity guard.
        verify_rank_uniform_env()
        ...
        if config.needs_ep_wrappers:
            ...
            if self._has_ep_layers:
                # The EP capacity generation rides the module the loop calls. EP patching registers
                # it on the model it patched, which a PEFT wrapper then reaches through .forward(),
        ...
        # FSDP2 replaces managed Parameter objects, so an id memoized pre-wrap names a dead object.
        ...
        # FSDP2's dynamic subclass is absent from _CAN_RECORD_REGISTRY, so router logits go uncaptured.
    ...
    def _validate_merge_expert_lora_save(self):
        ...
        - **No native expert adapters built** (dense models, MoE runs with no expert projection in
          ``lora_target_modules``, ``expert_tp_size > 1``). There is nothing to fold, and an
    ...
    def _require_tp_mesh(self) -> None:
        """The TP mesh names the TP group; without it every TP reduce becomes a no-op.

        ``tp_group()`` would return None, so the replicated-grad sync returns early (replicas drift
        apart across the TP axis) and the TP shard-norm bucket is never reduced. Nothing downstream
        can distinguish that from "no TP", so it raises here.
        """
    ...
    def _reject_fsdp_knobs_under_qlora(self) -> None:
        ...
        the FSDP-shaping knobs have nothing to act on. A multi-node run asking for ``use_hsdp``
        would otherwise do a flat replicated all-reduce instead of the topology the config names.
        """
    ...
    def parallel_dims(self) -> ParallelDims:
        """Typed view over the active device mesh, rebuilt each access from current state (cheap; it
        holds a reference) so it cannot go stale across the setup lifecycle. EP/ETP groups are read
    ...
    def _reject_cp_incompatible_collator(self, collator: Any):
        ...
        ``collators/factory.py`` applies this rule for configs that go through it; this covers a
        collator handed straight to the trainer. Ulysses drops ``attention_mask`` and calls
        flash-attn with no ``cu_seqlens``, so packed documents attend across each other and the loss
        differs from the non-CP run. Matched by ``isinstance`` against the two roots, which the
        completion-only variants subclass.
        """
    ...
    def _move_model_to_device(self, model, device):
        ...
        No loader path leaves either behind, so a tensor arriving here on meta holds no values;
        allocating it would train or score uninitialized memory that also differs per rank (a zero
        inv_freq degenerates RoPE to NoPE at a plausible loss). This is the last place to catch it.
        """
        meta_buffer_names = []
        meta_param_names = []
    ...
    def _top_level_model(self) -> nn.Module:
        ...
        ``keep_torch_compile=False``: accelerate otherwise keeps the ``OptimizedModule``, whose
        parameters are named ``_orig_mod.*``, so a save through it writes a checkpoint no loader can
        read and an isinstance check for the CP wrapper fails.
        """
    ...
    def get_total_train_batch_size(self, args) -> int:
        ...
        ``get_sp_size()``, which read state the toolkit never sets, and carries no PP or ETP term,
        so every non-DP axis would count as data parallelism. Step and epoch counts come from the
        DP-sharded dataloader and are unaffected; the "Total train batch size" banner,
        ``num_train_samples`` and ``train_samples_per_second`` under ``max_steps`` read this.
        """
    ...
    def _configure_fp32_output_conversion(self) -> None:
        ...
        upcasts the full ``[B, S, V]`` logits. bf16 compute comes from the FSDP mixed-precision
        policy (or from bf16 weights), not from autocast, so dropping it changes only the output
        dtype. Under fp16 it must stay on: ``native_amp`` also gates ``unscale_gradients``, so
        clearing it would leave clipping and the optimizer step on GradScaler-scaled gradients.
        """
    ...
    def _warn_knobs_unapplied_under_accelerate(self) -> None:
        """Warn for parallelism knobs an accelerate-managed FSDP/DDP launch drops.
    ...
    def _mixin_manages_gmm(self) -> bool:
        ...
        accelerator / skip-DDP path, which would convert the user's DDP choice into mixin FSDP2.
        """
    ...
    def _refuse_stock_optimizer_on_mixed_params(self, optim: str) -> None:
        ...
        parameter set. Refused at build time rather than at trainer construction, since only the
        optimizer is affected; ``train()`` builds it before the first step, so the run fails within
        seconds of starting.

        The predicate is read from the config so every PP stage agrees; a module walk would refuse
        on MoE stages while dense stages walked into the collective. At ``ep_group_size == 1`` with
        ``fsdp_shard_ep1_experts`` (the default) FSDP2 shards the experts too, and a MoE family with
        no registered EP wrapper class never gets wrapped; both are uniformly DTensor and step fine.
        ``fp32_non_ep_params`` routes to the tensor-type-grouped builder above instead.
        """
        config = self.parallelism_config
    ...
    def evaluation_loop(self, *args, **kwargs):
        """The eval loop's cross-cutting seams, one from each of two sub-mixins.
    ...
    def evaluate(self, *args, **kwargs):
        ...
        if (
            ...
            if getattr(self.args, "top_entropy_quantile", 1.0) < 1.0:
                # TRL's entropy threshold pads-and-gathers through this same accelerator; identity padding aborts.
                raise ValueError(
    ...
    def _validate_inputs(self, inputs: dict[str, Any]):
        ...
        immediately) and the ``.all()`` reduction is a device-to-host sync not worth paying every
        step.
        """
    ...
    def cleanup_ep(self):
        ...
        barrier()
        count = 0
        # Through the EP-layer registry rather than a hasattr check on ``dispatcher``, which matches too widely.
        for module in self._find_ep_modules():
            try:
        ...
        # Layers already collected freed nothing (a finalizer must not run a collective); swept here instead.
