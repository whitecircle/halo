...
def ctor_positions(trainer_cls: type, *names: str) -> dict[str, int]:
    ...
    slot with it; a stale index would hand :func:`ctor_value` a neighbouring argument.

    Raises:
        ValueError: a name is absent from the signature, meaning the installed TRL no longer matches
            the trainer's ctor gate; guessing a slot would gate on the wrong argument.
    """
...
class ParallelismValidationMixin:
    ...
    def _validate_expert_lora_peft_config(self):
        ...
        handed a ``peft_config`` directly (the documented route for AdaLoRA/(IA)3), which never
        passes through it. Checked here against the live config so both routes land on the same gate;
        a setting that reached only the attention half would leave two adapters under one recipe.
        """
    ...
    def _validate_lora_dropout_live(self):
        """Warn when the configured ``lora_dropout`` has been zeroed by the trainer.

        TRL's ``disable_dropout_in_model`` zeroes every ``nn.Dropout`` in the model, and PEFT builds
        ``lora_dropout`` as one. It runs after the adapter wrap on the preference, reward and
        offline-GRPO trainers (``disable_dropout`` defaults to ``True``), so the YAML value is inert
        there while it applies on SFT. Intended, for reference-vs-policy determinism.
        """
        peft_model = find_peft_model(self.model)
        spec = self.parallelism_config.expert_lora
        configured = max(
    ...
    def _validate_expert_lora_realized(self):
        """A peeled ``ExpertLoraSpec`` must materialize as EP grouped adapters, otherwise the expert
        LoRA targets are never built (e.g. MoE with ``use_grouped_gemm: false`` and no EP)."""
        spec = self.parallelism_config.expert_lora
        if spec is None:
            return
        model = self._top_level_model()
    ...
    def _validate_router_aux_loss_consumable(self):
        ...
        never enters the graph while the run still pays for ``output_router_logits``. A trainer can
        also re-add the term itself (TRL's ``KTOTrainer`` does), so consumption is declared per
        trainer class (``_consumes_router_aux_loss``) rather than inferred from the forward
        signature. Same contract :mod:`src.distributed.pipeline_parallel.split` enforces for a stage.
        """
        ...
        if self._moe_balancing == "auto":
            # ``auto`` set the flag before this trainer was built; clear it rather than pay for the tensor.
    ...
    def _validate_load_best_model_reloadable(self):
        ...
        reload in place. Under EP/CP a full fine-tune saves checkpoints that ship base weights,
        which the checkpoint loader refuses to reload, so raising here fails at setup rather than at
        the end of training.
