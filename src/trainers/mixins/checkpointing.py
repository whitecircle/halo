...
class CheckpointingMixin:
    ...
    def _load_best_model(self) -> None:
        ...
        the live model's names are EP-fused, CP-wrapper-prefixed or stage-local, so every key lands
        in ``unexpected_keys``, ``strict=False`` accepts them, and the run keeps the last weights,
        which ``save_model`` then exports as the best. Routing it here reuses the same name/shape
        handling as resume.
        """
    ...
    def _save_checkpoint(self, model, trial):
        ...
        optimizer view, so its optimizer.pt is replaced with per-rank shards plus the
        topology-fingerprint meta (see :meth:`OptimizerShardStore.save`). Non-sharded modes (single
        process, replicated DDP, ep_size==1 grouped-GEMM MoE without FSDP2) keep the base Trainer's
        optimizer.pt, whose replicated state round-trips as-is.

        Checkpoint rotation (the base save's last step, an rmtree of old checkpoints) is neutralized
        ...
        # base save calls this class's ``save_model``, whose saver gathers are world collectives, so
        ...
        # The base's optimizer.pt/.bin is rank 0's view alone, wrong under sharding. It is deleted
        # only now, with its replacement shards and meta on disk, so there is never a window with
        # neither. Both spellings, since the resume side checks both. FS-aware save rank, not rank
        # 0; fenced because a raise on one node's writer would block peers in the next collective.
    ...
    def _rotate_checkpoints_after_sidecars(self, trial) -> None:
        ...
        ``finally``: after a failed save the old checkpoints are the only complete ones, and
        rotating would delete them in favor of the torn new one.
        """
    ...
    def _persist_router_balancing_biases(self, output_dir: str) -> None:
        ...
        model checkpoint; without this a resumed run re-inits them to zero. Written on the FS-aware
        save rank(s); no-op when the model has no balancing router.

        Under PP each stage holds different routers, so local names are mapped through the stage's
        ``global_parameter_name`` and gathered to rank 0, which merges them and broadcasts the merged
        dict under unsplit-model names; stage peers contribute replica-identical duplicates that
        merge idempotently. Gather-then-broadcast rather than ``all_gather_object``, which would make
        every rank materialize one unpickled bias dict per rank.

        Fenced like every other sidecar write, since it runs before the saver's own collectives: an
        ENOSPC on one node's save rank would otherwise raise here while its peers proceed into them.
        """
        ...
        if self.parallelism_config.is_pp_mode:
            ...
            # Non-None on rank 0 alone: what gather_object requires, and it keeps the per-rank
            # copies on the single rank that merges them.
    ...
    def _restore_router_balancing_biases(self, checkpoint: str) -> None:
        ...
        When the file is absent on every rank, warns if the model carries balancing biases and
        returns. Copies each saved bias back in place; a name absent from the current model is
        skipped, and a live router missing from the file warns and keeps its zero-init biases. A
        sidecar matching no live router also warns, since the trained biases are then dropped (what
        a weight-sync RL leg does to a ``bias_update`` checkpoint). A saved bias whose shape differs
        from the live one raises (consensus'd), since ``copy_`` would broadcast it. The read goes
        through :func:`consensus_read`, so it is all-or-nothing across ranks: a node with a missing
        or torn file would otherwise keep zero-init biases while its peers restore.
        """
        ...
        for name, module in model.named_modules():
            ...
            # bias with a single saved entry instead of failing, affecting every routed token.
        ...
        # rank-local raise here would leave the peers blocked in the next collective.
        ...
        # A collective, entered by every rank: under PP a stage may legitimately hold no router, so
        # only a world-wide zero means the sidecar matched nothing.
    ...
    def save_model(self, output_dir: str = None, _internal_call: bool = False):
        ...
        # reads the wrong tensors, and the PEFT path additionally mis-routes on them: its DTensor
        # probe sees plain tensors, takes the rank-0 ``save_pretrained`` branch, and PEFT's
