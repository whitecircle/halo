...
class DataParallelDataLoaderMixin:
    ...
    def _cached_eval_dataloader(self, eval_dataset, build) -> DataLoader:
        """Resolve the eval split and hand it to ``build``, or reuse the loader already built.

        The persistent-worker cache holds the prepared loader (base ``Trainer`` semantics), since
        rebuilding one per ``evaluate()`` leaks a worker pool. The key is the split name, so a hit
        or a miss is the same on every rank and ``build``, which issues collectives, stays in
        lockstep.
        """
    ...
    def _loader_params(self, dataset, batch_size: int, description: str) -> tuple[Any, dict]:
        ...
        differ per split and stay with each caller.
        """
    ...
    def _needs_custom_dataloader(self) -> bool:
        ...
        batch. ``get_data_parallel_rank`` is stage-local, so the custom path gives all chain members
        the same shard.
        """
        return (
    ...
    def _install_dp_metric_gather(self) -> None:
        ...
        return the same rows. A world gather therefore repeats every replica once per sibling, and
        accelerate's end-of-dataloader remainder trim then keeps a prefix of the duplicates instead
        of the real tail, so a partial final batch reports a different sample set.
    ...
    def _rearm_dp_metric_gather(self) -> None:
        ...
        on its way out, so the install done once at construction covers the first loop only.
    ...
    def _dp_gather_for_metrics(self, input_data, **kwargs):
        ...
        The order matters because accelerate's trim keeps the first ``remainder`` rows of the
        gathered tensor, which under duplication are the wrong ones.
        """
