...
``gradient_accumulation_steps=1`` — the pipeline's microbatches ARE the accumulation, and the
user's configured accumulation is consumed as the microbatch count.
...
# Below _BUBBLE_WARN_FACTOR·pp microbatches the 1F1B bubble exceeds ~25% of the step (measured 22%
# at 2·pp on B300); _BUBBLE_TARGET_FACTOR·(pp-1) is the planning rule for a <10% bubble.
...
def pp_pad_values(pad_token_id: int, extra: Mapping[str, int] | None = None) -> dict[str, int]:
    ...
    ``PP_BATCH_PAD_VALUES`` leaves the pad id as ``None`` — it is the one tokenizer-specific fill —
    and an adapter's ``pad_spec`` adds the seq-shaped extra targets it ships.
    """
...
def wrapper_state_outside_stages(model) -> dict[str, torch.Tensor]:
    ...
    # An empty backbone prefix means the model IS the backbone: everything is stage-owned.
...
def collator_image_evidence(collator) -> str | None:
    ...
    Read off the columns it declares it needs — the same
    :data:`~src.data.vlm.VLM_IMAGE_COLUMNS` spellings the dataset probe reads — so
    a preprocessed-VLM collator whose dataset stores pixels rather than images is caught too.
    """
...
def admit_multimodal_wrapper(model, kwargs: dict) -> dict[str, torch.Tensor]:
    ...
    ``language_model`` — so the vision tower and projector, its SIBLINGS, are in no stage. For
    a run that feeds images that is fatal three times over, silently: the tower would get no
    gradient, ``pixel_values`` would be pruned by the runtime's column pin so no image reaches
    the model, and the placeholder tokens would train as text. Hence the refusal, naming the
    evidence — an image column or embedded image parts in any dataset, or a collator that
    consumes an image column (:data:`~src.data.vlm.VLM_IMAGE_COLUMNS`).

    A text-only run of the same checkpoint is the wrapper's text tower as the pipeline with the
    vision tower as dead weight: it is dropped from every stage, and its untouched tensors are
    kept on the save rank (returned here; the trainer holds them as ``_pp_wrapper_state``) so
    every checkpoint re-emits them under the wrapper layout — the export reloads and serves as
    the composite class with no reattach step, and a resume finds the tensors it plans for.
    """
    evidence = next(
        (
            found
    ...
    # Joined across ranks: the dataset probe is rank-local (a pre-sharded dataset can carry
    # images on one rank only), and a lone raise would strand the peers at the split's collectives.
...
def stash_wrapper_state(model) -> dict[str, torch.Tensor]:
    ...
    Cast here, through the caster built on the WHOLE wrapper: the artifact contract's keep-sets
    (its norms, fp32 pins) are tree-derived, and the stage the save holds no longer has the
    tower's modules to derive them from.
    """
...
class PipelineTrainerMixin:
    ...
    def _validate_pp_mode(self, ctor_args: tuple, kwargs: dict) -> None:
        ...
        Declared here so the gate has one name, one signature and one call site — driven by
        :meth:`_maybe_prepare_pipeline_model` once the PP early-out has passed, so an override needs
        no ``is_pp_mode`` guard of its own. Every ctor parameter is read out of ``kwargs``
        (``require_model_and_args_kwargs`` forces the positional ones there); ``ctor_args`` carries
        the trainer's remaining positionals for the slots TRL's signature allows. A trainer whose
        PP-relevant options are EXPLICIT ctor parameters cannot use this hook — the mixin never sees
        them — and gates them in its own ``__init__`` instead.
        """
    ...
    def _maybe_prepare_pipeline_model(self, kwargs: dict, training_args, ctor_args: tuple = ()) -> dict:
        ...
        # The name is what catches an UNREGISTERED VLM (remote code, a custom class), whose config
        # silence means nothing; passing the config keeps the probe off the hub.
        ...
        if context_window and training_args.max_length >= context_window:
            # `max_length: null` resolves to the context window, so every row would pay full-window
            # compute — a cost that, unlike dynamic padding, shows up as no shape the user can see.
            logger.warning(
    ...
    def _pp_wrap_collator(self):
        ...
        if isinstance(inner, DataCollatorWithFlattening):
            # Padding-free stays rejected on three independent counts: its width is the SUMMED true
            # document length (variable by construction, so no constant to pad to); its cu_seq_lens /
            # max_length_* side outputs are neither splittable on dim 0 nor re-basable per microbatch;
            # and its single row cannot be microbatched at all. Padding it to a constant would
            # reintroduce exactly the padding it exists to avoid. Packing gets the same throughput.
            raise ValueError(
    ...
    def _setup_pipeline_parallel(self):
        ...
        # Wrap FIRST: the collator decides which dataset columns it needs, and the pin below must
        # union them in. Pinning first would prune a packing collator's `seq_lengths`, which silently
        # collapses every packed row into ONE attended document instead of failing.
        self._pp_wrap_collator()

        # HF prunes dataset columns against the forward signature, and a stage's forward takes
        # hidden_states — without this pin, input_ids and labels are silently dropped.
        ...
        # UNCONDITIONAL, deliberately: create_stage_group issues pp_size collective dist.new_group
        ...
        # head weight's gradient is reassociated (see fused_causal_lm_token_loss), which is the price
        # of dropping the logits plane. HALO_PP_FUSED_HEAD_LOSS=0 restores the logits path.
        ...
        # this stage's rank block — a world reduce is a shape mismatch or a silent cross-stage blend.
    ...
    def _reshard_after_evaluation_loop(self) -> None:
        ...
        The forward-only drives leave the stage gathered across the loop's batches (one all-gather per
        eval, not per batch); everything after the loop — grad clipping, callbacks, the next optimizer
        step — must read the shards, which carry the pending gradients. Per-rank, no collective.
        """
    ...
    def _pp_clip_grad_norm(self, parameters, max_norm, norm_type=2):
        ...
        experts (ep1 ``fsdp_shard_ep1_experts`` grouped-GEMM MoE) partition the same way — their
        shards live on exactly one stage's mesh — so they need no separate expert leg here; stages
        holding plain-tensor EP expert shards use ``ep_clip_grad_norm_`` instead.
        """
        if norm_type != 2:
        ...
        # silently zero every grad. The clamped coefficient is applied unconditionally: no host sync.
    ...
    def _pp_default_normalizer(self, inputs) -> torch.Tensor:
        ...
        ``num_items_in_batch`` cannot supply this — a stage's forward has no loss kwargs so HF never
        computes it, and its world-gathered form would count every chain pp_size times. One scalar
        all-reduce over the stage group yields the stage sum; dividing by stage_world_size gives
        G/dp for any intra-stage composition (each DP replica appears stage_world_size/dp times in
        the sum). Trainers whose objective normalizes differently declare it on their adapter.
        """
    ...
    def _pp_training_step(self, inputs, num_items_in_batch=None) -> torch.Tensor:
        ...
        The schedule already ran backward for every microbatch, so no ``accelerator.backward`` —
        returning here hands HF a detached loss with gradients in place. ``num_items_in_batch`` is
        accepted for signature compatibility but deliberately unused: HF never computes it for a
        stage (no loss kwargs on the stage forward), and the normalizer below is derived
        first-hand instead.
        """
    ...
    def _pp_broadcast_output_from_last_stage(self, tensor: torch.Tensor | None) -> torch.Tensor:
        ...
        ``preprocess_logits_for_metrics`` cannot shrink what crosses here: HF applies it in its own
        evaluation loop, after ``prediction_step`` returns. The adapter's ``predictions_fn`` — which
        :meth:`_pp_hoist_metrics_reduction` fills from that very function — is the seam that reduces
        the output on the last stage instead.
        """
    ...
    def _pp_hoist_metrics_reduction(self, adapter: PPLossAdapter) -> PPLossAdapter:
        ...
        RAW output — ``[rows, max_length, vocab]``, 3.3 GB of bf16 per eval batch at vocab 201k and
        an 8192 window, allocated again on every other stage to receive it. HF applies
        ``preprocess_logits_for_metrics`` in its own evaluation loop, AFTER ``prediction_step``
        returns, so where it stands it cannot shrink that hop; run here it is the same function on
        the same tensor one stage earlier, and an argmax leaves 1/vocab of the plane on the wire. It
        is cleared afterwards so HF does not reduce twice (an argmax of an argmax).

        The reduction runs on the pipeline's FROZEN batch — the inert filler rows of a partial final
        eval batch are trimmed after the broadcast — so it must be row-independent. Argmax, top-k
        and every per-token reduction are.

        Resolved after the fused-head-loss decision, which is a property of the LOSS: the hoisted
        reduction shapes only the eval broadcast, and ``forward_only`` un-fuses for it either way.
        """
    ...
    def _pp_pin_metric_keys(self) -> None:
        """Pin the adapter's per-step metric names and prove every rank agrees on them.

        The per-step share broadcasts VALUES only, so a key set that differed across the chain would
        silently pair one stage's names with another's numbers (or hang on a length mismatch). The
        adapter's contract is that the names come from config, which makes them rank-uniform; this
        checks it once, here, instead of trusting it every step.
        """
        adapter = self._pp_adapter
        ...
        # Proves the trainer has somewhere to put them before the first step rather than at it.
    ...
    def _pp_broadcast_log_metrics(self, metrics: dict[str, float]) -> dict[str, float]:
        ...
        logs from — sits on the FIRST stage, so without this the last stage's numbers never reach a
        log line. One object hop per log step, not per training step.
        """
        shared = [metrics]
        dist.broadcast_object_list(shared, src=self._pp_last_stage_rank, group=self._pp_chain_group)
        return shared[0]
    ...
    def _pp_frozen_row_pads(self) -> dict[str, int]:
        ...
        that ``attention_mask`` fills with ONES — a fully-masked row would NaN the softmax, while
        unmasked garbage attention over pad tokens is deterministic and its labels are all ignore —
        and every extra target fills with 0 so it stays row-aligned with the padded batch (the
        runtime validates leading dims).
        """
    ...
    def _pp_pad_rows_to_frozen(self, inputs: dict, frozen_rows: int, pad_values) -> dict:
        ...
        Only keys listed in ``pad_values`` are padded; an over-frozen batch is a hard error — the
        ...
        for key, pad_value in pad_values.items():
            ...
            if key == "position_ids":
                ...
                # segment PER TOKEN, and the FA4 backward pays a fixed per-segment cost — a full
                # filler row would rebuild the 275x cliff on the eval path. The ramp keeps each
                # filler row a handful of no-op documents; their labels are all ignore regardless.
    ...
    def _pp_prediction_step(self, inputs, prediction_loss_only: bool, ignore_keys=None):
        ...
        schedule runs its own microbatched loss and no rank ever holds the merged output — the cheap
        default. With it, one forward-only pass yields the last stage's merged output, from which
        both the loss and the predictions derive.

        The adapter's ``predictions_fn`` is the seam that shrinks what crosses the chain, because it
        runs on the last stage before the broadcast; ``preprocess_logits_for_metrics`` is hoisted
        onto it at setup (:meth:`_pp_hoist_metrics_reduction`) and must not be re-applied here.

        A partial final eval batch is row-padded to the frozen batch size with the inert rows of
        :meth:`_pp_frozen_row_pads`: they contribute nothing to the loss, and the normalizer
        counts only real tokens.
        """
        del ignore_keys
        adapter = self._pp_adapter
        inputs = self._prepare_inputs(inputs)
        ...
        # preference adapter emits one row per PAIR, so slicing by the batch's row count keeps inert
        # pad pairs, which score as ties and silently inflate every partial final batch's metric.
