"""Expert-Parallel module/parameter introspection + EP-safe gradient checkpointing.

Identifies EP-patched modules and their params by module membership (not name matching, so shared
experts and dense MLPs are not misclassified), and owns EP GC setup with multi-group topology guards.
"""
...
def named_ep_layers(model: nn.Module) -> dict[str, EPMoELayerBase]:
    ...
    Single home for the walk (trainer introspection, the rollout weight-sync gathers, routing replay).
    ``isinstance``, not an ``ep_config`` probe: a PEFT ``modules_to_save`` wrapper forwards
    ``__getattr__`` to the EP layer it holds, so a probe matches the wrapper as well — and the
    wrapper's own type declares neither the family gather nor its export renames.
    """
...
class EpIntrospectionMixin:
    ...
    def _has_ep_layers(self) -> bool:
        ...
        Asked of the model directly rather than read off ``_ep_config``, which is populated only as a
        side effect of ``_find_ep_modules`` — and the CP-only and TP-only setup paths reach that call
        merely incidentally, through PEFT/EP validators that need not run. This property selects the
        checkpoint saver, the resume path and the EP grad-clip branch, so keying it on
        ``_ep_config`` would silently write MoE checkpoints through the wrong writer whenever that
        incidental call did not happen.
        """
        return bool(named_ep_layers(self.model))

    def _get_ep_param_ids(self) -> set:
        """Parameter ids owned by EP modules, by module membership (not name matching).

        Memoized — called every step (grad clipping, the deferred cross-node grad sweep, the TP
        pre-step hook). Ids are stable by the time any consumer runs: FSDP2 REPLACES managed
        Parameter objects rather than swapping ``.data`` in place, and every consumer (hence the
        lazy first population) runs at step time, after ``_setup_distributed_modes`` has finished
        wrapping. Do not call this before wrapping.
        """
    ...
    def _invalidate_param_id_caches(self) -> None:
        ...
        The ids are only meaningful against the Parameter objects that survive wrapping, so anything
        cached earlier — the fp32 upcast populates :meth:`_get_ep_param_ids` before FSDP runs — names
        replaced objects and would classify every EP param as non-EP (and, for the TP sets, every
        hand-sliced sink as replicated, averaging disjoint head slices across the TP group).
        """
    ...
    def _setup_ep_gradient_checkpointing(self):
        ...
        # Class-declared contract; absent attribute = supported.
        unsupported = sorted(
            {
