...
``DistributedTrainerMixin`` so its ``log`` flushes before ``super().log``.
"""
...
class StoredMetricsMixin:
    ...
    def log(self, logs: dict[str, float], start_time: float | None = None) -> None:
        ...
        Eval-bucket keys gain an ``eval_`` prefix (TRL convention) unless already prefixed —
        without it, eval flushes land on the same series as the train metrics.
        """
