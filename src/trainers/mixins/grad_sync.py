...
Mixed into :class:`~src.trainers.mixins.base.DistributedTrainerMixin` — a mixin class, not a function
module, because every method here reads the trainer's live parallelism state (``parallelism_config``,
``_ep_config``, ``_device_mesh``, ``_fsdp_wrapped``, the PP groups). The topology-independent pieces
every clip path shares live in the leaf :mod:`src.trainers.mixins.grad_clip`.
"""
...
def register_grad_sync_step_hook(
    ...
    zero-LR ``_init_optim_state`` step during optimizer-state restore — that step carries no
    gradients, and sweeping there makes the first real step's own guard fire against a phantom
    sweep. ``enabled`` is a callable so its state reads stay behind the ``optimizer is None`` check.
    """
...
class GradientSyncMixin:
    ...
    def _setup_qlora_gradient_sync(self):
        ...
        params that received a grad on THIS rank, so the all-reduce it issued had rank-local
        membership. A VLM whose vision adapters are in ``lora_target_modules`` and whose microbatch
        is text-only on one rank alone is enough to hang the job. Per-microbatch firing also ignored
        ``no_sync``, paying a full all-reduce per accumulation step.
        """
    ...
    def _sync_qlora_grads(self) -> None:
        ...
        skipped everywhere — materializing a zero grad there would hand the optimizer weight decay
        and momentum on a parameter it should have left alone.
        """
    ...
    def _setup_backward_reshard_window(self) -> None:
        ...
        Empty — and :meth:`_set_backward_reshard` inert, leaving torch's always-reshard default — for
        every run but ``fsdp_reshard_after_backward=False`` on a wrap this mixin owns. Call once, after
        every wrap: ``fully_shard`` is what makes a module an ``FSDPModule``.
        """
    ...
    def _set_backward_reshard(self, reshard: bool) -> None:
        ...
        1..n-1 (the saving), on for the last. Left off, ``post_backward`` clears the unsharded params'
        ``.grad`` before reduce-scattering onto the sharded DTensors and never re-registers those, so
        ``model.parameters()`` yields grad-less objects the optimizer never captured (0 grad norm,
        nothing clipped, ``isinstance(param.data, DTensor)`` false in the deferred EP sweep) while
    ...
    def _setup_ep_gradient_sync(self) -> None:
        ...
        inspects every parameter's dtype, so deriving it twice doubles that pass over a 400B model.
        """
        config = self.parallelism_config
    ...
    def _patch_gradient_clipping_for_ep(self):
        ...
        def ep_clip_grad_norm_(parameters, max_norm, norm_type=2):
            ...
            # Structural, never grad-presence: the EP branch issues collectives, so a rank-local early return hangs.
            if not all_params:
    ...
    def _sync_deferred_expert_grads(self) -> None:
        ...
        across nodes) or re-fire per microbatch (PP). Each grad must end as the ``/world_size`` DP
        ...
        if replica_avg_grads and replica_group is None:
            ...
            # Unreachable today (these grads are collected only under is_deferred_dp, which is what
            # builds the replica group), so it is a contract, not a fallback.
            raise RuntimeError(
    ...
    def _sharded_grad_bucket(grad: torch.Tensor, tp_disjoint: bool) -> str:
        ...
        ``tp_disjoint`` is what separates the last two: after FSDP2 wraps a run, a hand-sliced sink
        and a TP replica are both 1D ``dp`` DTensors, so the mesh alone cannot tell them apart (see
        :meth:`_tp_sharded_plain_param_ids`).
        """
    ...
    def _reduce_shard_norm_buckets(
        ...
        different layers, so a world reduce would add their norms into this stage's bucket and then
        the chain reduction would count them again. ``_pp_stage_group`` is ``None`` without PP, which
        is the default world group — the stage IS the world at ``pp_size == 1``.
        """
    ...
    def _fsdp_shard_group(mesh) -> dist.ProcessGroup:
        ...
        Raises on any other shape rather than falling back to the default world group: that group is
        wrong wherever it differs from the mesh — under PP it spans every stage, so the reduce would
        mix stages and the chain reduce would then count them a second time.
        """
        if mesh.ndim == 1:
    ...
    def _canonical_fsdp_shard_group(self, model, sharded_expert_ids) -> dist.ProcessGroup | None:
        ...
        rank-consistent, unlike capturing from the first non-None grad). With no FSDP-sharded backbone
        param the reduce has nothing to sum (``fsdp_full_sq`` is 0 on every rank), and it falls back to
        the pipeline STAGE group — the world group at ``pp_size == 1``, like every other norm reduce
        here, and never a cross-stage reduce that would mix stages holding different layers.
        """
    ...
    def _compute_global_grad_norm(self) -> torch.Tensor:
        ...
        non-expert params are Shard DTensors — shard norms local, then all-reduce over the TP group.
        Expert-TP: expert shards summed within the expert-TP group; expert norms aggregate across
        sub-EP (dispatch) groups. Iterates the FULL top-level model so ``lm_head`` (untied models,
        accelerate clips it) contributes.

        Kept on device deliberately: reading it back would block the launch queue on every optimizer
        step, and the only consumer that needs a host value is the logging path, which HF already
        reaches on ``logging_steps`` alone.
        """
        ...
        # Expert legs read straight off the EP config — the axis that owns those groups. A dense or
    ...
    def _tp_sharded_plain_param_ids(self) -> set:
        """Ids of params TP sharded as PLAIN tensors — a disjoint slice that LOOKS like a replica.

        Every plan-driven TP shard is a DTensor whose grad reduces itself; the plain slices are the
        hand-sliced params in ``model._tp_sharded_non_dtensor`` (GptOss sinks, whose forward
        concatenates them with already-sharded logits) — the entries the save path all-gathers.
        Their grads must NOT be TP-averaged (that blends another rank's heads into every slice);
        their norms are summed over the TP group instead of counted as replicated. Memoized.
        """
    ...
    def _tp_per_head_norm_param_ids(self) -> set:
        ...
        Registered by :func:`apply_tp_to_attention_only` as NAMES (FSDP2 replaces Parameter objects
        after TP is applied). Their true gradient is the SUM over the TP group, not the average the
        replicated bucket applies — a colwise projection's ``use_local_output=True`` ends the DTensor
        graph, so nothing else reduces them. Memoized.
        """
    ...
    def _sync_tp_replicated_grads(self, params: list) -> None:
        ...
        * **SUM over the per-head attention norms** (:meth:`_tp_per_head_norm_param_ids`) — each rank's
          gradient covers only its own heads, so the true gradient is the sum, and DP-sharded ones are
          reduced on their local shard (TP siblings hold the same shard of the same replica).

        Plain tensors the TP plan sharded by hand (:meth:`_tp_sharded_plain_param_ids`) are excluded
        from both — their per-rank slices are disjoint, so an average would mix another rank's
        attention heads into every sink gradient. No-op without a TP group. Materialize a zero grad
        for a param missing one (a router tie-break can leave grad=None on one rank only) so the
        collective count stays structural.
        """
    ...
    def _compute_tp_grad_norm(self, params: list) -> torch.Tensor:
        ...
        Reduces run even with empty ``params`` (zero contribution) — the collectives must stay
    ...
    def _logs_after_this_step(self) -> bool:
        ...
        (an epoch-strategy log, a flag another callback already raised, the final step) answers
        True, so no logged metric is ever dropped; and the whole predicate reads only rank-uniform
        trainer state, so every rank answers alike.
        """
    ...
    def _get_grad_norm(self, model, grad_norm=None):
        ...
        ``max_grad_norm <= 0`` HF instead asks for an UNCLIPPED norm purely for the log line, and
        every distributed clip path answers it with ``torch._foreach_norm`` plus an all-reduce over
        the DP/EP/TP/PP groups — discarded on every non-logging step, once per rank per step.

        Returning ``None`` there only omits a metric nothing reads. The grad sweeps that clip path
        also carries (deferred EP, TP-replicated, QLoRA) are covered by their optimizer step-pre-hook
        backstops, which run before the optimizer regardless.
        """
    ...
    def _register_tp_replicated_grad_sync_hook(self) -> None:
        ...
        when ``max_grad_norm == 0`` — which would drift the replicated weights across TP ranks. This
        step-pre-hook covers that. Syncs over ``model.parameters()`` (robust to Muon's nested optimizer).
        """
        ...
        def _sync() -> None:
            ...
            # Exclude EP experts (TP averaging corrupts them); never gate on rank-local grad presence — it hangs.
            expert_ids = self._get_sharded_expert_param_ids()
