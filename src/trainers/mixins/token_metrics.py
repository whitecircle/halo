"""Loss-contributing token counting — the ``train/total_output_tokens`` series.

Two phases so no collective lands on the per-micro-batch path: each step folds this rank's count
into an on-device accumulator, and one gather per log step turns those into the run total.

Mixed into :class:`~src.trainers.mixins.base.DistributedTrainerMixin`; the drain reads the trainer's
...
class TokenMetricsMixin:
    ...
    def _extract_output_token_count(self, inputs):
        ...
        ``tool_mask`` when present — under TRL-native tool use and in the environmental trainer the
        loss mask is ``completion_mask ∧ tool_mask``, ``completion_mask`` alone being the
        attention-valid mask that still includes tool-output tokens), offline GRPO
        (``completion_attention_mask``). Returns ``None`` for sequence-level trainers (classification /
        reward / embedding) and preference trainers (chosen/rejected split until ``compute_loss``).
        """
    ...
    def _drain_unmasked_output_tokens(self):
        ...
        from ``_add_parallelism_to_logs``, which every rank reaches together, keeping the gather
        in lockstep.

        Participation is structural: a rank with nothing accumulated (a trainer whose batches carry
        no token-level loss mask, or a rank that logged before its first step) enters with a zero,
        never by skipping the collective. The run total stays unset until some rank contributes, so
        sequence-level trainers do not log a meaningless zero.
        """
