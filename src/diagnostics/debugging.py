...
``assert_consistent`` / ``assert_tensor_shape_consistent`` are manual instrumentation: the toolkit
ships no call sites, so ``HALO_TP_CONSISTENCY_CHECK=1`` (raise instead of warn) takes effect only
once a call is added to the code under investigation. The helpers are inert when unused.
"""
...
# Default py-spy destinations, on the node's own temp filesystem rather than under HALO_DATA_ROOT, so
# a hang diagnosis does not itself block on a shared filesystem. py_spy_diag.py reads them back.
...
# py-spy attach budgets: a dump snapshots an already-stopped process, while a record needs headroom
# on top of its own ``duration``. pgrep is a local /proc walk, so its budget elapses only on a wedged
# host.
...
def assert_tensor_shape_consistent(
    ...
    *,
    ...
    """``assert_consistent`` comparing only ``tensor.shape``, for TP/EP collective deadlocks, which
    usually start as a shape mismatch."""
...
def _hash_value(value: Any) -> str:
    ...
    Tensor bytes go through a ``torch.uint8`` view rather than ``.numpy()``: bf16/fp8 have no numpy
    equivalent, and a ``TypeError`` on one rank inside an ``all_gather_object`` would hang its peers.
    """
    ...
    if isinstance(value, torch.Tensor):
        ...
        # Recurse over every element; the type/length prefix separates [x] from [[x]].
