...
def ensure_artifact_dir(output_dir: str) -> None:
    ...
    Not ``fs_aware_makedirs``: its barrier would deadlock against the ranks that ``ranks``
...
def start_memory_history(max_entries: int = 100_000) -> bool:
    ...
    True if started; no-op (False) when CUDA/the recorder API is unavailable. Has measurable
    overhead — scope to a few steps.
    """
    if not torch.cuda.is_available():
        return False
