...
def save_ep_model(
    ...
    ``named_modules()`` walk yields wrapper-prefixed EP-layer paths that misclassify every expert
    param as non-EP). ``sharded=True`` writes one shard per rank, requiring merge_ep_shards.py before
    reload; ``max_shard_size`` then bounds the shards that merge writes, not these. ``sharded=False``
    ...
    if is_peft_model(model):
        ...
        # key would carry an extra `model.` that cp_key_remap does not strip. LoRA layers stay in the
        # tree; peft_prefix normalizes their infixes.
...
def _save_ep_gathered(
    ...
    The expert all-gathers are collectives every rank enters; only the save rank retains the gathered
    tensors (``retain=`` on the layer gather). Gathered experts are streamed to disk one layer at a
    time: a fine-grained MoE holds nearly all its parameters there, so buffering the whole state dict
    would need the entire model resident in one host's RAM. On non-shared filesystems each node's
    local rank 0 saves.
    """
    ...
    # Held whole: reconcile_tie_word_embeddings compares them and needs both sides resident at once.
    state_dict = {}
    ...
    # walks the same list; a DTensor buffer resolves through a collective every rank must enter.
    buffers = (
    ...
    def stage_expert_layer(gathered: dict[str, torch.Tensor]) -> None:
        ...
        Runs inside the guard: a host OOM in the cast blocks the peers as a failing write would. The
        cast keys on the live name (the caster's keep-sets use that spelling); the respell follows."""
    ...
    # its real cause, rather than finalizing an index over tensors that were never written.
    guard.reject()

    # The finalize is the other ENOSPC-prone half (last flush, renames, stale sweep, index write),
...
def _check_ep_sharded_save_supported(
    ...
    rank and ``merge_ep_shards.py`` needs every shard co-located.
    """
    if not ep_layers:
        return
    ...
    # Attribute access, not getattr with a default: on anything but a finalized EPConfig the guards
    # below must raise rather than pass, or the sharded save merges to duplicated/missing experts.
...
def validate_ep_sharded_save(
    ...
    """Construction-time validation for ``save_sharded_ep=True``.

    Checks EP topology, filesystem and merge support up front, so an unsupported shape raises before
    training rather than at the first save.
    """
    ...
    ep_layers = find_ep_layers(model)
    if not ep_layers:
        # Accepting the flag on a run that can never shard would write ordinary gathered checkpoints
        # while a planned merge_ep_shards.py step waits for shards that never exist.
        raise ValueError(
...
def _save_ep_sharded(
    ...
    # Every rank writes its own shard, so write-failure odds scale with world size. Raising here
    # would leave the other ranks in the collectives below until the watchdog fires.
    ...
    # EP shards are key-disjoint (experts carry a per-rank ``.shard_N`` suffix, replicated keys are
    # rank-0-only), so the merge's collision check is a real gate here; it raises on every rank,
    # because a raise confined to the rank-0 gate below would leave the peers in the barrier.
    ...
    with barrier_on_exit():
        if rank == 0:
            ...
            # sharded save. The sweep matters here: these shards live in HF's own model-{i}-of-{n}
            # namespace, so a leftover gathered save is indistinguishable from a rank shard to
            # merge_ep_shards' regex.
            write_merged_index(
                output_dir,
                merged_map,
