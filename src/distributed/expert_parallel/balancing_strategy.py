"""Applying a resolved ``moe_balancing`` mode to a live model: the config writes, the bias-state
enable, and the export contract that keeps ``bias_update`` honest.

The mode itself is resolved by :func:`~src.models.moe_balancing.resolve_balancing_mode`. Both entry
points are COLLECTIVE — every rank must call them. Which families own a checkpoint-exported
balancing slot is read off the EP layer-class registry rather than a roster list here, so a new
family cannot silently fall off it.
"""
...
# Resolved-mode precedence for :func:`agree_balancing_mode`, most informed first: a stage that
# found a bias-capable router knows something a stage holding no MoE layer cannot.
...
def agree_balancing_mode(mode: BalancingMode) -> BalancingMode:
    ...
    stage without MoE layers resolves ``auto`` out of ignorance: a split verdict then sends stages
    down different branches of :func:`apply_balancing_strategy` — different collectives, and a raise
    on some ranks only. Identity where it cannot matter (outside distributed, or an explicit mode).
    """
...
def _enable_router_bias_balancing(model) -> tuple[int, list[str]]:
    ...
    Returns ``(enabled_count, hook_carrying_layer_class_names)`` so the caller can fail loud when
    nothing accepted the bias.
    """
    enabled = 0
...
def _enforce_bias_export_contract(model, mode: BalancingMode, is_moe: bool) -> None:
    """Refuse the spelling of the bias-update mode that misstates where the trained bias lands.

    ``bias_update`` promises the bias reaches every exported checkpoint, so a router left on the
    transient side-buffer raises here — the exported model would silently route without the bias it
    trained with, and ``bias_update_transient`` is the explicit opt-in for that. The transient
    spelling raises in turn where every balancing router adopts a native slot.

    COLLECTIVE — every rank must call it: both verdicts are about the WHOLE model while ``transient``
    is read off this rank's stage, so a PP stage holding no MoE layer would judge the opposite way.
    """
    ...
    if mode == "bias_update_transient":
        # ANY rank's transient router makes the whole model's bias transient; a stage that holds none
        # must not conclude "all native" on its own. Taken BEFORE the `is_moe` test, never on the
        # right of it: rank_consensus is a world all-reduce, so a rank that short-circuited past it
        # would leave every peer inside the collective.
...
def _export_native_balancing_config_flags(model) -> None:
    ...
    the very tensor the bias update trained."""
...
def apply_balancing_strategy(
    ...
    bias_update / bias_update_transient: zero ``router_aux_loss_coef``, force
                 ...
                 when nothing would accept it — then hold the two spellings to their meaning through
    ...
    if mode in BIAS_UPDATE_MODES:
        ...
        if coef not in (0, 0.0, None):
            ...
            # configures this run; the export restores the hub coefficient, or a later aux_loss
            # stage silently trains unbalanced off the destroyed value.
        ...
        # side-buffer at runtime (a DTensor-wrapped slot, an upstream rename), so the post-enable
        # state — not the class predicate — is its ground truth.
        ...
        # carries the trained bias but that engine's loader drops the key. Warned, not refused —
        ...
        # construction, and its raise would leave the stages that DID accept the bias hanging in
    ...
    if mode == "aux_loss":
        ...
        if not honors_output_router_logits_config(model):
            # bias_update is a remedy only where something would ACCEPT the bias; without an acceptor
            # it raises in turn, so name the knobs that CREATE one instead.
