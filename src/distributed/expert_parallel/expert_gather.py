...
:class:`EPExpertGatherMixin` reassembles this rank's expert shards across the expert-TP and
dispatch-EP groups into the layout the family's HuggingFace checkpoint uses, and provides the
shard-merge inverse of every gather. A family either declares its layout
(``_PER_EXPERT_UNFUSED_KEYS`` / ``_HUB_PER_EXPERT_KEYS``) or overrides the gather and its merge
together; ``__init_subclass__`` rejects one without the other.

The instance gathers are collective over those groups: each rank must call them, and ``retain=False``
joins every gather without assembling the result. The classmethod merge half runs single-process in
``scripts/after_training/merge_ep_shards.py``, off tensors already on disk.
"""
...
class EPExpertGatherMixin:
    ...
    Mixed into :class:`EPMoELayerBase`, whose construction sets up the state read here (expert
    ...
    # The same names without asserting that the base gather does the split: families that emit
    # per-expert themselves (Qwen3, Bailing) or gather fused (Qwen3.5/3.6, DSv4).
    ...
    def __init_subclass__(cls, **kwargs):
        """Enforce the layout contract on every EP layer that mixes this class in.

        Each raise closes a way for the declared and the gathered layout to drift apart into a
        checkpoint whose expert keys no serving engine reads.
        """
        ...
        # Gather and merge are inverses: overriding one alone writes a checkpoint whose layout
    ...
    def hub_per_expert_keys(cls) -> tuple[str, str, str] | None:
        """``(gate, up, down)`` key names for a family whose HF checkpoint stores one tensor per expert.

        Returns ``None`` for any other layout (fused halves, interleaved fused, a module per expert);
        ``scripts/after_training/unfuse_moe_experts.py`` rejects those families.
        """
    ...
    def _all_gather_cat(tensor: torch.Tensor, dim: int, group, world_size: int) -> torch.Tensor:
        ...
        most of a fine-grained MoE's parameters. Other dims still need the list plus concatenation.
        """
        if world_size <= 1:
            return tensor
    ...
    def _ep_all_gather_cat(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        :meth:`_tp_all_gather_cat`'s axis follows the projection instead."""
    ...
    def gather_expert_state_dict(self, device: str = "cpu", merge_lora: bool = False, retain: bool = True) -> dict:
        ...
        Non-writing ranks skip the post-gather layout work (split, re-interleave,
        transpose+``contiguous``, host copy).
        """
    ...
    def _gather_fused_experts(self, device: str, take: Callable[[str], torch.Tensor], retain: bool = True) -> dict:
        ...
        if self.expert_tp_size > 1:
            ...
            # Separately-stored GLU (Qwen3, Bailing): same shape the ETP branch rebuilds, so the
            # fused result is identical either way.
    ...
    def _unfuse_fused_to_per_expert(cls, fused: dict) -> dict:
        ...
        along the output dim, so split the halves and the expert dim (no transpose: the fused gather
        already returns ``F.linear`` convention). A classmethod, so the shard-merge side runs the
    ...
    def merge_shards_to_hf(cls, prefix: str, params: dict) -> dict:
        ...
        ``scripts/after_training/merge_ep_shards.py`` passes the merged runtime tensors (matmul
        convention, keyed by the wrapper attribute they were saved under) plus the module ``prefix``,
        and receives the keys ``from_pretrained`` expects.

        The base implementation is the fused-GLU path mirroring :meth:`gather_expert_state_dict`:
        transpose to ``F.linear`` convention, then the same ``_PER_EXPERT_UNFUSED_KEYS`` split. A
        family overriding the gather must override this too (enforced in ``__init_subclass__``).
        """
        state = cls._merge_fused_shards(params)
        if cls._PER_EXPERT_UNFUSED_KEYS is not None:
            # The split emits weights only, so a fused bias here would be dropped.
    ...
    def _merge_fused_shards(cls, params: dict) -> dict:
        ...
        discarded expert tensor produces a checkpoint that loads and is wrong.
        """
    ...
    def _individual_glu_hub_keys(cls) -> tuple[str, str, str]:
        """``_HUB_PER_EXPERT_KEYS``, or raise when a family routing its gather/merge through the
        separate-halves path declares none."""
    ...
    def gather_expert_lora_state_dict(self, device: str = "cpu", retain: bool = True) -> dict:
        ...
        Never ETP-sharded: ``EPConfig`` rejects ``expert_lora`` with ``expert_tp_size > 1`` at
    ...
    def load_expert_lora_state_dict(self, layer_state: dict) -> None:
        ...
        re-shard: ``EPConfig`` rejects ``expert_lora`` with ``expert_tp_size > 1`` at construction."""
        # A changed expert config between checkpoint and resume (GptOss renames attrs by grouped-GEMM
        # / ETP / arch) would otherwise leave adapters zero-init; an empty state is the same mismatch.
