...
:func:`resolve_conversion_steps` turns the declarative entries into the ordered steps the generic
walker consumes — scoped the way transformers scopes them: a key naming a SUB-model of the shell
(Step-3.7's ``step3p5_vision`` tower) applies only under that sub-model's path.

The op vocabulary, the translator and the key walker are family-agnostic and live in
``src/models/loading/lazy_safetensors/``; this module owns only the registry seam — which
family declares which conversion key, and where in the module tree each key lives.
"""
...
# A conversion source addressing one tensor PER EXPERT (``…experts.*.w1.weight``): that merge is
# the ExpertFuser's domain — the lazy loaders keep the per-expert on-disk form for ranged reads and
# fuse locally, so the walker must skip these rather than refuse the family's whole entry set.
...
def resolve_conversion_steps(model_type: str, model: nn.Module) -> tuple[Rename | Convert, ...] | None:
    ...
    A declared key that resolves to no entries raises. ``None`` is the caller's "this checkpoint is
    already canonical" answer, and ``_HUB_CONVERSION_KEYS`` states the opposite — collapsing the two
    would load the vendor-namespace tensors unconverted, which the key planner then skips as
    unmatched: a model at its meta-device init values, with no error.
    """
        ...
        for entry in entries:
            if isinstance(entry, WeightRenaming):
                ...
                if all(_PER_EXPERT_SOURCE.search(p) for p in entry.source_patterns):
                    ...
                    continue  # expert merge — the ExpertFuser owns it (see _PER_EXPERT_SOURCE)
...
def _conversion_scopes(model: nn.Module) -> dict[str, str]:
    ...
    transformers' ``get_model_conversion_mapping`` walk: every ``PreTrainedModel`` in the tree claims
    its class name and ``model_type``, the first (outermost) claimant wins, and a sub-model's entries
    see only the keys under its path relative to ``base_model_prefix`` while the root's see every
    key. A declared key naming no module (a text-only artifact loaded under the composite's key)
    is absent here and resolves to the root scope.
    """
