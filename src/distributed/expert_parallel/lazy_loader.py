...
ranks in parallel. :func:`load_ep_model_lazy` runs: meta-device shell → format detect →
...
individual per-expert checkpoints (Qwen3/Qwen3.5/Bailing) loaded local-only and fused into 3D
tensors. The generic machinery — index resolution, key alignment, ``WeightPlan`` materialization,
absent-module init, the meta shell — lives in ``src/models/loading/lazy_safetensors/`` and is shared
with the pipeline stage loader, which runs :func:`lazy_load_prologue` and then narrows the resulting
plan to its own stage.
"""
...
# Derived from the container attrs and fused tensor names the layer classes declare: an alternation
# missing a family's container mis-plans every one of its expert keys.
...
# Expert index nested under an extra module (``...experts.local_experts.N.linear_fc{1,2}``): matches
# neither pattern above, so detect() would classify it as FUSED and drop the expert plan.
...
def lazy_loader_supports_checkpoint(model_path: str) -> bool:
    ...
    Decided by the family's :attr:`~EPMoELayerBase._supports_lazy_loading` declaration, resolved from
    ``config.model_type``; callers route an unsupported family to the from_pretrained + EP-patch
    path. The structural probe covers a checkpoint whose ``model_type`` no registered family claims,
    where an expert index nested under an extra module (Zaya's
    ``...experts.local_experts.N.linear_fc{1,2}``) matches neither key pattern and ``detect()`` would
    classify it as FUSED. Best-effort: ``True`` when neither the config nor the index can be read.
    """
    ...
    if config.get("quantization_config"):
        # Checkpoint-native quantization (fp8 hub checkpoints), distinct from the run's TRL
        # quantization knobs: the lazy loaders read weight tensors raw and map nothing for the scale
        # tensors, which have no live counterpart, so the coverage gate never fires and the experts
        # load incorrect values. Quantized checkpoints go through from_pretrained.
        return False
    try:
...
def build_family_key_mapping(
    ...
    The family is read through ``text_config``, since a composite (VLM-wrapper) config keeps
    model_type on its text sub-config; a wrapper-only read returns "" and yields no renames, leaving
    a whole module absent from the load.
    """
...
def _check_fused_experts_mapped(ckpt_format: CheckpointFormat, weight_map: dict[str, str], n_shard: int) -> None:
    """Raise when a fused checkpoint's expert weights all missed the model's namespace.

    An empty expert plan would train freshly-initialized experts against loaded attention weights.
    """
...
class EPWeightPlanner:
    ...
    Fused 3D expert weights produce EXPERT_SHARD plans (slice dim 0); individual expert weights
    produce REPLICATE (local) or IGNORE (remote); everything else (attention, norms, router,
    embeddings) is REPLICATE.

    ``ep_config=None`` is the no-expert-sharding case (a dense model, or a pipeline stage loaded
    without EP): every key replicates, and the result is the whole-checkpoint plan the pipeline
    loader filters.
    """
    ...
    def __init__(self, ep_config: EPConfig | None):
        ...
        # materializing all experts unsharded on every rank of an EP job. "Shard no experts" is
        # spelled ``ep_config=None``, so an unfinalized config is a caller-order bug.
    ...
    def build(
        ...
        unexpected: list[str] = []
        fanout = fanout or {}
        # Siblings of a fan-in entry are read by the first source's plan (its leading Concat), never
        # planned or reported on their own.
        consumed = {
            sibling
        ...
        for disk_key, shard_file in weight_map.items():
            ...
            for model_key, ops in targets:
                if model_key not in model_keys:
                    continue
                # Two disk keys aligning to one model tensor is a layout error: which of the two
                # reads wins would depend on index order.
                ...
                # For a canonical family the two spellings are identical.
    ...
    def _is_expected_per_expert_key(self, disk_key: str) -> bool:
        """Whether a per-expert key will be consumed by the ExpertFuser or a per-expert live model.

        Such keys map to no fused model key by construction, and a fusion that misses an index or a
        GLU half raises in the fuser. ``False`` for an undeclared projection suffix (``detect_tasks``
        skips it, so nothing reads it) and for an expert index at or beyond the config's expert count
        (no rank holds it at any ep_size).
        """
        m = _INDIVIDUAL_EXPERT_PATTERN.search(disk_key)
        if not m:
            return False
...
class ExpertFuser:
    ...
    def detect_tasks(
        ...
        The fusion target is resolved through ``disk_to_model``, the disk→model mapping built by
        :func:`build_family_key_mapping`, rather than by re-deriving model names from the disk key: a
        family needing the ``base_model_prefix`` or a declared hub rename would resolve to a name the
        model does not have and skip every expert.
        ...
        for layer_prefix, fusion_groups in groups.items():
            for fusion_type, expert_dict in fusion_groups.items():
                ...
                # No fused target: valid only when the model stores one module per expert, which
                # EPWeightPlanner already covers. Anything else leaves the experts on meta until the
                # first _to_device.
                unresolved = sorted(
                    disk_key
                ...
                # A layer the model instantiates no parameter of is a declared-but-absent MTP tail
                # (GLM-4 num_nextn_predict_layers): dropping its experts matches how EPWeightPlanner
                # drops that layer's non-expert keys.
    ...
    def _check_local_range(self, expert_dict: dict[int, dict[str, tuple[str, str]]], model_key: str) -> None:
        ...
        index the checkpoint does not carry shifts every later expert down a slot, and the rank then
        trains and exports the wrong experts under each name.
        """
...
def lazy_load_prologue(
    ...
    *,
    ...
    The order matters: the expert assignment is finalized before any plan exists (the planner slices
    by this rank's expert range), and the meta shell exists before the checkpoint keys are aligned
    against it. Everything here is rank-uniform, and ``meta_init_phase`` is a store rendezvous every
    rank must enter. ``build_meta_shell`` receives whether the index names shard files absent from
    this filesystem, which a caller may answer with a config-only build.
    """
...
def load_ep_model_lazy(
    ...
    **model_kwargs,
    ...
    # A torn shard, a checkpoint short of one rank's experts or an OOM in the fuse raises on that
    ...
    # that sweep would re-initialize the whole model before this raised.
    ...
    # Only the shards this rank reads: SafetensorsWeightLoader safe_opens every file it is given,
    ...
    # Collective — every rank enters it: a rank that failed above surfaces its reason on all of them,
    # and the ranks that succeeded synchronize here before the EP patching below.
    guard.reject()

    logger.info(f"[Rank {rank}] Applying EP patching...")
    # Derived rather than assumed: the layers slice their own experts out of a full tensor unless
    # this load pre-sliced them. A checkpoint whose expert keys match no known pattern is planned as
    # REPLICATE, where claiming "already sharded" would give every rank global experts 0..E_local-1
    # and leave the rest untrained.
