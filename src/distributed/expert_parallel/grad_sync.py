...
Two post-accumulation grad hooks built from :class:`EPConfig`: a router hook (DP-average, since the
router runs on the local sequence chunk) and an expert hook (scale expert grads to match DDP/FSDP
averaging). Both no-op during gradient accumulation. The expert hook carries no collective; the
cross-replica sum multiple EP groups need runs in the trainer's post-backward sweep
(``EPConfig.defer_grad_sync``), which every rank enters structurally.
"""
...
def has_grad_sync_peers(ep_config: EPConfig) -> bool:
    ...
    Read where the hook is built rather than latched on the config: the default process group is
    initialized independently of config construction. Single-process runs have no peers and no default
    group, so every hook is inert there.
    """
    return dist.is_available() and dist.is_initialized() and ep_config.world_size > 1


def _noop_sync_hook(param):
    """Single-process grad hook: no peer ranks, nothing to reduce or rescale."""
...
def _make_hook(*, divisor: int, reduce: bool, group=None, fp32: bool = False):
    ...
    ``1/divisor``. ``reduce=False`` is the expert case: the shard already holds its full grad, so only
    the DP-averaging divide applies and ``group``/``fp32`` are unused."""
...
def create_router_grad_hook(ep_config: EPConfig):
    """Router-param grad sync: ``all_reduce(SUM)`` over the DP-scope group (the world group without PP;
    under PP this stage's rank block only), then divide by ``world_size``, that group's size.
...
def create_expert_grad_hook(ep_config: EPConfig):
    ...
    Collective-free. The cross-replica sum multiple EP groups need cannot ride this hook: it fires only
    where a grad was accumulated, and a rank whose dispatch delivered no tokens for a layer never
    touches its expert weights, leaving its replicas waiting in a collective it never enters. Those
    topologies use the trainer's post-backward sweep, which contributes every param structurally. No-op
