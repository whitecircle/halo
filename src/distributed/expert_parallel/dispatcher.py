...
:class:`DeepEPDispatcher` owns the EP communication buffer's lifetime, exposes autograd-aware
``dispatch`` / ``combine``, and short-circuits to no-op mode when ``ep_size <= 1`` (all experts local).

Buffer lifecycle and the four dispatch/combine ops live behind :class:`_DeepEPBackend`, selected once per
...
* :class:`_LegacyBackend` (``"legacy"``) — DeepEP V1 :class:`deep_ep.Buffer` (CUDA IPC over NVLink),
  ...
  elastic. Pick it for long-context ep8, where the elastic combine barrier races FSDP2's reduce-scatter
  and deadlocks.
...
# ``warn_once`` scope for the process-global DeepEP environment warnings: one line per process, not
# one per MoE layer (each layer builds its own backend instance, so an instance flag cannot bound it).
_WARNED_ENV: set = set()
...
# 100). The clock starts when a rank ENTERS the barrier, so this bounds rank SKEW, not idle time between
# steps: a rank that arrives while a peer is still writing a checkpoint traps once the peer is this far
# behind. Every save path ends on a collective (``DeferredRankFailure.reject``) and teardown opens with
# a barrier, both of which block the host, so the skew is absorbed under the much larger process-group
# timeout instead — raise this only if a legitimately skewed phase can outlast it.
...
# Opt-in override of the dispatch/combine SM count (0 => auto). Read here rather than inside a backend:
# both transports honour it, so a backend-local read would make it dead under the other.
_NUM_SMS = env_int("HALO_DEEPEP_NUM_SMS", None) or 0
# RDMA queue-pair count (0 => auto); more QPs help the latency-bound all-to-all on EFA Gin. ELASTIC ONLY
# — V1 takes no per-call QP count and its RDMA path is not wired up here, so the legacy backend warns
# rather than letting the knob look applied.
...
# forward. The group is kept as a strong ref for the same reason ``_SharedArena`` does: the key is its
# id, which the allocator could otherwise recycle onto a rebuilt group and serve it another group's
# capacity. ``destroy_all_dispatchers`` clears it, so the ref never outlives EP teardown.
...
# Latched by verify_rank_uniform_env once the join has actually run: the settings cannot change
# after import, so comparing them is worth exactly one collective per process.
_ENV_UNIFORMITY_VERIFIED = False


def _is_group_leader(group) -> bool:
    """Whether this rank is rank 0 of ``group`` — the one that logs a per-EP-group event."""
...
def rank_uniform_ep_settings() -> dict[str, object]:
    ...
    Resolved rather than raw, because each toolkit knob is consumed through :mod:`src.env`, where an
    absent variable and the variable set to its default mean the same thing. Comparing raw strings
    would reject the ordinary multi-node shape in which a launcher exports a default on the head node
    only — ``HALO_EP_CAPACITY_DEDUP=1`` next to unset is one behaviour, not two.

    ``EP_DISABLE_GIN`` is DeepEP-owned and read by DeepEP as a raw string, so it is compared as one.
    Left unset it is derived from topology by :meth:`_ElasticBackend._configure_env`, identically on
    every rank of a group; an explicit per-node value is a genuine divergence.

    ``HALO_DEEPEP_GIN_MAX_TOKENS_PER_RANK`` gates a raise against an all-reduced (rank-uniform)
    capacity, so a divergent ceiling makes one rank raise mid-forward while its peers enter the
    dispatch collective. ``HALO_GRAD_BUCKET_MB`` sets the chunk boundaries of
    :func:`~src.distributed.grad_reduce.reduce_grads_bucketed`, whose contract needs
    every rank of the reduction group to build identical chunks — and the deferred cross-replica
    sweep runs it across nodes.

    The two coordination timeouts belong here for the same reason, and they are not EP-specific: a
    rank left at the default aborts its process group (or gives up on a store key) while its peers
    still hold budget, so the job dies as a lone abort mid-collective and the survivors then fail on
    the NEXT collective, blaming it. Both are set exactly the way a divergence happens — per-node
    ``--env-file`` or launcher task (the SkyPilot tasks pin ``DIST_NCCL_TIMEOUT_MINUTES``).
    """
    return {
...
def verify_rank_uniform_env(force: bool = False) -> None:
    """COLLECTIVE. Refuse a job whose ranks disagree on a setting in :func:`rank_uniform_ep_settings`.

    These are read per rank at import, and at 512 GPUs they are set through a per-node
    ``--env-file`` or launcher, so a partial rollout is a realistic mistake rather than a
    hypothetical. It is not a slow run: ranks with ``HALO_EP_CAPACITY_DEDUP`` off run the capacity
    all-reduce on EVERY MoE layer while ranks with it on run it once, so from layer 2 the collective
    counts diverge inside one EP group and the job hangs with nothing naming the variable. The
    others are wire parameters both ends of the all-to-all must share, chunk boundaries every rank
    of a reduction group must build identically (the grad bucket), or the coordination timeouts a
    divergent rank would abort the whole job on ahead of its peers.

    Runs at most ONCE per process, and the entry scripts call it in distributed setup — before the
    weight load, not after. A world join placed after that FS-heavy phase is the first collective a
    straggler node can miss, so it dies at ``DIST_NCCL_TIMEOUT_MINUTES`` with a traceback naming
    this check rather than the load that ran long. The values are all known at process start, so
    there is nothing to gain by waiting. ``force`` re-runs it for a caller that reaches DeepEP
    without the script scaffold (a standalone test harness).

    The latch closes only once the join could actually run. Before ``init_process_group``
    :func:`reject_divergent_settings` no-ops, so latching on such a call would compare nothing and
    then permanently disarm :func:`~src.distributed.expert_parallel.patching.create_ep_buffers`'s
    backstop. A live single-rank group has no peer to disagree with, so it latches trivially.
    """
...
def destroy_all_dispatchers() -> None:
    ...
    gone faults with ``cudaErrorIllegalAddress`` (aborts teardown).

    The leading barrier is load-bearing. ``buffer.destroy()`` spins on the device-side NVLink barrier
    for ``HALO_DEEPEP_GPU_TIMEOUT_SECONDS`` (100 s), and teardown is reached after arbitrarily
    rank-asymmetric work: a gathered save streaming tens of GB on the writer rank, an adapter unmerge
    walking every layer, an eval. NCCL's barrier blocks the host, so aligning here puts that wait
    under the process-group timeout rather than the device one; without it the first rank in burns the
    whole budget and traps the job with ``DeepEP NVLink barrier timeout`` followed by
    ``cudaErrorLaunchFailure``, which reads as a DeepEP fault rather than as the skew it is.

    It aligns the ranks only if they still AGREE on the collective order — a job that has already
    desynchronized pairs this barrier with whatever its peer issues next, and arrives here skewed
    anyway. That is a bug upstream, not something teardown can repair.

    Unconditional (never gated on ``_LIVE_DISPATCHERS``): a pipeline stage holding no MoE layers has
    no dispatchers, and a gated barrier would then be rank-asymmetric — a deadlock in place of a
    timeout.
    """
    barrier()
...
def free_unclaimed_ep_buffers() -> int:
    ...
    COLLECTIVE — call only where every rank arrives, and only after a barrier. This is the sweep for
    claims dropped by the garbage collector, which cannot free a buffer itself
    (:meth:`_SharedArena.release`), so without it a model that went out of scope would leak its arena
...
def bump_forward_generation() -> None:
    ...
    ``__call__``; a caller that enters the backbone directly (TRL's chunked log-prob path peels
    ``base_model`` off the wrapper, and its FA4 dense arm forwards one row at a time, each a
    different length) has to open the scope itself.
    """
...
def register_forward_generation_hook(model: torch.nn.Module) -> None:
    ...
    Every MoE layer in a forward sees the same per-rank token count, so only the first need all-reduce the
    global max — the rest reuse it, saving N-1 collective + device-syncs per forward. The generation is
    rank-uniform (every rank bumps on the same forward), so the first-layer cache miss lands on all ranks
    together and the collective stays matched.

    Belongs on the OUTERMOST module the loop calls, and must be re-registered whenever a wrapper lands
    above that module: a task-typed ``PeftModel`` reaches the model it wraps through ``.forward()``, which
    runs no pre-hook. Idempotence is read off this module's own hooks — PEFT forwards a missing attribute
    down to the wrapped model, so a marker attribute would report an outer wrapper as already registered,
    while a ``torch.compile`` wrapper keeps an empty dict of its own and would take a second hook.
    A forward that enters the backbone directly is not reachable from here at all: it opens its own
    scope through :func:`bump_forward_generation`.
    """
...
class _SharedArena:
    ...
    A buffer per layer is the naive shape, and both backends pay for it by the layer count — the V2
    receive-side arena at ~4 GiB (ep64, 8k tokens/rank), the V1 chunked pipeline at a flat ~100 MiB.
    Sharing is sound on both because ``dispatch`` copies its results into allocator-owned tensors and
    the returned handle carries the routing layout rather than buffer state, so a later layer's
    dispatch leaves an earlier layer's pending backward intact. Measured per backend, running every
    layer's forward before any backward: bit-identical to private buffers.

    Subclasses build the buffer and name the shape that decides who may share one; this class owns
    the refcount and frees the buffer once the last layer releases it.

    Every arena is registered, shared or not, so that :meth:`free_unclaimed` is the one place a
    buffer is freed and no arena can escape it.
    """
    ...
    def acquire(cls, group, *shape) -> Self:
        ...
        # isinstance both narrows the base-typed registry value and states the key's invariant: it
        # carries the class name, so anything filed under it is one of ours.
        if not isinstance(arena, cls):
            # A private arena still gets registered, under a key nothing else can collide with.
    ...
    def release(self, *, free_buffer: bool) -> None:
        """Drop one layer's claim; the last one out frees the buffer, if ``free_buffer`` allows it.

        Freeing is a **collective** — ``buffer.destroy()`` spins on the device-side NVLink barrier
        for ``HALO_DEEPEP_GPU_TIMEOUT_SECONDS``. So a finalizer must pass ``free_buffer=False``: it runs
        whenever this rank's garbage collector happens to fire, which no other rank agrees on, and
        one rank entering that barrier alone burns the whole budget and aborts the job with
        ``cudaErrorLaunchFailure``. What a finalizer leaves behind is swept up instead by the
        explicit, host-aligned teardown — see :func:`free_unclaimed_ep_buffers`.
        """
        self._users -= 1
    ...
    def _free(self) -> None:
        """Destroy every buffer this arena owns (COLLECTIVE — host-aligned callers only)."""
    ...
    def free_unclaimed(cls) -> int:
        ...
        The sweep exists because a dispatcher dropped by the garbage collector decrements its arena
        without freeing it (see :meth:`release`), so an arena can sit at zero claims with nobody left
        to free it. Deliberately leaves claimed arenas alone: a second model in the same process (a
        distillation teacher, a reference policy) may still be dispatching through one.
        """
...
class _ElasticArena(_SharedArena):
    ...
    **independent of how many layers use it**: ``ep_size × tokens_per_rank × (2·hidden + 4·top_k)``,
    ~4 GiB at ep64 with 8k tokens/rank.

    Sharing needs one capacity for the whole forward, which is exactly what the capacity dedup
    provides. With dedup off a later layer may present more tokens than the first and grow the arena
    mid-forward, so each layer then keeps a private one instead.

    **Retained superseded arenas are the memory bound of a variable-length run.** A grow retires the
    previous buffer rather than freeing it (an in-flight microbatch's backward may still name it) and
    only teardown frees it, so the arena's HBM footprint is the SUM of every capacity it ever held.
    Capacity is the all-reduced max tokens/rank aligned by :func:`ep_dispatch_capacity`, so a run climbs
    at most ``max_tokens_per_rank / 256`` times — in practice a handful of growths early in an
    unpacked or lengthening-rollout run, ~4 GiB each at ep64/8k. Freeing a retirement earlier needs a
    point where no live handle can name it, and no rank-uniform such point exists here: the forward
    generation counter cannot bound the gap under a pipeline schedule, whose microbatch depth this
    module cannot see. Pre-size instead — pack the corpus, or keep the first forward's tokens/rank at
    ...
    def ensure(self, needed: int) -> bool:
        ...
        if self.buffer is not None:
            ...
            # so a multi-group topology grows independently and a global-rank-0 gate would hide every
            # group but its own.
...
class _LegacyArena(_SharedArena):
    ...
    Token-count-independent (measured: the same bytes at 1k and 8k tokens/rank), so it never grows
    and every layer on the group can always share one, whatever the capacity dedup is set to.
    """
...
class _DeepEPBackend(ABC):
    """A DeepEP transport backend owned by a :class:`DeepEPDispatcher`.

    Each subclass owns its buffer's lifetime and the four dispatch/combine ops, where ``dispatch.backward``
    is a combine and ``combine.backward`` is a handle-reuse dispatch (the ``handle`` carries the routing
    layout for both). Reads EP topology from the owning dispatcher.
    """
...
class _ElasticBackend(_DeepEPBackend):
    ...
    def _configure_env(self) -> None:
        ...
        - ``EP_DISABLE_GIN``: Gin (RDMA) is unavailable on a single NVLink node — disabled intra-node,
          enabled inter-node; explicit user value honored.
        ``EP_SUPPRESS_NCCL_CHECK`` cannot be set from here: DeepEP reads it inside ``check_nccl_so()``
        at ``import deep_ep``, which dispatcher construction has already triggered. Both images set it as an ENV,
        so warn rather than pretend.

        The environment is process-global but ``_env_configured`` is not: every MoE layer builds its
        own backend, so the lines below are bounded by :data:`_WARNED_ENV` (the warning, which every
        rank must be able to see) and by the main-process gate (the two INFO lines, identical on
        every rank of a group) — else a 92-layer model at 512 ranks opens with ~47k duplicate lines.
        """
    ...
    def ensure(self, num_tokens: int, num_topk: int) -> None:
        ...
        if cached is not None and cached[1] == _FORWARD_GENERATION and cached[2] == num_topk:
            # Valid while the arena still holds this layer's tokens. Judged against the aligned world
            # capacity, not this rank's own first-layer count: per-layer token counts are data-dependent
            # per rank (a family can dispatch a different count from a later layer), so only "does it
            # fit" is the invariant the shared arena needs. A rank that outgrows the arena raises here
            # while its peers reach the dispatch collective — DeepEP's GPU timeout
            # (HALO_DEEPEP_GPU_TIMEOUT_SECONDS) bounds that wait, and the message names both fixes.
            needed = cached[3]
    ...
    # Hidden padding lives on the wire only — stripped before results re-enter autograd, so callers see hidden_dim.
    ...
    # ``num_sms`` is), it recomputes ``get_theoretical_num_qps`` — so without it the knob would cover
...
class _LegacyBackend(_DeepEPBackend):
    ...
    built once, no ``hidden % 256`` padding, numerically identical to elastic. Pick it for long-context
    ...
    def prepare(self) -> None:
        ...
        # The PARSED value, like the QP gate above: a docker-compose pass-through exports the name
        # with an empty value when it is absent, which resolves to the default and is not a request.
    ...
    def ensure(self, num_tokens: int, num_topk: int) -> None:
        if self.buffer is not None:
            return
        d = self._d
        # Size hint takes a per-token BYTE size (hidden*2 for bf16), not a token count — buffer is token-independent.
...
class DeepEPDispatcher:
    """Token dispatcher for EP communication: owns the EP topology + the selected transport backend.
    ...
    def __init__(self, ep_config: EPConfig, num_experts: int, hidden_dim: int):
        ...
        # Public: the four dispatch/combine ops the autograd Functions drive are the backend's own
        # (:class:`_DeepEPBackend`); the dispatcher owns its lifetime, not a second copy of its API.
        ...
        if self._noop:
            # Main-process only: one line per MoE layer per rank buries the run's real diagnostics.
            if is_global_main_process():
    ...
    def _select_backend(self) -> _DeepEPBackend:
        ...
        if choice == "legacy":
            ...
            # for a hand-built EPConfig that never passed through it. The DISPATCH group is what
            # rides the buffer — under expert-TP it is narrower than the EP group.
            reject_legacy_backend_topology(
                self.ep_size,
    ...
    def destroy(self, *, free_buffer: bool = True) -> None:
        ...
        COLLECTIVE by default — only call it from a rank-synchronized point.
        """
        if self._destroyed:
            return
    ...
    def __del__(self, _is_finalizing=sys.is_finalizing):
        # Skip destroy() during interpreter shutdown (CUDA context may be corrupted → crash).
        # Bound as a default arg: interpreter teardown can clear this module's globals before the
        # finalizer runs, and a bare ``sys.is_finalizing()`` then raises — an "Exception ignored"
        # traceback at the end of every EP run. The skip still happened: this buys silence, not
        # correctness.
        if _is_finalizing():
            return
        ...
        # first rank in spins out its whole timeout budget and aborts the job. Drop the claim only;
