...
:class:`EPMoELayerBase` holds the machinery shared by every EP family wrapper: the construction
...
The checkpoint gather and shard merge (:mod:`~src.distributed.expert_parallel.expert_gather`) and the
router-balancing state (:mod:`~src.distributed.expert_parallel.balancing`) are mixed in from their own
modules.
"""
...
def has_grouped_mm() -> bool:
    ...
    try:
        ...
        # ``is_available()`` already returned True, so this is a broken CUDA context on this rank (a
        # fork re-init), not an old GPU. Logged because the answer decides a weight layout — GptOss
        # stores gate/up de-interleaved on the grouped path — so a rank that answers differently from
        # its peers enters the expert all-gather with different keys and the job hangs.
        logger.warning(
...
class EPMoELayerBase(EPExpertGatherMixin, EPRouterBalancingMixin, nn.Module, ABC):
    ...
    Three parts: the construction template, forward and compute kernels below; the checkpoint
    gather/merge (:class:`EPExpertGatherMixin`); and the router-balancing state
    ...
    # one flat key per layer type. transformers folds those into ``per_layer_config`` on load and
    # re-serializes only the folded form, which the pinned rollout server cannot parse, so exported
    # ``config.json`` files are rewritten back to the flat keys (``export_legacy_per_layer_config``).
    # Registered per claimed ``model_type`` into the ``moe_balancing`` leaf by ``__init_subclass__``.
    _LEGACY_PER_LAYER_CONFIG_KEYS: dict[str, tuple[str, str]] = {}

    # Attribute(s) an HF MoE block may hold its expert container under, tried in order. Shared by every
    # probe (container, expert count, the lazy loader's checkpoint-key regexes). A family whose block
    # spells it differently across revisions declares every spelling it accepts; an unknown spelling
    # raises rather than resolving off a name no family claims.
    ...
    # block carries becomes :attr:`_shared_expert_attr` and thus the export key. Empty = the family has
    # no shared expert. A declaring family whose block carries none (LFM-2) registers the slot as
    # ``None``, so the shared leg is skipped rather than raising on the family's own name.
    ...
    # upstream rename rather than a configuration, and adopting nothing would drop it from every output.
    ...
    # wrapped block names one — the family's architectural default.
    _DEFAULT_ACTIVATION: str = "silu"

    # Attribute the router hangs off, on the wrapped HF block (:meth:`_find_gate_or_router` reads it
    # there) and on this wrapper, which re-registers it under the same name. Declared once per family so
    # the probe, the fp32 upcast and the grad-sync hook cannot point at different modules. ``None`` = the
    # router is external to the wrapper (Gemma4's sibling module): nothing is adopted here, no routing
    # knob is read, and only expert grad-sync hooks register.
    ...
    # re-emits. Config writes forward the source ``config.json`` and modules instead
    ...
    def __init_subclass__(cls, **kwargs):
        """Register this family's config rewrites and native balancing slot under every ``model_type``
        it claims.

        They land in the ``moe_balancing`` leaf so the offline consumers (config writers, standalone
        conversion tools) read them without importing this package, and a wrapper-less run (no EP, no
        grouped GEMM) exports the same config as a wrapped one.
        """
    ...
    def __init__(self, original_layer: nn.Module, ep_config: EPConfig, weights_already_sharded: bool = False):
        ...
        The construction order shared by every family, each step consuming what the previous resolved:
        router and expert container, EP state, router adoption, routing knobs, shared expert,
        expert-compute constants, this rank's expert weights, then the shared tail (device → expert
        LoRA → fp32 upcast → grad-sync hooks) and the construction line. A family customizes a step by
        overriding its hook rather than restating the sequence.
    ...
    def _detect_hidden_dim(cls, router: nn.Module | None, experts: nn.Module) -> int:
        """The model hidden size this layer dispatches, read off the modules the wrapper adopts.

        The router projection's input width where the family's gate is a bare ``[num_experts, hidden]``
        matrix, otherwise the fused experts' ``gate_up_proj [E, 2M, H]`` — where a family with an
        external (Gemma4) or projection-less (Zaya's EDA gate) router keeps it. A classmethod: it runs
        before the module state exists.
        """
    ...
    def _init_routing(self, original_layer: nn.Module) -> None:
        ...
        Default: ``top_k`` alone. A family with more (DeepSeek-V3 group limiting, a selection function,
        a correction-bias buffer) overrides this and calls ``super()`` first; the router is adopted by
        then, so an override reads it as ``self.gate`` / ``self.router``.
        """
    ...
    def _init_shared_experts(self, original_layer: nn.Module) -> None:
        ...
        Resolved from :attr:`_SHARED_EXPERT_ATTRS`, so the attribute the wrapper registers — the export
        key ``replicated_named_params`` emits — is the one the block was read from.
        """
    ...
    def _init_expert_compute(self, original_layer: nn.Module, experts: nn.Module) -> None:
        ...
        fallback (:meth:`_resolve_activation`). A family whose combine carries extra constants (a clamp
        bound, GptOss's ``alpha``) reads them here directly, so an upstream rename raises instead of
        substituting a numeric the model was never trained with.
        """
    ...
    def _init_ep_state(self, ep_config: EPConfig, hidden_dim: int):
        ...
        Separate from :meth:`__init__` so a caller holding no HF MoE block (a construction probe) can
        build the state alone."""
        super().__init__()
        ...
        # Cursor over a mask spanning chunked forwards (wraps at exhaustion); the monotonic total lets
        # disarm distinguish "never consumed" from "k full passes".
    ...
    def _glu_combine_name(self) -> str:
        """The GLU combine this layer runs, for the construction-time summary.
    ...
    def _shared_expert_summary_field(self) -> tuple[str, ...]:
        ...
        Keyed on :attr:`_shared_expert_attr`, so the field names the module the forward calls and the
        save exports."""
    ...
    def _log_init_summary(self, *extras: str) -> None:
        """Emit this layer's construction-time line: the fields every family shares, then ``extras``.

        ``grouped_mm`` and ``glu_combine`` report the effective decision (:meth:`_grouped_mm_enabled`,
        :meth:`_glu_combine_name`) rather than the requested flag, and the shared-expert field names
        what :meth:`_init_shared_experts` adopted.

        The INFO line is main-process only and carries rank-uniform fields alone: a 92-layer MoE at 512
        ranks would otherwise emit ~47k near-identical lines at startup. This rank's expert range is
        rank-specific and goes to DEBUG on every rank.
        """
        logger.debug(
    ...
    def _in_gc_recompute() -> bool:
        ...
        The enclosing :class:`~src.distributed.expert_parallel.gc_scope.EPCheckpointScope` counts passes
        through the checkpointed body, so this holds for both checkpoint modes; grad mode alone would
        not, since the non-reentrant checkpoint runs both passes with grad enabled.
        """
    ...
    def _maybe_replace_selection(self, topk_indices: torch.Tensor) -> torch.Tensor:
        ...
        ``_forced_topk_indices`` is ``[T, top_k]`` long (global expert ids, flat token order); ``-1``
        keeps that token's natural selection. Selection is non-differentiable (``topk`` on detached
        scores), so swapping indices removes nothing from the graph: callers re-gather gate weights from
        live scores at the returned indices. Also accumulates the replay flip-rate (skipped during GC
        recompute).
        """
    ...
    def _gc_dispatch(self, flat, experts, weights):
        ...
        A second DeepEP dispatch issues a fresh all-to-all into the same ``ElasticBuffer`` and
        invalidates the handle the original forward's backward node still holds, corrupting every
        gradient in the stage without raising. The first pass through the checkpointed body therefore
        caches detached results in its
        :class:`~src.distributed.expert_parallel.gc_scope.EPCheckpointScope` and the recompute replays
        them. Outside a checkpoint (and for the inert ep1 dispatcher) this is a plain dispatch.
        """
        ...
        if not scope.is_recompute:
            ...
            # Detached aliases, not clones: DeepEP's copy epilogue lands every result in an
            # allocator-owned tensor (never a view of the shared arena) and nothing downstream writes
            # into them in place, so a clone would only add a recv-buffer memcpy per layer.
            slot.update(
    ...
    def _setup_gradient_sync(self, router: nn.Module, expert_params: list):
        ...
        reaches them and these hooks are the only sync:

        - Router + replicated submodules (shared expert + gate): replicated across DP ranks and computed
          on the local batch, so they take a plain DP average (``create_router_grad_hook``); otherwise
          they drift across DP ranks.
        - Expert FFN shards: EP/ETP-distributed; divisor ``world_size/expert_tp_size``
          (``create_expert_grad_hook``).
        """
        # Fully replicated experts (ep_group_size==1) are FSDP reduce-scattered, so EP hooks would double-
        # sync. This is not pure ETP (ep_size==1, expert_tp_size>1), which still needs them.
    ...
    def _expert_hook_grad_edge(self, out: torch.Tensor) -> torch.Tensor:
        ...
        tokens (the early returns), or an expert idling in the weighted loop, would leave its
        accumulated gradient undivided and ``world/expert_tp``× too large at the optimizer. A
        zero-valued edge onto every expert param that already accumulated this window keeps the divide
        reachable; params with no accumulated grad stay out, so an all-idle window leaves ``grad=None``
        and the optimizer skips them. Empty in the deferred and FSDP-managed regimes, whose sweep is
        structural.
        """
    ...
    def _fp32_router_input(self) -> bool:
        ...
        while the router is FSDP-ignored (``ep_group_size > 1``, or ep1 without
        ``fsdp_shard_ep1_experts``). Under FSDP management FSDP casts the weight to bf16 for compute, so
        an fp32 input would mismatch it; the fp32 master weight is preserved either way."""
    ...
    def _dp_averaged_named_params(self) -> list:
        ...
        the router/gate plus any fully replicated submodules (shared expert). Resolved live so a PEFT
    ...
    def reattach_router_grad_sync(self) -> int:
        ...
        Construction-time :meth:`_setup_gradient_sync` hooks the original router params, while PEFT
        ``modules_to_save`` trains a copy those hooks never see. Returns the number of params hooked.
        No-op unless this layer uses the in-backward router hook (the deferred post-backward sweep and
        the ep1 FSDP-sharded path already cover the copy); only trainable params are touched, so frozen
        originals are not double-hooked."""
    ...
    def synced_trainable_param_ids(self) -> set:
        ...
        all count. The in-backward hook path syncs only experts (+ LoRA), the router, and replicated
        submodules."""
    ...
    def _to_device(self):
        ...
        Follows the wrapped layer's own placement where it has one: a lazily-loaded (meta) or
        parameterless layer goes to the local CUDA device, while a layer already materialized on CPU
        stays there and is moved later by the trainer's ``_move_model_to_device``.
        """
    ...
    def _compute_experts_weighted(
        ...
        ``compute_fn(local_idx, x)`` returns one expert's output; token selection, weighting and
    ...
    def _activation_warmup_input(
        ...
        is latched), so every RNG consumer downstream of the MoE in that block — expert-LoRA dropout —
    ...
    def _expert_forward(self, idx: int, x: torch.Tensor) -> torch.Tensor:
        ...
        :meth:`_compute_experts_weighted` (GptOss's interleaved layouts override it).

        Base is the GLU forward: gate_up_proj → chunk → act_fn → down_proj, weights in matmul convention
        (``gate_up_proj [E, H, 2M]``, ``down_proj [E, M, H]``). Branches on the stored layout: families
        keeping the halves separate (Qwen3, Bailing) and every family under ETP — where slicing the
        fused halves along dim 2 would split gate from up across ranks — run the 3-projection path.
        """
        if not hasattr(self, "gate_up_proj"):
    ...
    def has_expert_lora(self) -> bool:
        ...
        Derived from what :meth:`_init_expert_lora` built rather than from the config, which can request
        adapters a family's live layout ends up with none of."""
    ...
    def set_lowp_compute(self, precision: GroupedGemmPrecision, *, weight_cacheable: bool) -> bool:
        """Set this layer's expert-compute precision; return whether the layer can apply it.

        Written through by :func:`~src.kernels.lowp.mixed_precision.apply_mixed_precision_compute`.
        False means this layer runs the per-expert loop, where the grouped kernels — and with them the
        fake-quant — never execute.
        """
    ...
    def _etp_shard_size(self, intermediate: int) -> int:
        ...
        Raises when ``intermediate`` is not divisible by ``expert_tp_size``: a non-divisible split drops
        the top units of every expert and produces a smaller gathered tensor on save."""
    ...
    def _etp_narrow(self, tensor: torch.Tensor, dim: int) -> torch.Tensor:
        ...
        carries the intermediate dim differs, since it moves with the transpose each layout applies.
        Divisibility is checked by :meth:`_etp_shard_size`. Returns a view; callers materialize it.
        """
    ...
    def _require_fused_experts(experts: nn.Module) -> nn.Module:
        ...
        tensor would register no expert weight on the wrapper, giving a layer that trains, saves and
    ...
    def _init_fused_glu_params(self, experts: nn.Module, weights_already_sharded: bool = False):
        ...
        intermediate dim shards coherently, since slicing the fused halves along dim 2 by ``2M/tp`` puts
        the gate half on rank 0 and the up half on rank 1, computing the GLU on misaligned pairs.
    ...
    def expert_named_params(self) -> list:
        ...
        Read by grad-sync, the FP32 upcast and the checkpoint skip-set. Base weights come from
        :meth:`_base_expert_named_params`, LoRA adapters are appended here. Override
        :meth:`_base_expert_named_params` rather than this method.
        """
    ...
    def replicated_named_params(self) -> list:
        """``(name, param)`` for this layer's fully replicated submodules (a shared expert and its gate).

        They run on the local batch and are FSDP-ignored, so like the router they need DP-average grad
        sync or they drift across DP ranks. Derived from :attr:`_shared_expert_attr`, so a family
        carrying the ordinary single shared expert only names the attribute; empty when that attribute
        is absent or ``None`` (LFM2). Only a family whose replicated set has a different shape overrides
        — Qwen3.5/3.6, whose gate is a second module.
        """
    ...
    def _find_gate_or_router(cls, layer: nn.Module) -> nn.Module | None:
        """The HF MoE layer's router module, read under this family's declared :attr:`_ROUTER_ATTR`, or
        ``None`` where the family declares its router external to the wrapper (Gemma4).

        Not a ``("gate", "router")`` probe: a block exposing both would resolve to whichever name the
        probe tried first, overriding the family's own declaration.
        """
    ...
    def detect_num_experts(cls, layer: nn.Module) -> int:
        ...
        from :attr:`_EXPERTS_CONTAINER_ATTRS`.
        """
    ...
    def _resolve_activation(self, *sources, original_layer: nn.Module | None = None, default: str = "silu") -> None:
        """Set ``self.act_fn`` from the first source that carries one, and latch the fused-GLU kernel.

        ``sources`` are searched in order, normally the HF experts module then its parent. Only when
        none carries a live ``act_fn`` is the activation rebuilt by name off ``original_layer``: its own
        ``hidden_act`` first, then its ``config``, then ``default``.

        The fused-combine latch is derived from the resolved activation via
        :func:`resolve_fused_glu_mul`, so a family gets a fused kernel by having a gate the kernels
        implement; any other gate falls back to the eager combine.
        """
    ...
    def _find_top_k(cls, layer: nn.Module) -> int:
        ...
        Uses :data:`ROUTER_TOPK_FIELDS` so this agrees with the other consumers (load metrics, the
        pipeline split's FFN cost model) on both the spellings and their order. The order matters: the
        registry puts bare ``top_k`` last because that name is also the generation sampling parameter,
        and it carries ``top_k_experts`` (Gemma4) and ``moe_router_topk``, which a two-name probe misses.

        The router is read under the family's own declaration for the reason
        :meth:`_find_gate_or_router` gives.
        """
    ...
    def _upcast_experts_to_fp32(self, expert_params: list) -> None:
        ...
        casts them for compute anyway, so the upcast would only mix fp32 experts into a bf16
        ``fully_shard`` group — skipped and logged instead. fp32 master weights in that state need
        ``fp32_non_ep_params`` with ``fsdp_shard_ep1_experts: false``.
        ...
        them would train the frozen base."""
    ...
    def _finalize_expert_init(self) -> None:
        ...
        Order matters. LoRA must exist before the fp32 upcast walks the expert params, since the upcast
        preserves the ``requires_grad`` the LoRA freeze set, and the router and expert params are
        re-read after their upcast: it rebinds each attribute to a new fp32 tensor, so a reference taken
        earlier would hook the discarded one.

        A family whose router is external (``_ROUTER_ATTR = None``) registers expert hooks only: the
        sibling router stays FSDP-managed, which also DP-syncs it, and its dtype cannot be raised under
        EP because DeepEP's combine is bf16-only.
        """
        self._to_device()
    ...
    def _compute_experts(
        ...
        per-expert weighted loop. Per-family variation goes into the grouped gate
        (:meth:`_grouped_mm_enabled`) and the compute kernels, not this branch.

        ``experts``/``weights`` are ``[T, top_k]`` — local expert ids and gate weights per dispatched
        token, one row per token. Every family's forward passes that shape and DeepEP preserves it
        (``ep_size == 1`` returns the inputs unchanged); both compute templates index the top-k axis, so
        a flat ``[T]`` selection is not a supported input to either.
        """
    ...
    def _dispatch_compute_combine(
        ...
        The expert-TP all-reduce runs in token space, outside the dispatch→combine span
        (``SumGradAcrossGroup`` pre-dispatch, ``ReduceFromExpertTP`` post-combine). It is exact because
        the combine is linear in expert outputs, so ``combine(sum) == sum(combine)``.

        It must stay outside the span: with ``ep_size > 1`` and ``expert_tp_size > 1`` the EP group
        splits into multiple DeepEP dispatch groups coupled by strided expert-TP groups, and an
        all-reduce between dispatch and combine couples those groups inside DeepEP's intranode combine
        barrier — under FSDP2 multi-stream drift they form a circular wait and the barrier times out. At
        the layer boundary each dispatch group runs independently.
        """
        ...
        if self.expert_tp_size > 1:
            # Both operands need the ETP backward sum; without either, the router and everything
...
class EPSharedExpertsMoELayerBase(EPMoELayerBase):
    ...
    Subclasses supply ``route_tokens_to_experts``; the construction template has already adopted the
    ``gate`` and the shared-expert module this forward reads under :attr:`_shared_expert_attr` (``None``
    where the family has none — LFM-2). ``_router_logits`` and ``forward`` stay overridable for a family
    whose router cannot be called bare (LFM-2) or whose combine scales the result (Cohere2). Families
    with a different router contract (Qwen3.5, Bailing) subclass ``EPMoELayerBase`` and keep their own
    ``forward``.
    """
    ...
    def _router_logits(self, flat: torch.Tensor) -> torch.Tensor:
        ...
        ``(logits, weights, indices)``: EP recomputes the latter two after routing replay / balancing but
        reuses the family-native (possibly soft-capped) logits. A family whose router module cannot be
        called bare (LFM2's needs the block's ``expert_bias``) overrides this with the logits
        computation alone.
        """
...
class EPGroupLimitedMoELayerBase(EPSharedExpertsMoELayerBase):
    ...
    One routing body for the families that share it (GLM-4 MoE Lite and Laguna, GLM-5 Next, Mistral4,
    Step-3.7), which differ in three declarations: the score function (:meth:`_routing_scores`), the
    ``norm_topk_prob`` fallback for a block that declares none, and the floor its weight
    renormalization adds to the sum. Which tensor biases selection, in what order, and which scores the
    weights come from are shared: a mis-ordered bias add changes routing with no shape, dtype or key
    moving.

    ``n_group: 1`` degenerates to plain top-k, so a family with no group limiting (Step-3.7) uses this
    class with the knob absent rather than a second implementation.
    """
    ...
    # required: absence means an upstream rename, and the neutral default would then be a wrong live
    # value (routed scaling 1.0 rather than the block's 2.5, under-scaling every routed weight), so the
    # resolution below raises instead of defaulting.
    ...
    def _init_routing(self, original_layer: nn.Module) -> None:
        ...
        Each is resolved off the block, then its router, then its config — the three places the roster's
        blocks hang them (5.14 moved most onto the ``*TopkRouter``, and Laguna's remote-code revisions
        put ``norm_topk_prob`` there while omitting the group knobs). A knob no source declares raises
        unless the family listed it in :attr:`_OPTIONAL_ROUTING_KNOBS`.
        """
        ...
        # The ``*TopkRouter`` modules spell it ``num_group``, configs ``n_group``; missing it disables
...
class EPSeparateGluMoELayerBase(EPMoELayerBase):
    ...
    The family's HF checkpoint stores one tensor per expert (:attr:`_HUB_PER_EXPERT_KEYS`) and the
    wrapper keeps the GLU halves apart — Qwen3 splits the pre-fused pair at init, Bailing stacks a
    per-expert ``nn.ModuleList`` — so the gather and its shard-merge inverse are the separate-halves
    pair for both. Declared once here so a fix cannot land in one family and leave the other's gathered
    save out of step with its sharded merge.
    """
...
def find_ep_layers(model: nn.Module) -> list[tuple[str, EPMoELayerBase]]:
    ...
    The gathered and sharded EP saves, a pipeline stage's export set and the optimizer's expert-replica
    writer all key off this walk, so they cannot disagree about which layers are EP.
    """
