...
class EPStep3p7MoELayer(EPGroupLimitedMoELayerBase):
    ...
    ACTIVATED gate — the post-activation mirror of DSv4/GLM-5's pre-activation clamp; most layers
    carry no clamp and run the base combine), sigmoid routing whose ``e_score_correction_bias``
    buffer perturbs SELECTION only (plain top-k — no group limiting), always-renormalized unbiased
    gate weights, the block-level routed scaling folded into those weights, and one always-on
    shared expert (``Step3p7MLP``, which carries its own ``swiglu_limits_shared`` clamp). Dense
    layers (``mlp_layer_types``) are plain MLPs and are never wrapped.
    """
    ...
    # The shared group-limited routing adds this exported native slot (an fp32 ``nn.Buffer``
    # upstream, hub key ``moe.router_bias``) to the selection score itself (selection-only,
    # DeepSeek-V3 style), so bias-update adoption rides it and the trained bias ships with every
    # checkpoint. The modeling has no aux-loss machinery at all (no ``output_router_logits``, no
    ...
    # the per-token weights instead — equivalent, the combine is linear in the expert outputs, and
    # required rather than defaulted: at 1.0 every routed weight is silently under-scaled by 2.5x.
    ...
    # answers None and ``unfuse_moe_experts`` refuses the family. The checkpoint keeps the vendor
    ...
    # A module-spelled save is a dead end here (no per-expert repair layout, and its reload consumes
    # no conversions), so the gathered save applies transformers' own save-side revert per chunk:
    # prefix renames back, ``mlp.gate`` → ``moe.gate``/``moe.router_bias``, ``shared_experts`` →
    # ``share_expert``, and ``gate_up_proj`` split back into the hub's two tensors. Chunk-safe — the
    # only reverse entry touching an EP layer is that single-source split; sharded EP saves are
    # refused. The RL weight sync reverts likewise, so vLLM's step3p5 loader gets the names it maps.
    ...
    # ABSORBS those spellings at load (``attribute_map`` plus ``__post_init__`` derivations) and
    # re-emits only the native ones, so a transformers-written ``config.json`` is a schema the server
    # refuses. Every toolkit config write carries the SOURCE config and modules forward instead.
    _EXPORTS_SOURCE_CONFIG_SCHEMA = True

    # The block always builds its shared expert, so an absent one is an upstream rename: adopting
    # nothing would silently drop it from every output.
    ...
    def _init_expert_compute(self, original_layer: nn.Module, experts: nn.Module) -> None:
        ...
        plus this layer's own clamp bound — ``inf`` = unclamped, which is most layers; Step-3.7-Flash
        clamps layers 43-44 at 7.0. A clamped layer replaces the latch with its own combine, whose
        Triton kernel hardcodes SiLU, so any other activation falls through to the eager one. Direct
        read: a substituted default silently un-clamps those layers."""
    ...
    def _act_then_clamp_eager(self, gate: torch.Tensor, up: torch.Tensor) -> torch.Tensor:
        """This layer's clamp for a gate no kernel implements — clamp AFTER the activation."""
