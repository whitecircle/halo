...
class EPGlm5NextMoELayer(EPGroupLimitedMoELayerBase):
    ...
    (``Glm5NextTextTopkRouter`` returns a ``(logits, weights, indices)`` 3-tuple — the wrapper
    reuses the logits and re-derives the rest), and one always-on shared expert
    (``Glm5NextTextMLP``, which carries its own clamp). The first ``first_k_dense_replace`` decoder
    layers are plain MLPs and are never wrapped.
    """
    ...
    # module, so the HF OutputRecorder still populates ``outputs.router_logits`` and the family's
    # fully-wired aux loss (the ForConditionalGeneration forward honors ``output_router_logits``)
    # keeps working under EP — ``moe_balancing: auto`` resolves there.
    _supports_bias_balancing = True
    # The shared group-limited routing adds this exported native slot (an fp32 ``nn.Buffer``
    # upstream) to the selection score itself (selection-only, DeepSeek-V3 style), so bias-update
    # adoption rides it and the trained bias ships with every checkpoint.
    ...
    # gather writes the FUSED tensor (no ``_PER_EXPERT_UNFUSED_KEYS``, no gather override), so a
    # save that bypassed ``save_pretrained``'s revert is repaired by ``unfuse_moe_experts`` under
    # these names.
    ...
    # sync would land nowhere — and no pinned rollout engine loads glm5_next.
    ...
    def _init_expert_compute(self, original_layer: nn.Module, experts: nn.Module) -> None:
        """``Glm5NextTextExperts._apply_gate`` hardcodes clamp-then-SiLU (no act_fn on the module, no
        config branch), so the combine latched into the base seam is unconditional; only the clamp
        bound comes off the live experts module. Direct read = fail loud on an upstream rename."""
