...
class EPDeepseekV4MoELayer(EPMoELayerBase):
    ...
    two router variants adopted whole as the ``gate`` module (so ``e_score_correction_bias`` /
    ``tid2eid`` stay checkpointable): ``DeepseekV4TopKRouter`` (``is_hash=False``) selects on
    ``scores + e_score_correction_bias`` and gates on the unbiased scores; ``DeepseekV4HashRouter``
    (``is_hash=True``) selects via the frozen ``tid2eid[input_ids]`` lookup. The hash variant needs
    ``input_ids`` in the MoE block, so this wrapper subclasses :class:`EPMoELayerBase` directly
    rather than the shared-experts forward base.
    """
    ...
    # ``_select_experts`` adds this native slot to the selection score (top-k variant only; hash
    # routing declines balancing), and the slot is part of the checkpoint.
    ...
    # untouched via the mapping's model-key fallback (``build_key_mapping``).
    ...
    def _init_expert_compute(self, original_layer: nn.Module, experts: nn.Module) -> None:
        """Read ``act_fn`` and ``limit`` off ``DeepseekV4Experts`` as attributes, not with defaults:
        a substituted default rescales the clamped SwiGLU on every expert.

        The clamp is latched into the base combine seam rather than overriding it, so the
        construction summary names the callable that runs. The fused kernel implements SiLU only;
        any other gate falls through to the eager form."""
    ...
    def _clamped_swiglu_eager(self, gate: torch.Tensor, up: torch.Tensor) -> torch.Tensor:
        """Eager fallback combine for a gate no kernel implements: same clamps, this family's act_fn."""
    ...
    def enable_bias_balancing(self) -> bool:
        """Only top-k layers create the bias state: a balancing bias cannot shift the frozen
        ``tid2eid`` selection a hash layer uses."""
        if self.is_hash:
            return False
