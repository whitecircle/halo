...
GLM-4 MoE Lite except the score function is **softmax** (not sigmoid) with no
...
class EPMistral4MoELayer(EPGroupLimitedMoELayerBase):
    ...
    # ``model_type: mistral3`` wrapping a ``mistral4`` text tower), and the merge/lazy-load
    # resolvers key on the top-level type — same dual declaration as gemma4/qwen3_5/inkling.
    ...
    # ``PixtralForConditionalGeneration``, which a plain-HF toolkit export cannot take — so the sync
    # would forward hub names into a native loader that does not exist. Refuse at construction.
    ...
    def _routing_scores(self, router_logits: torch.Tensor) -> torch.Tensor:
        ...
        group-limited roster uses a per-expert sigmoid."""
