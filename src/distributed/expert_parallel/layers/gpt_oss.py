...
def interleave_gate_up(gate: torch.Tensor, up: torch.Tensor) -> torch.Tensor:
    ...
    The single GptOss re-interleave, over the weights (``[E,H,M]`` → ``[E,H,2M]``) and their biases
    (``[E,M]`` → ``[E,2M]``) alike — the axis is the last one in both, so the rank is not a per-call
    difference. Shared by the live EP gather (:meth:`EPGptOssMoELayer.gather_expert_state_dict`) and
    the shard-merge transform (:meth:`EPGptOssMoELayer.merge_shards_to_hf`)."""
...
def split_interleaved_gate_up(interleaved: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """``(gate, up)`` views of an interleaved trailing axis — the inverse of :func:`interleave_gate_up`,
    for weights and biases alike. Views: every caller materializes with its own layout."""
...
class EPGptOssMoELayer(EPMoELayerBase):
    ...
    # and route with it (top-k on bias-inclusive logits, combine = softmax over the selected values), so
    # sign-updates landing here serve exactly as trained. Adoption re-registers the Parameter as a
    # buffer, freezing it out of gradient training: under bias_update the controller owns this tensor.
    ...
    # interleaved ``gate_up_proj``. Declared once: the weight-attr roots below and the shard merge's
    # accepted-key set both derive from it, so adding a layout member cannot reach only one of them.
    ...
    # EXTENDS the base roots (never restates them: a root added to the base and missed here would drop
    # an expert weight from grad-sync registration — DP divergence with no error). The additions cover
    # GptOss's biases and the grouped-mm de-interleaved pair; the inherited hasattr-filter picks
    # whichever of the three mutually-exclusive layouts this rank holds.
    ...
    def _init_expert_compute(self, original_layer: nn.Module, experts: nn.Module) -> None:
        """GptOss owns its interleaved clamped-SwiGLU compute paths, so it takes those two constants
        instead of an activation. Read directly (AttributeError = fail loud): a substituted default
        silently corrupts the clamped SwiGLU on every expert."""
    ...
    def _init_expert_params(self, experts: nn.Module, weights_already_sharded: bool = False):
        ...
        de-interleaved into separate gate_proj/up_proj — stride-2 slicing on grouped_mm outputs gives
        NaN backward gradients. weights_already_sharded skips dim-0 slicing. The expert BIASES stay
        optional (a bias-less GptOss variant is legal); the weights are not.
        """
    ...
    def _glu_combine_name(self) -> str:
        """GptOss owns its interleaved-bias compute paths and never reaches the base GLU seam."""
    ...
    def _warm_expert_activation(self, gate_up: torch.Tensor) -> torch.Tensor:
        ...
        Every path runs the same Triton kernel; they differ in how the halves reach it — grouped-GEMM
        and ETP store them de-interleaved and contiguous, the per-expert loop splits them out of one
        interleaved projection output (strided, which the kernel materializes).
        """
    ...
    def merge_shards_to_hf(cls, prefix: str, params: dict) -> dict:
        ...
        the grouped-GEMM de-interleave the runtime applies on SM90+ (``gate_proj_gmm`` / ``up_proj_gmm``
        back to one interleaved ``gate_up_proj``), exactly as the gather below does.

        Raises on any expert param outside the two layouts it re-interleaves, mirroring the base
        merge: a silently discarded expert tensor produces a checkpoint that loads and is wrong.
        """
        ...
        # An accepted key with no branch would otherwise vanish from the merged checkpoint — which
        # loads and is wrong, the exact failure the vocabulary check above claims to prevent.
    ...
    def gather_fused_expert_state_dict(
        ...
        """GptOss's checkpoint layout IS the fused one (interleaved gate/up + biases), so the fused
    ...
    def _gather_interleaved_experts(
        ...
        bias-less TP ranks cannot satisfy. The grouped-GEMM branch re-interleaves in the
    ...
    def _route_with_bias(self, router_input: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        The sign-updated bias enters via ``F.linear`` like at serving, ``_balancing_bias`` returns None
        (native mode), and softmax is monotone — so selection is the native top-k over bias-inclusive
        logits and the gate is the native softmax over the selected values. Trainer and server compute
        the identical route."""
        # The adopted bias integrates sign-steps in fp32, but the linear needs one dtype: under
        # ep1+fsdp_shard_ep1_experts the router computes in bf16 (see ``_fp32_router_input``), and
        # F.linear rejects a mixed-dtype bias — cast at the read, never at the state. ``None``-safe:
        # a transient-fallback route (adoption declined at enable time) reaches here with the
        # side-buffer applied inside ``_deepseek_biased_route`` and a possibly bias-less router.
    ...
    def _expert_forward(self, idx: int, x: torch.Tensor) -> torch.Tensor:
        """One local expert, off whichever layout this rank stores (see the base method): ETP keeps
        gate/up de-interleaved under the plain names, every other configuration keeps the pair
        interleaved."""
        if self.expert_tp_size > 1:
