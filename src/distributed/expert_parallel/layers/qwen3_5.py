...
class EPQwen3_5MoELayer(EPMoELayerBase):
    ...
    # the only balancing this family can get under PP, which rejects ``aux_loss`` outright.
    # ``_ep_severs_aux_loss`` stays False: the wrapper calls the real ``Qwen3_5MoeTopKRouter`` module,
    # so the HF OutputRecorder still fires — severing it would also flip every non-PP run to
    # ``bias_update``, and the recorder path is live for the text-only ``ForCausalLM`` sibling.
    _supports_bias_balancing = True

    # Hub layout: ``experts.{i}.{gate,up,down}_proj.weight`` per expert — transformers registers this
    # family against the ``qwen2_moe`` converter, which fuses those into ``experts.gate_up_proj`` on load
    # and reverts on save. The gather writes the FUSED tensor (no ``_PER_EXPERT_UNFUSED_KEYS``, no gather
    # override), so a save that bypassed ``save_pretrained``'s revert is repaired by ``unfuse_moe_experts``
    # under these names.
    ...
    # Singular, unlike the rest of the roster; the sigmoid gate below is a SECOND replicated module,
    # which is what ``replicated_named_params`` is overridden for.
    ...
    def forward(self, hidden_states: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        if self._balancing_bias(_router_logits) is not None:
            ...
            # bias-adjusted probabilities, gate on the unbiased renormalized softmax at those indices.
            # That is what Qwen3_5MoeTopKRouter computes, so a zero bias selects the same experts and
    ...
    def _gate_weights_at(self, router_logits: torch.Tensor, indices: torch.Tensor) -> torch.Tensor:
        """``Qwen3_5MoeTopKRouter`` weights at ``indices`` — the shared renormalized-softmax gating. This
        router carries no ``norm_topk_prob`` attribute and renormalizes unconditionally, which is the
        helper's default."""
