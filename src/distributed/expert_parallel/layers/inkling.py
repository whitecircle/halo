...
class EPInklingMoELayer(EPMoELayerBase):
    ...
    # slot is part of the checkpoint.
    _NATIVE_BALANCING_BIAS_ATTR = "gate.e_score_correction_bias"
    # The causal-LM loss never reads router_logits, so there is no aux-loss path either way and
    # ``moe_balancing: auto`` resolves to bias_update.
    ...
    def _init_routing(self, original_layer: nn.Module) -> None:
        ...
        if self.n_shared_experts < 1:
            # ``_route`` slices the last ``n_shared_experts`` logit columns; at 0 the ``[..., :-0]``
            # slice would drop every routed logit instead.
    ...
    def forward(self, hidden_states: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        # enters between the logits and the top-k, and the shared logits are needed for the joint
        # normalisation that produces ``gammas``.
    ...
    def _route(self, router_logits: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Routed and shared logits are normalised together, so a shared expert takes mass away from
        # the routed ones; splitting this into two softmaxes would change the weights.
