...
class EPQwen3MoELayer(EPSeparateGluMoELayerBase):
    ...
    ``Qwen3MoeExperts`` provides fused ``gate_up_proj``/``down_proj``; we split
    gate_up and store the halves in matmul convention.
    """
    ...
    # no balancing at all under pipeline parallelism, which rejects aux_loss outright.
    ...
    # Deliberately NO gather_fused_expert_state_dict: SGLang 0.5.17's qwen3_moe loader maps
    # per-expert names only (the ``*_fused`` variant is gpt_oss-only), so fused keys are dropped
    # silently and every synced step would serve a trained router over launch-weight experts. The
    # absent override keeps ``implements_fused_expert_layout()`` False, so the construction gate
    # refuses SGLang here; re-add only against a loader that verifiably consumes the fused pair.

    def _gate_weights_at(self, router_logits: torch.Tensor, indices: torch.Tensor) -> torch.Tensor:
        """``Qwen3MoeTopKRouter`` weights at ``indices`` — the shared renormalized-softmax gating, whose
        ``norm_topk_prob`` this family carries as a configurable (default-off) router attribute."""
    ...
    def forward(self, hidden_states: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        if self._balancing_bias(_logits) is not None:
            ...
            # unconditional renormalization in _deepseek_biased_route would not, and a zero bias has
            # to reproduce the unbalanced route exactly. _biased_topk applies the routing replay itself.
