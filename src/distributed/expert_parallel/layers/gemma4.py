"""EP wrapper for Gemma-4 routed experts (the router is in the parent decoder layer)."""
...
class EPGemma4MoELayer(EPMoELayerBase):
    ...
    Gemma4 inlines MoE compute inside ``Gemma4TextDecoderLayer.forward``, with no encapsulating
    ``*MoeBlock``. Only experts are replaced; the sibling ``Gemma4TextRouter`` stays FSDP-managed, so
    ``fp32_router`` has no effect and the router stays bf16 under EP (``fp32_non_ep_params`` is not a
    substitute: it would upcast the router, and DeepEP's combine is bf16-only). Tokens arrive flat
    ``[B*S, H]`` with routing weights already normalized and scaled.

    Expert TP is supported via the base fused-GLU helper; TP/CP for Gemma4 attention is not
    ...
    # The sibling router passes only pre-weighted (indices, weights), with nothing to re-derive.
    _supports_routing_replay = False

    # Unvalidated for this family and documented to fail at the first dispatch
    # (agent-docs/models/gemma4.md); rejected at load rather than as a raw DeepEP C++ assert.
    _supports_fp32_non_ep_params = False

    # ``gate_up_proj`` is ``[E, 2M, H]``, so its length is the expert count.
    ...
    def _find_experts_container(cls, layer: nn.Module) -> nn.Module:
        """The wrapped module is itself the expert container: Gemma4 inlines MoE in its decoder
        layer, so there is no block to read one off."""
        return layer

    def _init_routing(self, original_layer: nn.Module) -> None:
        """No-op: the sibling ``Gemma4TextRouter`` passes the routing decision in already made, and
        ``Gemma4TextExperts`` carries no routing knob."""
    ...
    def gather_expert_state_dict(self, device: str = "cpu", merge_lora: bool = False, retain: bool = True) -> dict:
        ...
        double to ``...experts.experts.gate_up_proj``. Strip it to match the checkpoint.
        """
        return {
