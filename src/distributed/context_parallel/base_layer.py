...
Model-family wrappers in :mod:`.layers` implement two hooks (Q/K/V projection,
rotary embedding); this base handles the all-to-alls, GQA flash attention, and
the legacy fallback. Two forward paths:

- **Optimized** (GptOss / Qwen3 default): RoPE on the local chunk *before* the
  all-to-all, then ``flash_attn_func`` with native GQA (no ``repeat_kv``, no
  transposes — both sides ``[B, S, H, D]``).
- **Legacy** (the MLA families, whose Q/K and V head dims differ): all-to-all first,
  gather position embeddings, then attention in ``[B, H, S, D]``.
"""
...
# spelling as an attn_implementation, and a mismatch would admit a model this loader cannot serve.
...
def _adapt_to_fa2_contract(func):
    ...
    Two mismatches, both silent-to-loud failures rather than wrong numbers:

    * ``flash_attn.cute.flash_attn_func`` (FA4) does not take ``dropout_p``; passing it raises
      TypeError. Kwargs the kernel lacks are dropped when they are neutral (dropout 0, full window)
      and raise when dropping would change semantics.
    * FA4 returns ``(out, lse)`` even at its default ``return_lse=False``, where FA2 returns the
      output tensor alone. CP transposes the result straight away, so the tuple has to be unwrapped
      here — otherwise every CP forward dies on ``'tuple' object has no attribute 'is_contiguous'``.
    """
...
def get_flash_attn_func(allow_fa4: bool = True):
    ...
    CP calls ``flash_attn_func`` directly, not via the model's attention module, so this probe — not
    the model's ``_attn_implementation`` — decides which kernel a CP run actually executes. Each
    arch-specific probe must therefore come BEFORE the FA2 one: both images ship flash-attn 2, so an
    FA2 probe placed first wins everywhere and the arch kernel below it is dead code.

    ``allow_fa4=False`` vetoes the Blackwell probe for the families whose FA4 backward emits NaN
    gradients. Because this probe overrides the configured label, those models cannot opt out by
    requesting FA2 the way they can on the non-CP path — the veto is their only protection.
    """
    # Hopper (sm_90) must not take the FA2 path: the image stubs out the split-K kernels that FA2's
    ...
    # FA4 only on data-center Blackwell (SM100+): its CuTe kernels do not run elsewhere. This is the
    # backend the model was labelled with and whose kernels warmup_fa4_kernels already pre-compiled,
    # so CP running FA2 here would both diverge from the rest of the toolkit and pay FA2's slower
    ...
    try:
        ...
        # Chained, not swallowed: this last probe also hits the hub, so a cache/auth/kernel failure
        # here is NOT "flash-attn is not installed" and must not be reported as one.
        raise ImportError(
...
class UlyssesAttentionBase(nn.Module, ABC):
    ...
    # False for the families off the optimized path — set on :class:`MLAUlyssesAttentionBase`,
    # whose Q/K and V head dims differ.
    ...
    # replaces wholesale, the running kernel coming from get_flash_attn_func. False declares that the
    # wrapper's geometry is flash-compatible for a family whose modeling code cannot carry a flash
    # label at all (v4-era remote code declaring `_supports_flash_attn_2`, which transformers v5
    # ignores in favour of `_supports_flash_attn`), so validation reads that guarantee instead of a
    # label about dead code.
    ...
    # sequence, so a family hook needing positions there needs these — not this rank's chunk.
    ...
    def __init__(
        ...
        # This wrapper's kernel probe overrides the configured label, so a family whose FA4
        # backward emits NaN gradients cannot protect itself by requesting FA2 the way it can on
        # the non-CP path. Resolved once here, not per forward.
        self._allow_fa4 = not model_fa4_backward_nan_prone(self.config)

        # LAST: a family's own derived state, after every field above exists — the hook a family
        # extends instead of overriding __init__ for one derived attribute.
    ...
    def _resolve_scaling(self) -> float:
        """The softmax scale. Families whose attention module computes it inline in their own forward
        (Bailing divides the scores by ``sqrt(head_dim)``) override; ``self.head_dim`` is already set."""
        return self.original_attention.scaling

    @staticmethod
    def _resolve_num_kv_heads(attention: nn.Module) -> int:
        """The head count of the K/V tensors :meth:`_project_qkv` emits, read off the UNPATCHED HF
        module so this wrapper and :func:`~src.distributed.context_parallel.validation.
        validate_model_for_ulysses` answer from one seam rather than two readings that can drift.
    ...
    def _output_projection(self) -> nn.Module:
        """The attention output projection. Resolved per call — PEFT wraps the projection after this
        wrapper is built. Families that name it something other than ``o_proj`` (Bailing's ``dense``)
        override."""
    ...
    def _project_qkv(
        ...
        nothing is stashed on the module (re-entrant under gradient-checkpoint recompute).
        """
    ...
    def _project_qkv_plain(
        ...
        The whole projection for the families that add nothing before RoPE (GptOss, Cohere2 MoE);
        head counts are spelled out rather than inferred, so a projection whose width disagrees with
        the config raises here instead of silently reshaping.
        """
    ...
    def _apply_rotary_pos_emb(
        ...
        Reached only from :meth:`_forward_legacy`, so only the families off the optimized path
        (``_optimize_attention = False``) implement it. Not an abstractmethod for exactly that
        reason: as one it would force every optimized family to write an unreachable, untested copy.
        """
    ...
    def _apply_rotary_core(
        ...
        the rotate-half convention over the leading ``cos.shape[-1]`` channels — what Qwen3,
        Qwen3.5/3.6 and Bailing all use. Families with a different rotary override (GptOss's
        split-concat, Cohere2's interleaved fp32).

        A default rather than a raise, so the families sharing the convention do not restate it. The
        cost: a NEW optimized family with a different rotary silently gets rotate-half, so adding one
        means checking its rotary against this default (``tests/cpu/parallelism/
        test_cp_qwen3_rotary.py`` is the per-family oracle).
        """
    ...
    def _post_rope(
        ...
        ``position_ids`` describe the sequence axis Q/K actually carry at this point: this rank's
        LOCAL chunk on the optimized path (RoPE before the all-to-all), the wrapper-published
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Per-family choice only: validate_model_for_ulysses already guaranteed a flash backend.
    ...
    def _forward_optimized(
        ...
        """Optimized forward: RoPE before all-to-all + native GQA flash attention.

        Vs legacy: no position-embedding all-gather, no ``repeat_kv`` (flash_attn
        handles GQA natively, saves ~6 GB/forward on 20B-class models), no transposes.
        """
    ...
    def _flash_call_kwargs(self, kv_len: int) -> dict:
        """The flash-attn call kwargs every CP attention path shares (one home for the sliding-window
        expression — HF convention: the window applies only past its own length, minus 1 for the
        diagonal). MLA families carry no sliding window, so their path degenerates to ``(-1, -1)``.
        """
        return {
...
class MLAUlyssesAttentionBase(UlyssesAttentionBase):
    ...
    Owns the whole shared MLA layer: the geometry read off the wrapped module, the two rotate-half
    rotary helpers, the compressed-KV :meth:`_project_qkv`, the rope/nope-split
    :meth:`_apply_rotary_pos_emb`, and the V-padding attention fn. Subclasses declare only what is
    family-specific — their HF class names and, where they have one, a hook of their own.
    """

    # MLA's compressed projections and nope/rope split need the [B, H, S, D] legacy path, and
    # ``v_head_dim`` may differ from ``qk_head_dim`` (guarded V-padding below) — flash-attn's
    # optimized GQA path assumes plain per-head QKV of one head dim.
    ...
    def __init__(
        ...
        # MLA families spell the query/key head dim ``qk_head_dim``; the parent reads ``head_dim``
        # off the wrapped module. Back-fill before delegating, and only where the family carries no
        # ``head_dim`` of its own.
        ...
        # Whether Q rides the LoRA-style compression, asked of the modules :meth:`_project_qkv`
        # actually calls; KV is always compressed. Presence alone is not the verdict: transformers
        # keeps the unused branch's attribute present but None.
        self.use_qkv_lora = getattr(original_attention, "q_a_proj", None) is not None

        # Read unconditionally, exactly as both families' own forwards do — their configs default it
        # to True, so a defensive ``getattr(..., False)`` would silently invert the family's rotary.
    ...
    def _resolve_num_kv_heads(attention: nn.Module) -> int:
        ...
        head — ``config.num_key_value_heads`` does not describe that tensor. Derived from the
        projection itself so the wrapper's head math cannot disagree with the weights it reshapes."""
    ...
    def _flash_attention(
        ...
        dims already match (GLM4-MoE-Lite and Mistral4 both ship them equal). MLA families have no
        sliding window, so the shared kernel runs unwindowed.
        """
    ...
    def _project_qkv(
        ...
        Q optionally rides the LoRA-style compression (``use_qkv_lora``); KV is always compressed —
