...
def patch_attention_for_ulysses(
    ...
    Raises:
        UlyssesConfigError: If validation is enabled and the model is incompatible, or if no
            attention layer was patched — a CP run with zero wrapped layers attends over each
            rank's local sequence chunk only.
    """
    if validate:
    ...
    def build(path: str, attention: nn.Module, wrapper_cls: type) -> nn.Module:
        ...
        # The wrapper is fully constructed here, so per-layer geometry is logged from this point
        # rather than once per family __init__.
