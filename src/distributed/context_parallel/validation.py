...
# Every rejection below says the same thing about a different mechanism: a layer carrying state ALONG
# the sequence axis cannot be sharded by it, because the Ulysses path halo-exchanges nothing across
# chunk boundaries. One sentence, one (mechanism, remedy) pair per family — so a family cannot borrow
# another's reason and the shared half cannot drift between them.
...
# Config-only rejection for the ``linear_attention`` layer type: every family that declares it runs a
# causal convolution and a recurrent scan (the instantiated-tree scan names the class once the modules
# exist).
...
# Layers crossing sequence chunks (Conv1d, recurrent scans, window pooling). Name -> rejection detail
# in one table: a second, parallel name ladder would let a new entry raise some other family's message.
...
def validate_model_for_ulysses(model: nn.Module, cp_size: int) -> None:
    ...
    # The one reader of the resolved backend: a composite (VLM) wrapper records it on the TEXT
    # sub-config and only sometimes mirrors it up, so reading the top level first reports the
    # wrapper's absence for a model that is running flash attention.
    ...
    # for the families whose own attention forward is a real fallback path (see the wrapper flag).
    ...
    # describes a tensor this path never builds. One seam, or the two readings drift.
    try:
