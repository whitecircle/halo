...
def _reject_left_padding(attention_mask) -> None:
    ...
    without CP. Reachable from ``DataCollatorForSMPO``, which left-pads prompts unconditionally.

    Runs on EVERY forward. No batch can certify the ones after it: the collator this guard exists for
    left-pads prompts and right-pads completions, so a batch whose prompts happen to be equal-length
