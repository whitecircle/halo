...
Standard softmax GQA inside a transformers-v4-era remote-code shell, so the wrapper carries the
family's spellings rather than any new attention math:
...
are claimed because they hold the same weights and CP replaces their forward either way. Ling 3.0
(``BailingMoeV3*``) and the Lightning-Attention-2 sibling are separate architectures — the latter is
rejected by :mod:`~src.distributed.context_parallel.validation`, whose class names it shares.
"""
...
class BailingMoeV2UlyssesAttention(UlyssesAttentionBase):
    ...
    # in favour of ``_supports_flash_attn``, so the model refuses to build under any flash label and
    # every Bailing run is labelled sdpa. That label describes the family's own forward, which this
    # wrapper replaces; the geometry (uniform head_dim 128 GQA, no sinks, no additive bias) is what
    # CP's flash kernel needs.
    ...
    def forward(
        ...
        **kwargs,
        ...
        handed straight back the way the family's own forward does — CP never writes one.
        """
