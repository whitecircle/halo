...
class GptOssUlyssesAttention(UlyssesAttentionBase):
    ...
    # Flipped on the instance by the first forward's check; a class-level default needs no ctor.
    _sinks_checked = False

    @property
    def sinks(self):
        """Live view of the owner's sinks — never a second binding.

        An ``__init__``-time alias would also REGISTER the parameter on this wrapper: the FA2 sinks
        policy's rebind would then land here while ``original_attention`` kept the live pretrained
        tensor, which the save paths surface under the clean HF key (prefix-strip) — a trained-with-
        no-sinks checkpoint shipping live sinks. Read-only: the policy rebinds through the owner.
        """
        return self.original_attention.sinks

    def _check_sinks_neutralized(self) -> None:
        """Fail loud at first forward if the sinks were NOT neutralized.

        The CP flash paths never pass sinks to the kernel, so frozen pretrained sinks
        (reset_sinks: false) would give a wrong softmax normalization in every layer. Reset
        sinks (None, or dtype-min fill contributing exp(min)==0) match eager. Checked at forward,
        not construction, because ``_reset_gpt_oss_sinks`` runs after the CP wrap.
        """
