...
class Mistral4UlyssesAttention(MLAUlyssesAttentionBase):
    ...
    def __init__(
        ...
        # Fail loud rather than train silently unscaled: Mistral4Attention.forward applies this
        # scale unconditionally, so a CP run that skipped it would train a different attention than
        # the family defines — and than every non-CP run of the same checkpoint.
    ...
    def _post_rope(
        ...
        """Scale Q by the position-dependent llama-4 factor (legacy ``[B, H, S, D]`` layout)."""
