...
def _load_tp_model(
    ...
    # No max_concurrent_loading throttle here, unlike every other loader: this load ends in
    # ``tie_weights``, whose equality check on a tied pair of DTensors is an all-reduce on the DEFAULT
    # process group, and a rank-serialized region deadlocks it (the loading rank blocks in the
    # collective while its peers wait their turn). The throttle would buy little anyway — transformers streams the checkpoint
    # key by key and places each rank's shard straight on its GPU, so host RAM never holds the model.
    ...
    # throughput. Read off the applied plan, which names the failure (a plan covering no parameter,
    # or covering them only with transform-only styles) rather than reporting an absent shard.
