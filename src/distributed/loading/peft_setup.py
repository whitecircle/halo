...
Folding a saved adapter back into its base is the artifact side of the same story and lives with the
rest of the adapter format (:mod:`src.checkpoint.adapters`)."""
...
def _reject_lora_target_parameters_under_ep(model: PreTrainedModel, model_config: ModelConfig) -> None:
    ...
    ``lora_target_parameters`` makes PEFT wrap the module *owning* the parameter and swap it into its
    parent — for an expert weight that module is the ``EPMoELayerBase`` itself, so the adapters land
    above the EP layer where neither EP validator looks and no EP gradient sync reaches. Gated on the
    live model, not on "is this MoE": without EP wrappers (``ep_size<=1`` + ``use_grouped_gemm: false``,
    the accelerate route) the experts are ordinary modules and this is stock PEFT working as intended.
    """
...
def _reject_unsupported_expert_lora_knobs(model_config: ModelConfig, projections: set[str]) -> None:
    ...
    Stock PEFT never sees the expert projections — they are peeled to :class:`ExpertLoraSpec`, which
    carries ``r``/``alpha``/``dropout``/``use_rslora`` and nothing else. Anything outside that set
    would apply to the attention half of the very same run and silently not to the expert half, so it
    is refused here rather than half-applied.
    """
...
def split_expert_lora_targets(model_config: ModelConfig) -> ExpertLoraSpec | None:
    ...
    EP experts are grouped tensors stock PEFT can't wrap, so expert names are peeled out (mutating
    ...
    # string naming no projection is a sentinel or regex ("all-linear", ".*gate_proj") and peels nothing.
    ...
    # The peel rewrites the user's list in place, and it is name-based, not module-based: on a hybrid
    # MoE the same names also spell plain nn.Linear MLPs (dense prefix layers, shared experts) that
    # neither half then adapts. Say so — the run record downstream shows only the remainder.
    warnings.warn(
...
def _pins_a_decoder_layer_index(pattern: str) -> bool:
    ...
    Structural, not a digit scan of the whole pattern: the segment that matters is the one FOLLOWING
    the decoder-layer list (:data:`DECODER_LAYER_LIST_ATTRS`, the repo-wide spelling), and it pins an
    index whenever it carries a digit anywhere. That covers the character-class ranges real configs
    use — ``model.layers.5[6-9].*``, ``model.layers.[6-8][0-9].*`` — which a per-segment
    ``str.isdigit`` reads as index-free because ``"5[6-9]".isdigit()`` is False. It also stops
    ``*.mlp.experts.0.*`` from being refused: an expert index is identical on every stage.
...
def _reject_layer_indexed_patterns_under_pp(model, patterns, flag: str) -> None:
    ...
    Under PP the stage-aware loader hands back a model holding only THIS stage's layers, re-based to
    index 0. A global-space pattern like ``model.layers.30.*`` therefore matches nothing on most
    stages and a different layer wherever local index 30 exists — silently, since the freeze path has
    no matched-nothing check. Index-free patterns (``*.self_attn.sinks``) mean the same thing on every
    stage and stay allowed.
    """
...
def setup_peft_model(
    ...
    # None targets = architecture defaults, [] = expert-only; conflating them full-finetunes a `use_peft` run.
    targets = model_config.lora_target_modules
    ...
    if not attention_peft:
        # Expert-LoRA only: no PeftModel exists, so every remaining LoraConfig field is inert. The one
        # a user can reasonably expect to still apply is modules_to_save, and its failure would be a
        # silently frozen router/head rather than an error.
...
def freeze_modules_by_patterns(model, patterns):
    ...
    Raises when a pattern matches nothing, mirroring :func:`unfreeze_modules_by_patterns`: freezing
    is additive, so a typo'd entry leaves exactly the parameters it was meant to hold still training
    while every other pattern works — the run looks entirely normal. Under pipeline parallelism this
    holds each stage to patterns every stage carries, the same bar its counterpart already sets by
    refusing a stage with nothing trainable.
    """
    ...
    # World-uniform: under PP a stage whose patterns miss must take every rank down with the
    # diagnostic — a stage-local raise leaves its peers hanging in the next collective.
...
def unfreeze_modules_by_patterns(model, patterns):
    ...
    Raises when the patterns leave nothing trainable: this freezes everything first, so a typo'd
    pattern would train a fully frozen model — the optimizer accepts empty parameter groups and the
    loss simply never moves.
    """
    ...
    # Same world-uniform contract as freeze_modules_by_patterns: a stage-local verdict must not
    # leave peer stages hanging in the next collective.
...
def peft_bf16_autocast(casted_to_bf16: bool, device: torch.device) -> contextlib.AbstractContextManager:
    ...
    a hardcoded "cuda" would be inert on other backends — and bf16 is explicit because torch's cuda
    autocast defaults to fp16, which would narrow the adapters again.
    """
...
def _peft_module_casting_to_bf16(model):
    for name, module in model.named_modules():
        if isinstance(module, BaseTunerLayer):
            ...
            # Class-derived, like every other norm classification here: a family whose norm modules
            # sit at a path without "norm" in it (or spell the class LayerNorm-alike) is still kept
            # in fp32, and a non-norm module under a "norm"-named parent is not upcast.
