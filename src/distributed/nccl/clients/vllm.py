"""NCCL weight sync and HTTP generation for vLLM servers.

The trainer joins as rank 0 and the vLLM workers as rank 1+; weights go over a packed NCCL broadcast.
The vllm package is never imported here.
"""
...
# Server control-plane routes used from more than one call site (the call, its async-call label, and
# the error text naming it), so a route rename cannot leave a stale label behind.
...
class VLLMWeightSyncClient(BaseWeightSyncClient):
    ...
    def __init__(
        ...
        # Kept so close_communicator can release the listener; the group carries only the NCCL id.
        self._process_group: StatelessProcessGroup | None = None
        # Persistent producer streams: per-call streams leave every sync's pack allocations in
        # unreachable per-stream allocator pools (see packed_broadcast_producer). Created lazily on
        ...
        # Derived rather than a literal, since the call blocks every peer at the next collective and
        # must stay under the watchdog, including when the watchdog is lowered.
    ...
    def probe_generation(self):
        ...
        Only a hang counts as wedged: the completion below is a plain request, so a timeout is
        conclusive rather than something the session would retry away.
        """
        # A paused engine queues new requests instead of rejecting them, so the probe below would only
        # time out and report a wedged scheduler. Ask the server first.
    ...
    def init_communicator(self, device: torch.device | str | int = 0):
        ...
        # Detect a paused or generation-wedged server before forming the group or running rollouts.
    ...
    def _require_communicator(self) -> PyNcclCommunicator:
        """The live communicator, raising when there is none; an aborted one broadcasts into nothing."""
    ...
    def begin_weight_update(self):
        ...
        Unwinds itself on failure: nothing has been broadcast yet, so a server left paused behind a
        half-opened phase would wedge the next rollout round.
        """
        ...
        try:
            ...
            # Single-shot: /start_weight_update is a collective RPC and a session retry would issue two.
                ...
                fn=lambda: self._post_once(
                    ...
                    json={
                        "update_info": {
                            ...
                            "shapes": shapes,
                            "packed": True,
                            # Sent explicitly: the consumer derives its chunk boundaries from these,
                            # and a mismatch desynchronizes every broadcast count.
    ...
    def end_weight_update(self, tail: list[tuple[str, torch.Tensor]]):
        """Send the final chunk, close the layerwise reload and resume; the close runs on every path."""
        try:
    ...
    def _close_phase_and_resume(self):
        """Close a started-but-unfinished phase and lift the pause. Does not raise: it runs where the
    ...
    def close_communicator(self):
        ...
        if self.communicator is not None:
            # ncclCommAbort rather than a dropped reference: PyNcclCommunicator has no __del__, so the
            # communicator and the device memory NCCL pinned for it would outlive every retired client,
            # one leak per reconnect and per served endpoint.
            self.communicator.abort()
        self.communicator = None
        self._packed_streams = None
        if self._process_group is not None:
            # Frees group_port for the next client on this host; the group only carried the NCCL id.
    ...
    def _completion_ids(self, choice: dict) -> list[int]:
        ...
        Not reconstructed from the decoded text: a BPE re-merge need not reproduce the sampled stream,
        and ids that disagree with the logprobs desync the GRPO importance ratio. A missing field means
        the server ignored ``return_token_ids``, which raises.
        """
    ...
    def generate(
        ...
        **kwargs,
        ...
        # /v1/completions needs logprobs >= 1 to populate token_logprobs; GRPO defaults to 0.
        ...
        # One batched request: continuous batching schedules all prompts together (wall clock ~max).
