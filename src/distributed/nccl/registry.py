...
def _client_registry() -> dict[str, type[BaseWeightSyncClient]]:
    ...
    Restricted to the clients shipped in this package. ``__subclasses__()`` returns every class
    that ever subclassed the base, including one defined outside it — a test double, or this
    module tree loaded a second time under another name — and such a class silently capturing a
    backend key would swap the production client out with no error at all.
    """
...
def rollout_backends() -> list[str]:
    """Every selectable rollout backend — the test seam pinning the registry against the config
    ``Literal`` (``rollout_backend``), so a new client cannot land unselectable."""
