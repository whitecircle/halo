...
class StatelessProcessGroup:
    ...
    def broadcast_obj(self, obj: Any, src: int) -> Any:
        ...
        Send-only: the trainer owns this group as rank 0 and every exchange it drives has ``src=0``,
        so a receiving rank here would be a group built wrong rather than a supported shape.
        """
    ...
    def create(
        ...
        multi-homed node to accept the peer regardless of NIC while still advertising routable ``host``.
        """
    ...
    def close(self) -> None:
        ...
        The C++ store owns the listening fd (handed over as ``master_listen_fd``) and its daemon
        thread polls it, so closing the Python socket under a live store aborts that thread
        ("Unexpected poll revent on the master's listening socket"). Detaching hands the fd to the
        store alone; dropping the last store reference then runs the destructor, which stops the
        daemon and closes the fd. Dropping the reference HERE rather than letting the group be
        garbage-collected is what makes the release deterministic: whoever still holds this group
        (a thread parked in ``ncclCommInitRank``, a traceback) no longer holds the port with it.
        """
