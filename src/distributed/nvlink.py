...
``NVLINK_DOMAIN_SIZE`` against the fabric the GPUs actually report, and the Multi-Node NVLink
prerequisites.

Verdicts only — nothing here builds a process group. Consumed by
...
# Long enough for a loaded 64-node host, short enough that a wedged driver cannot stall startup.
_NVIDIA_SMI_TIMEOUT_S = 30

# Definite "this GPU is NOT on an MNNVL fabric" — distinct from ``None`` ("could not tell"), which is
# what lets the domain cross-check reject a too-wide domain. Negative: never a real clique id.
NO_FABRIC = -1
...
def get_nvlink_domain_size(default: int) -> int:
    ...
    ``NVLINK_DOMAIN_SIZE`` env → ``default``; the caller owns the fallback because only it knows its
    per-node count (a hand-built ``ParallelismConfig`` sets ``gpus_per_node``, which
    ...
    if domain is not None:
        if domain > 0:
            return domain
        # 0 is ParallelismConfig's "unset" sentinel: it slips the validators and dies as a bare
...
def _parse_fabric_clique(nvidia_smi_query: str) -> int | None:
    ...
    Keeping the three apart is what lets the cross-check judge a declared domain on non-MNNVL
    hardware instead of no-op'ing there: :data:`NO_FABRIC` (no ``Fabric`` block, or clique 0) is the
    DEFINITE "not on an MNNVL fabric" every plain HGX/NVL8 part answers, while ``None`` (registration
    not ``COMPLETED``, or an unparseable id) is "could not tell" over a possibly real fabric.
    ...
    for line in nvidia_smi_query.splitlines():
        ...
        if key == "State":
            ...
            clique = value
    if indent is None:
        return NO_FABRIC  # no Fabric block at all — a definite "not an MNNVL part"
...
def nvlink_fabric_clique_id() -> int | None:
    ...
    inside the C call — uncatchable, on every rank, during ``ParallelismConfig.__post_init__``.
    """
    try:
...
def _fabric_clique_for_device(device_index: int) -> int | None:
    ...
    Fabric membership is fixed once the driver is up, and the read costs ~60 ms — which every
    ``ParallelismConfig`` construction reaches through the MNNVL advisory. Invisible to a training
    process (one config); worth an hour to the config grid sweep, which builds tens of thousands.
    """
    try:
        # By PCI bus id, not index: nvidia-smi enumerates every host GPU while the local rank indexes
        # the CUDA_VISIBLE_DEVICES-masked view, so under masking the clique read is another rank's.
...
def node_placement_contradiction(host_ids: list[str], gpus_per_node: int) -> str | None:
    ...
    (:mod:`~src.distributed.group_layout`), which is a machine only while rank ``r`` sits on node
    ``r // gpus_per_node``. One torchrun per node makes that true by construction; a launcher that
    numbers ranks round-robin over the allocation (``srun --distribution=cyclic`` with an exported
    rendezvous) does not, and nothing else would notice: the groups straddle machines while
    ``requires_rdma`` still reports False, so DeepEP takes its intranode CUDA-IPC path to peers on
    another host.
    """
...
def validate_nvlink_domain_against_fabric(nvlink_domain_size: int, world_size: int, gpus_per_node: int = 0) -> None:
    ...
    ``nvlink_domain_size`` is an unchecked assertion about physics whose two failure modes are both
    silent: a domain straddling two racks makes every "node-local" group half RDMA while
    ``requires_rdma`` still reports False, and one left at ``gpus_per_node`` on an NVL72 rack caps
    expert sharding at 4-8 GPUs.

    COLLECTIVE and rank-uniform by construction — every rank contributes its clique id and decides
    from the same gathered list, so this can never raise on a subset of ranks (the caller has already
    rejected a per-node drift in the arguments themselves). A ``None`` anywhere no-ops the FABRIC
    legs: a partial view is not evidence. :data:`NO_FABRIC` IS evidence, and ``gpus_per_node`` then
    rejects a domain wider than the node its ranks sit on (omit it to skip that leg). Every fabric
    verdict is taken per DOMAIN — one job may hold an NVLink island and fabric-less nodes at once.

    The machine identity rides along in the same gather and carries its own verdict
    (:func:`node_placement_contradiction`), taken BEFORE the fabric legs because it is the
    assumption they and the whole rank layout rest on, and no clique read has to succeed for it.
    """
    if not (dist.is_available() and dist.is_initialized()):
        return
    # Size the buffer from the LIVE group, never the declared world: all_gather_object fills exactly
    ...
    # NO_FABRIC is one sentinel shared by every fabric-less node, never an island of that many ranks,
...
def imex_channels_present() -> bool:
    ...
    A filesystem fact, readable with no NVIDIA tooling; an unlistable caps dir is a definite "no",
    never a raise (this runs inside a rank-uniform gate that must reach its collective).
    """
    try:
...
def mnnvl_prerequisite_reason(imex_ok: bool, fabric_clique: int | None) -> str | None:
    ...
    Pure verdict for one rank, asked only where the topology REQUIRES MNNVL. There a ``None`` clique
    is not the benign "could not tell" it is elsewhere: it is what an incomplete Fabric Manager
    registration reports, which is the failure this check exists to catch.
    """
    if not imex_ok:
        return (
...
def check_mnnvl_prerequisites(nvlink_domain_size: int, gpus_per_node: int) -> None:
    ...
    Manager fails by name instead of as a peer-memory fault mid-dispatch — a rank-0-only look would
    miss exactly the failing node.

    Below that threshold, and outside a live multi-rank job, this only advises: a fabric present
    while ``NVLINK_DOMAIN_SIZE`` is unset caps node-local TP/CP/EP at one node for nothing.
    """
