...
Single source of truth for group membership (``ParallelismConfig`` and ``EPConfig``/``CPConfig``
construction both call these). Dependency-free leaf so the CP path shares it without importing DeepEP.
...
def etp_dispatch_coords(ep_rank: int, ep_size: int, expert_tp_size: int, node_local: bool) -> tuple[int, int]:
    ...
    ETP partners share ``dispatch_ep_rank`` and therefore a DP batch — this single formula keys both
    ``EPConfig``'s group construction and ``ParallelismConfig.get_data_parallel_rank``.
    """
    if node_local:
...
def cross_node_layout(world_size: int, ep_group_size: int, nvlink_domain_size: int) -> tuple[int, int, int]:
    ...
    Raises:
        ValueError: if ``ep_group_size`` cannot tile the world as equal contiguous per-domain
            blocks (each group spanning every NVLink domain). The message points at the two
            always-valid shapes: a single global group, or NVLink-local EP.
    """
...
def reject_node_local_ep_group(
    ...
    """Refuse a node-local EP group that does not tile its NVLink domain.

    A node-local dispatch group is a contiguous block inside one domain, so it must both fit in the
    domain and tile it evenly. Enforced twice on purpose — ``ParallelismConfig`` gates the whole run
    at config time, ``EPConfig`` guards direct construction — so the predicate and the invariant half
    of the message live here and each caller appends its own ``remedy`` (the two APIs spell the escape
    hatch differently).
    """
...
def reject_cross_node_ep_group(
    ...
    """Refuse a cross-node (global-scope) EP group that does not fit and divide its scope.

    ``scope_desc``/``scope_size`` name the scope in the caller's own terms — the world for a plain
    job, one pipeline stage's world when ``pp_size > 1``.
    """
    shape = f"ep_group_size = ep_size({ep_size}) * expert_tp_size({expert_tp_size})."
...
def reject_cross_node_etp_shape(
    ...
    """Refuse a cross-node EP+ETP shape that is not exactly one ETP group per NVLink domain.
