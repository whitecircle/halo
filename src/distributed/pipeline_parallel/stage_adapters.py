...
Each function here is that family's ``*TextModel``/``*Model`` forward with exactly one change: the
already-widened stream from the previous stage is fed straight to the layer loop instead of being
re-widened. Everything else — mask construction, rotary embeddings, the layer loop's kwargs, the
final ``norm(hc_head(...))`` — mirrors transformers' own forward for the pinned release, so a stage
stays numerically identical to the unsplit model. A family declares its own through
...
def _boundary_stream(backbone, inputs_embeds: torch.Tensor | None) -> torch.Tensor:
    ...
    Fails loud on anything but ``[B, S, hc_mult, D]``: a 3-D tensor here means the previous stage
    collapsed the stream (or a caller drove this stage like an unsplit model), and silently widening
    it by replication would train on a lossy mean of the four streams.
    """
...
def glm5_next_stream_forward(
    ...
    **kwargs,
    ...
    The one other deliberate difference: the attention mask is selected by each layer's own
    ...
    spellings are equivalent today; keying by the layer keeps that true if they ever diverge.
    """
    hidden_states = _boundary_stream(self, inputs_embeds)
    embeds_ref = hidden_states[:, :, 0]
    if use_cache and past_key_values is None:
