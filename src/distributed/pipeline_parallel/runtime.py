"""The pipeline runtime — owns the stage, the schedule, and the microbatched step.

The single seam over ``torch.distributed.pipelining``: the schedule replaces the inherited HF
training step and drives every microbatch's forward and backward itself, so PP owns gradient
accumulation and the caller must not call ``.backward()``.
...
constructing :class:`PipelineRuntime` raises.
The batch contract (:data:`PP_BATCH_PAD_VALUES`) and the runtime's public signatures are the
stable seam the trainer mixin builds against — see ``agent-docs/parallelism/pipeline-parallelism.md``.
"""
...
# Batch contract: each key the pipeline consumes → its pad value (``None`` = tokenizer pad id). The
# trainer's dataset-column pin and fixed-shape padding both derive from it, so they cannot drift.
