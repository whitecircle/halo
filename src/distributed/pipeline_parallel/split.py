...
A pipeline stage is the real backbone with its decoder-layer list sliced and the parts the stage does
not hold neutralized (see :mod:`.stage`), so each family's rotary embeddings, mask construction,
``layer_types``, sliding-window and MoE handling are inherited rather than reimplemented. The
splitter is therefore model-agnostic by default; only families whose forward threads state a
contiguous cut would sever declare themselves here.
...
# Config fields a family may use to declare multi-token-prediction tail layers; the spellings differ
# per family (DeepSeek/Bailing num_nextn_predict_layers, Qwen3.5/3.6 mtp_num_hidden_layers, Inkling
# num_mtp_layers). _reject_live_mtp_layers checks all of them.
...
class PPModelSpec:
    ...
    # HF backbone class names this spec claims (the module holding ``.layers``).
    ...
    # The norm is applied after the last decoder layer, so non-last stages neutralize it; otherwise
    # the hidden state is normalized mid-network. The decoder-layer list is not here —
    # ``backbone_with_layers`` resolves that spelling repo-wide (``DECODER_LAYER_LIST_ATTRS``).
    ...
    # hyper-connection collapse ``hc_head``). Exact attribute names, not probes: each must exist, is
    # neutralized to Identity on non-last stages, and counts as last-stage state for the stage-aware
    # loader's droppable-prefix filter.
    TAIL_MODULE_ATTRS: tuple[str, ...] = ()

    # True only where a sliced stage selects every per-layer input by the layer's own attributes
    # (``layer_idx``/``block_type``, set at construction from the global index) rather than by list
    # position — either in the upstream forward, or in the mid-chain forward this spec's
    # ``adapt_stage_backbone`` installs. Exempts the family from ``reject_layer_type_rebase`` and
    # from period-snapping the automatic partition.
    ...
    def adapt_stage_backbone(cls, backbone: nn.Module, *, is_first: bool, is_last: bool) -> None:
        ...
        binding rather than a class patch, so stage 0 and any unsplit model in the same process keep
        the real forward; ``nn.Module.__call__`` resolves ``self.forward`` through the instance, so
        FSDP2/GC hooks and the stage wrapper see the bound method. A family declaring no
        ``STREAM_FORWARD`` keeps its own forward on every stage.
        """
    ...
    def validate_partition(cls, model: nn.Module, partition: list[tuple[int, int]]) -> None:
        ...
        invalid split fails every rank together rather than leaving peers at the first collective.
...
class ZayaPPSpec(PPModelSpec):
    """Zaya carries two tensors across every layer boundary; one of them accumulates."""
...
class DeepSeekV4PPSpec(PPModelSpec):
    ...
    routers, which the partition gate confines to stage 0, the only stage that has the ids. Nothing
    in the loop indexes by list position (masks are loop-invariant, per-layer state hangs off each
    module's own ``layer_idx``), so any split offset is exact.
    """
    ...
    def validate_partition(cls, model: nn.Module, partition: list[tuple[int, int]]) -> None:
        ...
        ``input_ids`` and would fail on the ``None`` dereference mid-schedule.
        """
...
class Glm5NextPPSpec(PPModelSpec):
    ...
    sibling in transformers); a text-only run reaches this spec through the trainer's multimodal
    admission, which stashes the vision tower and projector for the save.
    """
    ...
    def validate_partition(cls, model: nn.Module, partition: list[tuple[int, int]]) -> None:
        ...
        A shared layer reuses the previous full indexer layer's top-k indices, handed layer-to-layer
        by the model loop, so a stage starting on one raises mid-schedule ("Shared DSA layers
...
def resolve_pp_spec(backbone: nn.Module) -> type[PPModelSpec]:
    """The spec claiming ``backbone``'s class, or the generic base."""
...
def tie_flag_configs(config, prefix: str = "") -> dict[str, object]:
    ...
    level, and transformers resolves the tie across every submodel, so the top level alone is not the
    whole declaration. Keyed by path rather than position so the values can be put back on a
...
def _reject_tied_embeddings(model: nn.Module) -> None:
    ...
    Stage 0 holds ``embed_tokens`` and the last stage ``lm_head``; splitting gives each rank only its
...
def _reject_live_mtp_layers(model: nn.Module) -> None:
    ...
    ``inputs_embeds`` path a non-first stage would fail on the ``input_ids=None`` dereference
    ...
    never built, which a declaration-only gate would refuse. Every declared spelling still counts, so
    a family that starts building its declared head is caught.
    """
    text = text_config(getattr(model, "config", None))
    ...
    # A pre-sliced stage (the stage-aware loader) holds fewer layers than num_hidden_layers; its
    # unsplit shell was already checked before slicing.
...
def validate_model_supports_pp(model: nn.Module, moe_balancing: str = "auto") -> None:
    ...
    Takes the run's resolved ``moe_balancing`` so the aux-loss gate judges the strategy the run
    applies; a stage severs the aux-loss path.
    """
...
def resolve_module_attr(module: nn.Module, candidates: tuple[str, ...], what: str) -> str:
    ...
    The single probe used for the layer list, the final norm and the task head.

    Raises:
        ValueError: none is present. Always fatal — a caller would otherwise skip a step (an
            un-neutralized norm, an unsliced layer list) and produce wrong numbers.
    """
...
def compute_layer_partition(
    ...
    *,
    ...
    ``pp_split`` (per-stage decoder-layer counts) overrides everything: the manual escape hatch for a
    measured imbalance. Otherwise ``head_layer_equivalents`` > 0 rebalances the last stage, which
    carries the ``lm_head`` matmul + CE (the dominant non-layer cost, since the embedding is a
    lookup): its layer budget shrinks by that many layer-equivalents, in whole multiples of
    ``boundary_period`` (heterogeneous ``config.layer_types`` select the attention mask by list
    position, so stage boundaries must land on the pattern period). Any leftover goes to the middle
    stages first. With no head weighting and an exact division every stage gets the same count.
...
def _balanced_sizes(num_layers: int, pp_size: int, head_layer_equivalents: float, period: int) -> list[int]:
    ...
    (``num_layers % period``) stays on the last stage, since ``reject_layer_type_rebase`` refuses a
    mid-period boundary.
    """
...
def head_cost_layer_equivalents(config) -> float:
    ...
    FLOPs/token of a matmul are proportional to its parameters; the attention quadratic term and the
    (free) embedding lookup are ignored, which only softens the correction. A MoE layer counts its
    per-token activated expert parameters. Returns 0.0 (uniform split) when the config lacks the
    needed fields.
    """
    text = text_config(config)
    hidden = getattr(text, "hidden_size", None)
    vocab = getattr(text, "vocab_size", None)
    # Attention and FFN widths are read through the per-layer-aware accessor: on a
    # per-layer-heterogeneous family (step3p7's 64 full / 96 sliding heads) the bare attribute raises
    # transformers' AmbiguousGlobalPerLayerAttributeError, which a getattr default does not swallow.
    # This is a cost ceiling, so ``max`` is the right reduce.
    ...
    if not intermediate:
        return 0.0
    # A MoE layer's per-token cost scales with the router width, read through the registry holding
    # its config spellings: a missed spelling would cost the layer as dense and skew the split. Gated
    # on the model being MoE, since the ``top_k`` spelling doubles as the legacy generation sampling
    # parameter a dense config may carry.
...
def layer_types_shift_invariant(layer_types, shift: int) -> bool:
    ...
    Both PP layer-type rules read this: :func:`layer_types_period` returns the smallest shift
    satisfying it, so an automatic partition can snap to whole periods, and
...
def layer_types_period(config) -> int:
    ...
    Stage boundaries must land on whole periods — several families select the attention mask by list
    position. Requiring the pattern to tile ``len(layer_types)`` exactly would miss ragged lists
    (step3p7's 45 layers = 11 whole ``full,s,s,s`` periods plus one trailing layer) and leave their
    default partition un-snapped, hence refused.
    """
    text = text_config(config)
...
def resolve_layer_partition(
    ...
    """The partition every PP consumer shares: manual ``pp_split``, else the head-weighted default."""
    backbone = backbone_with_layers(model)
    if backbone is None:
        raise ValueError(
    ...
    # need no period snapping, which for aperiodic layer_types would disable the head-weighted
    # rebalance entirely (GLM-5's 45-layer L,L,L,D pattern has no divisor period).
