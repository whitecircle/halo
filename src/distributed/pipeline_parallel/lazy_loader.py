    ...
    **model_kwargs,
    ...
    Raises:
        ...
        RuntimeError: a planned key does not exist on the sliced stage, a live tensor was never
            assigned, or this stage carries no EP MoE layer. Each is a STAGE-local verdict routed
            through :func:`~src.distributed.runtime.reject_across_ranks`, so the whole world
            raises together.
    """
    ...
    base = lazy_load_prologue(
        ...
        # (agent-docs/parallelism/pipeline-parallelism.md, Checkpoints). from_pretrained opens every indexed
        # file, so the meta shell must then be built config-only, and the plan gate below decides
        # whether the absent tensors are the legitimately-droppable cross-stage ones.
    ...
    # Collective (all_gather_object): the barrier this replaces, plus the uniform raise on any
    # rank's deferred failure.
    guard.reject()
