...
guard skips the embedding. Every family's rotary embeddings, mask construction, ``layer_types``,
...
def rebase_layer_index(name: str, src_root: str, dst_root: str, delta: int) -> str | None:
    ...
    The single index-rebase behind both directions of the stage naming map
    (:meth:`PipelineStageModule.global_parameter_name` / :meth:`~PipelineStageModule.local_parameter_name`)
    and behind the stage-aware loader's key filter, so a slice and its inverse cannot drift apart.
    Returns ``None`` when ``name`` is not a layer path under ``src_root``.
    """
...
class PipelineStageModule(nn.Module):
    ...
    :meth:`global_parameter_name` is applied EXPLICITLY by whoever serializes — that mapping keeps
    stage 1 from writing stage 0's layer names. The accessors must NOT be overridden to return
    global names: HF derives the weight-decay set from ``named_children`` (local) and
    ``load_state_dict``/DCP resolve keys against the real module tree.
    """
    ...
    # and the remote-code class whose modeling module the save must copy. ``config`` is deliberately
    # left un-defaulted — ``save_model_config`` skips a model that carries none, and a class-level
    # ``None`` would turn that skip into an attribute error.
    ...
    def __init__(
        ...
        *,
        ...
        # then returns the scalar loss instead of logits, so no [mb, S, V] plane is ever built.
        ...
        # The EP capacity generation rides the module the schedule actually calls. EP patching
        # registers it on the model it patched — under PP the un-sliced CausalLM, whose forward no
        # pipeline ever runs — so without this the generation never advances and every microbatch of
        # every step reuses the FIRST forward's DeepEP capacity, refusing the first longer batch.
    ...
    def can_generate(self) -> bool:
        ...
        else. Reward and classification stages correctly report False, as their unsplit models do.
        """
    ...
    def gradient_checkpointing_enable(self, gradient_checkpointing_kwargs=None, every_n_layers: int = 1) -> None:
        ...
        Mirrors ``PreTrainedModel.gradient_checkpointing_enable`` with two deliberate deviations:
        ...
        and there is no ``enable_input_require_grads`` (a non-first stage has no input embedding,
        and its recv buffer already requires grad). ``every_n_layers`` counts from this stage's
        first local layer, so every stage checkpoints the same fraction of its own layers.
        """
    ...
    def local_parameter_name(self, global_name: str) -> str:
        ...
        Only meaningful for names this stage owns; a layer outside its range comes back with a
    ...
    def checkpoint_name_map(self) -> dict[str, str]:
        ...
        The one filter+map the PP save and the PP resume both walk, so a key can never be written
        under one policy and read back under another. EP MoE entries are excluded: they hold this
        rank's internal shard layout (local expert ranges, de-interleaved splits) and are re-emitted
        in HF layout by the family gather.
...
def module_path(model: nn.Module, module: nn.Module) -> str:
    """Dotted path of ``module`` inside ``model`` — identity match, never name assumptions.
    ...
    Raises:
        ValueError: ``module`` is not part of ``model``'s tree. Fatal for every caller: the path is
            ...
            matches on, so a miss would otherwise surface as a bare ``StopIteration`` or as a
            silently unfiltered key set.
    """
...
def resolve_layer_root(model: nn.Module) -> str:
    ...
    stage module's own :attr:`PipelineStageModule.global_layer_root` composes the same string from
...
def slice_backbone_to_stage(model: nn.Module, lo: int, hi: int) -> None:
    ...
    materialized, :func:`build_pipeline_stage` applies it to a whole-model load, and neither can
...
def reject_layer_type_rebase(model: nn.Module, layer_offset: int, pp_rank: int) -> None:
    ...
    periods silently swaps sliding-window and full causal masks. Stage 0 and a uniform
    ...
    # the same seam ``layer_types_period`` uses: this gate rejects the offsets that seam's period
    # does not already snap the partition to, so a config the two read differently would leave a
    # manual ``pp_split`` un-snapped AND un-gated.
...
def build_pipeline_stage(
    ...
    stage does not own are dropped so they hold no memory. A model that arrives already sliced (the
    stage-aware loader stamps its partition on ``PP_STAGE_PARTITION_ATTR``) keeps that split — the
    partition cannot be re-derived from a list that is already one stage long. The returned module is
    the only handle that should be used afterwards.
    ...
    # Validate EVERY stage's offset: a rank-local raise leaves the others hanging at the P2P handshake.
    ...
    # input_ids-consuming layers): every rank judges every boundary, so all fail together.
    ...
    # families this rebinds the mid-chain forward that accepts the widened boundary stream.
    ...
    # cannot re-derive it from this nn.Module carrier. Stamp the live class here, where the same
    # carry already happens for the generation config and the remote-code class; an ``architectures``
    # -keyed consumer (vLLM, TGI, Auto* resolution) would otherwise serve the wrong head.
    ...
    # Same carry for a remote-code model's defining class: the stage module's own MRO has no
    # ``transformers_modules`` entry, so without it the save ships config + auto_map but no
    # modeling_*.py — an unloadable checkpoint (resume re-from_pretrains the saved dir).
