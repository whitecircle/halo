...
def save_tp_checkpoint(ctx: CheckpointContext, output_dir: str) -> None:
    """TP-only / TP+DP. ``save_tp_model`` owns the second gather its hand-sliced params need."""
...
def save_ep_checkpoint(ctx: CheckpointContext, output_dir: str) -> None:
    ...
    # unmerge on exit is what leaves the adapters trainable after an intermediate merged save.
...
def is_pp_shard_writer(config, shared_fs: bool) -> bool:
    ...
    NODE, so each node holds a directory it can resume from. The per-node widening is sound only
...
def save_pp_checkpoint(ctx: CheckpointContext, output_dir: str) -> None:
    ...
    # which is what the caster's tree-derived keep-sets speak.
