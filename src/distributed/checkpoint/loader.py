...
:class:`CheckpointLoader` owns every weight restore path on resume; the optimizer state and LR
scheduler are :class:`~src.distributed.checkpoint.optimizer.OptimizerShardStore`'s half, and the
adapter restore :func:`~src.distributed.checkpoint.peft.restore_adapters`'. Reads a
:class:`CheckpointLoadContext` (never the trainer) so collective/rank invariants stay explicit.

Resume POLICY only: the file reads it drives (:class:`StreamingCheckpointReader`,
:func:`read_checkpoint_key_set`) live in the format leaf, so the standalone tools share them.
"""
...
def _weights_read_from(model) -> str | None:
    """Where the live model's weights were actually READ, or None for a from-config random init.
...
class CheckpointLoader:
    ...
    def load_model(self, resume_from_checkpoint: str, model=None, *, for_best_model: bool = False) -> None:
        ...
        if self._needs_skip_weight_load():
            ...
            # Base weights only load at construction, so both refusals below turn on one question: does
            # this checkpoint ship base weights? (An adapter-only one has none to reload.) Decided on rank
            # 0 — ``realpath`` resolves locally, so per-rank branching splits the world on a non-shared FS.
            has_base = False
    ...
    def _load_pp_stage(self, checkpoint: str, model=None) -> None:
        ...
        # EP experts only load at construction, so a model not built FROM the checkpoint would
        # silently resume base experts. The signal is ``_weights_read_from`` (None for a from-config
        # init), not ``config._name_or_path``, which survives a build that read no weights.
        # Every rank computes the verdict and joins it before acting: both inputs are rank-local
        # (``ep_moe_layers()`` is stage-dependent, ``realpath`` resolves per node), so raising inside
        # the gate would strand the offended ranks while their peers enter the consensus below.
    ...
    def _restore_extra_trained_params(self, resume_from_checkpoint: str, model) -> None:
        ...
        base reload never sees them. Params are declared via ``_extra_checkpoint_param_names``;
        read only those and broadcast into the live
        (possibly FSDP2-sharded) params. Collective-safe: rank 0 reads, every rank enters
        ``broadcast_from_rank0`` ``set_model_state_dict``. No-op when none declared, and a raise
        (not a warning) when the checkpoint cannot be read — see below.
        """
        unwrapped = unwrap_framework_wrappers(model)
    ...
    def _load_fsdp2(self, resume_from_checkpoint: str, model=None, *, for_best_model: bool = False) -> None:
        ...
        # ``realpath`` resolves locally. Best-model loads must still read: the live weights trained past it.
