...
host copy; these primitives encapsulate that asymmetry.
"""
...
def resolve_retained(items: Iterable[tuple[str, torch.Tensor]], *, retain: bool) -> dict[str, torch.Tensor]:
    ...
    hangs a gathered save: the writer enters the all-gather and its peers never do.
    ``materialize_dtensor`` is that collective without the device-to-host copy, letting non-writers
    join without holding N× model bytes per node. Public because the EP gathered save walks its own
    key space and needs the same symmetry for its non-expert tensors.
    """
...
def conversion_chunk_key(name: str) -> str:
    ...
    :func:`~src.checkpoint.format.revert_load_conversions` cannot take one tensor at a time: a reverse
    fusion (a vision tower's q/k/v into ``in_proj``, a per-expert hub split) needs every source key of
    that entry together. Conversion entries are declared per layer and never straddle two, and
    everything outside the layer list shares one chunk, so both chunks stay closed.
    """
...
def chunked_saveable_tensors(
    ...
    *,
    ...
    identical on every rank, and each chunk resolves through the same ``retain`` gate. The writer's
    host RAM peaks at one decoder layer plus one pending shard rather than the whole model.
    """
...
def saveable_items(
    ...
    Public for callers that subtract from that walk before handing it back as ``items`` (the TP save
    drops the hand-sliced params its own trailing gather covers), where re-spelling the walk would
...
def stream_gathered_checkpoint(
    ...
    *,
    ...
    (per chunk, which is why the chunks are conversion-closed), tie reconcile, config and index, with
    host RAM bounded by one chunk plus one pending shard. The tied pair is held across the stream
...
def exchange_shard_index(
    ...
    """Fold every rank's ``(weight-map fragment, bytes written)`` into one merged index.
    ...
    would leave every rank holding one fragment per rank of a map only rank 0 needs. The merge verdict
    travels with the map, so a collision raises on every rank.
    """
    payload = (weight_map, total_bytes) if contribute else None
    if not (dist.is_available() and dist.is_initialized()):
        return merge_shard_index([payload])

    # Non-None on rank 0 alone, as ``gather_object`` requires, which narrows the merge below to the
    # one rank holding every fragment.
    gathered: list[tuple[dict[str, str], int] | None] | None = (
        [None] * dist.get_world_size() if dist.get_rank() == 0 else None
    )
    dist.gather_object(payload, gathered, dst=0)

    # The exception object is the verdict, so every rank raises the identical diagnostic naming the
    # colliding key. Any exception, not just the collision: one escaping rank 0 here would leave
    # every peer in the next collective until the watchdog fires.
...
def merge_shard_index(gathered: list[tuple[dict[str, str], int] | None]) -> tuple[dict[str, str], int]:
    """Fold the gathered fragments into one ``(weight_map, total_size)``, rejecting a collision.
    ...
    bytes twice, producing a checkpoint that loads while holding the wrong writer's tensor; both
    callers are key-disjoint by construction, so a second claim is a defect.
