...
# Single source of truth for the supported schedules: field type, validation and runtime table.
PPSchedule = Literal["1f1b", "gpipe"]
PP_SCHEDULES: tuple[str, ...] = get_args(PPSchedule)
PP_DEFAULT_SCHEDULE: PPSchedule = "1f1b"

# Same pattern for the other two enum-like fields: the type IS the validation table.
...
class ParallelismConfig:
    ...
    # The run's per-rank shape, derived from the training config by ``parallelism_config_from_args``
    # for the dispatch-ceiling gate below: rows one MoE forward carries per device
    # (rows_per_forward x per_device_train_batch_size), and the config's own max_length. A 0 length
    # is ``max_length: null``, which the gate resolves against the model's context window; 0 rows (a
    # hand-built config, or a trainer with no max_length knob at all) declares no budget and leaves
    # the ceilings to the dispatcher's runtime backstop.
    ...
    # FSDP2 otherwise reshards after EVERY microstep's backward and re-all-gathers the full model on
    # the next — pure waste when nothing changed in between, and catastrophic when NCCL is forced onto
    # sockets (rollout_backend=sglang: ~15s × gradient_accumulation_steps per optimizer step). Leaves one
    # re-gather per optimizer step, costs one unsharded param copy per GPU; plain-DP torchrun path only.
    ...
    def _validate_ep_group(self):
        ...
        if self.ep_group_size > 1:
            if self.ep_scope == "node":
                ...
                # world_size whenever pp_size == 1. Name the unit or an ep16+pp2 user reads
                # "cannot exceed world size (8)" on a 16-GPU job.
    ...
    def is_racy_single_domain_multigroup_ep(self) -> bool:
        ...
        DIFFERENT membership. The single shared topology predicate for both the config-time validator
        and the trainer-side guard.
    ...
    def racy_ep_topology_message(self) -> str:
        """The one wording of the racy-EP rejection, shared by the config gate and the trainer guard."""
        return (
    ...
    def _validate_ep_token_budget(self, model_config) -> None:
        ...
        as undeclared switched the gate off exactly where it binds.
