...
transformers shards module by module and ties afterwards, so the two ends of a tied pair must land on
the same side of the TP plan or the first forward meets mixed plain/DTensor operands. The load-time
context manager below arranges that; the post-load check raises when it did not hold.
"""
...
def _tied_module_names(concrete) -> tuple[set[str], set[str]]:
    ...
    Empty when the class declares no ``_tied_weights_keys``; the post-load check raises in that case.
    """
...
def consistent_tied_tp_plan(model_class, model_config):
    ...
    pair ends as one parameter. Normally both ends are planned: a tied config injects
    ``embed_tokens: embedding_rowwise`` into the backbone plan and a ForCausalLM's
    ``lm_head: colwise_gather_output`` agrees with it on ``Shard(0)`` of the vocab dim. Where only one
    end carries a plan entry (a multimodal wrapper class declaring no ``lm_head`` entry, a backbone
    shipping no ``base_model_tp_plan``) the tie hands the unplanned end a weight of the other kind and
    ...
    # wrapper's own base_model_tp_plan is None (Qwen3.5), so the outer config reports no entry.
...
def validate_tied_pair_consistent(model, applied_plan: dict) -> None:
    """Raise when a tied embedding/head pair survived the load in an unusable shape.

    Two cases, both fatal at the first forward or the first step:

    * two distinct parameters, meaning transformers refused the tie (a checkpoint carrying both keys
      with different values), so each end would train on half the tied gradient;
    * a pair whose sharding disagrees with the plan: a sharded weight in an un-transformed head, or a
      replicated weight in a head the plan gave TP's input/output transforms. Either fails inside
    ...
    # By module rather than parameter name: the tie makes both ends one object, which
    # ``named_parameters`` emits only under the embedding's name.
