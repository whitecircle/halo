...
— not ``tp_rank == 0``, which several TP groups per node would race on identical files.
"""
...
def _tp_chunks(model: nn.Module, model_to_save: nn.Module, *, retain: bool):
    ...
    reconstructs them and they need their own collective pass. They carry no hub-conversion entry —
    the toolkit slices them, no family mapping names them — so a chunk of their own is
    conversion-closed. Collective on every TP-group rank; only the writer retains.

    They are dropped from the parameter walk for exactly that reason: it emits the LIVE parameter,
    which is this rank's ``[heads/tp]`` slice, and the streamed writer has already flushed it by the
    time the gathered tensor arrives under the same key. The exclusion is read off the same
    ``_tp_sharded_non_dtensor`` registry the gather iterates, so the two cannot disagree about which
    params the trailing chunk owns.
    """
