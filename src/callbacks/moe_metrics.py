...
A forward hook counts each router's own selection, since a balancing bias or group-limited routing
moves it away from ``topk(router_logits)`` (the fallback for a router publishing logits alone).
``on_step_end`` all-reduces the per-step counts and ``on_log`` merges the summary. Asking for metrics
does not turn ``output_router_logits`` on by itself, because that also couples the router aux loss
...
def compute_moe_load_metrics(
    ...
    # A single device-to-host sync for all six scalars; separate .item() calls stall the stream once
    # each.
...
def _selected_experts(router_output, logits_index: int) -> torch.Tensor | None:
    ...
    Identified structurally rather than per family: the selection is the integer tensor sharing the
...
class MoELoadMetricsCallback(TrainerCallback):
    ...
    ``reduce_group``, which the PP mixin sets by scanning for this base type so a third such callback
    is covered too. ``reduce_group`` narrows the expert-load all-reduce to a subset of the world
    (``None`` = world); under PP a world reduce would blend stages holding different routers.
...
class MoEMetricsCallback(MoELoadMetricsCallback):
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        ...
        if router_logits_forced_off(cfg):
            # Re-enabling here would undo the balancing strategy's decision to leave it off.
            if is_global_main_process():
                logger.warning(
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        # differently-shaped payload, which would hang its peers in NCCL, and it still joins on its
        # CUDA device since NCCL rejects CPU tensors.
        ...
        if num_layers == 0 or max_e == 0:
            # Only a hooked callback can diagnose an empty capture; when wiring was declined the
            # reason was already logged at train begin, so this warning would name the wrong cause.
