...
# Headroom over that bound; a larger per-step delta indicates a corrupt counter, not real work.
...
def resolve_max_seq_len(*sources) -> int | None:
    ...
    Several sources are checked because on the RL surface the bound is split across objects:
    ``trl.GRPOConfig`` declares neither ``max_length`` nor ``max_prompt_length``, and the scripts
    carry the latter on their own args.
    """
...
def _per_step_metrics(metrics) -> dict[str, float]:
    ...
    The remaining fields are run constants reported once in the setup summary; emitting them every
...
def _fp32_compute_precision() -> str:
    ...
    whose peak is higher, so keying MFU off ``"fp32"`` there would report TFLOPS above peak.
    """
...
def _detect_precision(model: torch.nn.Module | None, training_args, lowp_precision: str = "bf16") -> str:
    ...
    ``lowp_precision`` takes precedence where it is on: the masters stay bf16/fp32 while the GEMMs
    run in fp8/fp4, so the parameter dtype would name the wrong peak and scale every MFU figure by
    2-4x. Otherwise the training arguments decide, falling back to the model dtype. ``model`` is None
    until the Trainer hands over a model reference, where the args flags are the only signal (bf16 is
    the default).
    """
...
class EfficiencyCallback(transformers.TrainerCallback):
    ...
    divides by ``world_size`` (not by ``ep_size``: EP is orthogonal to DP, and each EP rank sees a
    distinct batch) and again by ``cp_size``, since CP splits the sequence inside ``forward()``.
    ...
    def _fallback_step_tokens(self, world_size: int, measured_cluster: int | None) -> int:
        """Substitute the padded-length estimate for an unusable measured token delta, with a warning.
        ...
        constant rather than a measurement. The warning names that cause.
        """
    ...
    def _compute_token_metrics(self, state: TrainerState, step_time: float) -> int:
        ...
        # Only ranks sharing the same batch are non-independent (TP/ETP, CP, and the pp_size ranks of
        # one pipeline chain); EP is orthogonal to DP, so data_parallel_size does not divide ep_size.
    ...
    def _initialize_model_flops(self, model: torch.nn.Module, seq_len: int):
        ...
        # The shared router probe rather than a caller-declared pair, so the balancing wiring and the
        # MoE load metrics classify a family the same way.
        ...
        # group's tokens, so per-rank active FLOPs/token is ep-invariant. expert_tp_size does not
