...
plain attribute rather than a buffer, keeping it out of ``state_dict()`` exports and out of the PP
lazy loader, which materializes state-dict tensors only.
"""
...
# Overshoot guard for the transient side-buffer, whose bias adds to selection scores of uniform scale
# 1/num_experts. Warn once a single sign step exceeds this fraction of uniform, where the controller
# oscillates instead of converging, and quote a rate that would keep it at the threshold.
...
class RouterBiasBalancingCallback(MoELoadMetricsCallback):
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        ...
        # sits in the family's own score space, where GPT-OSS logits call for a larger rate.
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        # sign(0-0)=0 makes the update a no-op, so an all-zero counter is reported.
        ...
        # strict: ``update``'s first dimension is ``counters``', stacked from ``routers``. A length
        # mismatch means the router list changed under the callback, and truncating would leave the
        # tail of the model balancing on a frozen bias.
