...
Every count is the RANK's own (``local_numel`` reads the TP/PP shard off the parameter, the layer
count comes off this rank's module tree), so the estimate describes what this GPU computes.
"""
...
# Fallback when nothing declares a bound. Not a real upper bound — callers must not clamp to it.
...
def _is_expert_param(name: str) -> bool:
    ...
    are always-active. Both vocabularies are registry-derived, never a hardcoded name list.
    """
...
def compute_expert_params(model, trainable_only: bool = False) -> float:
    ...
    ``trainable_only=True`` restricts to ``requires_grad`` experts; default counts every expert (a frozen
    LoRA base still runs expert forward + input-grad).
    """
    return float(
        sum(
            local_numel(p)
...
def estimate_model_flops_per_token(
    ...
    # Frozen params (LoRA base) cost 4N, not 6N — counting only trainable would read MFU ~0 for adapters.
...
def estimate_attention_flops(model, seq_length: int, pp_size: int = 1, tp_size: int = 1) -> float:
    ...
    The term is derived from ``hidden_size``, which stays global on every rank, so the ``tp_size``
    division is explicit here while the ``6·N`` term gets it from the DTensor shard; CP does not
    divide it (already per-token). ``hidden_size`` comes off ``config.get_text_config()`` so a VLM
    contributes its language tower rather than dropping the term — its vision attention is not
    modelled, making MFU on a VLM a slight UNDER-estimate.
    """
    config = getattr(model, "config", None)
    if config is None:
        return 0.0
    ...
    # The rank's OWN layers, not config.num_hidden_layers / pp_size: the default pipeline partition is
    # head-weighted, not even (gpt-oss-20b at pp4 splits 8/8/6/2 against an assumed 6).
