...
def _metric_log_period(training_config) -> int:
    ...
    Only ``on_log`` consumes them, so deriving them more often than the trainer logs buys nothing and
    costs a host sync (plus a collective for :class:`MoEMetricsCallback`) per step. ``logging_steps``
    below 1 is a RATIO of total steps, carrying no step period — every step there.
    """
...
def build_perf_callbacks(
    ...
    if args.enable_efficiency_metrics:
        ...
        # cheap at 8 ranks, a throughput tax at hundreds. The callback estimates without it.
