"""The fused-linear-cross-entropy ``forward`` installed by every toolkit Liger applier.
    ...
    def lce_forward(
        ...
        **kwargs,
        ...
        if skip_logits:
            ...
            # LigerForCausalLMLoss, not the bare loss module: it applies the shift, the
            # `num_items_in_batch` normalisation that matches transformers' ForCausalLMLoss (a mean
            # reduction would normalise per-rank, scaling the loss by world_size), and the token
            # accuracy TRL reads whenever use_liger_kernel is on.
        ...
        # Liger's own output class, not the plain transformers one: TRL reads token_accuracy off the
        # output whenever use_liger_kernel is on (the toolkit default), and a class without that field
        # loses the metric and emits a per-step warning.
