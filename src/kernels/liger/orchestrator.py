...
``fallback_model_type`` (text-config for multimodal wrappers). A delegating spec still resolves on the
toolkit branch, so every rule here reaches it. Parallelism safety filters
(:func:`liger_parallelism_overrides`) then force kernels off, and a family no applier covers warns loudly
and refuses an explicit per-kernel request.
"""
...
# Kernels a parallelism override makes merely INERT rather than wrong, so an explicit request is
# honored. A wrong-loss kernel (CE/FLCE under TP/CP/PP) is forced off even when asked for.
...
# by the role-driven builder — the ones upstream Liger has no entry for, and the ones whose spec
# delegates to upstream's applier and adds a role on top. One registry for all of them: a family that
# self-registered into Liger's own dict instead would resolve on the upstream branch and miss every
# rule this module derives from the toolkit registry.
_TOOLKIT_LIGER_APPLIERS = build_liger_appliers(LIGER_FAMILY_SPECS)

# Model types whose fused GLU survives an EP wrapper: a toolkit spec names the dense and
# shared-expert MLPs, which every wrapper adopts unchanged. A DELEGATING spec that names none has
# only upstream's routed-expert swap, which the wrapper replaces — force it off as for any upstream
# family.
...
# the two cannot drift. Built from the REAL appliers at import, so a substituted applier still gets them.
_PER_MODEL_DEFAULTS = {
    model_type: {
        name: param.default
...
def _applier_identities(apply_fn) -> set[str]:
    ...
    What is broken about an applier belongs to the function that runs, so a spec delegating to it
    inherits the defect (and the default that works around it) rather than shadowing it.
    """
...
def _liger_model_types(model_config) -> tuple[str | None, str | None]:
    ...
    through it; both the resolution and the EP predicate below must read the same pair.
    """
...
def liger_ep_disables_fused_glu(needs_ep_wrappers: bool, model_config) -> bool:
    """The ``has_ep_wrapped_experts`` argument BOTH Liger application sites must pass, whole, so they
    cannot drift.

    True only when an EP wrapper really replaces every module the resolved applier's GLU patch swaps:
    upstream's MoE appliers set the ROUTED-experts class, which ``patch_moe_model_for_ep`` replaces
    wholesale (inert under EP), while a toolkit spec names the dense and shared-expert MLPs every wrapper
    adopts unchanged (still paying on every token). Read off the specs, not a model_type list.
    """
...
def liger_parallelism_overrides(
    *,
    ...
    """Liger kernels a parallelism axis makes wrong or inert, mapped to why.

    The single home for that decision. Liger is applied twice — once at model load
    (:func:`apply_liger_kernel`) and once by the trainer mixin, which re-sanitizes
    ``liger_kernel_config`` before TRL can re-apply it — and a filter present at one site but not the
    other is silently undone by whichever runs second. Both call this instead of restating the rules.
    """
    ...
    if engaged:
        ...
        # `skip_logits` gate never fires; CE goes too — upstream rebinds F.cross_entropy PROCESS-WIDE.
...
def apply_liger_parallelism_overrides(user_config: dict, forced_off: dict[str, str]) -> dict:
    ...
    Sharing the rule table is not enough: both application sites must also FOLD it the same way, or
    one silently undoes the other.
    """
...
def warn_if_flce_unreachable(model_config: AutoConfig, trainer_name: str) -> None:
    """Warn when FLCE was applied to a model whose trainer's loss can never fire it.

    A trainer computing its loss outside the model's forward (the GRPO family) passes no ``labels``, so
    Liger's ``skip_logits`` gate never engages and the full logits plane materializes as unfused. Forcing
    it off is impossible here — the load-time patch site cannot know the trainer, and re-applying would
    not unpatch ``lce_forward`` — so the one site that does know warns instead.
    """
...
def apply_liger_kernel(
    ...
    forced_off = liger_parallelism_overrides(
        ...
        # fused expert path — and so does one whose applier never patched the routed experts.
...
def _apply_liger_for_standard_models(
    ...
    Returns the applied (signature-filtered) config dict, or ``None`` when no applier ran. A family
    no applier covers WARNS — ``use_liger_kernel`` defaults on, so an info line would let a whole run
    train unfused unremarked — and RAISES when the config asked for a specific kernel, which nothing
    can deliver there.
    """
    ...
    # A key this applier does not accept is dropped by the filter below. Dropping a kernel the user
    # asked for silently is the same failure the no-applier branch raises over: a config requesting
