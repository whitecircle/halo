...
with its reason here, because a silently different activation or normalization is a wrong model, not a
slow one. A family upstream Liger covers sets ``delegates_to_upstream`` and names only the roles it ADDS.
"""
...
# Multimodal wrappers are absent by design: the orchestrator resolves them through
# `text_config.model_type`, where the decoder classes these specs name live, so a wrapper whose text
# tower can be the dense or the MoE sibling (`lfm2_vl`, `cohere2_vision`) is never pinned to one.
#
