"""Role-driven Liger applier builder — one implementation for every family the toolkit covers.

A :class:`LigerFamilySpec` names the classes filling each ROLE; the roles it leaves empty are absent from
the built applier's signature, which is what the orchestrator reads for per-family defaults. Each role is
patched by SUBCLASSING the family's own class, so the constructor variance that blocks Liger's own class
swaps never arises and the original class name every downstream lookup keys on survives.
"""
...
# The flag every toolkit-patched family's gated MLP is gated on. Liger spells the switch twice
# (`swiglu`/`geglu`) and the orchestrator defaults and overrides both together; which kernel a module
# actually gets is resolved from its own activation by `resolve_fused_glu_mul`, not from the spelling.
_GLU_FLAG = "swiglu"

# The gates `fla`'s fused gated norm implements. Its kernel dispatches on the string with no else
# branch, so one it does not know applies NO gate — the ungated norm, at the right shape.
...
class LigerFamilySpec:
    ...
    # LigerRMSNorm cannot express them and the swap would change the model silently.
    rms_norm: tuple[str, ...] = ()
    ...
    # above are added on top. Which roles it serves is read off Liger's registry, never restated here.
    ...
    def __post_init__(self) -> None:
        ...
        # is caught for real by :func:`_named_class`, on the class it actually replaced.
...
def _named_class(module: ModuleType, name: str, spec: LigerFamilySpec) -> type:
    """The class ``name`` in ``module``, or a raise naming the family — never a silent skip."""
    ...
    # the replacement would stack the two. `_rebrand` keeps the toolkit's own swaps family-owned.
...
def _rebrand(patched: type, original: type, role: str) -> type:
    ...
    EP wrapper discovery, the PP spec registry and the module-structure helpers all key on
    ``type(module).__name__``; a subclass announcing its own name would drop the module out of every
    one of them while remaining an ``isinstance`` of the original.
    """
...
def _liger_rms_norm_class(original: type, *, offset: float, casting_mode: str) -> type:
    ...
    In-place follows Liger's own convention: the llama casting mode reuses the incoming gradient buffer,
    the gemma one (weight multiply in fp32) does not. Reusing it is safe only while no family adds a norm
    output straight into a residual — ``AddBackward`` hands both inputs the same gradient object. Every
    family here is pre-norm; ``tests/cpu/kernels/test_liger_family_coverage.py`` fails if one stops being.
    """
    ...
    class _LigerRMSNorm(original):
        def __init__(self, *args, **kwargs):
            ...
            # The family spells eps either way; Liger's forward reads `variance_epsilon`. A norm
            # spelling it a third way would otherwise hand `None` to the Triton kernel, which fails
            # deep inside the launcher with nothing naming the family.
...
def _bridge_gated_norm(module: nn.Module) -> None:
    ...
    ``variance_epsilon`` and register no bias. The activation is passed through as the family's OWN,
    which is what makes the fused gate provably the one the eager module declared.
    """
    ...
    # fla applies a bias when one is present; these norms have none, and silently gaining one would
    # change the function.
...
def _fused_gated_rms_norm_class(original: type) -> type:
    ...
    Liger has no gated-norm kernel; `flash-linear-attention`, already a hard dependency for this roster's
    delta-rule kernels, has exactly this one. It keeps the weight multiply in fp32 where the eager modules
    round first — the one deliberate deviation from the bit-for-bit rule, and the strictly more accurate
    side of it. Imported inside the factory: ``fla`` costs ~4.3 s and probes Triton at import time, while
    the orchestrator is on the import path of every run and every CPU test.
    """
    ...
    class _FusedGatedRMSNorm(original):
        ...
        def forward(self, hidden_states: torch.Tensor, gate: torch.Tensor | None = None) -> torch.Tensor:
            ...
            # keyword caller must not land on fla's `x`/`g` spelling.
...
def _fused_glu_mlp_class(original: type) -> type:
    ...
    (:func:`~src.kernels.fused_glu.resolve_fused_glu_mul`) — the same behavioural gate the EP
    wrappers use, so a config that swaps ``hidden_act`` to something the kernels do not compute
    falls back to the family's own forward instead of silently changing the activation.
    """
...
def _patch_module(module: ModuleType, spec: LigerFamilySpec, flags: dict) -> list[str]:
    ...
    A flag the spec names no class for patches nothing — the roles upstream's applier already ran.
    """
    patched: list[str] = []
...
def _patch_instance(model, spec: LigerFamilySpec, flags: dict) -> None:
    ...
    if flags.get(_GLU_FLAG):
        for module in model.modules():
            ...
            # which upstream's own instance patch does, on the very MLPs a delegating spec names, in
            # the call this one follows. Re-binding is what makes the toolkit's patch the final word.
...
class LigerApplier:
    ...
    def __init__(self, spec: LigerFamilySpec):
        ...
        # Resolved once: the signature the orchestrator reads and the defaults a call starts from are
        # the same fact, and under delegation deriving it means inspecting upstream's signature.
    ...
    def _declared_parameters(self):
        """One parameter per role that will run — a role nothing fills must not be offered.

        Under delegation the names and defaults are upstream's; the toolkit imposes only its own loss
        convention (CE on, FLCE off — the reverse of Liger's, because CE keeps the logits metrics read).
        """
        spec = self.spec
    ...
    def __call__(self, **kwargs) -> list[str]:
        ...
        patched: list[str] = []
        if self.upstream is not None:
            # Upstream owns every role it declares, the loss included — hence the skip below. What
            # follows adds only the classes the spec names, which `_named_class` proves are unclaimed.
