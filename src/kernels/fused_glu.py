...
Two kernel pairs: an unclamped one (SiLU or tanh-GELU plus the multiply) and one covering the clamped
family (GptOss, DeepSeek-V4, GLM-5 Next, Step-3.7) from a single compilation, with ``alpha`` and the
bound as runtime arguments. They stay separate so the unclamped path avoids an ``alpha`` multiply.
"""
...
class _FusedClampedGLU(torch.autograd.Function):
    """Kernel pair for the clamped GLU family: ``alpha`` and the bound are runtime arguments, so every
    layer and family in the process shares one compilation, while the clamp placement and GptOss's
    ``up + 1`` are ``tl.constexpr`` and cost nothing in the emitted code."""
