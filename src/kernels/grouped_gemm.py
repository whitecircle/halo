"""Grouped GEMM op for MoE expert FFNs across precisions, called by the EP and non-EP layers.
...
oracle or the opt-in native DeepGEMM under :mod:`src.kernels.lowp`. Master weights stay bf16/fp32.
"""
...
class GroupedGemmPrecision(str, Enum):
    ...
    MXFP4 = "mxfp4"  # e2m1 elements + e8m0 (1x32) block scales
    NVFP4 = "nvfp4"  # e2m1 elements + e4m3 (1x16) block scales
...
def grouped_gemm(
    ...
    *,
    ...
    fmt = precision.value
    # Native DeepGEMM fp8/nvfp4: opt-in only, not auto-selected (fp4 tolerates K-not-÷256 experts, the
