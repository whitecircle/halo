...
class LinearPrecision(str, Enum):
    ...
    FP4 = "fp4"  # nvfp4 block-scale (e2m1 + e4m3 1×16)
    MXFP4 = "mxfp4"  # mxfp4 block-scale (e2m1 + e8m0 1×32)
...
def _fake_quant_contraction_axis(t: torch.Tensor, fmt: str) -> torch.Tensor:
    ...
    quantizing the full tensor. It also keeps the kernel off DTensor dispatch, which has no sharding rule
    for fp4's ``bucketize``.
    """
...
class LowPrecisionLinear(nn.Linear):
    ...
    assignment, so a direct construction raises rather than defaulting to a precision.
    """
    ...
    def convert_(cls, linear: nn.Linear, precision: LinearPrecision) -> LowPrecisionLinear:
        ...
        The module is not rebuilt: tensor parallelism attaches its semantics to the module object
        (DTensor params, forward transforms, hooks), so a replacement would drop the collectives and
        train on unsynced partial sums. The retype relies on the class ``forward`` resolving; an
        instance-level ``forward`` (transformers' TP installs its transforms as one, closed over the
        bound ``nn.Linear.forward``) shadows it and keeps computing in bf16, so it is rejected here and
        the caller re-installs the transforms around the retyped module
