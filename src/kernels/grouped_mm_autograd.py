"""Autograd wrapper over ``torch.nn.functional.grouped_mm`` — the primitive
:mod:`src.kernels.grouped_gemm` dispatches its bf16 path to.

Materializes the broadcast (zero-stride) grads native backward rejects on SM100+ Blackwell
...
def grouped_mm_grads(mat_a, mat_b, grad_output, offs, needs_a: bool, needs_b: bool):
    ...
    Callers own dtype handling: ``grad_output`` and the operands must already share a dtype
    (mixed-dtype ``grouped_mm`` raises).
    """
    if any(s == 0 for s in grad_output.stride()):
        if grad_output.numel() == 0:
            # ``contiguous()`` is a no-op on a 0-element broadcast grad (keeps the [0, 0] strides),
            # and the kernel validates strides against sizes even at M == 0 — reachable under EP
