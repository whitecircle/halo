...
# Matched by PREFIX because the name carries a per-run suffix (``.halo_fs_probe_<ns>``). A job killed
# inside the probe window never reaches rank 0's removal, and the inert leftover would otherwise make
# the next launch refuse an output_dir holding nothing but the toolkit's own droppings.
...
def _validate_output_dir(output_dir: str) -> None:
    ...
    An unreadable directory is the same verdict — the run cannot own it — so the probe's ``OSError``
    is translated here rather than escaping as a second error type through the cross-rank seam.
    """
...
def _validate_output_dir_across_ranks(output_dir: str) -> None:
    """Run ``_validate_output_dir`` on the FS-aware setup ranks and raise UNIFORMLY on every rank.

    A rank-local raise would desync the job, so the per-node verdict goes through the cross-rank
    rejection seam — carrying the ``ValueError`` type every caller of this guard matches on.
    """
...
def run_training(main_fn):
    ...
    sticky ``cudaErrorIllegalAddress`` that buries the real traceback. The traceback is printed
    BEFORE that teardown because ``destroy_process_group`` is itself a collective: a rank failing
    while its peers are still in one blocks there, and an uncaught exception prints only once the
    ``finally`` returns, so the real error would surface as a silent hang.
    """
...
def setup_training_environment(args, training_config, script_name: str = "train") -> None:
    ...
    # ceil(tokens/1024)) with every peer spinning in the next collective.
    ...
    # never loads through load_distributed_model, and the image's TF32 default collapses adjacent RoPE
    # positions past 2048 tokens.
    configure_float32_matmul_precision()

    # output_dir may carry per-rank strftime codes; broadcast rank 0's value to avoid split-brain dirs.
...
def _setup_tracking_env_vars(args, training_config, script_name: str) -> None:
    ...
    # wandb takes its name from args.run_name, which transformers passes straight to wandb.init
    # (WANDB_NAME never gets a look), so the shortening reaches it only through the config.
...
def detect_resume_checkpoint(training_config) -> str | None:
    ...
    when it does not exist (degrading to a from-scratch run would overwrite the output_dir);
    ...
    fresh, so the output_dir guard ``setup_training_environment`` skipped re-runs here.
    """
...
def _checkpoint_has_full_model_weights(checkpoint: str) -> bool:
    ...
    WINS over any whole-model file beside it: a directory carrying both is a gathered save torn by a
    later sharded one, and resuming off the stale gathered half would be silent.
    """
...
def resolve_resume_weights_source(checkpoint: str | None, model_config, parallelism_config) -> str:
    ...
    The Trainer's loader REFUSES the checkpoint weight reload under the EP wrappers (their fused
    params cannot ingest HF-format keys) and CP, and a TP model constructed FROM the checkpoint skips
    the re-read entirely — so those resumes return the checkpoint dir. Under the default
    ...
    neither loadable nor an adapter raises (it would silently continue from BASE). Decided on rank 0
    and broadcast for FS-agnostic consistency.
    """
    base = model_config.model_name_or_path
    if checkpoint is None:
        return base
    # TP is listed on its own rather than riding on ``needs_ep_wrappers`` (true only via
    # ``use_grouped_gemm``): turning that knob off would otherwise make TP resume a dead end.
