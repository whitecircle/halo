"""Run logging: per-rank transformers verbosity, and the ``run.log`` console tee.

A leaf between the two phases that need it — the parser installs the tee as soon as ``output_dir``
is final, ``setup_training_environment`` sets the verbosity — so neither pulls in the other's stack.
"""
...
# The run.log this process's fds are already redirected into. A path, not a bool: the redirect is
# process-global and one-shot, so a later call naming a DIFFERENT output_dir would otherwise no-op in
# silence and leave that run's console output in the first run's file.
_TEE_LOG_PATH: str | None = None

# Grace for tee to flush its block-buffered tail once the fds are restored at exit. It is draining an
# already-closed pipe, so anything longer means the process is stuck, not busy.
...
def install_log_tee(output_dir: str | None) -> None:
    ...
    The writer is ``fs_aware_save_rank``: run.log is an output artifact, so it follows the checkpoint
    writers' predicate and their rank-AGREED shared-filesystem flags (``init_distributed`` runs
    first, in ``H4ArgumentParser.parse``) — a per-node env override cannot make two nodes both
    believe they are the sole writer and interleave one ``run.log``.
    """
