...
``apply_distributed_trainer_config`` → ``run_trainer``) plus the helpers whose meaning must be
identical at every call site — window pins, tokenizer/attention resolution, callback assembly, and
the ``reject_*`` guards. Parsing tuples, collators, and trainer construction stay in the scripts.
"""
...
def init_training_script(
    ...
    **parallelism_kwargs,
    ...
    # Before the weight load (a join placed after it is the first collective a straggler misses, and
    # the watchdog then blames the check) and after the device bind (the gather rides NCCL).
    verify_rank_uniform_env()

    # Peeled BEFORE the config is built: __post_init__ validates expert_lora (PP rejects it) and
    # create_ep_config caches its EPConfig, so a later assignment never reaches the layers.
    ...
    # output_dir is final and still unwritten — the last moment a mis-declared DIST_*_SHARED_FILESYSTEM
    # is a startup error rather than a resume that restarts every non-zero node at global_step=0.
...
def reject_images_under_text_only_model(args, datasets, *, text_only_model: bool) -> None:
    """Refuse image data on a run that loaded a multimodal checkpoint through its text-only class.

    Under ``text_only_model`` the loaded config is the text sub-config, so ``is_vlm_run`` can never
    route to a VLM data path: an image column would be pruned unseen and the run would train on the
    rows' text alone. Called once the dataset is in hand, before the modality dispatch.
    """
...
def load_script_datasets(
    ...
    **loader_kwargs,
    ...
    # environmental GRPO) refuse the knob through reject_unsupported_args before their load.
...
def log_script_dataset_examples(datasets: dict, tokenizer: PreTrainedTokenizer, args, training_config) -> None:
    ...
    Absent splits pass through as ``None`` and are skipped, so callers never assemble the dict
    conditionally.
    """
...
def distributed_trainer_kwargs(args, dist_args, parallelism_config: ParallelismConfig, *, dataset_presharded: bool):
    """The constructor kwargs every ``DistributedTrainerMixin`` trainer takes, spelled once.

    A site passing three of the four gathers a save the config asked to shard, or re-shards a
    dataset that is already per-DP-rank.
    """
    return {
...
def prepare_script_preference_data(
    ...
    *,
    ...
    The VLM arm passes the rows through UNTOUCHED — both trainers chat-template and tokenize vision
...
def apply_max_length(
    ...
    of the one passed in. The GRPO family pins its two-knob budget through
...
def enforce_text_path_padding_side(tokenizer: PreTrainedTokenizer, vlm_run: bool) -> None:
    ...
    the packing collator refuse it outright and the padding ones read leading pads as an attended
    document. A VLM run keeps the processor's side — its collators pad through the processor.
    """
    if not vlm_run:
...
def apply_prompt_completion_window(
    ...
    *,
    ...
    """Pin the tokenizer's window for the GRPO family, whose budget is two knobs, not one.
    ...
    half into a cap nobody chose. ``completion_budget_required`` marks the on-policy callers, where
    ...
    callers must use it in place of the one passed in — and the window, ``None`` when unbounded.
    """
...
def reject_unsupported_args(context: str, **unsupported) -> None:
    """Fail loud on parsed knobs this script cannot honor.

    Shared config dataclasses expose fields no single script implements; parsing one and doing
    nothing trains something other than what the config states. Only truthy values are reported —
    an unset field is not a request.
    """
...
def reject_non_default_args(context: str, args, *field_names: str) -> None:
    """Fail loud on parsed knobs this script cannot honor, judged against their DECLARED default.
    ...
    defaulted field of ``args`` — a typo is a dead guard, so it raises here.
    """
...
def _reject_ignored_fields(context: str, set_fields: list[str]) -> None:
    """Shared raise for both rejection forms, so they report identically."""
    if set_fields:
        raise ValueError(
