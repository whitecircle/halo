...
_TOOLKIT_DEFAULTS = {
    ...
    # and logs a running average in a NaN's place — the silent fallback this toolkit refuses.
    "logging_nan_inf_filter": False,
}

# A toolkit default applies only when its guard holds: bf16 must yield to an explicitly-enabled
# fp16, else it forms the fp16+bf16 pair TrainingArguments.__post_init__ exists to reject.
...
# would let glibc eat it (`%-d` is a no-padding day-of-month).
...
def is_true_string(value: str) -> bool:
    ...
    (``--resume_from_checkpoint=true``), so their consumers read it as ``--packing=true`` is read.
    """
...
class PercentSafeHelpFormatter(argparse.ArgumentDefaultsHelpFormatter):
    ...
    takes ``--help`` down for the whole script. Already-escaped ``%%`` and argparse's own
...
def _is_bool_only(annotation) -> bool:
    ...
    A string on such a field can only be a spelling mistake — every non-empty string is truthy, so
    it would silently invert an intended false. A union that also admits ``str`` is left alone.
    """
...
def _argv_yaml_path() -> str | None:
    ...
    Single home for "was this invoked as ``<script> config.yaml [--overrides]``" — the parse dispatch
    and the explicitly-set-field scan must branch on the same answer.
    """
...
class H4ArgumentParser(HfArgumentParser):
    ...
    def parse_yaml_file(self, yaml_file: str, allow_extra_keys: bool = False):
        ...
        the string :meth:`_validate_field_values` refuses. No spelling is migrated: an unrecognized
    ...
    def parse_yaml_and_args(self, yaml_arg: str, other_args: list[str] | None = None) -> list[Any]:
        ...
        for arg in other_args or []:
            ...
            # Reject a repeated flag (--lr=1 --lr=2); the dict would otherwise silently last-wins it.
        ...
        for data_yaml in arg_list:
            ...
            for arg, val in parsed_other_args.items():
                ...
                if literal_choices is not None:
                    ...
                    # cast alike. A literal choice spelled "none" outranks None-clearing, so
                    # --moe_balancing=none selects the string; --field=None still clears an Optional.
                    ...
                    # unions with no confident value cast (--report_to=none on None | str | list[str],
                    # the standard way to silence logging on a smoke run). Optional BOOLS are exempt:
                    # --bf16=none reads as a mistyped boolean, and clearing it would silently flip
                    # the run's precision (or re-arm an auto-default) — the bool branch raises instead.
                    cast_val = None
                elif _is_cli_uncastable(annotations[arg]):
                    # setattr-ing the raw string would silently corrupt the field: __post_init__
                    # conversions do not re-run, and a string in a list field is iterated char-wise.
                    raise ValueError(
                    ...
                    # string equals no enum member, so a typo'd --save_strategy would silently
                    # disable checkpointing rather than fail. Let the enum's ValueError propagate.
    ...
    def parse(self) -> Any:
        ...
        a per-script shape, which is why the annotation can say no more than ``Any``.
        """
    ...
    def _get_explicitly_set_fields() -> set:
        ...
        for arg in cli_args:
            ...
            # reads field names — a dashed record would let the default reverse the user's value.
    ...
    def _format_output_dir(parsed_objects: list) -> None:
        ...
        would then tee ``run.log`` into a directory that is not the run's output_dir.
        """
        for obj in parsed_objects:
