...
memory and honouring ``save_max_shard_size`` are one fix: flush whenever the pending batch would
overflow. A MANY-writer caller (the PP save, one rank per stage) hands its weight-map fragment back
through :meth:`StageShardWriter.close` to be merged, because no writer knows the global part count;
a single writer knows it at the last flush, so :meth:`StageShardWriter.close_as_hf_checkpoint`
renames its parts to HF's own names and writes the index itself.
"""
...
class StageShardWriter:
    ...
    collective gather — share the call sites without a branch at every one.
    """
    ...
    def add(self, key: str, tensor: torch.Tensor) -> None:
        ...
        caller's chunk sequence claims one tensor from two sources, so it is refused, not overwritten.
        """
        if not self.enabled:
            return
    ...
    def close_as_hf_checkpoint(self) -> int:
        ...
        oversized tensor rather than carrying it alongside, the peak it exists to bound), so both
        layouts load identically but neither the file count nor the key→file map is comparable.

        Only valid when this writer owns every ``model*.safetensors`` in the directory: it deletes
        the ones it did not write, as HF's splitter does, because ``from_pretrained`` prefers a
        leftover single ``model.safetensors`` over the index. The sweep runs AFTER the parts are on
        disk — a re-save transiently holds both checkpoints (2× peak disk), where sweeping first
        would destroy the good one if this save failed.
        ...
        self._flush()
        if not self._part:
            # An enabled writer with no parts means the caller gathered nothing — a caller bug, and
            # the sweep below would delete the PREVIOUS checkpoint to put nothing in its place.
            raise RuntimeError(
        ...
        for part_name, final_name in renamed.items():
            # Overwrites a same-named leftover, which is the intended outcome; the stale sweep below
            # then only has to consider names this save did NOT claim.
    ...
    def _flush(self) -> None:
        if not self._pending:
            return
        self._part += 1
        # Numbered even when the stage produces a single part, so it can never collide with a stray
