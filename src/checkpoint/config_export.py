...
a no-op for a family that needs none, so writers call the set rather than picking.
"""
...
# Stamped by load_distributed_model on the loaded model: where its weights were actually READ
# (None under init_from_scratch). One spelling for the writer and the resume-coverage reader.
LOADED_WEIGHTS_FROM_ATTR = "_loaded_weights_from"

# "this serialized config does not spell the key" in the source-schema diff — distinct from a key
...
def hf_architecture_name(model) -> str | None:
    ...
    Re-derived because the load-time value is the HUB's class, which a task-head swap invalidates:
    a reward run would ship ``*ForCausalLM`` and every ``architectures[0]``-keyed consumer (vLLM,
    TGI, ``Auto*``) would serve the wrong head.
    """
    return next(
        (
            cls.__name__
...
def restore_model_type(config, output_dir: str) -> None:
    ...
    sharded-EP merge then refuses the checkpoint it just produced and hub renames degrade to none.
    """
    live = getattr(config, "model_type", "")
    config_file = os.path.join(output_dir, CONFIG_NAME)
    if not live:
        # The live config is family-less too — nothing correct to write, so say so loudly.
        logger.warning(
...
def flatten_per_layer_config(section: dict, legacy_keys: dict[str, tuple[str, str]]) -> bool:
    ...
    section's global value otherwise. ``per_layer_config`` is then dropped: a 5.14-line reader refuses
    a config carrying it at all, a 5.16 reader rebuilds it. Returns whether the section changed.

    The flat keys are the whole of what a reload can rebuild, so a config they cannot express is
...
def export_legacy_per_layer_config(output_dir: str) -> None:
    ...
    Runs after every config write the toolkit owns, because transformers 5.16's ``to_dict()`` has no
    flat form to offer — the family's config class dropped the legacy fields that the pinned rollout
...
def _source_config_payload(source: str) -> dict | None:
    ...
    None — with a warning naming the consequence — when the source is unreadable or declares no
    ``auto_map``: such a source was never servable by the engines that read this family through
    remote code, so there is nothing to carry. Not a raise; the artifact is still trainable.
    """
    try:
...
def _copy_auto_map_modules(source: str, auto_map: dict, output_dir: str) -> None:
    ...
    Transitively, through each module's relative imports: a modeling module that imports a sibling
    (Step-3.7's ``vision_encoder``) loads nowhere without it, and only the module the ``auto_map``
    entry spells is named there.
    """
    pending = {
        value.split(".")[0]
...
def _serialized_config(config, output_dir: str) -> dict:
    ...
    Every step the writers run before the source-schema carry, so the payloads being differenced
    were produced identically — one of them skipping a rewrite would read as a run-made change.
    """
...
def _apply_config_changes(source_section: dict, before: dict, after: dict, config, path: str = "") -> None:
    ...
    ``attribute_map`` — and under its own name where the source spells neither. Never both: the
    source's spelling reaches ``__post_init__`` after the field defaults and would win.

    Nothing here can prove the result parses back to the live values (the source schema also derives
    native fields from legacy keys this cannot see), so the caller re-parses and compares.
    """
...
def export_source_config_schema(config, output_dir: str, *, source: str | None) -> None:
    ...
    Runs after every config write the toolkit owns, one step past
    :func:`export_legacy_per_layer_config`: for these families transformers 5.16's serialization is
    not a dialect the server misreads but a schema it has no class for. The vendor spellings are
    absorbed at load and never re-emitted, so only the source config can produce them — carried over
    with this run's changed values applied under its spellings, plus the modules its ``auto_map``
    names, without which the directory does not load at all. A 5.16 reload is unaffected.
    """
    ...
    # ``auto_map`` names the source's modules and rides across by construction; the live config
    # carries it only when it was loaded from that source, so it is out of the comparison.
...
def finalize_exported_config(config, output_dir: str, *, source: str | None) -> None:
    ...
    no-op for a family that needs none, so the writers call the set rather than picking.

    One function because the parallel writers and the single-GPU save must produce the SAME
    directory: a checkpoint missing any of these trains fine and fails only in the merge tools or
    the rollout server, on a family the run never mentioned.
    """
...
def save_model_config(model, output_dir: str) -> None:
    ...
    by hand), so what that would have emitted is reproduced here. Each piece is a silent-corruption
    trap, not a formality:

    * ``PretrainedConfig`` does not carry the generation config, so custom ``eos_token_id`` sets
      (harmony/GPT-OSS), stop strings and sampling defaults would live nowhere — and vLLM reads that
      file directly.
    ...
    * ``config.architectures`` still holds the HUB's class after a task-head swap, so every
      ``architectures[0]``-keyed consumer would serve the wrong head.
    ...
    # configure THIS run, not the artifact: serialize with the pre-mutation values restored.
    with config_export_ready(config):
        config.save_pretrained(output_dir)
    # Ship the modeling modules the config's ``auto_map`` names — the write above copies only the
    # configuration module. From the MRO, not ``type(model)``: FSDP2 rewrites ``model.__class__`` in
    # place, so a live-class check would skip every remote-code model saved from a sharded run.
