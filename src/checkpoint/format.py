...
# Exempt from that skip: dropping these silently restarts the LR schedule / zeroes the router biases.
...
# EXACT names, never prefixes: ``original_adapter_config/`` is aux data the copy owes the artifact.
...
def save_dtype_caster(model: torch.nn.Module):
    ...
    family's fp32 pins — so a direct EP/TP save of an fp32-master run matches its merged-shards save
    and the export quantizes neither the balancing state nor a family's declared fp32 modules away.
...
def normalize_gathered_state_dict(model: torch.nn.Module, state_dict: dict) -> dict:
    ...
    The order is load-bearing: the caster's keep-set is derived from the module tree, so it speaks
    the LIVE key spelling and must run BEFORE the revert respells the expert keys. Shared by the
    FSDP2/CP and TP gathered writers, whose artifacts must be byte-identical for the same model.
    """
...
def copy_checkpoint_aux_files(
    ...
    Skips every top-level weight file and safetensors index — the caller writes fresh ones — but
    ...
    Subdirectories are copied whole, weight files included: a SentenceTransformer module directory or
    ``original_adapter_config/`` carries weights no caller rewrites, and filtering them out leaves
    ``modules.json`` pointing at modules that no longer exist. Three kinds stay behind — a nested
    ``checkpoint-N`` (resume state, not the artifact), a vendor weight dump, and anything hidden
    (``.git``/``.cache`` belong to the workspace).
