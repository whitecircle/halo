"""Checkpoint-directory I/O contract shared by the standalone conversion tools in ``scripts/``.

Which safetensors files make up a checkpoint, whether the requested conversion is writable, the one
full-model write path every tool owes its output directory, the staged-publish contract for a tool
that may be replacing the only copy, and the training-state sidecars an exported artifact carries.
What an exported ``config.json`` must contain belongs to :mod:`src.checkpoint.config_export`, the
saved-adapter shape to :mod:`src.checkpoint.adapters`, the flags the tools parse to
:mod:`scripts._common`.
"""
...
# Sibling suffixes a staged publish writes through: the output is built BESIDE the target and swapped
# in, so an interrupted write never replaces a good directory with a partial one. Spelled once here
# because a failed swap prints one of these paths for a human to find.
STAGING_SUFFIX = ".halo-staging"
DISPLACED_SUFFIX = ".halo-displaced"

# The training-state records an exported artifact carries beside its weights: how the run's routing
# was balanced, and what training-time model state a later merge has to re-apply.
...
def save_full_checkpoint(model, output_dir: str, processing_class=None, source_dir=None, **save_kwargs) -> None:
    """``save_pretrained`` + everything a conversion tool owes a full model directory.

    The contract is the ORDER: ``source_dir``'s non-weight files are carried over BEFORE the save so
    the live config and tokenizer overwrite the copies; stale weight files are swept only AFTER it,
    under :func:`~src.checkpoint.format.sweep_after_full_save`'s failure-safe rule; and the legacy
    ...
    FULL-model saves only: an unmerged ``PeftModel`` save writes adapter files alone, so the sweep
    would delete the directory's weights and the restore would stamp a foreign family into its config.
    """
...
def resolve_checkpoint_source(model_id: str, revision: str | None = None) -> str:
    ...
    The one resolver behind every ``--model_id`` that may name a Hub repo, so the input gates run on
    a local directory whichever spelling the operator used and ``HF_HOME`` owns where it lands.
    """
...
def clear_staging_path(target: str | os.PathLike, suffix: str = STAGING_SUFFIX) -> str:
    ...
    The staged-publish contract for a tool that may be rewriting the only copy of its input (an
    in-place sink reset, a dataset republish): build the new directory here, verify it, swap it onto
    the target. Anything found here at the START of a run belongs to a run that failed before its
    swap, so it is never the only copy and is cleared; anything left AFTER a failed swap IS the only
    complete copy, and the caller must name that path in its error and leave it in place.
...
def reject_sibling_adapter(input_dir: str) -> None:
    ...
    merged artifact would ADVERTISE an adapter it does not hold. The sharded EP save refuses every
...
def reject_sharded_checkpoint(checkpoint_dir: str) -> None:
    ...
    downstream can tell them apart — a ``from_pretrained`` tool reports the real keys as MISSING and
    RANDOMLY INITIALIZES them with a warning, a raw-tensor tool re-indexes a fraction of each weight
    as if it were whole — so every tool that opens a checkpoint directory calls this first.
    """
...
def _reject_absent_index_shards(checkpoint_dir: str) -> None:
    ...
    The one artifact that lands this way by design is a PP save on a non-shared output filesystem:
    each node holds its own stage's parts under a copy of the world-wide index, so no node's
    directory is loadable until they are gathered. A half-copied checkpoint reads identically, and
    both would otherwise surface as random-initialized keys or a failure inside ``safe_open``.
    """
...
def _reject_indexless_ep_shards(checkpoint_dir: str) -> None:
    ...
    truncation with its real cause rather than as this refusal.
    """
...
def apply_training_sidecars(model, source_dir: str) -> list[str]:
    """Re-apply to ``model`` everything ``source_dir``'s sidecars record about how it was TRAINED.

    One sequence for every tool that assembles an exportable model out of a training run's output,
    so two exports of the same run cannot disagree on what they carry: the training provenance, then
    the router-balancing state at the precision it was trained in (a conversion loads the checkpoint
    at the target dtype, and a bf16 round trip quantizes away the ~1e-3 sign steps the balancing
    update writes) — filled from ``router_balancing_biases.pt`` where the weights cannot carry it at
    all, since an artifact assembled from BASE weights would otherwise serve pretrained routing.
    ...
    # module tree, which an EP run's is not the hub tree. It travels with the artifact instead.
...
def copy_training_sidecars(source_dir: str, output_dir: str) -> None:
    ...
    For the writes that do NOT go through :func:`save_full_checkpoint` — an unmerged PEFT save, or a
    full save whose ``source_dir`` is the base model rather than the adapter directory holding the
...
def _balancing_tensors_at_trained_precision(model, source_dir: str) -> dict[str, torch.Tensor]:
    ...
    and the load may have downcast it); where it has none — a PEFT adapter directory — the slot is
    given fp32 storage so the fp32 sidecar lands unrounded. Entries already at the right dtype are
    omitted, so an untouched model returns ``{}``.

    The keys come from the registry-derived :func:`is_balancing_state_key`, since the tools here hold
    a plain hub model with none of the EP wrapper's adoption marks. Stored tensors match on the
    module path, not the full name: a family whose export renames its slot stores it under the hub
    spelling while the live tree uses its own, and a router carries exactly one balancing tensor.
    """
...
def _read_checkpoint_tensors(checkpoint_dir: str, wanted: Callable[[str], bool]) -> dict[str, torch.Tensor]:
    ...
    ``{}`` when the directory holds no model weights at all (a PEFT adapter directory, a hub id
    that was never resolved to a local path) — an absence the callers here treat as "nothing to
    read back", not as an error.
    """
    try:
...
def detect_model_type(checkpoint_dir: str) -> str:
    ...
    The single reader behind every family gate here, so a tool resolves the family the way the
    sharded merge does instead of sniffing key spellings. A composite VLM config with no top-level
    ``model_type`` falls back to its ``text_config``, where the language family lives.
    """
...
def checkpoint_shard_files(checkpoint_dir: str) -> list[str]:
    ...
    The shared layout cascade first — its index leg names exactly the model shards, so a co-located
    ``adapter_model.safetensors`` is never picked up — then a ``model*.safetensors`` glob, the leg
    only these standalone tools need since a training resume refuses an indexless sharded checkpoint.
    Raises when neither matches, rather than returning an empty list a caller reads as "no tensors";
    a legacy ``pytorch_model.bin`` reads as absent.
...
def iter_checkpoint_shards(checkpoint_dir: str) -> Iterator[tuple[str, Any]]:
    ...
    The reader is live for its own iteration step only — a caller that needs a shard's keys
    together (a weight/scale co-location check) reads them inside the step; a reader kept past it
    is closed.
    """
...
def iter_checkpoint_shard_entries(checkpoint_dir: str) -> Iterator[tuple[str, Any, str]]:
    ...
    The one walk behind every tool that reads a checkpoint's tensors, headers or key set: each shard
    opens exactly once and the live reader is handed out, so the caller takes the tensor
    (``get_tensor``), the header (``get_slice``) or neither — which is what keeps a header-only or
    key-only scan free of tensor reads. Lazy: a caller that stops early opens none of the rest.
    """
...
def iter_checkpoint_tensors(
    ...
    materializes the tensors it is about to discard — the difference between reading one expert bank
...
def preflight_resource_warning(tool: str, output_dir: str, *, disk_bytes: int | None, ram_bytes: int | None) -> None:
    ...
    Warn-and-continue by design: both figures are estimates (file sizes stand in for resident bytes,
    the output is assumed input-sized), so failing hard would refuse conversions that fit — the
    warning only exists so a 397B merge does not discover ENOSPC hours in. ``None`` skips a side,
    as does an unreadable ``/proc/meminfo``.
    """
    if ram_bytes:
...
def preflight_model_load_resources(
    ...
    *,
    ...
    (``writes_full_model=False`` for adapter-only outputs). Silent for a hub id or a weightless
    directory, and for a per-rank-sharded one — whether that input is usable belongs to the tool's
    own gates, never to a warn-only estimate.
    """
