...
# The key a keyless local rollout server (vLLM / SGLang without --api-key) accepts. The SDK refuses
# to construct a client without a key, so a None default would break the documented local flow.
...
class RequestTools:
    ...
    def hash_sample(self) -> str | None:
        """Identity of the complete tool-declaration set, for the resume-checkpoint key.

        A key built from a prefix of the declarations would let a run with unchanged prompts but
        edited tool schemas resolve to another run's checkpoint and replay its completions.
        """
        if self.raw is None:
            return None
...
async def generate_openai_response(
    ...
    ``extra_body`` is passed through the request body verbatim, for fields outside the OpenAI chat
    schema; the rollout engines' generation field names come from
    :func:`~src.environments.engine_wire.generation_control_fields`. ``top_p`` is sent only when set,
    so the served default otherwise stands. Without ``use_native_json_schema`` the schema is injected
    as prompt text.
    """
...
async def parallel_openai_requests(
    ...
    async def process_message_wrapper(idx, message):
        try:
            ...
            # Non-fatal so one bad row cannot end the batch: the row is left un-checkpointed,
            # returns as None, and the cause is logged with its traceback.
    ...
    # ``disable_checkpoints`` gate, so a branch-local definition would risk a NameError.
    new_results_buffer = []
    ...
    if not disable_checkpoints:
        ...
        async def save_checkpoint_incremental():
            ...
            try:
                ...
                # A lost append only means those completed rows re-run on resume; results stay correct.
...
def _sequence_digest(items: list) -> str:
    """Stable identity of a complete request sequence (prompt rows or tool definitions), for the
    resume-checkpoint key.

    Every element is hashed because callers pass the post-resume pending subset: two disjoint subsets
    of one dataset can be identical up to a long shared system prompt, and a colliding filename would
    replay the earlier run's results onto unrelated rows by index. Elements are fed in one at a time
    (NUL-delimited) so a 100k-row dataset is not materialized twice.
    """
