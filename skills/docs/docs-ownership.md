...
Always also check whether the change affects: the **nav** (`mkdocs.yml`), the
...
| any new trainer | `agent-docs/reference/trainer-architecture.md` + new method page + `mkdocs.yml` nav + `CLAUDE.md` index |

## Distributed / parallelism

| `src/` area | Owning doc page(s) |
|---|---|
| `src/distributed/parallelism_config.py` (ParallelismConfig, validation) | `agent-docs/parallelism/*` (all), `agent-docs/reference/configuration-reference.md`, the parallelism matrix in `CLAUDE.md` + `agent-docs/index.md` |
...
| `src/checkpoint/config_export.py` (what an exported `config.json` must contain: model_type restore, flat legacy keys, source schema) | `agent-docs/reference/checkpoints.md`, `agent-docs/models/index.md` |
...
| `src/distributed/loading/model_loading.py`, `src/distributed/loading/warmup.py`, `src/models/loading/model_preparation.py`, `src/models/patches/remote_code_compat.py`, `src/models/patches/remote_code_hooks.py` | `agent-docs/models/index.md`, `agent-docs/models/adding-a-model.md`, the affected per-family page |
...
| `src/models/loading/config_levels.py` (composite-config field access, run-scoped writes, `config_export_ready`) | `agent-docs/models/index.md`, `agent-docs/reference/checkpoints.md`, `agent-docs/training-methods/callbacks.md` |
| `src/models/modality.py` (multimodal checkpoint detection) | `agent-docs/data/dataset-formats.md`, `agent-docs/models/index.md` |
| `src/models/attention_geometry.py` (head-dim and KV-head resolution across composite/per-layer configs) | `agent-docs/models/index.md`, `agent-docs/optimization/flash-attention.md` |
| new model support | new `agent-docs/models/<family>.md` + model matrices in `expert-parallelism.md`/`grouped-gemm.md` + `mkdocs.yml` nav + `CLAUDE.md` index |
...
| `src/data/pipeline/vlm_dataset.py` (raw-VLM map, schema, over-length filter) | `agent-docs/data/dataset-formats.md`, `agent-docs/models/index.md` |
...
| `src/models/moe_balancing.py` (`resolve_balancing_mode`, the router field registries) and `src/distributed/expert_parallel/balancing_strategy.py` (`apply_balancing_strategy`, the export contract) | `agent-docs/training-methods/callbacks.md` (MoE balancing modes), `agent-docs/models/index.md` |
...
| `src/` area | Owning doc page(s) |
|---|---|
| `src/environments/base.py`, `episode.py`, `registry.py` | `agent-docs/training-methods/grpo/environments/index.md`, `custom-environments.md` |
...
| `.github/workflows/**` (lint, docs, GPU tier), `Makefile` test targets | `agent-docs/infrastructure/ci.md`; tier composition lives in `agent-docs/contributing/index.md` ("Tests") |
...
- A change to what parallelism combinations are valid → the parallelism matrix in
  `CLAUDE.md` and the mode list in `agent-docs/index.md`, plus the owning
  `agent-docs/parallelism/` page.
- A new page → wire it into `mkdocs.yml` `nav:` and the `CLAUDE.md` Documentation
  index, then run `mkdocs build --strict`.
