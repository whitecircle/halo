...
| OOM / attention error on **gemma4** at long seq | head_dim=512 blocks FA2; needs mem-efficient SDPA + manual KV repeat | — | Auto-applied: the FA→SDPA swap in `resolve_attn_implementation` and `patch_sdpa_for_gemma4_long_seq` via `apply_family_attention_patches` (both `src/models/patches/attention.py`, called from the loader; no `enable_gqa`). Cap seq (~20k) if still tight |
...
| `CheckpointError` (recomputed tensor shape ≠ forward) under **EP + gradient checkpointing**, often in GRPO | `use_reentrant=False` validates recompute shapes; the extra no-grad GRPO forwards make the recompute take a different path than the forward it must reproduce | Watch for the override warning at trainer init | The mixin forces `use_reentrant=True` under EP **or CP** everywhere except PP (which requires non-reentrant) even when the config (every GRPO template) sets `false` — `src/trainers/mixins/ep_introspection.py` and `mixins/base.py`. Let it; do not pin `use_reentrant: false` under EP |
| Embedding index OOB / CUDA assert early in training after switching model | A stale HF `map` cache keyed without the tokenizer → cross-model token-ID reuse | — | The cache key folds the closure-captured tokenizer — by **content**, plus its chat template and special-token ids — and `functools.partial` bound args (`src/data/pipeline/processing.py`). A value it cannot fingerprint takes a one-time `Dataset-map cache fingerprint … skips a value of type …` warning — thread that value through `cache_key_extras`; clearing `HF_DATASETS_CACHE` is the blunt fallback |
| Every resume re-runs the dataset map, or rows tokenized by an old tokenizer after editing it in place | Reading the tokenizer's content raised (a custom Python component the fast backend cannot serialize, a remote-code `get_vocab`), so the map cache keys on its `name_or_path` | The one-time `Dataset-map cache key cannot hash the content of tokenizer … and keys on its name_or_path … instead` warning (`_tokenizer_content_sig`, `src/data/pipeline/processing.py`) names the tokenizer class and the error | The re-map on resume is the only cost for an unchanged tokenizer; after editing one in place, save it to a new path or clear `HF_DATASETS_CACHE` |
| EP checkpoint reload → meta-tensor / missing-bias error | A stale checkpoint whose EP gather dropped 2D `gate_up_proj_bias`/`down_proj_bias`, or exported fused experts vLLM can't read | — | The EP gather keeps the biases — the fused key set in `src/distributed/expert_parallel/expert_gather.py`, gathered per layer by `expert_weights.py`. Repair an old checkpoint from source biases, or re-emit the family's per-expert layout with `scripts/after_training/unfuse_moe_experts.py` |
...
export TORCH_NCCL_DEBUG_INFO_TEMP_FILE=$HALO_DATA_ROOT/halo_diag/nccl_trace
...
with cuda_memory_history("$HALO_DATA_ROOT/profiling/oom", ranks="all"):
...
For TPS / peak-memory per step (not a trace), use `EfficiencyCallback`
(`enable_efficiency_metrics: true`); MFU / S-MFU / achieved TFLOPS reach the log only with
`report_mfu_diagnostics: true` on top. On MoE read **S-MFU** and judge it against the model class
