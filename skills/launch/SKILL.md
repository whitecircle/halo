...
# Publishing (moving + dated tag per image)
make ecr-login
make push-blackwell                          # also push-hopper, push-vllm, push-sglang, push-all
...
# In-image (mkdocs and its plugins ship in the image, not on the host)
make docs                                    # mkdocs build --strict
make docs-serve                              # mkdocs serve on :8000 (--network host)
...
Default to **blackwell** (it's the Makefile `IMAGE` default and the usual host).
